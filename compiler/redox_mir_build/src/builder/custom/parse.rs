        ...
        match &expr.kind {
            $(
                ExprKind::Call { ty, fun: _, args: $args, .. } if {
                    match ty.kind() {
                        ty::FnDef(did, _) => {
                            $self.tcx.is_diagnostic_item(redox_span::sym::$name, *did)
                        }
                        _ => false,
                    }
                } => $call_expr,
            ...
            $(
                ExprKind::Adt(box AdtExpr { adt_def, variant_index, .. }) if {
                    $self.tcx.is_diagnostic_item(redox_span::sym::$adt, adt_def.did()) &&
                    adt_def.variants()[*variant_index].name == redox_span::sym::$variant
                } => $variant_expr,
            )*
            $(
                $pat $(if $guard)? => $expr,
