...
use redox_errors::{Applicability, Diag, ErrorGuaranteed, MultiSpan, a_or_an, listify, pluralize};
use redox_hir as hir;
use redox_hir::attrs::DivergingBlockBehavior;
use redox_hir::def::{CtorKind, CtorOf, DefKind, Res};
use redox_hir::def_id::DefId;
use redox_hir::intravisit::Visitor;
use redox_hir::{Expr, ExprKind, HirId, LangItem, Node, QPath, is_range_literal};
use redox_hir_analysis::check::potentially_plural_count;
use redox_hir_analysis::hir_ty_lowering::{HirTyLowerer, PermitVariants};
use redox_index::IndexVec;
use redox_infer::infer::{BoundRegionConversionTime, DefineOpaqueTypes, InferOk, TypeTrace};
use redox_middle::ty::adjustment::AllowTwoPhase;
use redox_middle::ty::error::TypeError;
use redox_middle::ty::{self, IsSuggestable, Ty, TyCtxt, TypeVisitableExt};
use redox_middle::{bug, span_bug};
use redox_session::Session;
use redox_span::{DUMMY_SP, Ident, Span, kw, sym};
use redox_trait_selection::error_reporting::infer::{FailureCode, ObligationCauseExt};
use redox_trait_selection::infer::InferCtxtExt;
use redox_trait_selection::traits::{self, ObligationCauseCode, ObligationCtxt, SelectionContext};
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
                ...
                let opt_ty = expected.to_option(self).and_then(|ty| match ty.kind() {
                    ...
                    &ty::Pat(base, _) if base.is_integral() => {
                        ...
                        match layout.backend_repr {
                            redox_abi::BackendRepr::Scalar(scalar) => {
    ...
    /// ```
    fn get_expr_coercion_span(&self, expr: &hir::Expr<'_>) -> redox_span::Span {
...
impl<'tcx> Visitor<'tcx> for FindClosureArg<'tcx> {
    type NestedFilter = redox_middle::hir::nested_filter::All;
...
impl<'a, 'b, 'tcx> ArgMatchingCtxt<'a, 'b, 'tcx> {
    ...
    fn suggest_confusable(&self, err: &mut Diag<'_>) {
        ...
        //  - methods marked as `redox_confusables` with the provided arguments
        //  - methods with the same argument type/count and short levenshtein distance
        //  - methods marked as `redox_confusables` (done)
