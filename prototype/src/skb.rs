...
impl RuleDatabase {
    ...
    /// guards what derives from here — which is now both callers.
...
mod tests {
    ...
    /// The per-database sum must equal the total, or `--emit-skb` writes a
    /// tree that is quietly short of rules.
    ///
    /// `rule_count()` is `builtin_rules().len()` — the rules themselves. The
    /// per-database figures come from `rule_counts_by_db()`, which walks a
    /// **hardcoded array** of the eight `RuleDatabase` variants, and
    /// `rmi_ontology_adapter::unified_search` keeps a second copy of that same
    /// array under a comment promising it scans "all rules across the eight
    /// databases". Neither array is exhaustive in the compiler's eyes: a match
    /// on the enum would be, an array literal is not.
    ///
    /// **`check-skb-tree.sh` cannot see the gap.** It diffs the committed
    /// `skb/` tree against a fresh `--emit-skb`, and `emit_tree` builds that
    /// fresh copy by iterating `rule_counts_by_db()` — the same array. Drop a
    /// variant and *both sides of the comparison* lose those rules together,
    /// so the checker reports the tree matches. Its reference is generated by
    /// the thing it is checking, which makes this whole class of drift
    /// structurally invisible to it.
    ///
    /// The one assertion that is not: an array-derived figure against a
    /// source-of-truth figure. A ninth database, or a variant dropped from
    /// either array, fails here instead of silently shrinking `skb/`.
    #[test]
