...
use redox_errors::{Applicability, Diag, DiagCtxtHandle, struct_span_code_err};
    ...
    pub(crate) fn cannot_borrow_across_coroutine_yield(
        ...
        if matches!(coroutine_kind, hir::CoroutineKind::Coroutine(_)) {
            ...
            let span = match capture_clause {
                redox_hir::CaptureBy::Value { move_kw } => move_kw.shrink_to_lo(),
                redox_hir::CaptureBy::Use { use_kw } => use_kw.shrink_to_lo(),
                redox_hir::CaptureBy::Ref => fn_decl_span.shrink_to_lo(),
            };
