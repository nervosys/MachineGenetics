//! THIR datatypes and definitions. See the [redox dev guide] for more info.
//!
...
//!
//! [redox dev guide]: https://redox-dev-guide.rust-lang.org/thir.html
...
use redox_abi::{FieldIdx, Integer, Size, VariantIdx};
use redox_ast::{AsmMacro, InlineAsmOptions, InlineAsmTemplatePiece, Mutability};
use redox_hir as hir;
use redox_hir::def_id::DefId;
use redox_hir::{BindingMode, ByRef, HirId, MatchSource, RangeEnd};
use redox_index::{IndexVec, newtype_index};
use redox_macros::{HashStable, TyDecodable, TyEncodable, TypeVisitable};
...
impl<'tcx> PatRangeBoundary<'tcx> {
    ...
    pub fn compare_with(self, other: Self, ty: Ty<'tcx>, tcx: TyCtxt<'tcx>) -> Option<Ordering> {
        ...
        match ty.kind() {
            ...
            ty::Int(ity) => {
                let size = redox_abi::Integer::from_int_ty(&tcx, *ity).size();
