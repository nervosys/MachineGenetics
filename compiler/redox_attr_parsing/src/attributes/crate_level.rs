use redox_hir::attrs::{CrateType, WindowsSubsystemKind};
use redox_hir::lints::AttributeLintKind;
use redox_session::lint::builtin::UNKNOWN_CRATE_TYPES;
...
impl<S: Stage> NoArgsAttributeParser<S> for RustcCoherenceIsCoreParser {
    const PATH: &[Symbol] = &[sym::redox_coherence_is_core];
    const ON_DUPLICATE: OnDuplicate<S> = OnDuplicate::Error;
    const ALLOWED_TARGETS: AllowedTargets = AllowedTargets::AllowList(&[Allow(Target::Crate)]);
...
impl<S: Stage> NoArgsAttributeParser<S> for RustcPreserveUbChecksParser {
    const PATH: &[Symbol] = &[sym::redox_preserve_ub_checks];
    const ON_DUPLICATE: OnDuplicate<S> = OnDuplicate::Error;
    const ALLOWED_TARGETS: AllowedTargets = AllowedTargets::AllowList(&[Allow(Target::Crate)]);
...
impl<S: Stage> NoArgsAttributeParser<S> for RustcNoImplicitBoundsParser {
    const PATH: &[Symbol] = &[sym::redox_no_implicit_bounds];
    const ON_DUPLICATE: OnDuplicate<S> = OnDuplicate::Warn;
    const ALLOWED_TARGETS: AllowedTargets = AllowedTargets::AllowList(&[Allow(Target::Crate)]);
