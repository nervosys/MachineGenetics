...
use redox_ast::token;
use redox_ast::util::literal::LitError;
...
use redox_target::spec::{SplitDebuginfo, StackProtector, TargetTuple};
...
pub(crate) enum AppleDeploymentTarget {
    ...
    #[diag(
        "deployment target in {$env_var} was set to {$version}, but the minimum supported by `redox` is {$os_min}"
    )]
