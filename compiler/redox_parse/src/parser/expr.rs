...
use redox_ast::token::{self, Delimiter, InvisibleOrigin, MetaVarKind, Token, TokenKind};
use redox_ast::tokenstream::TokenTree;
use redox_ast::util::case::Case;
use redox_ast::util::classify;
use redox_ast::util::parser::{AssocOp, ExprPrecedence, Fixity, prec_let_scrutinee_needs_par};
...
use redox_span::{BytePos, ErrorGuaranteed, Ident, Pos, Span, Spanned, Symbol, kw, respan, sym};
    ...
    fn interpolated_or_expr_span(&self, expr: &Expr) -> Span {
        match self.prev_token.kind {
            ...
            token::CloseInvisible(InvisibleOrigin::MetaVar(_)) => {
                ...
                // `redox_expand::mbe::transcribe::transcribe`.)
    ...
    fn parse_expr_bottom(&mut self) -> PResult<'a, Box<Expr>> {
        ...
        if let Some(expr) = self.eat_metavar_seq_with_matcher(
            ...
            |this| {
                ...
                // `compiler/redox_index/src/bit_set/tests.rs`.
                    ...
                    this.expect_one_of(&[exp!(Comma)], &[exp!(CloseBrace)]).map_err(|mut err| {
                        if this.token == token::FatArrow {
                                ...
                                && expr_lines.lines.len() == 2
                            {
                                ...
                                } else if arm_start_lines.lines[0].end_col + redox_span::CharPos(1)
                                    == expr_lines.lines[0].end_col
                                {
                                    // similar to the above, but we may typo a `.` or `/` at the end of the line
                                    let comma_span = arm_start_span
                                        .shrink_to_hi()
                                        .with_hi(arm_start_span.hi() + redox_span::BytePos(1));
    ...
    pub(crate) fn mk_closure_expr(&self, span: Span, body: Box<Expr>) -> Box<Expr> {
        self.mk_expr(
            ...
            ast::ExprKind::Closure(Box::new(ast::Closure {
                binder: redox_ast::ClosureBinder::NotPresent,
                constness: redox_ast::Const::No,
                movability: redox_ast::Movability::Movable,
                capture_clause: redox_ast::CaptureBy::Ref,
                coroutine_kind: None,
                fn_decl: Box::new(redox_ast::FnDecl {
                    inputs: Default::default(),
                    output: redox_ast::FnRetTy::Default(span),
                }),
                fn_arg_span: span,
                fn_decl_span: span,
                body,
