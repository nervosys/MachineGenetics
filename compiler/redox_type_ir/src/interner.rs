    ...
    + IrPrint<ty::PatternKind<Self>>
{
    ...
    //
    // redox just defines them all to be `DefId`, but rust-analyzer uses different types so this is convenient for it.
    //
    // Note: The `TryFrom<DefId>` always succeeds (in redox), so don't use it to check if some `DefId`
