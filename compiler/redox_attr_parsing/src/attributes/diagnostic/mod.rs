...
use redox_errors::E0232;
use redox_hir::AttrPath;
use redox_hir::attrs::diagnostic::{
...
fn merge_directives<S: Stage>(
    ...
    if let Some((_, first)) = first {
        if first.is_redox_attr || later.1.is_redox_attr {
...
fn parse_directive_items<'p, S: Stage>(
    ...
    Some(Directive {
        is_redox_attr: matches!(mode, Mode::RustcOnUnimplemented),
        condition,
        subcommands,
        message,
        label,
