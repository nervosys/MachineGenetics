...
Unlike PGO (controlled by the `redox` flags `-Cprofile-generate` and
...
```shell
redox -O -Zdebug-info-for-profiling main.rs -o main
perf record -b ./main
create_llvm_prof --binary=main --out=code.prof
redox -O -Zprofile-sample-use=code.prof main.rs -o main2
```
...
used by `redox` to guide optimizations in producing the binary `main2`.
