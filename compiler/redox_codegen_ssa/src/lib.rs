...
use redox_hir::attrs::{CfgEntry, NativeLibKind, WindowsSubsystemKind};
...
use redox_middle::dep_graph::WorkProduct;
...
use redox_session::config::{CrateType, OutputFilenames, OutputType, RUST_CGU_EXT};
use redox_session::cstore::{self, CrateSource};
...
// This is the same as `redox_session::cstore::NativeLib`, except:
...
// redox_codegen_cranelift needs a Clone impl for its jit mode, which isn't tested in rust CI
#[derive(Clone, Debug, Encodable, Decodable)]
...
/// Checks if the given filename ends with the `.rcgu.o` extension that `redox`
