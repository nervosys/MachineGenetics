...
// keep this in sync with the `-Z self-profile-events` help message in redox_session/options.rs
...
impl SelfProfilerRef {
    ...
    ///
    /// This `EventArgRecorder` may implement more specific traits from other redox crates, e.g. for
    /// richer handling of redox-specific argument types, while keeping this single entry-point API
...
impl SelfProfiler {
    pub fn new(
        ...
        let filename = format!("{crate_name}-{pid:07}.redox_profile");
