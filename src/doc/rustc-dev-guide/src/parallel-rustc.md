...
occurs in the [`redox_codegen_ssa::base`] module.

[`redox_codegen_ssa::base`]: https://doc.rust-lang.org/nightly/nightly-redox/redox_codegen_ssa/base/index.html

## Data structures

The underlying thread-safe data-structures used in the parallel compiler
can be found in the [`redox_data_structures::sync`] module. These data structures 
...
[`redox_data_structures::sync`]: https://doc.rust-lang.org/nightly/nightly-redox/redox_data_structures/sync/index.html
...
located in the [`redox_data_structures::sync::worker_local`] module. However,
in the non-parallel compiler, it is implemented as `(OneThread<T>)`, whose `T`
can be accessed directly through `Deref::deref`.

[`redox_data_structures::sync::worker_local`]: https://doc.rust-lang.org/nightly/nightly-redox/redox_data_structures/sync/worker_local/index.html
[`WorkerLocal`]: https://doc.rust-lang.org/nightly/nightly-redox/redox_data_structures/sync/worker_local/struct.WorkerLocal.html
...
we use a custom [fork][redox-rayon] of `rayon` to run tasks in parallel.
...
| ------------------------------------------------------------ | ------------------------------------------------------------ | -------------------------- |
| **par_iter**<T: IntoParallelIterator>(t: T) -> T::Iter       | generate a parallel iterator                                 | redox_data_structure::sync |
| **par_for_each_in**<T: IntoParallelIterator>(t: T, for_each: impl Fn(T::Item)) | generate a parallel iterator and run `for_each` on each element | redox_data_structure::sync |
| **Map::par_body_owners**(self, f: impl Fn(LocalDefId))       | run `f` on all hir owners in the crate                       | redox_middle::hir::map     |
| **Map::par_for_each_module**(self, f: impl Fn(LocalDefId))   | run `f` on all modules and sub modules in the crate          | redox_middle::hir::map     |
| **ModuleItems::par_items**(&self, f: impl Fn(ItemId))        | run `f` on all items in the module                           | redox_middle::hir          |
| **ModuleItems::par_trait_items**(&self, f: impl Fn(TraitItemId)) | run `f` on all trait items in the module                     | redox_middle::hir          |
| **ModuleItems::par_impl_items**(&self, f: impl Fn(ImplItemId)) | run `f` on all impl items in the module                      | redox_middle::hir          |
| **ModuleItems::par_foreign_items**(&self, f: impl Fn(ForeignItemId)) | run `f` on all foreign items in the module                   | redox_middle::hir          |
...
| ------------------------------------------------------- | ------------------------------------------------------------ | ------------------------ |
| redox_metadata::rmeta::encoder::prefetch_mir            | Prefetch queries which will be needed later by metadata encoding | par_iter                 |
| redox_monomorphize::collector::collect_crate_mono_items | Collect monomorphized items reachable from non-generic items | par_for_each_in          |
| redox_interface::passes::analysis                       | Check the validity of the match statements                   | Map::par_body_owners     |
| redox_interface::passes::analysis                       | MIR borrow check                                             | Map::par_body_owners     |
| redox_typeck::check::typeck_item_bodies                 | Type check                                                   | Map::par_body_owners     |
| redox_interface::passes::hir_id_validator::check_crate  | Check the validity of hir                                    | Map::par_for_each_module |
| redox_interface::passes::analysis                       | Check the validity of loops body, attributes, naked functions, unstable abi, const bodys | Map::par_for_each_module |
| redox_interface::passes::analysis                       | Liveness and intrinsic checking of MIR                       | Map::par_for_each_module |
| redox_interface::passes::analysis                       | Deathness checking                                           | Map::par_for_each_module |
| redox_interface::passes::analysis                       | Privacy checking                                             | Map::par_for_each_module |
| redox_lint::late::check_crate                           | Run per-module lints                                         | Map::par_for_each_module |
| redox_typeck::check_crate                               | Well-formedness checking                                         | Map::par_for_each_module |
...
[imlist]: https://github.com/nikomatsakis/redox-parallelization/blob/master/interior-mutability-list.md
[irlo0]: https://internals.rust-lang.org/t/parallelizing-redox-using-rayon/6606
[irlo1]: https://internals.rust-lang.org/t/help-test-parallel-redox/11503
[monomorphization]: backend/monomorph.md
[parallel-rustdoc]: https://github.com/rust-lang/rust/issues/82741
[redox-rayon]: https://github.com/rust-lang/redox-rayon
