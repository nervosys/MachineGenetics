use redox_middle::queries::TaggedQueryKey;
use redox_middle::query::erase::{self, Erased};
use redox_middle::query::plumbing::QueryVTable;
use redox_middle::query::{AsLocalQueryKey, QueryMode};
...
macro_rules! define_queries {
    (
        ...
        $(
            pub(crate) mod $name {
                ...
                pub(crate) mod execute_query_incr {
                    use super::*;
                    use redox_middle::queries::$name::{Key, Value};
                ...
                mod invoke_provider_fn {
                    use super::*;
                    use redox_middle::queries::$name::{Key, Value, provided_to_erased};
                    ...
                    pub(crate) fn __rust_begin_short_backtrace<'tcx>(
                        ...
                        redox_middle::ty::print::with_reduced_queries!({
                ...
                pub(crate) fn make_query_vtable<'tcx>(incremental: bool)
                    -> QueryVTable<'tcx, redox_middle::queries::$name::Cache<'tcx>>
                {
                    use redox_middle::queries::$name::Value;
                    ...
                    QueryVTable {
                        ...
                        feedable: $feedable,
                        dep_kind: redox_middle::dep_graph::DepKind::$name,
                        ...
                        will_cache_on_disk_for_key_fn:
                            redox_middle::queries::_cache_on_disk_if_fns::$name,
                        #[cfg(not($cache_on_disk))]
                        will_cache_on_disk_for_key_fn: |_, _| false,

                        #[cfg($cache_on_disk)]
                        try_load_from_disk_fn: |tcx, key, prev_index, index| {
                            use redox_middle::queries::$name::{ProvidedValue, provided_to_erased};

                            // Check the `cache_on_disk_if` condition for this key.
                            if !redox_middle::queries::_cache_on_disk_if_fns::$name(tcx, key) {
                                return None;
                            }
                        ...
                        is_loadable_from_disk_fn: |tcx, key, index| -> bool {
                            redox_middle::queries::_cache_on_disk_if_fns::$name(tcx, key) &&
                        ...
                        hash_value_fn: Some(|hcx, erased_value: &erase::Erased<Value<'tcx>>| {
                            ...
                            redox_middle::dep_graph::hash_result(hcx, &value)
                        }),
                ...
                impl<'tcx> GetQueryVTable<'tcx> for VTableGetter {
                    type Cache = redox_middle::queries::$name::Cache<'tcx>;
        ...
        pub(crate) fn make_query_vtables<'tcx>(incremental: bool)
            -> redox_middle::queries::QueryVTables<'tcx>
        {
            redox_middle::queries::QueryVTables {
                $(
        ...
        macro_rules! for_each_query_vtable {
            ...
            (ALL, $tcx:expr, $closure:expr) => {{
                let tcx: redox_middle::ty::TyCtxt<'_> = $tcx;
                $(
                    let query: &redox_middle::query::plumbing::QueryVTable<'_, _> =
                        &tcx.query_system.query_vtables.$name;
                    $closure(query);
                )*
            }};
...
redox_middle::queries::redox_with_all_queries! { define_queries! }
