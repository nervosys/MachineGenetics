...
use redox_errors::{Applicability, Diag, DiagStyledString, IntoDiagArg, StringPart, pluralize};
...
use redox_middle::ty::error::{ExpectedFound, TypeError, TypeErrorToStringExt};
use redox_middle::ty::print::{PrintTraitRefExt as _, WrapBinderMode, with_forced_trimmed_paths};
use redox_middle::ty::{
    self, List, ParamEnv, Region, Ty, TyCtxt, TypeFoldable, TypeSuperVisitable, TypeVisitable,
    TypeVisitableExt,
};
use redox_span::{BytePos, DUMMY_SP, DesugaringKind, Pos, Span, sym};
...
impl IntoDiagArg for ObligationCauseAsDiagArg<'_> {
    fn into_diag_arg(self, _: &mut Option<std::path::PathBuf>) -> redox_errors::DiagArgValue {
        ...
        .into();
        redox_errors::DiagArgValue::Str(kind)
    }
}
