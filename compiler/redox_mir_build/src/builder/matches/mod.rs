...
use redox_abi::{FIRST_VARIANT, FieldIdx, VariantIdx};
use redox_data_structures::fx::FxIndexMap;
use redox_data_structures::stack::ensure_sufficient_stack;
use redox_hir::{BindingMode, ByRef, LangItem, LetStmt, LocalSource, Node};
use redox_middle::middle::region::{self, TempLifetime};
use redox_middle::mir::*;
use redox_middle::thir::{self, *};
use redox_middle::ty::{self, CanonicalUserTypeAnnotation, Ty, ValTree, ValTreeKind};
use redox_middle::{bug, span_bug};
use redox_pattern_analysis::constructor::RangeEnd;
use redox_pattern_analysis::redox::{DeconstructedPat, RustcPatCtxt};
...
impl<'a, 'tcx> Builder<'a, 'tcx> {
    ...
    ///
    /// In redox, we opt instead for the "backtracking automaton" approach. This guarantees we never
...
impl<'a, 'tcx> Builder<'a, 'tcx> {
    ...
    fn bind_and_guard_matched_candidate(
            ...
            && let Some(guard) = arm.guard
        {
            ...
            let mut guard_span = redox_span::DUMMY_SP;
    ...
    pub(crate) fn static_pattern_match(
        ...
        for (&arm_id, branch) in it {
            ...
            if let redox_pattern_analysis::redox::Constructor::Or = pat.ctor() {
    ...
    fn static_pattern_match_inner(
        ...
        use redox_pattern_analysis::constructor::{IntRange, MaybeInfiniteInt};
        use redox_pattern_analysis::redox::Constructor;
