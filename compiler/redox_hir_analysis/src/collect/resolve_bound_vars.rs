...
use redox_middle::middle::resolve_bound_vars::*;
use redox_middle::query::Providers;
use redox_middle::ty::{self, TyCtxt, TypeSuperVisitable, TypeVisitor};
use redox_middle::{bug, span_bug};
use redox_span::def_id::{DefId, LocalDefId};
use redox_span::{Ident, Span, sym};
...
impl<'a, 'tcx> Visitor<'tcx> for BoundVarContext<'a, 'tcx> {
    ...
    fn visit_opaque_ty(&mut self, opaque: &'tcx redox_hir::OpaqueTy<'tcx>) {
...
impl<'a, 'tcx> BoundVarContext<'a, 'tcx> {
        ...
        let result = loop {
            match *scope {
                ...
                Scope::Binder { ref bound_vars, scope_type, s, where_bound_origin, .. } => {
                    ...
                    {
                        let mut diag: redox_errors::Diag<'_> = redox_session::parse::feature_err(
                            &self.tcx.sess,
    ...
    fn emit_opaque_capture_errors(&self) -> Option<ErrorGuaranteed> {
        ...
        let mut span = redox_errors::MultiSpan::from_span(errors.capture_spans[0]);
        for &capture_span in &errors.capture_spans[1..] {
            span.push_span_label(capture_span, "");
        }
        let decl_span = redox_errors::MultiSpan::from_spans(errors.decl_spans);
            ...
            let obligations = predicates.iter_identity_copied().filter_map(|(pred, _)| {
                ...
                match bound_predicate.skip_binder() {
                    ty::ClauseKind::Trait(data) => {
                        ...
                        // `redox_middle::ty::predicate::Clause::instantiate_supertrait`
