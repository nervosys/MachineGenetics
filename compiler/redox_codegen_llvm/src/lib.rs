...
use redox_session::config::{OptLevel, OutputFilenames, PrintKind, PrintRequest};
use redox_span::{Symbol, sym};
use redox_target::spec::{RelocModel, TlsModel};
...
impl CodegenBackend for LlvmCodegenBackend {
    ...
    fn init(&self, sess: &Session) {
        ...
        {
            use redox_session::config::AutoDiff;
...
"#
    ...
    fn codegen_crate<'tcx>(&self, tcx: TyCtxt<'tcx>, crate_info: &CrateInfo) -> Box<dyn Any> {
        Box::new(redox_codegen_ssa::base::codegen_crate(LlvmCodegenBackend(()), tcx, crate_info))
    }
    ...
    fn join_codegen(
        ...
        let (compiled_modules, work_products) = ongoing_codegen
            .downcast::<redox_codegen_ssa::back::write::OngoingCodegen<LlvmCodegenBackend>>()
