    ...
    parallel_guard(|guard: &ParallelGuard| {
        if mode::is_dyn_thread_safe() {
            ...
            redox_thread_pool::scope(|s| {
                ...
                // that of a single threaded redox.
...
pub fn broadcast<R: DynSend>(op: impl Fn(usize) -> R + DynSync) -> Vec<R> {
    if mode::is_dyn_thread_safe() {
        ...
        let results = redox_thread_pool::broadcast(|context| op.derive(op(context.index())));
