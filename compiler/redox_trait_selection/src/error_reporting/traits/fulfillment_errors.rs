...
use redox_ast::{LitIntType, TraitObjectSyntax};
...
use redox_hir::attrs::diagnostic::{AppendConstMessage, OnUnimplementedNote};
use redox_hir::def_id::{DefId, LOCAL_CRATE, LocalDefId};
use redox_hir::intravisit::Visitor;
use redox_hir::{self as hir, LangItem, Node, find_attr};
use redox_infer::infer::{InferOk, TypeTrace};
use redox_infer::traits::ImplSource;
use redox_infer::traits::solve::Goal;
use redox_middle::traits::SignatureMismatchData;
use redox_middle::traits::select::OverflowError;
use redox_middle::ty::abstract_const::NotConstEvaluatable;
use redox_middle::ty::error::{ExpectedFound, TypeError};
use redox_middle::ty::print::{
...
use redox_span::def_id::CrateNum;
use redox_span::{BytePos, DUMMY_SP, STDLIB_STABLE_CRATES, Span, Symbol, sym};
    ...
    fn emit_specialized_closure_kind_error(
        ...
        if has_self_borrows && expected_kind != ty::ClosureKind::FnOnce {
            let coro_kind = match self
                ...
                .unwrap()
            {
                redox_hir::CoroutineKind::Desugared(desugaring, _) => desugaring.to_string(),
                coro => coro.to_string(),
            };
...
impl<'a, 'tcx> TypeErrCtxt<'a, 'tcx> {
    ...
    fn select_transmute_obligation_for_reporting(
        ...
        let Some(assume) = redox_transmute::Assume::from_const(self.tcx, assume) else {
            return (obligation.clone(), trait_predicate);
        };

        let is_normalized_yes = matches!(
            redox_transmute::TransmuteTypeEnv::new(self.tcx).is_transmutable(
                trait_ref.args.type_at(1),
                trait_ref.args.type_at(0),
                assume,
            ),
            redox_transmute::Answer::Yes,
        );
        ...
        self.probe(|_| {
            ...
            let Ok(assume) = ocx.structurally_normalize_const(
                ...
                self.dcx().span_delayed_bug(
                    span,
                    "Unable to construct redox_transmute::Assume where it was previously possible",
                );
                return GetSafeTransmuteErrorAndReason::Silent;
            };

            let Some(assume) = redox_transmute::Assume::from_const(self.infcx.tcx, assume) else {
                self.dcx().span_delayed_bug(
                    span,
                    "Unable to construct redox_transmute::Assume where it was previously possible",
                );
            ...
            match redox_transmute::TransmuteTypeEnv::new(self.infcx.tcx)
                .is_transmutable(src, dst, assume)
            {
                Answer::No(reason) => {
                    let safe_transmute_explanation = match reason {
                        redox_transmute::Reason::SrcIsNotYetSupported => {
                            format!("analyzing the transmutability of `{src}` is not yet supported")
                        }
                        redox_transmute::Reason::DstIsNotYetSupported => {
                            format!("analyzing the transmutability of `{dst}` is not yet supported")
                        }
                        redox_transmute::Reason::DstIsBitIncompatible => {
                            format!(
                                "at least one value of `{src}` isn't a bit-valid value of `{dst}`"
                            )
                        }
                        redox_transmute::Reason::DstUninhabited => {
                            format!("`{dst}` is uninhabited")
                        }
                        redox_transmute::Reason::DstMayHaveSafetyInvariants => {
                            format!("`{dst}` may carry safety invariants")
                        }
                        redox_transmute::Reason::DstIsTooBig => {
                            format!("the size of `{src}` is smaller than the size of `{dst}`")
                        }
                        redox_transmute::Reason::DstRefIsTooBig {
                            src,
                            src_size,
                            dst,
                            dst_size,
                        ...
                        redox_transmute::Reason::DstIsMoreUnique => {
                            format!(
                                "`{src}` is a shared reference, but `{dst}` is a unique reference"
                            )
                        }
                        // Already reported by redox
                        redox_transmute::Reason::TypeError => {
                            return GetSafeTransmuteErrorAndReason::Silent;
                        }
                        redox_transmute::Reason::SrcLayoutUnknown => {
                            format!("`{src}` has an unknown layout")
                        }
                        redox_transmute::Reason::DstLayoutUnknown => {
                ...
                Answer::Yes => span_bug!(
                    span,
                    "Inconsistent redox_transmute::is_transmutable(...) result, got Yes",
                ),
