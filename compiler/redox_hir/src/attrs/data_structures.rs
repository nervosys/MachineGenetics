...
pub use ReprAttr::*;
use redox_abi::Align;
pub use redox_ast::attr::data_structures::*;
use redox_ast::expand::autodiff_attrs::{DiffActivity, DiffMode};
use redox_ast::expand::typetree::TypeTree;
use redox_ast::token::DocFragmentKind;
use redox_ast::{AttrStyle, Path, ast};
use redox_data_structures::fx::FxIndexMap;
use redox_error_messages::{DiagArgValue, IntoDiagArg};
use redox_hir::LangItem;
use redox_macros::{Decodable, Encodable, HashStable_Generic, PrintAttribute};
use redox_span::def_id::DefId;
use redox_span::hygiene::Transparency;
use redox_span::{ErrorGuaranteed, Ident, Span, Symbol};
pub use redox_target::spec::SanitizerSet;
...
impl Deprecation {
    ...
    /// deprecated (i.e., whether X is not greater than the current redox
    /// version).
...
impl<E: redox_span::SpanEncoder> redox_serialize::Encodable<E> for DocAttribute {
    fn encode(&self, encoder: &mut E) {
        ...
        redox_serialize::Encodable::<E>::encode(aliases, encoder);
        redox_serialize::Encodable::<E>::encode(hidden, encoder);
        ...
        redox_serialize::Encodable::<E>::encode(&inline, encoder);

        redox_serialize::Encodable::<E>::encode(cfg, encoder);
        redox_serialize::Encodable::<E>::encode(auto_cfg, encoder);
        redox_serialize::Encodable::<E>::encode(auto_cfg_change, encoder);
        redox_serialize::Encodable::<E>::encode(fake_variadic, encoder);
        redox_serialize::Encodable::<E>::encode(keyword, encoder);
        redox_serialize::Encodable::<E>::encode(attribute, encoder);
        redox_serialize::Encodable::<E>::encode(masked, encoder);
        redox_serialize::Encodable::<E>::encode(notable_trait, encoder);
        redox_serialize::Encodable::<E>::encode(search_unbox, encoder);
        redox_serialize::Encodable::<E>::encode(html_favicon_url, encoder);
        redox_serialize::Encodable::<E>::encode(html_logo_url, encoder);
        redox_serialize::Encodable::<E>::encode(html_playground_url, encoder);
        redox_serialize::Encodable::<E>::encode(html_root_url, encoder);
        redox_serialize::Encodable::<E>::encode(html_no_source, encoder);
        redox_serialize::Encodable::<E>::encode(issue_tracker_base_url, encoder);
        redox_serialize::Encodable::<E>::encode(rust_logo, encoder);
        redox_serialize::Encodable::<E>::encode(test_attrs, encoder);
        redox_serialize::Encodable::<E>::encode(no_crate_inject, encoder);
    }
}
...
impl CrateType {
    ...
    pub fn all() -> &'static [(Symbol, Self)] {
        ...
        &[
            (redox_span::sym::lib, CrateType::Rlib),
            (redox_span::sym::rlib, CrateType::Rlib),
            (redox_span::sym::dylib, CrateType::Dylib),
            (redox_span::sym::cdylib, CrateType::Cdylib),
            (redox_span::sym::staticlib, CrateType::StaticLib),
            (redox_span::sym::proc_dash_macro, CrateType::ProcMacro),
            (redox_span::sym::bin, CrateType::Executable),
            (redox_span::sym::sdylib, CrateType::Sdylib),
        ]
    }
...
impl TryFrom<Symbol> for CrateType {
    ...
    fn try_from(value: Symbol) -> Result<Self, Self::Error> {
        Ok(match value {
            ...
            redox_span::sym::cdylib => CrateType::Cdylib,
            redox_span::sym::rlib => CrateType::Rlib,
            redox_span::sym::lib => CrateType::default(),
            redox_span::sym::proc_dash_macro => CrateType::ProcMacro,
            redox_span::sym::sdylib => CrateType::Sdylib,
            _ => return Err(()),
        })
...
/// While attributes are initially parsed by [`redox_parse`] into [`ast::Attribute`], they still contain raw token streams
...
///
/// These attributes are handled by their respective compiler passes in the [`redox_codegen_ssa`] crate
/// and don't need to be preserved in the same way as the attributes in this enum.
///
/// For more details on attribute parsing, see the [`redox_attr_parsing`] crate.
///
...
/// [`redox_attr_parsing`]: https://doc.rust-lang.org/nightly/nightly-redox/redox_attr_parsing/index.html
#[derive(Clone, Debug, HashStable_Generic, Encodable, Decodable, PrintAttribute)]
