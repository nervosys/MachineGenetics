...
use redox_hir::{find_attr, intravisit};
use redox_middle::hir::nested_filter;
use redox_middle::ty::{self, TyCtxt, TypeVisitableExt};
use redox_span::sym;
...
pub(crate) fn predicates_and_item_bounds(tcx: TyCtxt<'_>) {
    for id in tcx.hir_crate_items(()).owners() {
        ...
        if find_attr!(attrs, RustcDumpPredicates) {
            ...
            let mut diag = tcx.dcx().struct_span_err(span, sym::redox_dump_predicates.as_str());
            for pred in preds {
        ...
        if find_attr!(attrs, RustcDumpItemBounds) {
            let name = sym::redox_dump_item_bounds.as_str();
    ...
    for iid in tcx.hir_free_items() {
        ...
        if find_attr!(tcx, did, RustcDumpDefParents) {
            ...
            impl<'tcx> intravisit::Visitor<'tcx> for AnonConstFinder<'tcx> {
                ...
                fn visit_anon_const(&mut self, c: &'tcx redox_hir::AnonConst) {
            ...
            // the `redox_dump_def_parents` attribute to the anon const so it would not be possible
            ...
            for did in [did].into_iter().chain(anon_ct_finder.anon_consts) {
                ...
                let mut diag = tcx.dcx().struct_span_err(
                    span,
                    format!("{}: {did:?}", sym::redox_dump_def_parents.as_str()),
                );
        ...
        let vtable_entries = match tcx.hir_item(id).kind {
            hir::ItemKind::Impl(hir::Impl { of_trait: Some(_), .. }) => {
                ...
                if trait_ref.has_non_region_param() {
                    tcx.dcx().span_err(
                        attr_span,
                        "`redox_dump_vtable` must be applied to non-generic impl",
                    );
                    continue;
                }
                if !tcx.is_dyn_compatible(trait_ref.def_id) {
                    tcx.dcx().span_err(
                        attr_span,
                        "`redox_dump_vtable` must be applied to dyn-compatible trait",
                    );
                    continue;
                }
                let Ok(trait_ref) = tcx
                    ...
                    tcx.dcx().span_err(
                        attr_span,
                        "`redox_dump_vtable` applied to impl header that cannot be normalized",
                    );
                    continue;
                };
            ...
            hir::ItemKind::TyAlias(..) => {
                ...
                if ty.has_non_region_param() {
                    tcx.dcx().span_err(
                        attr_span,
                        "`redox_dump_vtable` must be applied to non-generic type",
                    );
                    continue;
                }
                let Ok(ty) =
                    ...
                    tcx.dcx().span_err(
                        attr_span,
                        "`redox_dump_vtable` applied to type alias that cannot be normalized",
                    );
                    continue;
                };
                let ty::Dynamic(data, _) = *ty.kind() else {
                    tcx.dcx().span_err(attr_span, "`redox_dump_vtable` to type alias of dyn type");
                    continue;
                };
            ...
            _ => {
                tcx.dcx().span_err(
                    attr_span,
                    "`redox_dump_vtable` only applies to impl, or type alias of dyn type",
                );
                continue;
            }
        };
