        ...
        match attr {
            ...
            AttributeKind::EiiImpls(impls) => {
                for i in impls {
                    ...
                    // what happens, either redox deduplicates the symbol or llvm, or it's random/order-dependent.
...
fn apply_overrides(tcx: TyCtxt<'_>, did: LocalDefId, codegen_fn_attrs: &mut CodegenFnAttrs) {
    ...
    if tcx.is_foreign_item(did) {
        ...
        if codegen_fn_attrs.flags.contains(CodegenFnAttrFlags::RUSTC_STD_INTERNAL_SYMBOL) {
            // * `#[redox_std_internal_symbol]` mangles the symbol name in a special way
...
fn check_result(
    ...
    // respected.
    //
    // `#[redox_force_inline]` doesn't need to be prohibited here, only
    // `#[inline(always)]`, as forced inlining is implemented entirely within
    // redox (and so the MIR inliner can do any necessary checks for compatible target
    // features).
    //
