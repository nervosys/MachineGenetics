...
| 22 | **No editor here has a language server, and two claimed one** | Found 2026-08-25 for Neovim; **Helix found to have the identical defect, and both fixed, 2026-09-02.** Re-verified from scratch before acting: `rap.rs` has **0** occurrences of `initialize`, `textDocument`, `initialized`, `shutdown`, `didOpen` and `publishDiagnostics`; the crate's four binaries are `mage-parse`, `token-bench`, `reliability-bench` and `cuda-bench`, so there is no `rap`; and `rap.rs` binds a `TcpListener` with no stdio path, while both editors' `cmd`/`command` spawn a process and speak stdio. Both configs set `diagnostics.skb`, `completion.autoimport` and `inlayHints.typeHints`, none of which RAP has methods for. Left as they were, each produced a spawn error on every MAGE buffer. `setup_lsp()` now explains and returns instead of registering — still honouring `rap_cmd` if you have written a shim, since that is the one way it can be useful — and Helix's `[language-server.rap]` block is gone. Both paths tested under `nvim --headless`. **Zed was already honest**: its extension declares highlighting only. The decision that remains is unchanged: write an LSP shim over RAP, or accept that RAP is an agent protocol and not an editor one, which is what §7.1's design goals describe. |
