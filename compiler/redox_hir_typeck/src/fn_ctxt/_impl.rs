...
use redox_hir::{self as hir, AmbigArg, ExprKind, GenericArg, HirId, Node, QPath, intravisit};
use redox_hir_analysis::hir_ty_lowering::errors::GenericsArgsErrExtend;
use redox_hir_analysis::hir_ty_lowering::generics::{
    check_generic_arg_count_for_call, lower_generic_args,
};
use redox_hir_analysis::hir_ty_lowering::{
...
use redox_infer::infer::{DefineOpaqueTypes, InferResult};
use redox_lint::builtin::SELF_CONSTRUCTOR_FROM_OUTER_ITEM;
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
                ...
                .is_some_and(|(output, inputs)| {
                    output.is_unit()
                        && inputs
                            ...
                            .is_some_and(redox_ast::Mutability::is_mut)
                })
        };
    ...
    pub(crate) fn instantiate_value_path(
        ...
        let (res, implicit_args) = if let Res::Def(DefKind::ConstParam, def) = res {
            ...
            // self type has any generic types during redox_resolve, which is what we use
    ...
    pub(crate) fn try_structurally_resolve_const(
            ...
            && let ty::ConstKind::Unevaluated(..) = ct.kind()
        {
            ...
            redox_trait_selection::traits::evaluate_const(&self.infcx, ct, self.param_env)
        } else {
            ct
        }
    }
