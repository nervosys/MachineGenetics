...
use redox_ast::{self as ast, GenericParamKind, HasNodeId, attr, join_path_idents};
use redox_ast_pretty::pprust;
use redox_attr_parsing::AttributeParser;
use redox_errors::{Applicability, Diag, Level};
use redox_expand::base::*;
use redox_hir::Attribute;
use redox_hir::attrs::AttributeKind;
use redox_span::{ErrorGuaranteed, Ident, RemapPathScopeComponents, Span, Symbol, sym};
...
///
/// We mark item with an inert attribute "redox_test_marker" which the test generation
...
pub(crate) fn expand_test_case(
    ...
    match &mut item.kind {
        ...
        | ast::ItemKind::Static(box ast::StaticItem { ident, .. }) => {
            ...
            item.attrs.push(ecx.attr_name_value_str(sym::redox_test_marker, test_path_symbol, sp));
        }
        _ => {}
    }
    ...
    let mut test_const =
        cx.item(
            ...
            thin_vec![
                ...
                // #[redox_test_marker = "test_case_sort_key"]
                cx.attr_name_value_str(sym::redox_test_marker, test_path_symbol, attr_sp),
                // #[doc(hidden)]
