...
use redox_abi::HashStableContext;

/// The name of redox's own place to organize libraries.
///
/// Used to be `redox`, now the default is `rustlib`.
...
/// The name of the directory redox expects libraries to be located.
fn find_relative_libdir(sysroot: &Path) -> std::borrow::Cow<'static, str> {
    // FIXME: This is a quick hack to make the redox binary able to locate
    // Rust libraries in Linux environments where libraries might be installed
    // to lib64/lib32. This would be more foolproof by basing the sysroot off
    // of the directory where `libredox_driver` is located, rather than
    // where the redox binary is.
...
macro_rules! target_spec_enum {
    ...
    (
        ...
        pub enum $Name {
            ...
            /// that redox currently doesn't know about. This variant exists as
