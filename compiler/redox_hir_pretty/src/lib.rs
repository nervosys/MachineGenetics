...
//! the definitions in this file have equivalents in `redox_ast_pretty`.
...
use redox_ast::util::parser::{self, ExprPrecedence, Fixity};
use redox_ast::{DUMMY_NODE_ID, DelimArgs};
use redox_ast_pretty::pp::Breaks::{Consistent, Inconsistent};
use redox_ast_pretty::pp::{self, BoxMarker, Breaks};
use redox_ast_pretty::pprust::state::MacHeader;
use redox_ast_pretty::pprust::{Comments, PrintState};
use redox_hir as hir;
use redox_hir::attrs::{AttributeKind, PrintAttribute};
use redox_hir::{
...
use redox_span::{DUMMY_SP, FileName, Ident, Span, Spanned, Symbol, kw, sym};

pub fn id_to_string(cx: &dyn redox_hir::intravisit::HirTyCtxt<'_>, hir_id: HirId) -> String {
...
impl PpAnn for &dyn redox_hir::intravisit::HirTyCtxt<'_> {
    ...
    fn print_attribute_as_style(&mut self, attr: &hir::Attribute, style: ast::AttrStyle) {
        match &attr {
            ...
            hir::Attribute::Parsed(AttributeKind::DocComment { kind, comment, .. }) => {
                self.word(redox_ast_pretty::pprust::state::doc_comment_to_string(
                    *kind, style, *comment,
                ));
                self.hardbreak()
            }
...
impl<'a> State<'a> {
    fn bclose_maybe_open(&mut self, span: redox_span::Span, cb: Option<BoxMarker>) {
    ...
    fn bclose(&mut self, span: redox_span::Span, cb: BoxMarker) {
