//! # RMIL Shape Inference
//!
//! A pre-flight pass that walks an RMIL [`Expr`] pipeline and threads
//! tensor shapes layer-by-layer, catching dimension mismatches before the
//! first compute-backend dispatch.
//!
//! The inferer handles the same op set as [`crate::rmil_compute`]:
//!
//! | Op          | Shape rule                                            |
//! |-------------|-------------------------------------------------------|
...
fn walk(
    ...
    match expr {
        ...
        Expr::Par(a, _b) => {
            // Take the left branch only — matches rmil_compute::walk semantics.
            walk(a, current, mismatches, unknown);
        }
        _ => {}
    }
...
fn apply_op(
    ...
    match op {
        ...
        Op::CONV2D => {
            // Arg schema matches rmil_compute::dispatch_conv2d:
