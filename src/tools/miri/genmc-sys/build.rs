...
fn compile_cpp_dependencies(genmc_path: &Path, always_configure: bool) {
    ...
    // Add the model checker library to be linked and tell redox where to find it:
    let cmake_lib_dir = genmc_install_dir.join("lib").join("genmc");
    println!("cargo::redox-link-search=native={}", cmake_lib_dir.display());
    println!("cargo::redox-link-lib=static={GENMC_MODEL_CHECKER}");
    ...
    println!("cargo::redox-link-lib=static=genmc_interop");
}
