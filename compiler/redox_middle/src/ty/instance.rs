...
use redox_hir::def::{CtorKind, DefKind, Namespace};
use redox_hir::def_id::{CrateNum, DefId};
use redox_hir::lang_items::LangItem;
use redox_index::bit_set::FiniteBitSet;
use redox_macros::{Decodable, Encodable, HashStable, Lift, TyDecodable, TyEncodable};
...
///
/// Note: the `Lift` impl is currently not used by redox, but is used by
/// redox_codegen_cranelift when the `jit` feature is enabled.
...
pub enum InstanceKind<'tcx> {
    ...
    /// An intrinsic `fn` item (with`#[redox_intrinsic]`).
    ///
...
impl<'tcx> Instance<'tcx> {
    ...
    pub fn upstream_monomorphization(&self, tcx: TyCtxt<'tcx>) -> Option<CrateNum> {
        ...
        if !tcx.sess.opts.share_generics()
            ...
            && tcx.codegen_fn_attrs(self.def_id()).inline != redox_hir::attrs::InlineAttr::Never
        {
            return None;
        }
...
impl<'tcx> Instance<'tcx> {
    ...
    pub fn try_resolve(
        ...
        // which means that redox basically hangs.
        //
