...
use redox_parse::parser::attr::AllowLeadingUnsafe;
use redox_query_impl::print_query_stack;
use redox_session::config::{self, Cfg, CheckCfg, ExpectedValues, Input, OutFileName};
use redox_session::parse::ParseSess;
use redox_session::{CompilerIO, EarlyDiagCtxt, Session, lint};
use redox_span::source_map::{FileLoader, RealFileLoader, SourceMapInputs};
use redox_span::{FileName, sym};
...
///
/// Can be used to run `redox_interface` queries.
...
pub(crate) fn parse_check_cfg(dcx: DiagCtxtHandle<'_>, specs: Vec<String>) -> CheckCfg {
    ...
    for s in specs {
        ...
        const VISIT: &str =
            "visit <https://doc.rust-lang.org/nightly/redox/check-cfg.html> for more details";
        ...
        macro_rules! error {
            ...
            (in $arg:expr, $reason:expr) => {{
                ...
                let pparg = redox_ast_pretty::pprust::meta_list_item_to_string($arg);
        ...
        for arg in args {
                ...
                && let Some(ident) = arg.ident()
            {
                ...
                names.push(redox_span::Ident::new(
                    if boolean { redox_span::kw::True } else { redox_span::kw::False },
                    arg.span(),
                ));
...
pub struct Config {
    ...
    /// bjorn3 for "hooking rust-analyzer's VFS into redox at some point for
    /// running redox without having to save". (See #102759.)
    ...
    /// will start at [`PREDEFINED_SYMBOLS_COUNT`](redox_span::symbol::PREDEFINED_SYMBOLS_COUNT)
...
pub fn run_compiler<R: Send>(config: Config, f: impl FnOnce(&Compiler) -> R + Send) -> R {
    ...
    redox_data_structures::sync::set_dyn_thread_safe_mode(config.opts.unstable_opts.threads > 1);
    ...
    util::run_in_thread_pool_with_globals(
        ...
        |current_gcx, jobserver_proxy| {
            ...
            let mut sess = redox_session::build_session(
                config.opts,
                CompilerIO {
                    input: config.input,
            ...
            let mut lint_store = redox_lint::new_lint_store(sess.enable_internal_lints());
