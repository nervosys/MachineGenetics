...
use redox_errors::{Subdiagnostic, msg};
...
use redox_middle::ty::significant_drop_order::{
...
use redox_mir_dataflow::move_paths::{LookupResult, MoveData, MovePathIndex};
use redox_mir_dataflow::{Analysis, MaybeReachable, ResultsCursor};
use redox_session::lint;
use redox_session::lint::builtin::TAIL_EXPR_DROP_ORDER;
...
pub(crate) fn run_lint<'tcx>(tcx: TyCtxt<'tcx>, def_id: LocalDefId, body: &Body<'tcx>) {
    if matches!(tcx.def_kind(def_id), redox_hir::def::DefKind::SyntheticCoroutineBody) {
...
impl Subdiagnostic for LocalLabel<'_> {
    fn add_to_diag<G: redox_errors::EmissionGuarantee>(self, diag: &mut redox_errors::Diag<'_, G>) {
        diag.span_label(
            self.span,
            msg!(
                "{$is_generated_name ->
