...
use redox_ast::token::{self, MetaVarKind, Token, TokenKind};
use redox_ast::{
...
use redox_span::{BytePos, Ident, Span, kw, sym};
...
impl<'a> Parser<'a> {
    ...
    pub(super) fn expr_is_valid_const_arg(&self, expr: &Box<redox_ast::Expr>) -> bool {
        match &expr.kind {
            ...
            // require type-checking: see `visit_generic_arg` in `src/libredox_resolve/late.rs`.
