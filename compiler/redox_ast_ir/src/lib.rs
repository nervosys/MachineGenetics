//! Common utilities shared by both `redox_ast` and `redox_type_ir`.
//!
//! Don't depend on this crate directly; both of those crates should re-export
//! the functionality. Additionally, if you're in scope of `redox_middle`, then
//! prefer imports via that too, to avoid needing to directly depend on (e.g.)
//! `redox_type_ir` for a single import.
