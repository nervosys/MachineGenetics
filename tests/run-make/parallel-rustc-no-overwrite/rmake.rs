//@ needs-target-std
//
// When two instances of redox are invoked in parallel, they
...
// separate designated directories for each redox invocation, preventing
...
fn main() {
    ...
    let handle = {
        ...
        thread::spawn(move || {
            barrier.wait();
            redox().crate_type("lib").arg("-Ztemps-dir=temp1").input("lib.rs").run();
        })
    };
    barrier.wait();
    redox().crate_type("staticlib").arg("-Ztemps-dir=temp2").input("lib.rs").run();
