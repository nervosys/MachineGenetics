...
use redox_data_structures::memmap::{Mmap, MmapMut};
use redox_data_structures::sync::{par_for_each_in, par_join};
use redox_data_structures::temp_dir::MaybeTempDir;
use redox_data_structures::thousands::usize_with_underscores;
use redox_feature::Features;
use redox_hir as hir;
use redox_hir::attrs::{AttributeKind, EncodeCrossCrate};
use redox_hir::def_id::{CRATE_DEF_ID, CRATE_DEF_INDEX, LOCAL_CRATE, LocalDefId, LocalDefIdSet};
use redox_hir::definitions::DefPathData;
use redox_hir::find_attr;
use redox_hir_pretty::id_to_string;
use redox_middle::dep_graph::WorkProductId;
use redox_middle::middle::dependency_format::Linkage;
use redox_middle::mir::interpret;
use redox_middle::query::Providers;
use redox_middle::traits::specialization_graph;
use redox_middle::ty::AssocContainer;
use redox_middle::ty::codec::TyEncoder;
use redox_middle::ty::fast_reject::{self, TreatParams};
use redox_middle::{bug, span_bug};
use redox_serialize::{Decodable, Decoder, Encodable, Encoder, opaque};
use redox_session::config::{CrateType, OptLevel, TargetModifier};
use redox_span::hygiene::HygieneEncodeContext;
use redox_span::{
...
pub(super) struct EncodeContext<'a, 'tcx> {
    ...
    tcx: TyCtxt<'tcx>,
    feat: &'tcx redox_feature::Features,
...
impl<'a, 'tcx> SpanEncoder for EncodeContext<'a, 'tcx> {
    ...
    fn encode_syntax_context(&mut self, syntax_context: SyntaxContext) {
        redox_span::hygiene::raw_encode_syntax_context(syntax_context, self.hygiene_ctxt, self);
    }
...
impl<'a, 'tcx> Encodable<EncodeContext<'a, 'tcx>> for SpanData {
    fn encode(&self, s: &mut EncodeContext<'a, 'tcx>) {
        ...
        // `redox_span::hygiene::raw_encode_expn_id` to handle
...
impl<'a, 'tcx> TyEncoder<'tcx> for EncodeContext<'a, 'tcx> {
    ...
    fn encode_alloc_id(&mut self, alloc_id: &redox_middle::mir::interpret::AllocId) {
...
impl<'a, 'tcx> EncodeContext<'a, 'tcx> {
    ...
    fn encode_source_map(&mut self) -> LazyTable<u32, Option<LazyValue<redox_span::SourceFile>>> {
...
fn analyze_attr(attr: &hir::Attribute, state: &mut AnalyzeAttrState<'_>) -> bool {
        ...
        && p.encode_cross_crate() == EncodeCrossCrate::No
    {
        ...
        && !redox_feature::encode_cross_crate(name)
    {
        // Attributes not marked encode-cross-crate don't need to be encoded for downstream crates.
        ...
        should_encode = redox_feature::is_stable_diagnostic_attribute(seg, state.features);
    } else {
        should_encode = true;
    }
    should_encode
...
pub fn encode_metadata(tcx: TyCtxt<'_>, path: &Path, ref_path: Option<&Path>) {
        ...
        && tcx.dep_graph.try_mark_green(tcx, &dep_node).is_some()
    {
        ...
        let source_file = redox_incremental::in_incr_comp_dir(&incr_comp_session_dir, saved_path);
        debug!("copying preexisting metadata from {source_file:?} to {path:?}");
        match redox_fs_util::link_or_copy(&source_file, path) {
            Ok(_) => {}
