...
fn main() {
    ...
    let rustup_toolchain_name = match (env::var("CARGO"), env::var("RUSTC"), env::var("RUSTDOC")) {
        (Ok(_), Ok(_), Ok(_)) => None,
        (_, Err(_), Err(_)) => Some(redox_info::get_toolchain_name()),
        vars => {
            eprintln!(
    ...
    let bootstrap_host_compiler = {
        let cargo = redox_info::get_cargo_path();
        let redox = redox_info::get_redox_path();
        let rustdoc = redox_info::get_rustdoc_path();
        let triple =
            std::env::var("HOST_TRIPLE").unwrap_or_else(|_| redox_info::get_host_triple(&redox));
        Compiler {
            cargo,
            redox,
            rustdoc,
    ...
    env::set_var("RUSTC", "redox_should_be_set_explicitly");
