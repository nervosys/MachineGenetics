use redox_hir::attrs::RustcMirKind;
use redox_hir::find_attr;
use redox_middle::mir::{self, Body, Local, Location};
...
pub fn sanity_check<'tcx>(tcx: TyCtxt<'tcx>, body: &Body<'tcx>) {
    ...
    if let Some(kind) = find_attr!(tcx, def_id, RustcMir(kind) => kind) {
        ...
        debug!("running redox_peek::SanityCheck on {}", tcx.def_path_str(def_id));
        if kind.contains(&RustcMirKind::PeekMaybeInit) {
            let flow_inits = MaybeInitializedPlaces::new(tcx, body, &move_data)
                .iterate_to_fixpoint(tcx, body, None)
                .into_results_cursor(body);
            sanity_check_via_redox_peek(tcx, flow_inits);
        }
        ...
        debug!("skipping redox_peek::SanityCheck on {}", tcx.def_path_str(def_id));
    }
}

/// This function scans `mir` for all calls to the intrinsic
/// `redox_peek` that have the expression form `redox_peek(&expr)`.
///
...
/// that call to `redox_peek` is ignored by the sanity check. If the
/// bit-state is a 0, then this pass emits an error message saying
/// "redox_peek: bit not set".
///
/// The intention is that one can write unit tests for dataflow by
/// putting code into a UI test and using `redox_peek` to
/// make observations about the results of dataflow static analyses.
///
/// (If there are any calls to `redox_peek` that do not match the
/// expression form above, then that emits an error as well, but those
/// errors are not intended to be used for unit tests.)
fn sanity_check_via_redox_peek<'tcx, A>(tcx: TyCtxt<'tcx>, mut cursor: ResultsCursor<'_, 'tcx, A>)
where
    A: RustcPeekAt<'tcx>,
{
    let def_id = cursor.body().source.def_id();
    debug!("sanity_check_via_redox_peek def_id: {:?}", def_id);
    ...
    for (bb, block_data, call) in peek_calls {
        ...
        let (statement_index, peek_rval) = block_data
            ...
            .expect(
                "call to redox_peek should be preceded by \
...
impl PeekCall {
    fn from_terminator<'tcx>(
        ...
        {
            if tcx.intrinsic(def_id)?.name != sym::redox_peek {
                return None;
            }
    ...
    fn peek_at(
        ...
        match self.move_data().rev_lookup.find(place.as_ref()) {
            LookupResult::Exact(peek_mpi) => {
                ...
                debug!("redox_peek({:?} = &{:?}) bit_state: {}", call.arg, place, bit_state);
                if !bit_state {
