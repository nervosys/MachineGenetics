...
| 26 | **`benchmarks/cross_lang/run.sh` is cited by the README and run by nothing** | Found 2026-09-02. `README.md` says of its benchmark section "Every number below is produced by **actually compiling and running** code ... Reproduce with `benchmarks/cross_lang/run.sh`", and all three benchmark scripts were run by neither CI nor any checker. Two are now wired in and made to assert (see the commit); this one is not, because it shells out to `rustc`, `javac`, `go` and `node`, and a missing toolchain makes it report a language as **failing** rather than absent — the 5/5 executability table would silently become a table about the runner. Wiring it up means first deciding what the benchmark measures when a toolchain is absent, which is a judgement about the claim, not a step to add. Until then the front page's cross-language numbers rest on a manual run from 2026-06-11. |
| 27 | **`test-all.ps1 -CheckDocs` cannot pass, and has not for a long time** | Found 2026-09-02 while checking whether the new benchmark pins broke the Windows path. They did not — it was already broken. The ps1 emits **6 keys** (the five crate counts plus `total`) and pipes them to `check-doc-counts.sh`, which now has **91** pins; the other 46 have nothing to compare against, so it prints `INCOMPLETE - 46 documented count(s) had nothing to compare against; 45 matched` and exits 1. Measured by feeding exactly what the ps1 supplies. The bash twin grew `doc_blocks`, `doc_evals`, `floor_*`, `onto_*`, `rmi_api_items`, `unsafe_*`, `mg_*` and now the benchmark figures; the ps1 grew none of them. CI runs the bash version, so nothing noticed. **The file's own comment states the principle it needs**: "One implementation of the check, in bash, rather than two that can drift" — true of the *check*, and the *measurements* are the half still duplicated. The fix is to lift the emission block out of `test-all.sh` into something both call, rather than porting forty lines of measurement to PowerShell. Not done here: it is a refactor of the harness both CI and the docs depend on, and this branch is already five deep. |
