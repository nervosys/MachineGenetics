...
use redox_abi::{FIRST_VARIANT, FieldIdx, Integer, VariantIdx};
...
use redox_middle::thir::{self, Pat, PatKind, PatRange, PatRangeBoundary};
...
impl<'p, 'tcx: 'p> RustcPatCtxt<'p, 'tcx> {
    ...
    /// `redox_mir_build::thir::pattern::check_match::MatchVisitor::lower_pattern`.
    pub fn lower_pat(&self, pat: &'p Pat<'tcx>) -> DeconstructedPat<'p, 'tcx> {
        ...
        match &pat.kind {
            ...
            PatKind::Range(patrange) => {
                ...
                let end = match end {
                    redox_hir::RangeEnd::Included => RangeEnd::Included,
                    redox_hir::RangeEnd::Excluded => RangeEnd::Excluded,
                };
                ctor = match ty.kind() {
                    ty::Char | ty::Int(_) | ty::Uint(_) => {
    ...
    fn print_pat_range(&self, range: &IntRange, ty: RevealedTy<'tcx>) -> String {
        ...
        if matches!((range.lo, range.hi), (NegInfinity, PosInfinity)) {
            ...
            let mut end = redox_hir::RangeEnd::Included;
            ...
            let hi = if let Some(hi) = range.hi.minus_one() {
                ...
                end = redox_hir::RangeEnd::Excluded;
                range.hi
            };
...
impl<'p, 'tcx: 'p> PatCx for RustcPatCtxt<'p, 'tcx> {
    ...
    fn lint_non_contiguous_range_endpoints(
        ...
        if range.end != redox_hir::RangeEnd::Excluded {
            return;
        }
        ...
        let suggested_range: String = {
            ...
            suggested_range.end = redox_hir::RangeEnd::Included;
