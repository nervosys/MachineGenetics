...
#[proc_macro]
pub fn redox_queries(input: TokenStream) -> TokenStream {
    query::redox_queries(input)
}
...
decl_derive!(
    ...
    /// generics. This is suitable for use in crates like `redox_type_ir`.
...
decl_derive! {
    ...
    /// This macro is pretty specific to `redox_hir::attrs` and likely not that useful in
