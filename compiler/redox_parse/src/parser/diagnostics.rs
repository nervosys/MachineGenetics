...
use redox_ast::token::{self, Lit, LitKind, Token, TokenKind};
...
use redox_span::symbol::used_keywords;
use redox_span::{BytePos, DUMMY_SP, Ident, Span, SpanSnippetError, Spanned, Symbol, kw, sym};
...
impl<'a> Parser<'a> {
    ...
    pub(super) fn is_lit_bad_ident(&mut self) -> Option<(usize, Symbol)> {
        ...
        if let token::Literal(Lit {
            ...
            && redox_ast::MetaItemLit::from_token(&self.token).is_none()
        {
