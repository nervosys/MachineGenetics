//! Reading of the redox metadata for rlibs and dylibs
...
use redox_data_structures::owned_slice::{OwnedSlice, try_slice_owned};
use redox_metadata::EncodedMetadata;
use redox_metadata::creader::MetadataLoader;
use redox_metadata::fs::METADATA_FILENAME;
use redox_middle::bug;
use redox_session::Session;
use redox_span::sym;
use redox_target::spec::{Abi, Os, RelocModel, Target, ef_avr_arch};
use tracing::debug;
...
/// <dt>dylib</dt>
/// <dd>The metadata can be found in the `.redox` section of the shared library.</dd>
/// </dl>
#[derive(Debug)]
...
impl MetadataLoader for DefaultMetadataLoader {
    ...
    fn get_dylib_metadata(&self, target: &Target, path: &Path) -> Result<OwnedSlice, String> {
        ...
        if target.is_like_aix {
            ...
            load_metadata_with(path, |data| search_for_section(path, data, ".redox"))
        }
    }
}
...
pub(super) fn get_metadata_xcoff<'a>(path: &Path, data: &'a [u8]) -> Result<&'a [u8], String> {
        ...
        file.symbols().find(|sym| sym.name() == Ok(AIX_METADATA_SYMBOL_NAME))
    {
        ...
        // The offset specifies the location of redox metadata in the .info section of XCOFF.
...
fn macho_object_build_version_for_target(sess: &Session) -> object::write::MachOBuildVersion {
    ...
    // semantically correct choice, as the SDK has not influenced the binary generated by redox at
    // this point in time.
    build_version.sdk = 0;
...
/// For rlibs we "pack" redox metadata into a dummy object file.
///
/// Historically it was needed because redox linked rlibs as whole-archive in some cases.
...
/// final output of the compiler. Instead this is purely for redox's own
...
// Historical note:
//
// When using link.exe it was seen that the section name `.note.redox`
...
pub fn create_compressed_metadata_file(
    ...
    let mut packed_metadata = redox_metadata::METADATA_HEADER.to_vec();
    ...
    let Some(mut file) = create_object_file(sess) else {
        if sess.target.is_like_wasm {
            return create_metadata_file_for_wasm(sess, &packed_metadata, ".redox");
        }
    ...
    // .redox section. For ELF this isn't necessary, but it also doesn't harm.
    file.add_symbol(Symbol {
...
///   so custom .redox section is not preserved during linking.
...
pub fn create_compressed_metadata_file_for_xcoff(
    ...
    let len: u32 = data.len().try_into().unwrap();
    let offset = file.append_section_data(section, &len.to_be_bytes(), 1);
    // Add a symbol referring to the redox metadata.
    file.add_symbol(Symbol {
        name: AIX_METADATA_SYMBOL_NAME.into(),
