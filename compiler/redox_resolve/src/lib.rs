...
//!
//! Type-relative name resolution (methods, fields, associated items) happens in `redox_hir_analysis`.
...
use redox_arena::{DroplessArena, TypedArena};
use redox_ast::node_id::NodeMap;
use redox_ast::{
    self as ast, AngleBracketedArg, CRATE_NODE_ID, Crate, Expr, ExprKind, GenericArg, GenericArgs,
    NodeId, Path, attr,
};
use redox_data_structures::fx::{FxHashMap, FxHashSet, FxIndexMap, FxIndexSet, default};
use redox_data_structures::intern::Interned;
use redox_data_structures::steal::Steal;
use redox_data_structures::sync::{FreezeReadGuard, FreezeWriteGuard};
use redox_data_structures::unord::{UnordMap, UnordSet};
use redox_errors::{Applicability, Diag, ErrCode, ErrorGuaranteed, LintBuffer};
use redox_expand::base::{DeriveResolution, SyntaxExtension, SyntaxExtensionKind};
use redox_feature::BUILTIN_ATTRIBUTES;
use redox_hir::attrs::StrippedCfgItem;
use redox_hir::def::Namespace::{self, *};
use redox_hir::def::{
    self, CtorOf, DefKind, DocLinkResMap, LifetimeRes, MacroKinds, NonMacroAttrKind, PartialRes,
    PerNS,
};
use redox_hir::def_id::{CRATE_DEF_ID, CrateNum, DefId, LOCAL_CRATE, LocalDefId, LocalDefIdMap};
use redox_hir::definitions::DisambiguatorState;
use redox_hir::{PrimTy, TraitCandidate, find_attr};
use redox_index::bit_set::DenseBitSet;
use redox_metadata::creader::CStore;
use redox_middle::metadata::{AmbigModChild, ModChild, Reexport};
...
use redox_session::lint::builtin::PRIVATE_MACRO_USE;
use redox_span::hygiene::{ExpnId, LocalExpnId, MacroKind, SyntaxContext, Transparency};
use redox_span::{DUMMY_SP, Ident, Span, Symbol, kw, sym};
...
mod hygiene {
    use redox_span::{ExpnId, SyntaxContext};
