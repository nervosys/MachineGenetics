...
use redox_data_structures::sync::DynSend;
use redox_error_messages::{DiagArgMap, DiagArgName, DiagArgValue, IntoDiagArg};
use redox_lint_defs::{Applicability, LintExpectationId};
...
impl EmissionGuarantee for redox_span::fatal_error::FatalError {
    fn emit_producing_guarantee(diag: Diag<'_, Self>) -> Self::EmitResult {
        diag.emit_producing_nothing();
        redox_span::fatal_error::FatalError
    }
}

/// Trait implemented by error types. This is rarely implemented manually. Instead, use
/// `#[derive(Diagnostic)]` -- see [redox_macros::Diagnostic].
///
...
pub struct DiagInner {
    ...
    /// we print where in redox this error was emitted.
...
redox_data_structures::static_assert_size!(Diag<'_, ()>, 3 * size_of::<usize>());
...
impl<'a, G: EmissionGuarantee> Diag<'a, G> {
    ...
    ///
    /// In case of short messages and a simple suggestion, redox displays it as a label:
    ///
    /// ```text
    ...
    /// [redox_macros::Subdiagnostic]). Performs eager formatting of any messages
