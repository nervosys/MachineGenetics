...
use redox_middle::middle::dependency_format::{Dependencies, DependencyList, Linkage};
use redox_middle::ty::TyCtxt;
use redox_session::config::CrateType;
use redox_session::cstore::CrateDepKind;
use redox_session::cstore::LinkagePreference::{self, RequireDynamic, RequireStatic};
use redox_span::sym;
...
fn calculate_type(tcx: TyCtxt<'_>, ty: CrateType) -> DependencyList {
    ...
    if tcx.features().redox_private() {
        // We need this to prevent users of `redox_driver` from linking dynamically to `std`
        // which does not work as `std` is also statically linked into `redox_driver`.
    ...
    for (cnum, kind) in ret.iter_enumerated() {
        ...
        match *kind {
            ...
            kind => {
                ...
                if crate_name.as_str().starts_with("redox_") {
    ...
    match m.get(&cnum) {
        Some(&link2) => {
            ...
            if link2 != link || link == RequireStatic {
                let linking_to_redox_driver = tcx.sess.psess.unstable_features.is_nightly_build()
                    && tcx.crates(()).iter().any(|&cnum| tcx.crate_name(cnum) == sym::redox_driver);
                tcx.dcx().emit_err(CrateDepMultiple {
                    ...
                    non_static_deps: unavailable_as_static
                        ...
                        .collect(),
                    redox_driver_help: linking_to_redox_driver,
                });
            }
        }
        None => {
