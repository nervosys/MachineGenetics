...
use redox_errors::{Applicability, Diag, ErrorGuaranteed, StashKey, msg};
use redox_hir::def::{self, CtorKind, Namespace, Res};
use redox_hir::def_id::DefId;
use redox_hir::{self as hir, HirId, LangItem, find_attr};
use redox_hir_analysis::autoderef::Autoderef;
use redox_infer::infer::BoundRegionConversionTime;
use redox_infer::traits::{Obligation, ObligationCause, ObligationCauseCode};
use redox_middle::ty::adjustment::{
    Adjust, Adjustment, AllowTwoPhase, AutoBorrow, AutoBorrowMutability,
};
use redox_middle::ty::{self, GenericArgsRef, Ty, TyCtxt, TypeVisitableExt};
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    fn confirm_builtin_call(
        ...
        let (fn_sig, def_id) = match *callee_ty.kind() {
            ty::FnDef(def_id, args) => {
                ...
                // `#[redox_evaluate_where_clauses]` trigger special output
                // to let us test the trait evaluation system.
                if self.has_redox_attrs && find_attr!(self.tcx, def_id, RustcEvaluateWhereClauses) {
        ...
        if fn_sig.abi == redox_abi::ExternAbi::RustCall {
    ...
    fn report_invalid_callee(
            ...
            && call_expr.span.contains(callee_expr.span)
        {
            ...
            unit_variant =
                Some((removal_span, descr, redox_hir_pretty::qpath_to_string(&self.tcx, qpath)));
        }
