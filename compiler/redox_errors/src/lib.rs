//! Diagnostics creation and emission for `redox`.
//!
...
use redox_hashes::Hash128;
use redox_lint_defs::LintExpectationId;
pub use redox_lint_defs::{Applicability, listify, pluralize};
pub use redox_macros::msg;
use redox_macros::{Decodable, Encodable};
pub use redox_span::ErrorGuaranteed;
pub use redox_span::fatal_error::{FatalError, FatalErrorMarker, catch_fatal_errors};
...
redox_data_structures::static_assert_size!(PResult<'_, ()>, 24);
#[cfg(target_pointer_width = "64")]
redox_data_structures::static_assert_size!(PResult<'_, bool>, 24);
...
struct DiagCtxtInner {
    ...
    /// Used to suggest redox --explain `<error code>`
...
pub struct DiagCtxtFlags {
    ...
    /// (redox: see `--allow warnings` and `--cap-lints`)
    pub can_emit_warnings: bool,
    /// If Some, the Nth error-level diagnostic is upgraded to bug-level.
    /// (redox: see `-Z treat-err-as-bug`)
...
impl Drop for DiagCtxtInner {
    fn drop(&mut self) {
        // For tools using `interface::run_compiler` (e.g. redox, rustdoc)
...
impl DiagCtxt {
    ...
    ///
    /// NOTE: *do not* call this function from redox. It is only meant to be called from external
...
impl<'a> DiagCtxtHandle<'a> {
        ...
        if can_show_explain && are_there_diagnostics {
            ...
            if !error_codes.is_empty() {
                ...
                if error_codes.len() > 1 {
                    ...
                    let msg2 = format!(
                        "For more information about an error, try `redox --explain {}`.",
                        &error_codes[0]
                    );
                    ...
                    let msg = format!(
                        "For more information about this error, try `redox --explain {}`.",
                        &error_codes[0]
                    );
    ...
    pub fn emit_unused_externs(
        &self,
        lint_level: redox_lint_defs::Level,
        loud: bool,
        unused_externs: &[&str],
    ) {
        let mut inner = self.inner.borrow_mut();
