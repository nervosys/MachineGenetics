...
use redox_mir_dataflow::impls::{MaybeInitializedPlaces, MaybeUninitializedPlaces};
...
impl<'a, 'tcx> DropElaborator<'a, 'tcx> for ElaborateDropsCtxt<'a, 'tcx> {
    ...
    fn field_subpath(&self, path: Self::Path, field: FieldIdx) -> Option<Self::Path> {
        redox_mir_dataflow::move_path_children_matching(self.move_data(), path, |e| match e {
    ...
    fn deref_subpath(&self, path: Self::Path) -> Option<Self::Path> {
        redox_mir_dataflow::move_path_children_matching(self.move_data(), path, |e| {
...
impl<'a, 'tcx> ElaborateDropsCtxt<'a, 'tcx> {
    ...
    fn drop_flags_for_args(&mut self) {
        let loc = Location::START;
        redox_mir_dataflow::drop_flag_effects_for_function_entry(
            self.body,
            &self.env.move_data,
            |path, ds| {
    ...
    fn drop_flags_for_locs(&mut self) {
        ...
        for (bb, data) in self.body.basic_blocks.iter_enumerated() {
            ...
            for i in 0..(data.statements.len() + 1) {
                ...
                redox_mir_dataflow::drop_flag_effects_for_location(
                    self.body,
                    &self.env.move_data,
                    loc,
