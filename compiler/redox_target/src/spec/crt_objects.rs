...
//!
//! Use cases for redox linking the CRT objects explicitly:
//!     - redox needs to add its own Rust-specific objects (mingw is the example)
...
//! However, for some targets (musl, mingw) redox historically provides a more self-contained
//! installation not requiring users to install the native target's toolchain.
//! In that case redox distributes the objects as a part of the target's Rust toolchain
//! and falls back to linking with them manually.
//! Unlike native toolchains, redox only currently adds the libc's objects during linking,
//! but not gcc's. As a result redox cannot link with C++ static libraries (#36710)
