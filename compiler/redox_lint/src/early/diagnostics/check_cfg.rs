...
use redox_session::config::ExpectedValues;
use redox_span::edit_distance::find_best_match_for_name;
use redox_span::{ExpnKind, Ident, Span, Symbol, sym};
...
fn redox_macro_help(span: Span) -> Option<lints::UnexpectedCfgRustcMacroHelp> {
...
pub(super) fn unexpected_cfg_name(
    ...
    let is_from_cargo = redox_session::utils::was_invoked_from_cargo();
    ...
    let invocation_help = if is_from_cargo {
        ...
        lints::unexpected_cfg_name::InvocationHelp::Rustc {
            help,
            macro_help: redox_macro_help(name_span),
        }
    };
...
pub(super) fn unexpected_cfg_value(
    ...
    // defined by redox/Rust itself. Users can still do this if they wish, but should not be
    // encouraged to do so.
    let can_suggest_adding_value = !sess.psess.check_config.well_known_names.contains(&name)
        // Except when working on redox or the standard library itself, in which case we want to
        ...
        || (matches!(sess.psess.unstable_features, redox_feature::UnstableFeatures::Cheat)
