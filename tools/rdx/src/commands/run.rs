/// Run a MechGen program (parse-only at this stage).
pub fn run(args: &[String], verbose: bool) -> Result<(), String> {
    let src_dir = super::find_src_dir()?;

    // Determine main entry point
    let main_rdx = src_dir.join("main.mg");
    if !main_rdx.exists() {
        return Err("src/main.mg not found — only binary projects can be run".into());
    }

    let source = std::fs::read_to_string(&main_rdx)
        .map_err(|e| format!("cannot read main.mg: {e}"))?;

    // Verify main function exists
    if !source.contains("+f main(") && !source.contains("f main(") {
        return Err("no main() function found in src/main.mg".into());
    }
    ...
    println!("[rdx] parse-only mode: full execution requires the MechGen backend");
...
mod tests {
    ...
    fn test_run_requires_main() {
        // run() looks for src/main.mg relative to Forge.toml, which
