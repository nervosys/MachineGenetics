...
use redox_hir::def::{CtorOf, DefKind, Namespace, Res};
...
use redox_middle::ty::adjustment::{Adjust, Adjustment, AutoBorrow, DerefAdjustKind};
use redox_middle::ty::print::{FmtPrinter, PrettyPrinter, Print, Printer};
use redox_middle::ty::{
...
use redox_span::{BytePos, DUMMY_SP, Ident, Span, sym};
...
impl IntoDiagArg for UnderspecifiedArgKind {
    fn into_diag_arg(self, _: &mut Option<std::path::PathBuf>) -> redox_errors::DiagArgValue {
        ...
        redox_errors::DiagArgValue::Str(kind.into())
    }
}
