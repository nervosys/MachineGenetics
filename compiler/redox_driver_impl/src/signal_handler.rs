//! Signal handler for redox
//! Primarily used to extract a backtrace from stack overflow

use std::alloc::{Layout, alloc};
use std::{fmt, mem, ptr, slice};

use redox_interface::util::{DEFAULT_STACK_SIZE, STACK_SIZE};
...
unsafe extern "C" fn print_stack_trace(signum: libc::c_int) {
    ...
    raw_errln!("error: redox interrupted by {signame}, printing backtrace\n");
    ...
    if (cyclic || stack.len() > random_depth()) && signum == libc::SIGSEGV {
        ...
        // redox only arrived in this signal handler because bad things happened
        // and this message is for explaining it's not the programmer's fault
        raw_errln!("note: redox unexpectedly overflowed its stack! this is a bug");
        written += 1;
    }
    ...
    if signum == libc::SIGSEGV {
        ...
        raw_errln!(
            "help: you can increase redox's stack size by setting RUST_MIN_STACK={new_size}"
        );
        written += 1;
    }
    if written > 24 {
