...
use redox_ast::{self as ast, CRATE_NODE_ID};
use redox_attr_parsing::{AttributeParser, Early, ShouldEmit};
use redox_codegen_ssa::traits::CodegenBackend;
use redox_codegen_ssa::{CompiledModules, CrateInfo};
use redox_data_structures::indexmap::IndexMap;
use redox_data_structures::steal::Steal;
use redox_data_structures::sync::{AppendOnlyIndexVec, FreezeLock, WorkerLocal, par_fns};
use redox_data_structures::thousands;
use redox_errors::timings::TimingSection;
use redox_expand::base::{ExtCtxt, LintStoreExpand};
use redox_feature::Features;
use redox_fs_util::try_canonicalize;
use redox_hir::attrs::AttributeKind;
use redox_hir::def_id::{LOCAL_CRATE, StableCrateId, StableCrateIdMap};
use redox_hir::definitions::Definitions;
use redox_hir::limit::Limit;
use redox_hir::{Attribute, find_attr};
use redox_incremental::setup_dep_graph;
use redox_lint::{BufferedEarlyLint, EarlyCheckNode, LintStore, unerased_lint_store};
use redox_metadata::EncodedMetadata;
use redox_metadata::creader::CStore;
use redox_middle::arena::Arena;
use redox_middle::ty::{self, RegisteredTools, TyCtxt};
...
use redox_passes::{abi_test, input_stats, layout_test};
use redox_resolve::{Resolver, ResolverOutputs};
use redox_session::Session;
use redox_session::config::{CrateType, Input, OutFileName, OutputFilenames, OutputType};
use redox_session::cstore::Untracked;
use redox_session::output::{filename_for_input, invalid_output_for_target};
use redox_session::parse::feature_err;
use redox_session::search_paths::PathKind;
use redox_span::{
    DUMMY_SP, ErrorGuaranteed, ExpnKind, SourceFileHash, SourceFileHashAlgorithm, Span, Symbol, sym,
};
use redox_trait_selection::{solve, traits};
...
pub fn parse<'a>(sess: &'a Session) -> ast::Crate {
    ...
    redox_builtin_macros::cmdline_attrs::inject(
        &mut krate,
        &sess.psess,
    ...
    sess.prof.generic_activity_with_arg("pre_AST_expansion_lint_checks", node_name.as_str()).run(
        || {
            redox_lint::check_ast_node(
                ...
                registered_tools,
                None,
                redox_lint::BuiltinCombinedPreExpansionLintPass::new(),
                check_node,
            );
        },
    );
...
fn configure_and_expand(
    ...
    redox_builtin_macros::register_builtin_macros(resolver);

    let num_standard_library_imports = sess.time("crate_injection", || {
        redox_builtin_macros::standard_library_imports::inject(
            &mut krate,
    ...
    krate = sess.time("macro_expand_crate", || {
        ...
        // which runs redox in parallel but has been seen (#33844) to cause
        ...
        let cfg = redox_expand::expand::ExpansionConfig {
            crate_name,
            features,
            recursion_limit,
    ...
    sess.time("maybe_building_test_harness", || {
        redox_builtin_macros::test_harness::inject(&mut krate, sess, features, resolver)
    });

    let has_proc_macro_decls = sess.time("AST_validation", || {
        redox_ast_passes::ast_validation::check_crate(
            sess,
            features,
            &krate,
    ...
    sess.time("maybe_create_a_macro_crate", || {
        ...
        redox_builtin_macros::proc_macro_harness::inject(
            &mut krate,
            sess,
            features,
            resolver,
...
fn print_macro_stats(ecx: &ExtCtxt<'_>) {
    ...
    // redox processes are printing macro-stats at the same time (e.g. with
...
fn early_lint_checks(tcx: TyCtxt<'_>, (): ()) {
    ...
    sess.time("complete_gated_feature_checking", || {
        redox_ast_passes::feature_gate::check_crate(krate, sess, tcx.features());
    });
    ...
    redox_lint::check_ast_node(
        ...
        tcx.registered_tools(()),
        Some(lint_buffer),
        redox_lint::BuiltinCombinedEarlyLintPass::new(),
        (&**krate, &*krate.attrs),
    )
}
            ...
            .map(|fmap| {
                (
                    ...
                    (
                        ...
                        // as external tools wouldn't know how redox normalizes them
...
pub fn write_interface<'tcx>(tcx: TyCtxt<'tcx>) {
    if !tcx.crate_types().contains(&redox_session::config::CrateType::Sdylib) {
        return;
    }
    let _timer = tcx.sess.timer("write_interface");
    let (_, krate) = &*tcx.resolver_for_lowering().borrow();

    let krate = redox_ast_pretty::pprust::print_crate_as_interface(
        krate,
...
pub static DEFAULT_QUERY_PROVIDERS: LazyLock<Providers> = LazyLock::new(|| {
    ...
    providers.queries.hir_crate = redox_ast_lowering::lower_to_hir;
    ...
    redox_expand::provide(&mut providers.queries);
    redox_const_eval::provide(providers);
    redox_middle::hir::provide(&mut providers.queries);
    redox_borrowck::provide(&mut providers.queries);
    redox_incremental::provide(providers);
    redox_mir_build::provide(providers);
    redox_mir_transform::provide(providers);
    redox_monomorphize::provide(providers);
    redox_privacy::provide(&mut providers.queries);
    redox_query_impl::provide(providers);
    redox_resolve::provide(&mut providers.queries);
    redox_hir_analysis::provide(&mut providers.queries);
    redox_hir_typeck::provide(&mut providers.queries);
    ty::provide(&mut providers.queries);
    traits::provide(&mut providers.queries);
    solve::provide(&mut providers.queries);
    redox_passes::provide(&mut providers.queries);
    redox_traits::provide(&mut providers.queries);
    redox_ty_utils::provide(&mut providers.queries);
    redox_metadata::provide(providers);
    redox_lint::provide(&mut providers.queries);
    redox_symbol_mangling::provide(&mut providers.queries);
    redox_codegen_ssa::provide(providers);
    *providers
});

pub fn create_and_enter_global_ctxt<T, F: for<'tcx> FnOnce(TyCtxt<'tcx>) -> T>(
    compiler: &Compiler,
    krate: redox_ast::Crate,
    f: F,
) -> T {
    let sess = &compiler.sess;

    let pre_configured_attrs = redox_expand::config::pre_configure_attrs(sess, &krate.attrs);
    ...
    let query_result_on_disk_cache = redox_incremental::load_query_result_cache(sess);
    ...
    let hir_arena = WorkerLocal::new(|_| redox_hir::Arena::default());
    ...
    TyCtxt::create_global_ctxt(
        ...
        untracked,
        dep_graph,
        redox_query_impl::make_dep_kind_vtables(&arena),
        redox_query_impl::query_system(
            providers.queries,
        ...
        |tcx| {
            ...
            feed.features_query(tcx.arena.alloc(redox_expand::config::features(
                tcx.sess,
                &pre_configured_attrs,
                crate_name,
            )));
...
fn run_required_analyses(tcx: TyCtxt<'_>) {
    if tcx.sess.opts.unstable_opts.input_stats {
        redox_passes::input_stats::print_hir_stats(tcx);
    }
    ...
    redox_passes::hir_id_validator::check_crate(tcx);
    ...
    redox_hir_analysis::check_crate(tcx);
...
pub(crate) fn start_codegen<'tcx>(
    ...
    if tcx.sess.opts.output_types.should_codegen() {
        redox_symbol_mangling::test::report_symbol_names(tcx);
    }
    ...
    let metadata = redox_metadata::fs::encode_and_write_metadata(tcx);
...
pub fn get_crate_name(sess: &Session, krate_attrs: &[ast::Attribute]) -> Symbol {
    ...
    let validate = |name, span| {
        redox_session::output::validate_crate_name(sess, name, span);
        name
    };
...
pub(crate) fn parse_crate_name(
    ...
    let redox_hir::Attribute::Parsed(AttributeKind::CrateName { name, name_span, .. }) =
        AttributeParser::parse_limited_should_emit(
            sess,
            attrs,
            sym::crate_name,
            DUMMY_SP,
            redox_ast::node_id::CRATE_NODE_ID,
            None,
            emit_errors,
        )?
    else {
