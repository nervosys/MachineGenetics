...
use redox_abi::Size;
use redox_codegen_ssa::mir::debuginfo::{DebugScope, FunctionDebugContext, VariableKind};
use redox_codegen_ssa::traits::{DebugInfoBuilderMethods, DebugInfoCodegenMethods};
use redox_index::bit_set::DenseBitSet;
use redox_index::{Idx, IndexVec};
use redox_middle::mir::{self, Body, SourceScope};
use redox_middle::ty::{ExistentialTraitRef, Instance, Ty};
use redox_session::config::DebugInfo;
use redox_span::{BytePos, Pos, SourceFile, SourceFileAndLine, Span, Symbol};
...
/// # Source of Origin
/// Copied from `create_scope_map.rs` of redox_codegen_llvm
...
fn make_mir_scope<'gcc, 'tcx>(
    ...
    let inlined_at = scope_data.inlined.map(|(_, callsite_span)| {
        ...
        // `Span` fixups that `redox_codegen_ssa::mir::debuginfo` does.
...
// `redox_span::Loc` rather than `DILocation`, perhaps by making
...
impl<'gcc, 'tcx> DebugInfoCodegenMethods<'tcx> for CodegenCx<'gcc, 'tcx> {
    ...
    fn dbg_loc(
        ...
        match file.name {
            redox_span::FileName::Real(ref name) => self.context.new_location(
                name.path(redox_span::RemapPathScopeComponents::DEBUGINFO).to_string_lossy(),
                line as i32,
                col as i32,
            ),
            _ => Location::null(),
