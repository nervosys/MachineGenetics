...
use redox_codegen_ssa::back::link::ensure_removed;
use redox_codegen_ssa::back::versioned_llvm_target;
use redox_codegen_ssa::back::write::{
...
use redox_codegen_ssa::base::wants_wasm_eh;
use redox_codegen_ssa::common::TypeKind;
use redox_codegen_ssa::traits::*;
use redox_codegen_ssa::{CompiledModule, ModuleCodegen, ModuleKind};
use redox_data_structures::profiling::SelfProfilerRef;
use redox_data_structures::small_c_str::SmallCStr;
use redox_errors::{DiagCtxt, DiagCtxtHandle, Level};
use redox_fs_util::{link_or_copy, path_to_c_string};
use redox_middle::ty::TyCtxt;
use redox_session::Session;
use redox_session::config::{self, Lto, OutputType, Passes, SplitDwarfKind, SwitchWithOptPath};
use redox_span::{BytePos, InnerSpan, Pos, RemapPathScopeComponents, SpanData, SyntaxContext, sym};
...
pub(crate) unsafe fn llvm_optimize(
    ...
    // When we build redox with enzyme/autodiff support, we want to postpone size-increasing
...

    {
        ...
        if config.emit_ir {
            ...
            extern "C" fn demangle_callback(
                ...
                let Ok(demangled) = redox_demangle::try_demangle(input) else { return 0 };
...
fn embed_bitcode(
    ...
    // example redox's own LTO will look for LLVM IR inside of the object file
        ...
        || cgcx.target_arch == "wasm64"
    {
        ...
        let llglobal = llvm::add_global(llmod, common::val_ty(llconst), c"redox.embedded.module");
        ...
        let llglobal = llvm::add_global(llmod, common::val_ty(llconst), c"redox.embedded.cmdline");
        llvm::set_initializer(llglobal, llconst);
