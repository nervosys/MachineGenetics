...
> Derived from mechgen_PROPOSAL.md Section 11: Phased Implementation Plan.
...
- [x] **Step 1**: Create automated rename script (`scripts/rebrand.py`) that renames all `rustc_*` crate directories under `compiler/` to `mechgen_*`, updates every `Cargo.toml` package name, and rewrites all `use rustc_*` / `extern crate rustc_*` imports across the entire tree. Validate with `grep -r "rustc_" compiler/` showing zero hits post-rename. Reference: mechgen_PROPOSAL.md §4.2 Compiler Crate Ontology.
- [x] **Step 2**: Update the build system (`src/bootstrap/`, `x.py`, `configure`, `bootstrap.example.toml`) to use `mechgen_*` crate names. Update `Cargo.toml` workspace members list. Ensure `python x.py check` succeeds with the rebranded crates.
- [x] **Step 3**: Rename the top-level compiler driver from `compiler/rustc/` to `compiler/MechGen/` and update all references. The produced binary should be `redoxc` (or `MechGen` as a compiler command). Update `src/tools/` references.
- [x] **Step 4**: Establish CI/CD pipeline: add `.github/workflows/MechGen-ci.yml` with build, test, and lint jobs for the rebranded compiler. Verify the compiler bootstraps successfully (stage 0 → stage 1 at minimum).

### Lexer: Token-Compressed Keywords & MechGen Tokens

- [x] **Step 5**: Extend `mechgen_lexer` (`compiler/mechgen_lexer/src/lib.rs`) — add new `TokenKind` variants for MechGen-specific tokens: `@@` (discovery), `?=` (refinement), `~>` (pipeline), `@` prefix for compact attributes. Add unit tests for each new token.
...
- [x] **Step 9**: Create `compiler/mechgen_grammar/` crate containing the formal LL(1) grammar specification for MechGen canonical syntax as a structured data file (grammar rules, FIRST/FOLLOW sets). This serves as the single source of truth for the parser. Include grammar validation tests (no LL(1) conflicts).
- [x] **Step 10**: Modify `mechgen_parse` to support dual-mode parsing: add a `SyntaxMode` enum (`Canonical`, `Legacy`) to the parser state, controlled by the session configuration. In `Legacy` mode, the parser behaves identically to upstream `rustc_parse`. In `Canonical` mode, it accepts the new LL(1) grammar. Add plumbing tests.
- [x] **Step 11**: Implement `spec` block parsing in `mechgen_parse` (canonical mode). A `spec` block contains `@req` (precondition), `@ens` (postcondition), `@perf` (cost bound), and `@fx` (effect declaration) clauses. Parse into new AST nodes. Add parser tests with valid and invalid spec blocks.
- [x] **Step 12**: Implement contract syntax parsing in `mechgen_parse`: `@req(expr)`, `@ens(expr)`, `@inv(expr)` as function/method attributes in canonical mode. Parse into `ContractAttr` AST nodes. Add parser tests.
...
### AST: MechGen Node Types

- [x] **Step 16**: Extend `mechgen_ast` with new AST node types: `SpecBlock`, `ContractAttr`, `EffectDecl`, `EffectAnnotation`, `CapabilityDecl`, `CapabilityBlock`, `RefinementType`, `PerfAnnotation`. Add visitor trait implementations for all new nodes. Add serialization support.
- [x] **Step 17**: Extend `mechgen_ast_pretty` to pretty-print all new MechGen AST nodes in both canonical (compact) and expanded (human-readable) forms. Add round-trip pretty-print tests: parse → print → re-parse should be identity.

### Safety Elision Pass

- [x] **Step 18**: Add a `MechGen.toml` configuration file format (parsed via a new `mechgen_config` crate or extension of `mechgen_session`). Support `[safety]` section with `mode = "agent" | "human" | "ci"`, `borrow-check`, `lifetime-check`, `bounds-check`, `overflow-check` settings per the proposal §9.4. Add config parsing tests.
- [x] **Step 19**: Implement the safety elision pass in `mechgen_ast_lowering` (or as a MIR pass). When `safety.mode = "agent"`, strip all explicit lifetime annotations, borrow annotations, and `unsafe` blocks during lowering. The compiler should infer these from usage context. Add tests showing identical compilation output with and without explicit annotations.
- [x] **Step 20**: Implement safety-free type inference extensions in `mechgen_hir_typeck` / `mechgen_infer`. When in agent mode, the compiler should infer: `&` vs `&mut` from usage, `move` vs `ref` from context, `dyn` vs `impl` from call sites. Add tests with the safety-free function signatures from the proposal §5.2.1.
...
- [x] **Step 23**: Implement the Structured Diagnostics Protocol in `mechgen_diagnostics` (extending `mechgen_errors`). Emit diagnostics as JSON diagnostic graphs with: error node, cause chain, fix candidates (each with token cost and confidence), related locations. Reference: mechgen_PROPOSAL.md §6.2. Add tests for structured diagnostic output.
- [x] **Step 24**: Create `mechgen_query` crate (or extend `mechgen_public`) that externalizes core compiler queries as a stable API. Agents should be able to query: `tokens_of(file)`, `ast_of(file)`, `type_of(expr)`, `impl_of(trait, type)`, `diagnostics_of(file)`. Add integration tests.

### Abbreviation Registry

- [x] **Step 25**: Create the standard abbreviation registry v1 as a data file in `compiler/mechgen_lexer/` (or `compiler/mechgen_config/`). Map all core types (`V[T]`→`Vec<T>`, `S[T]`→`String`, `?T`→`Option<T>`, `R[T,E]`→`Result<T,E>`, etc.), traits (`D`→`Debug`, `Cl`→`Clone`, `Cp`→`Copy`, `Df`→`Default`, etc.), and derives. The lexer and parser should use this registry. Add registry lookup tests.
...
- [x] **Step 27**: Set up MLIR build infrastructure. Add MLIR as a dependency alongside the existing LLVM dependency in `src/llvm-project/`. Create `compiler/mechgen_mlir/` crate with Rust FFI bindings to the MLIR C API. Verify that the MLIR libraries build and link correctly. Add a smoke test that creates and destroys an MLIR context.
- [x] **Step 28**: Define the MechGen MLIR dialect using TableGen ODS (in `compiler/mechgen_mlir/dialect/`). Define types: `OwnedType`, `RefType`, `RegionType`, `EffectType`, `CapabilityType`. Define operations: `MechGen.move`, `MechGen.copy`, `MechGen.borrow`, `MechGen.drop` (ownership); `MechGen.effect.decl`, `MechGen.effect.perform`, `MechGen.effect.handle` (effects); `MechGen.contract.require`, `MechGen.contract.ensure`, `MechGen.contract.invariant` (contracts); performance annotation ops. Reference: mechgen_PROPOSAL.md §14. Add dialect registration and verification tests.
- [x] **Step 29**: Implement MIR → MechGen MLIR dialect translation layer in `compiler/mechgen_mlir/`. Translate MIR basic blocks, terminators, places, rvalues, and operands into MechGen MLIR operations. This is a thin boundary layer where MIR semantics map to dialect operations. Add translation tests for basic MIR patterns (assignment, function call, branch, drop).
- [x] **Step 30**: Implement MLIR progressive lowering pipeline: MechGen Dialect → standard MLIR dialects (Linalg, Affine, Vector, SCF) → LLVM Dialect. Wire the LLVM backend codegen through MLIR LLVM Dialect, replacing the direct MIR→LLVM IR path. Verify that a simple MechGen program compiles end-to-end through the MLIR pipeline. Add integration test.
...
- [x] **Step 31**: Build the Safety Knowledge Base (SKB) as `compiler/mechgen_skb/`. Implement the rule schema (category, subcategory, severity, condition, remedy, references). Seed with an initial corpus: ownership rules (2,847), borrowing rules (1,203), lifetime rules (894), type safety rules (3,412), concurrency rules (567), FFI rules (234) — totaling ~9,157 rules. Reference: mechgen_PROPOSAL.md §15.
...
- [x] **Step 33**: Make all safety compiler passes opt-in via `MechGen.toml` safety profiles. Implement profile presets: `agent-dev` (all checks skipped), `human-dev` (all checks enforced), `ci-pipeline` (all enforced), `production` (all enforced). Wire profile selection into the compiler session. Add tests.

### Agent Infrastructure

- [x] **Step 34**: Build `mechgen_index` — a persistent semantic knowledge graph that indexes all symbols, types, traits, impls, and their relationships across crates. Store as a queryable database (embedded, e.g., SQLite or custom B-tree). Expose via the query API.
...
- [x] **Step 36**: Extend `mechgen_metadata` with capability manifest serialization. Each compiled crate produces a capability manifest JSON alongside its rlib/dylib. Implement the schema from mechgen_PROPOSAL.md §10.2.
- [x] **Step 37**: Build prototype RAP (MechGen Agent Protocol) server, merging rust-analyzer-style IDE support with direct compiler query access. The server should expose: `query.*`, `tokens.*`, `ast.*`, `type.*`, `diagnostic.*` endpoints. Reference: mechgen_PROPOSAL.md §8.2.
...
- [x] **Step 40**: Implement token budget reporting: `MechGen build --token-report` outputs per-file and per-function token counts, with compact vs expanded savings metrics.
- [x] **Step 41**: Implement the Cost Oracle (mechgen_PROPOSAL.md P38): per-target cost queries for expressions, types, and operations. Expose via `cost.query(expr, target)` API. Implement multi-target cost comparison (`cost.compare`). Seed with initial cost models for x86-64, AArch64, WASM.
...
- [x] **Step 43**: Build the CRDT-based semantic merge engine for concurrent AST/HIR modifications. Implement the CRDT types from mechgen_PROPOSAL.md §7.4: `SemanticCRDT` with `InsertNode`, `DeleteNode`, `MoveNode`, `RenameSymbol`, `ChangeType`, `AddImport` operations. Add merge conflict resolution tests.
- [x] **Step 44**: Implement the swarm message bus with zero-copy FlatBuffers-inspired serialization. Support three transport layers: shared memory (<100ns), Unix domain sockets (~1μs), TCP/TLS (network). Implement the wire protocol from mechgen_PROPOSAL.md §16 (frame header, routing header, CRC-32C). Add latency benchmarks.
- [x] **Step 45**: Validate MLIR→LLVM backend targets: x86-64, AArch64, WASM. Run the MechGen compiler test suite producing binaries for each target. Verify correctness with target-specific test harnesses.
...
- [x] **Step 47**: Define and implement the full MechGen Agent Protocol (RAP) specification: JSON-RPC request/response format, capability negotiation, session management.
...
- [x] **Step 50**: Build the swarm SDK (`compiler/mechgen_swarm/` or `library/mechgen_swarm/`) with orchestrator, synthesizer, and verifier agent roles.
...
- [x] **Step 66**: Expose all ACI services via RAP endpoints: `aci.warnings`, `aci.debug`, `aci.perf`, `aci.swarm`.
- [x] **Step 67**: Phase 2 integration testing: full agent swarm performing coordinated compilation with ACI assistance.

---

## Phase 3: Language Evolution + Synthesis + Grammar Extensions (Steps 68–88)

- [x] **Step 68**: Implement the effect type system in `mechgen_hir_analysis`: effect declarations, effect inference, effect polymorphism, effect handling.
- [x] **Step 69**: Implement contract syntax and checking in `mechgen_contracts`: preconditions, postconditions, invariants with compile-time and runtime verification modes.
...
- [x] **Step 85**: Define `MechGen-2026` edition with all new features including token-compact canonical form.
...
- [x] **Step 93**: Publish MechGen language specification.
