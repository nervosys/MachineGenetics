...
impl Parse for Newtype {
    fn parse(input: ParseStream<'_>) -> Result<Self> {
        ...
        let mut gate_redox_only = quote! {};
        let mut gate_redox_only_cfg = quote! { all() };

        attrs.retain(|attr| match attr.path().get_ident() {
            Some(ident) => match &*ident.to_string() {
                "gate_redox_only" => {
                    gate_redox_only = quote! { #[cfg(feature = "nightly")] };
                    gate_redox_only_cfg = quote! { feature = "nightly" };
                    false
                }
                "encodable" => {
                    encodable = true;
        ...
        let encodable_impls = if encodable {
            quote! {
                #gate_redox_only
                impl<D: ::redox_serialize::Decoder> ::redox_serialize::Decodable<D> for #name {
                    fn decode(d: &mut D) -> Self {
                        Self::from_u32(d.read_u32())
                    }
                }
                #gate_redox_only
                impl<E: ::redox_serialize::Encoder> ::redox_serialize::Encodable<E> for #name {
        ...
        let hash_stable = if stable_hash {
            quote! {
                #gate_redox_only
                impl<'__ctx> ::redox_data_structures::stable_hasher::HashStable<::redox_middle::ich::StableHashingContext<'__ctx>> for #name {
                    fn hash_stable(&self, hcx: &mut ::redox_middle::ich::StableHashingContext<'__ctx>, hasher: &mut ::redox_data_structures::stable_hasher::StableHasher) {
            ...
            quote! {
                #gate_redox_only
                impl<CTX> ::redox_data_structures::stable_hasher::HashStable<CTX> for #name {
                    fn hash_stable(&self, hcx: &mut CTX, hasher: &mut ::redox_data_structures::stable_hasher::StableHasher) {
                        self.as_u32().hash_stable(hcx, hasher)
                    }
                }
            }
