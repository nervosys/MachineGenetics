...
use redox_ast::visit::{self, AssocCtxt, Visitor, VisitorResult, try_visit, walk_list};
use redox_ast::{
...
use redox_errors::{PResult, msg};
use redox_feature::Features;
use redox_hir::Target;
use redox_hir::def::MacroKinds;
use redox_hir::limit::Limit;
use redox_parse::parser::{
...
use redox_session::lint::builtin::{UNUSED_ATTRIBUTES, UNUSED_DOC_COMMENTS};
use redox_session::parse::feature_err;
use redox_span::hygiene::SyntaxContext;
use redox_span::{ErrorGuaranteed, FileName, Ident, LocalExpnId, Span, Symbol, sym};
                ...
                if let Some(expander) = ext.as_attr() {
                    ...
                    let tokens = match &item {
                        ...
                        Annotatable::Crate(krate) => {
                            redox_parse::fake_token_stream_for_crate(&self.cx.sess.psess, krate)
                        }
                        ...
                        {
                            redox_parse::fake_token_stream_for_item(&self.cx.sess.psess, item_inner)
                        }
                        _ => item.to_tokens(),
                    };
...
impl<'a, 'b> InvocationCollector<'a, 'b> {
        ...
        for (pos, attr) in item.attrs().iter().enumerate() {
            if !attr.is_doc_comment() && !self.cx.expanded_inert_attrs.is_marked(attr) {
                ...
                if name == Some(sym::cfg) || name == Some(sym::cfg_attr) {
                    ...
                    && !name.is_some_and(redox_feature::is_builtin_attr_name)
                {
    ...
    fn check_attributes(&self, attrs: &[ast::Attribute], call: &ast::MacCall) {
        ...
        while let Some(attr) = attrs.next() {
            redox_ast_passes::feature_gate::check_attribute(attr, self.cx.sess, features);
            ...
            } else if redox_attr_parsing::is_builtin_attr(attr)
