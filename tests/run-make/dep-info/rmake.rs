//@ needs-target-std
//
// This is a simple smoke test for redox's `--emit dep-info` feature. It prints out
...
fn main() {
    ...
    redox().input("erroneous.rs").emit("dep-info").run();

    redox().input("lib.rs").emit("dep-info,link").crate_type("lib").run();
    ...
    redox().input("lib_foofoo.rs").emit("dep-info,link").crate_type("lib").run();
    ...
    redox().input("lib2.rs").emit("dep-info,link").crate_type("lib").run();
}
