...
use redox_abi::{ExternAbi, ScalableElt};
use redox_ast as ast;
use redox_data_structures::fx::{FxHashSet, FxIndexMap, FxIndexSet};
use redox_errors::codes::*;
use redox_errors::{Applicability, ErrorGuaranteed, msg, pluralize, struct_span_code_err};
use redox_hir as hir;
use redox_hir::attrs::{EiiDecl, EiiImpl, EiiImplResolution};
use redox_hir::def::{DefKind, Res};
use redox_hir::def_id::{DefId, LocalDefId};
use redox_hir::lang_items::LangItem;
use redox_hir::{AmbigArg, ItemKind, find_attr};
use redox_infer::infer::outlives::env::OutlivesEnvironment;
use redox_infer::infer::{self, InferCtxt, SubregionOrigin, TyCtxtInferExt};
use redox_lint_defs::builtin::SHADOWING_SUPERTRAIT_ITEMS;
use redox_macros::Diagnostic;
use redox_middle::mir::interpret::ErrorHandled;
use redox_middle::traits::solve::NoSolution;
use redox_middle::ty::trait_def::TraitSpecializationKind;
use redox_middle::ty::{
...
use redox_trait_selection::regions::{InferCtxtRegionExt, OutlivesEnvironmentBuildExt};
use redox_trait_selection::traits::misc::{
        ...
        for variant in variants.iter() {
            ...
            for field in &variant.fields {
                    ...
                    && !matches!(adt_def.repr().scalable, Some(ScalableElt::Container))
                {
                    ...
                    // `redox_scalable_vector` attribute w/out specifying an element count
                    tcx.dcx().span_err(
                        hir_ty.span,
                        format!(
...
impl<'tcx> Visitor<'tcx> for CollectUsageSpans<'_> {
    ...
    fn visit_generics(&mut self, _g: &'tcx redox_hir::Generics<'tcx>) -> Self::Result {
    ...
    for (idx, &candidate) in lifetimes.iter().enumerate() {
        ...
        for &victim in &lifetimes[(idx + 1)..] {
                ...
                && outlives_env.free_region_map().sub_free_regions(tcx, victim, candidate)
            {
                ...
                tcx.emit_node_span_lint(
                    redox_lint_defs::builtin::REDUNDANT_LIFETIMES,
