...
use redox_errors::ColorConfig;
use redox_errors::emitter::HumanReadableErrorType;
use redox_hir::attrs::{CollapseMacroDebuginfo, NativeLibKind};
...
use redox_session::utils::{CanonicalizedPath, NativeLib};
use redox_session::{CompilerIO, EarlyDiagCtxt, Session, build_session, getopts};
use redox_span::edition::{DEFAULT_EDITION, Edition};
use redox_span::source_map::{RealFileLoader, SourceMapInputs};
use redox_span::{FileName, SourceFileHashAlgorithm, sym};
    ...
    F: FnOnce(Session, Cfg),
{
    ...
    let target = redox_session::config::build_target_config(
        &early_dcx,
    ...
    redox_span::create_session_globals_then(DEFAULT_EDITION, &[], sm_inputs, || {
...
fn test_unstable_options_tracking_hash() {
    ...
    tracked!(default_visibility, Some(redox_target::spec::SymbolVisibility::Hidden));
    ...
    tracked!(sanitizer_dataflow_abilist, vec![String::from("/redox/abc")]);
    tracked!(sanitizer_kcfi_arity, Some(true));
    tracked!(sanitizer_memory_track_origins, 2);
    tracked!(sanitizer_recover, SanitizerSet::ADDRESS);
    tracked!(saturating_float_casts, Some(true));
    tracked!(share_generics, Some(true));
    tracked!(simulate_remapped_rust_src_base, Some(PathBuf::from("/redox/abc")));
