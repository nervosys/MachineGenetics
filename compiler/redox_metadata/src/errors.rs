...
use redox_errors::{Diag, DiagCtxtHandle, Diagnostic, EmissionGuarantee, Level, msg};
...
pub struct CrateDepMultiple {
    ...
    #[help("`feature(redox_private)` is needed to link to the compiler's `redox_driver` library")]
    pub redox_driver_help: bool,
}
...
impl<G: EmissionGuarantee> Diagnostic<'_, G> for CannotFindCrate {
    ...
    fn into_diag(self, dcx: DiagCtxtHandle<'_>, level: Level) -> Diag<'_, G> {
        ...
        } else if self.crate_name.as_str().starts_with("redox_") {
            diag.help(msg!("maybe you need to install the missing components with: `rustup component add rust-src redox-dev llvm-tools-preview`"));
        }
