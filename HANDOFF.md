...
| `scripts/check-ci-paths.sh` | that CI is actually triggered by every file a checker reads — the instrument on the instruments. Three directions, per trigger: a file `check-doc-counts.sh` pins that no `paths` entry covers; a file a *content* checker compiles that no entry covers; and an entry naming a file that does not exist (a typo covers nothing and looks exactly like coverage). **It found two gaps, the second much larger than the first.** Seven of the fourteen pinned files were uncovered — `HANDOFF.md`, which alone carries fourteen counts, and `SECURITY_AUDIT.md` among them. Then, because `check-doc-blocks.sh` walks the whole tree and `check-mg-sources.sh` finds every `.mg`: **76 of 101 markdown files and 83 of 101 MAGE sources**, including `MAGE_SPEC.md` and the whole of `quick-start/`, `agent-guide/`, `cookbook/` and `training/prompts/` — the documents an agent reads to learn the language. Both closed by two globs. The script's own first header said "most of the repository is documentation no checker reads", which was false, and the correction is in it. |
