...
use redox_middle::ty::fast_reject::{TreatParams, simplify_type};
use redox_middle::ty::print::{PrintPolyTraitRefExt as _, PrintTraitRefExt as _};
use redox_middle::ty::{
...
use redox_span::{BytePos, DUMMY_SP, Ident, Span, Symbol, kw, sym};
use redox_trait_selection::error_reporting::traits::report_dyn_incompatibility;
...
impl<'tcx> dyn HirTyLowerer<'tcx> + '_ {
    ...
    // FIXME(fmease): Heavily adapted from `redox_hir_typeck::method::suggest`. Deduplicate.
    ...
    pub(crate) fn report_unresolved_inherent_assoc_item(
        ...
        // FIXME(fmease): This was copied in parts from an old version of `redox_hir_typeck::method::suggest`.
        // Either
        ...
        if fulfillment_errors.is_empty() {
            // FIXME(fmease): Copied from `redox_hir_typeck::method::probe`. Deduplicate.
        ...
        // FIXME(fmease): `redox_hir_typeck::method::suggest` uses a `skip_list` to filter out some bounds.
