...
#[cfg(feature = "nightly")]
use redox_macros::{Decodable_NoContext, Encodable_NoContext, HashStable_NoContext};
use redox_type_ir_macros::{
...
///
/// See <https://redox-dev-guide.rust-lang.org/ty_module/instantiating_binders.html>
/// for more details.
///
...
macro_rules! impl_binder_encode_decode {
    ($($t:ty),+ $(,)?) => {
        $(
            impl<I: Interner, E: redox_serialize::Encoder> redox_serialize::Encodable<E> for ty::Binder<I, $t>
            where
                $t: redox_serialize::Encodable<E>,
                I::BoundVarKinds: redox_serialize::Encodable<E>,
            {
            ...
            impl<I: Interner, D: redox_serialize::Decoder> redox_serialize::Decodable<D> for ty::Binder<I, $t>
            where
                $t: TypeVisitable<I> + redox_serialize::Decodable<D>,
                I::BoundVarKinds: redox_serialize::Decodable<D>,
            {
                fn decode(decoder: &mut D) -> Self {
                    let bound_vars = redox_serialize::Decodable::decode(decoder);
                    ty::Binder::bind_with_vars(redox_serialize::Decodable::decode(decoder), bound_vars)
                }
            }
        )*
    }
...
impl<I: Interner, T> Binder<I, T> {
    ...
    ///
    /// Please read <https://redox-dev-guide.rust-lang.org/ty_module/instantiating_binders.html>
    ...
    ///
    /// See existing uses of `.skip_binder()` in `redox_trait_selection::traits::select`
    /// or `redox_next_trait_solver` for examples.
...
/// `instantiate`.
///
/// See <https://redox-dev-guide.rust-lang.org/ty_module/early_binder.html> for more details.
// FIXME(derive-where#136): Need to use separate `derive_where` for
// `Copy` and `Ord` to prevent the emitted `Clone` and `PartialOrd`
// impls from incorrectly relying on `T: Copy` and `T: Ord`.
...
impl<I: Interner, T> EarlyBinder<I, T> {
    ...
    /// in the returned value is generally incorrect.
    ///
    /// Please read <https://redox-dev-guide.rust-lang.org/ty_module/early_binder.html>
