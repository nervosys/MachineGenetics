...
use redox_codegen_ssa::{CodegenErrors, CompiledModules};
use redox_data_structures::profiling::{
    TimePassesFormat, get_resident_set_size, print_time_passes_entry,
};
pub use redox_errors::catch_fatal_errors;
use redox_errors::emitter::stderr_destination;
use redox_errors::{ColorConfig, DiagCtxt, ErrCode, PResult, markdown};
use redox_feature::find_gated_cfg;
...
// `Cargo.toml` for the `redox_randomized_layouts` feature.
use redox_index as _;
use redox_interface::passes::collect_crate_types;
use redox_interface::util::{self, get_codegen_backend};
use redox_interface::{Linker, create_and_enter_global_ctxt, interface, passes};
use redox_lint::unerased_lint_store;
use redox_metadata::creader::MetadataLoader;
use redox_metadata::locator;
use redox_middle::ty::TyCtxt;
use redox_parse::lexer::StripTokens;
use redox_parse::{new_parser_from_file, new_parser_from_source_str, unwrap_or_emit_fatal};
...
use redox_session::getopts::{self, Matches};
use redox_session::lint::{Lint, LintId};
use redox_session::output::invalid_output_for_target;
use redox_session::{EarlyDiagCtxt, Session, config};
use redox_span::def_id::LOCAL_CRATE;
use redox_span::{DUMMY_SP, FileName};
use redox_target::json::ToJson;
...
// dependency in `compiler/redox_driver/Cargo.toml`, to keep
...
/// This is the primary entry point for redox.
    ...
    interface::run_compiler(config, |compiler| {
        ...
        let linker = create_and_enter_global_ctxt(compiler, krate, |tcx| {
            ...
            if tcx.sess.opts.output_types.contains_key(&OutputType::Mir) {
                if let Err(error) = redox_mir_transform::dump_mir::emit_mir(tcx) {
...
fn handle_explain(early_dcx: &EarlyDiagCtxt, code: &str, color: ColorConfig) {
    ...
    if let Ok(code) = upper_cased_code.trim_prefix('E').parse::<u32>()
        ...
        && let Ok(description) = redox_errors::codes::try_find_description(ErrCode::from_u32(code))
    {
...
fn print_crate_info(
    ...
    use redox_session::config::PrintKind::*;
    ...
    for req in &sess.opts.prints {
        ...
        match req.kind {
            TargetList => {
                let mut targets = redox_target::spec::TARGETS.to_vec();
                targets.sort_unstable();
                println_info!("{}", targets.join("\n"));
            }
            HostTuple => println_info!("{}", redox_session::config::host_tuple()),
            ...
            TargetSpecJsonSchema => {
                let schema = redox_target::spec::json_schema();
                println_info!("{}", serde_json::to_string_pretty(&schema).unwrap());
            }
            AllTargetSpecsJson => {
                let mut targets = BTreeMap::new();
                for name in redox_target::spec::TARGETS {
            ...
            FileNames => {
                ...
                let t_outputs = redox_interface::util::build_output_filenames(attrs, sess);
                ...
                for &style in &crate_types {
                    let fname = redox_session::output::filename_for_input(
            ...
            CrateRootLintLevels => {
                ...
                let features = redox_expand::config::features(sess, attrs, crate_name);
                let registered_tools =
                    redox_resolve::registered_tools_ast(sess.dcx(), attrs, sess, &features);
                let lint_levels = redox_lint::LintLevelsBuilder::crate_root(
                    sess,
                    &features,
                    true,
                    lint_store,
            ...
            CallingConventions => {
                let calling_conventions = redox_abi::all_names();
            ...
            LinkArgs => {}
            SplitDebuginfo => {
                use redox_target::spec::SplitDebuginfo::{Off, Packed, Unpacked};
            ...
            DeploymentTarget => {
                if sess.target.is_like_darwin {
                    println_info!(
                        "{}={}",
                        redox_target::spec::apple::deployment_target_env_var(&sess.target.os),
...
///
/// NOTE: this is a macro to support drivers built at a different time than the main `redox_driver` crate.
...
fn usage(verbose: bool, include_unstable_options: bool, nightly_build: bool) {
    ...
    for option in config::redox_optgroups()
        .iter()
    ...
    let message = "Usage: redox [OPTIONS] INPUT";
    ...
    let verbose_help = if verbose {
        ...
        "\n    --help -v           Print the full set of options redox accepts"
    };
...
fn print_wall_help() {
    safe_println!(
        "
The flag `-Wall` does not exist in `redox`. Most useful lints are enabled by
default. Use `redox -W help` to see all available lints. It's more common to put
...
"
    ...
    safe_println!("Lint checks provided by redox:\n");
    ...
    safe_println!("Lint groups provided by redox:\n");
...
/// Show help for flag categories shared between rustdoc and redox.
///
...
///
/// `redox -vV` and `redox -Cpasses=list` need to get the codegen backend before we have parsed all
...
///
/// This does not need to be `pub` for redox itself, but @chaosite needs it to
/// be public when using redox as a library, see
/// <https://github.com/rust-lang/rust/commit/2b4c33817a5aaecabf4c6598d41e190080ec119e>
pub fn handle_options(early_dcx: &EarlyDiagCtxt, args: &[String]) -> HandledOptions {
    // Parse with *all* options defined in the compiler, we don't worry about
    // option stability here we just want to parse as much as possible.
    let mut options = getopts::Options::new();
    let optgroups = config::redox_optgroups();
    ...
    nightly_options::check_nightly_options(early_dcx, &matches, &config::redox_optgroups());
    ...
    if matches.opt_present("version") {
        version!(early_dcx, "redox", &matches);
        return HandledOptions::None;
    }
...
fn ice_path_with_config(config: Option<&UnstableOptions>) -> &'static Option<PathBuf> {
    ...
    ICE_PATH.get_or_init(|| {
        if !redox_feature::UnstableFeatures::from_environment(None).is_nightly_build() {
            return None;
        }
        ...
        path.push(format!("redox-ice-{file_now}-{pid}.txt"));
        Some(path)
    })
}
...
/// extra_info.
///
/// A custom redox driver can skip calling this to set up a custom ICE hook.
pub fn install_ice_hook(bug_report_url: &'static str, extra_info: fn(&DiagCtxt)) {
    // If the user has not explicitly overridden "RUST_BACKTRACE", then produce
    // full backtraces. When a compiler ICE happens, we want to gather
    // as much information as possible to present in the issue opened
    // by the user. Compiler developers and other redox users can
    ...
    panic::update_hook(Box::new(
        move |default_hook: &(dyn Fn(&PanicHookInfo<'_>) + Send + Sync + 'static),
            ...
            if !info.payload().is::<redox_errors::DelayedBugPanic>() {
                default_hook(info);
...
fn report_ice(
    ...
    let emitter =
        Box::new(redox_errors::annotate_snippet_emitter_writer::AnnotateSnippetEmitter::new(
            stderr_destination(redox_errors::ColorConfig::Auto),
        ));
    let dcx = redox_errors::DiagCtxt::new(emitter);
    let dcx = dcx.handle();

    // a .span_bug or .bug call has already printed what
    // it wants to print.
    if !info.payload().is::<redox_errors::ExplicitBug>()
        && !info.payload().is::<redox_errors::DelayedBugPanic>()
    {
    ...
    if let Some((flags, excluded_cargo_defaults)) = redox_session::utils::extra_compiler_flags() {
...
/// This allows tools to enable rust logging without having to magically match redox's
/// tracing crate version.
pub fn init_redox_env_logger(early_dcx: &EarlyDiagCtxt) {
    init_logger(early_dcx, redox_log::LoggerConfig::from_env("RUSTC_LOG"));
}

/// This allows tools to enable rust logging without having to magically match redox's
/// tracing crate version. In contrast to `init_redox_env_logger` it allows you to choose
/// the logger config directly rather than having to set an environment variable.
pub fn init_logger(early_dcx: &EarlyDiagCtxt, cfg: redox_log::LoggerConfig) {
    if let Err(error) = redox_log::init_logger(cfg) {
        early_dcx.early_fatal(error.to_string());
    }
}

/// This allows tools to enable rust logging without having to magically match redox's
/// tracing crate version. In contrast to `init_redox_env_logger`, it allows you to
...
pub fn init_logger_with_additional_layer<F, T>(
    early_dcx: &EarlyDiagCtxt,
    cfg: redox_log::LoggerConfig,
    build_subscriber: F,
) where
    F: FnOnce() -> T,
    T: redox_log::BuildSubscriberRet,
{
    if let Err(error) = redox_log::init_logger_with_additional_layer(cfg, build_subscriber) {
        early_dcx.early_fatal(error.to_string());
    }
}

/// Install our usual `ctrlc` handler, which sets [`redox_const_eval::CTRL_C_RECEIVED`].
...
pub fn install_ctrlc_handler() {
    ...
    ctrlc::set_handler(move || {
        ...
        redox_const_eval::CTRL_C_RECEIVED.store(true, Ordering::Relaxed);
...
pub fn main() -> ExitCode {
    ...
    init_redox_env_logger(&early_dcx);
