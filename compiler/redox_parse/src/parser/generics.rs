...
use redox_errors::{Applicability, PResult};
use redox_span::{Ident, Span, kw, sym};
...
impl<'a> Parser<'a> {
    ...
    fn parse_contract_requires(
        &mut self,
    ) -> PResult<'a, (ThinVec<redox_ast::Stmt>, Option<Box<redox_ast::Expr>>)> {
        Ok(if self.eat_keyword_noexpect(exp!(ContractRequires).kw) {
            ...
            let precond = match decls_and_precond.stmts.pop() {
                Some(precond) => match precond.kind {
                    redox_ast::StmtKind::Expr(expr) => expr,
    ...
    fn parse_contract_ensures(&mut self) -> PResult<'a, Option<Box<redox_ast::Expr>>> {
    ...
    pub(super) fn parse_spec_block(&mut self) -> PResult<'a, Option<Box<ast::SpecBlock>>> {
        ...
        while !self.eat(exp!(CloseBrace)) {
            ...
            // Each clause starts with `@` + keyword (unfused) or a
            // `CompactAttribute` token (fused by the lexer in canonical mode).
            let clause_kind = if let token::CompactAttribute(compact) = self.token.kind {
                match compact {
                    token::CompactAttr::Req => {
                        self.bump();
                    ...
                    _ => {
                        return Err(self
                            .dcx()
                            .create_err(errors::UnknownSpecClause { span: self.token.span }));
                    }
                }
            } else if self.eat(exp!(At)) {
                // Unfused `@` + keyword form (fallback).
                if self.token.is_ident_named(sym::req) {
                    self.bump();
                    self.expect(exp!(OpenParen))?;
                    let expr = self.parse_expr()?;
                    self.expect(exp!(CloseParen))?;
            ...
            clauses.push(ast::SpecClause { kind: clause_kind, span: clause_lo.to(clause_hi) });
        }

        let hi = self.prev_token.span;
        Ok(Some(Box::new(ast::SpecBlock { clauses, span: lo.to(hi) })))
    }
