...
use redox_index::Idx;
use redox_index::bit_set::SparseBitMatrix;
use redox_index::interval::{IntervalSet, SparseIntervalMatrix};
use redox_middle::mir::{BasicBlock, Location};
use redox_middle::ty::{self, RegionVid};
...
impl LivenessValues {
    ...
    /// [`location`][redox_middle::mir::Location].
    ...
    /// Returns whether `region` is marked live at the given
    /// [`point`][redox_mir_dataflow::points::PointIndex].
    #[inline]
...
/// redox to the internal `PlaceholderIndex` values that are used in
/// NLL.
