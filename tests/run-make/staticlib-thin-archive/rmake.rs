...
// checks that redox can read thin archive. Before the object crate added thin
// archive support redox would add emit object files to the staticlib and after
...
use run_make_support::{llvm_ar, path, rfs, rust_lib_name, redox, static_lib_name};

fn main() {
    rfs::create_dir("archive");

    // Build a thin archive
    redox().input("simple_obj.rs").emit("obj").output("archive/simple_obj.o").run();
    llvm_ar()
        .obj_to_thin_ar()
    ...
    redox().input("bin.rs").extern_("rust_lib", rust_lib_name("rust_lib")).run();
}
