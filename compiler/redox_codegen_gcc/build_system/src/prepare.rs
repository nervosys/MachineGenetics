...
use crate::redox_info::get_redox_path;
use crate::utils::{
...
fn prepare_libcore(
    ...
    if let Some(path) = sysroot_source {
        ...
        let redox_path = match get_redox_path() {
            Some(path) => path,
            None => return Err("`redox` path not found".to_string()),
        };

        let parent = match redox_path.parent() {
            Some(path) => path,
            None => return Err(format!("No parent for `{}`", redox_path.display())),
        };
