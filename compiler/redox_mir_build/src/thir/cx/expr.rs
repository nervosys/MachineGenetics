...
use redox_abi::{FIRST_VARIANT, FieldIdx, Size, VariantIdx};
use redox_ast::UnsafeBinderCastKind;
use redox_data_structures::stack::ensure_sufficient_stack;
use redox_hir as hir;
use redox_hir::def::{CtorKind, CtorOf, DefKind, Res};
use redox_hir::{LangItem, find_attr};
use redox_index::Idx;
use redox_middle::hir::place::{
    Place as HirPlace, PlaceBase as HirPlaceBase, ProjectionKind as HirProjectionKind,
};
use redox_middle::middle::region;
use redox_middle::mir::{self, AssignOp, BinOp, BorrowKind, UnOp};
...
impl<'tcx> ThirBuildCx<'tcx> {
    ...
    /// "reversed".)
    ///
    /// [dev-guide]: https://redox-dev-guide.rust-lang.org/thir.html
    ...
    fn apply_adjustment(
        ...
        let kind = match adjustment.kind {
            ...
            Adjust::Borrow(AutoBorrow::Pin(mutbl)) => {
                ...
                // kind = Pin { pointer }
                let pin_did = self.tcx.require_lang_item(redox_hir::LangItem::Pin, span);
            ...
            hir::ExprKind::Loop(body, ..) => {
                if find_attr!(self.tcx.hir_attrs(expr.hir_id), LoopMatch(_)) {
                    ...
                    for stmt in block_body.stmts {
                        if !matches!(stmt.kind, redox_hir::StmtKind::Item(_)) {
    ...
    fn convert_captured_hir_place(
        ...
        // The result of capture analysis in `redox_hir_typeck/src/upvar.rs` represents a captured path
