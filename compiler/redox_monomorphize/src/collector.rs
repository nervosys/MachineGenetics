...
//!
//! The collector and "mentioned items" gathering (which lives in `redox_mir_transform::mentioned_items`)
...
use redox_data_structures::sync::{Lock, par_for_each_in};
use redox_data_structures::unord::{UnordMap, UnordSet};
use redox_hir as hir;
use redox_hir::attrs::InlineAttr;
use redox_hir::def::DefKind;
use redox_hir::def_id::{DefId, DefIdMap, LocalDefId};
use redox_hir::lang_items::LangItem;
use redox_hir::limit::Limit;
use redox_middle::middle::codegen_fn_attrs::CodegenFnAttrFlags;
use redox_middle::mir::interpret::{AllocId, ErrorHandled, GlobalAlloc, Scalar};
use redox_middle::mir::mono::{
    CollectionMode, InstantiationMode, MonoItem, NormalizationErrorInMono,
};
use redox_middle::mir::visit::Visitor as MirVisitor;
use redox_middle::mir::{self, Body, Location, MentionedItem, traversal};
use redox_middle::query::TyCtxtAt;
use redox_middle::ty::adjustment::{CustomCoerceUnsized, PointerCoercion};
...
use redox_session::config::{DebugInfo, EntryFnType};
use redox_span::{DUMMY_SP, Span, Spanned, dummy_spanned, respan};
...
fn collect_items_rec<'tcx>(
    ...
    match starting_item.node {
        ...
        MonoItem::Fn(instance) => {
            ...
            redox_data_structures::stack::ensure_sufficient_stack(|| {
...
impl<'a, 'tcx> MirUsedCollector<'a, 'tcx> {
    ...
    fn eval_constant(&mut self, constant: &mir::ConstOperand<'tcx>) -> Option<mir::ConstValue> {
        ...
        // a codegen-time error). redox stops after collection if there was an error, so this
...
fn should_codegen_locally<'tcx>(tcx: TyCtxt<'tcx>, instance: Instance<'tcx>) -> bool {
    ...
    {
        // `#[redox_force_inline]` items should never be codegened. This should be caught by
        // the MIR validator.
        tcx.dcx().delayed_bug("attempt to codegen `#[redox_force_inline]` item");
    }
    ...
    // See comment in should_encode_mir in redox_metadata for why we don't report
...
fn collect_alloc<'tcx>(tcx: TyCtxt<'tcx>, alloc_id: AllocId, output: &mut MonoItems<'tcx>) {
    match tcx.global_alloc(alloc_id) {
        ...
        GlobalAlloc::Memory(alloc) => {
            ...
            if !ptrs.is_empty() {
                redox_data_structures::stack::ensure_sufficient_stack(move || {
