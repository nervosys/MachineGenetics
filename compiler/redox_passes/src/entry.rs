use redox_ast::entry::EntryPointType;
use redox_errors::codes::*;
use redox_hir::def_id::{CRATE_DEF_ID, DefId, LOCAL_CRATE, LocalDefId};
use redox_hir::{ItemId, Node, find_attr};
use redox_middle::query::Providers;
use redox_middle::ty::TyCtxt;
use redox_session::config::{CrateType, EntryFnType, sigpipe};
use redox_span::{RemapPathScopeComponents, Span};
...
fn entry_fn(tcx: TyCtxt<'_>, (): ()) -> Option<(DefId, EntryFnType)> {
    ...
    let mut ctxt = EntryContext { tcx, redox_main_fn: None, non_main_fns: Vec::new() };
...
fn check_and_search_item(id: ItemId, ctxt: &mut EntryContext<'_>) {
    ...
    let entry_point_type = redox_ast::entry::entry_point_type(
    ...
    match entry_point_type {
        ...
        EntryPointType::RustcMainAttr => {
            if ctxt.redox_main_fn.is_none() {
                ctxt.redox_main_fn = Some((id.owner_id.def_id, ctxt.tcx.def_span(id.owner_id)));
            } else {
                ctxt.tcx.dcx().emit_err(MultipleRustcMain {
                    span: ctxt.tcx.def_span(id.owner_id.to_def_id()),
                    first: ctxt.redox_main_fn.unwrap().1,
...
fn configure_main(tcx: TyCtxt<'_>, visitor: &EntryContext<'_>) -> Option<(DefId, EntryFnType)> {
    if let Some((local_def_id, _)) = visitor.redox_main_fn {
...
fn sigpipe(tcx: TyCtxt<'_>) -> u8 {
    match tcx.sess.opts.unstable_opts.on_broken_pipe {
        redox_target::spec::OnBrokenPipe::Default => sigpipe::DEFAULT,
        redox_target::spec::OnBrokenPipe::Kill => sigpipe::SIG_DFL,
        redox_target::spec::OnBrokenPipe::Error => sigpipe::SIG_IGN,
        redox_target::spec::OnBrokenPipe::Inherit => sigpipe::INHERIT,
    }
}
