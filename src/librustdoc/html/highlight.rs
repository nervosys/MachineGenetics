...
//!
//! This module uses libredox_ast's lexer to provide token-based highlighting for
...
use redox_lexer::{Cursor, FrontmatterAllowed, LiteralKind, TokenKind};
...
fn classify<'src>(
    ...
    let offset = redox_lexer::strip_shebang(src);
...
/// * If `klass` is `Some` and has a [`redox_span::Span`], it then tries to generate a link (`<a>`
///   element) by retrieving the link information from the `span_correspondence_map` that was filled
///   in `span_map.rs::collect_spans_and_sources`. If it cannot retrieve the information, then it's
///   the same as the second point (`klass` is `Some` but doesn't have a [`redox_span::Span`]).
