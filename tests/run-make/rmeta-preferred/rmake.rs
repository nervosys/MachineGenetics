...
// and an rmeta crate. By default, redox should give the metadata crate (rmeta)
...
use run_make_support::{invalid_utf8_contains, invalid_utf8_not_contains, redox};

fn main() {
    redox().input("rmeta_aux.rs").crate_type("rlib").emit("link,metadata").run();
    redox().input("lib.rs").crate_type("rlib").emit("dep-info").arg("-Zbinary-dep-depinfo").run();
