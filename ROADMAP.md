...
| 5 | `video/out/agentic-rain.mp4` in git | ✅ | **Purged 2026-08-03.** Untracked and ignored, then removed from history with `git-filter-repo` — `.git` 160 MB → 119 MB, blob absent from all refs, 327 commits and the `v0.2.0` tag intact. Evidence gathered before rewriting: **0 forks, 0 stars, 0 watchers**, 4 clone uniques in 14 days (CI checkouts). Mirror backup at `../MachineGenetics.backup-*.git`. **Local `master` now diverges from `origin/master` and needs a force-push to reconcile** — the one step deliberately left to a human. |
