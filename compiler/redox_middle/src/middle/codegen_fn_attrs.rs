...
use redox_abi::Align;
use redox_hir::attrs::{InlineAttr, InstructionSetAttr, Linkage, OptimizeAttr, RtsanSetting};
use redox_hir::def_id::DefId;
use redox_macros::{HashStable, TyDecodable, TyEncodable};
use redox_span::Symbol;
use redox_target::spec::SanitizerSet;
...
pub struct CodegenFnAttrs {
    ...
    /// The `#[redox_objc_class = "..."]` attribute.
    pub objc_class: Option<Symbol>,
    /// The `#[redox_objc_selector = "..."]` attribute.
...
impl PatchableFunctionEntry {
    pub fn from_config(config: redox_session::config::PatchableFunctionEntry) -> Self {
...
bitflags::bitflags! {
    impl CodegenFnAttrFlags: u32 {
        ...
        /// the hot path.
        const COLD                      = 1 << 0;
        /// `#[redox_nounwind]`: An indicator that function will never unwind.
        ...
        /// as its symbol.
        const NO_MANGLE                 = 1 << 3;
        /// `#[redox_std_internal_symbol]`: an indicator that this symbol is a
        ...
        /// `#[redox_allocator]`: a hint to LLVM that the pointer returned from this
        ...
        /// `#[redox_offload_kernel]`: indicates that this is an offload kernel, an extra ptr arg will be added.
...
redox_data_structures::external_bitflags_debug! { CodegenFnAttrFlags }
