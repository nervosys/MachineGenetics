...
use redox_errors::{Diag, IntoDiagArg};
use redox_hir::def::Namespace;
use redox_hir::def_id::{CRATE_DEF_ID, DefId};
use redox_middle::bug;
use redox_middle::ty::error::ExpectedFound;
use redox_middle::ty::print::{FmtPrinter, Print, PrintTraitRefExt as _, RegionHighlightMode};
use redox_middle::ty::{self, GenericArgsRef, RePlaceholder, Region, TyCtxt};
    ...
    T: for<'a> Print<'tcx, FmtPrinter<'a, 'tcx>>,
{
    fn into_diag_arg(self, _: &mut Option<std::path::PathBuf>) -> redox_errors::DiagArgValue {
        redox_errors::DiagArgValue::Str(self.to_string().into())
    }
}
            ...
            if self_ty.value.is_closure() && self.tcx().is_fn_trait(expected_trait_ref.value.def_id)
            {
                let closure_sig = self_ty.map(|closure| {
                    if let ty::Closure(_, args) = closure.kind() {
                        self.tcx()
                            .signature_unclosure(args.as_closure().sig(), redox_hir::Safety::Safe)
                    } else {
