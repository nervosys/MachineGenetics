...
> sensible output shapes (P86). `rmil_bridge` resolves **80+ layer
> names** to opcodes. Measured RMIB compression on a real transformer
...
> win is binary IR (RMIB) for AI items + reliability pipeline.
>
...
> claim and found it ~tied with Rust; Phase 28 commits to **RMIL
...
neurosymbolic IR **RMIL**, neural / symbolic / distributed / federated
primitives, and seven compute backends).

## Goals

1. **Token-efficient codegen.** A MechGen `net` definition should lower to
   RMIL — a transformer block in ~50 bytes on the wire instead of hundreds of
...
```
              ...
              systems items  │            │  net / kb / agent / swarm
                             ...
                             ▼            ▼
                  ┌──────────────┐  ┌──────────────────────────┐
                  │  MLIR codegen│  │  RMIL Expr (rmi::lang)   │
...
### `prototype/src/rmil_bridge.rs` — AST → RMIL

| MechGen surface                         | RMIL lowering                                       |
|-----------------------------------------|-----------------------------------------------------|
...
`LayerNorm`, `Attention`, `GELU`, …) to RMIL opcodes. Unknown layers lower
to `Op::IDENTITY` with a diagnostic.

`OpFamilyRouter::route(item)` returns `IrTarget::Mlir`, `Rmil`, or `Both`,
...
```sh
MechGen-parse --target=rmil prototype/examples/unified.mg
```

emits per-item RMIL statistics:

```
// MechGen → RMIL lowering for prototype/examples/unified.mg
// MLIR-routed items: 2
// RMIL-routed items: 4
...
| Surface / IR relationship         | RMIL is a **peer** to MLIR; both reachable          |
...
### Forward-pass walker (`ForwardWalker` in `rmil_bridge.rs`)

`NetTranslator` now walks `forward { ... }` blocks and emits RMIL that
respects the user-written data flow:

| Forward shape               | RMIL produced                                |
|-----------------------------|----------------------------------------------|
...
A bridge test now constructs an RMIL expression, runs it through
...
### VM execution end-to-end (`--target=rmil-run`)

A second CLI flag walks every RMIL-routed item, executes through
`rmi::lang::Vm::eval`, and reports per-item status:

```
// MechGen → RMIL → VM execution for prototype/examples/unified.mg
...
`rmil_bridge.rs`. Math, control, memory, and meta ops run fine.
...
### RMIL → MechGen decompiler (`decompile` in `rmil_bridge.rs`)

A new `decompile(expr, net_name) -> DecompileResult` walks an RMIL `Expr`
...
`translate ∘ decompile ∘ translate` yields the same RMIL content hash as
`translate` alone — closing the loop for evolutionary code generation.

### Compute backend dispatch (`rmil_compute.rs` + `--target=rmil-compute`)

A new module dispatches activation-only RMIL pipelines to a real
[`rmi::compute::Backend`] (CPU today, GPU once features are enabled):

```
MechGen-parse --target=rmil-compute prototype/examples/unified.mg
// MechGen → RMIL → CpuBackend dispatch
...
`rmil_compute.rs` now dispatches `LINEAR` and `MATMUL` against real
...
`--target=rmil-run` switched from `Vm::eval` to `Vm::eval_jit`. Pure
...
3. **Multi-tail forward blocks.** Explicit branching (`if cond { a } else
   { b }`, multi-output nets) lowers as best-effort and needs RMIL
...
5. **Distributed transport runtime wiring.** Phase 3 emits the transport
   sym into RMIL; an actual backend pass that consumes
...
| `prototype/src/main.rs`                             | + 3 mod decls, `--target=rmil` flag |
| `prototype/src/rmil_bridge.rs`                      | **new** (415 lines, 5 tests) |
...
- **Add a new RMIL-routed surface form?** Add a variant to `ItemKind`,
...
- **Add a new neural primitive?** Add the opcode to
  ...
  to `layer_name_to_op` in `rmil_bridge.rs`.
...
RMIL compute dispatch). Phase 27 re-anchored on the actual mission and
...
## Phase 45: RAP carries `application/rmib`

`prototype/src/rmib.rs` lifts the binary IR container codec
(`encode_module`, `decode_container`, `to_hex`, `from_hex`,
`RMIB_MAGIC`, `RMIB_VERSION`) into a single shared module. CLI emitter
`main.rs::run_emit_rmil_bytes` refactored to call it. Three new RAP
methods expose the path over JSON-RPC:

| Method | In | Out |
|---|---|---|
| `rmil/encode` | `source` | `magic`, `version`, `container_bytes`, `items[]`, `rmib_hex` |
| `rmil/decode` | `rmib_hex` | `container_bytes`, `items[name, layers, content_hash, skipped]` |
| `rmil/run` | `source` | encode then dispatch to `CpuBackend`, per-item `status` (`dispatched`/`stub`/`error`) |
...
since `rmi::lang::Op` is not `Serialize`. Tests: +6 rmib + 7 rap.
...
Composes `recover::recover` + `rmib::encode_module` so an agent gets
recovered MechGen + RMIB bytes in one call. On `ok:false` returns
...
from `rmil_bridge::layer_name_to_op`), rap_methods, heal_patterns
(via new `heal::pattern_names()` public enumerator), recovery_stages,
and rmib container constants. Tests: +8 ontology + 3 rap.
...
RAP methods, ~10 heal patterns, 7 recovery stages, RMIB layout, and
...
2. **Binary IR (RMIB)** for AI-routed items (`net`/`kb`/`agent`/
...
- **P74**: end-to-end integration test
  `framewerx_examples_compile_to_rmib` walks every example through
  lex -> parse -> bridge::lower_module -> rmib::encode -> decode ->
...
| RecursiveMachineIntelligence-MG | 14 `.mg` files, end-to-end-tested through RMIB codec |
...
example actually compiles to RMIL bytecode, content-hash stability
...
- **P79** Worked examples exercising P78 mappings end-to-end:
  ...
  mixture_of_experts. All added to `framewerx_examples_compile_to_rmib`
...
- **P80** Tangible UX demonstration: `scripts/demo_agent_workflow.sh`
  ...
  → RMIB 147 B = 68.8% size reduction**. That's the size win the
...
- **P83** Agent self-test: bootstrapped from `MECHGEN_ONTOLOGY.json`
  ...
  **Surfaced a real bug**: `rmil_compute::run_pipeline` was called
...
- **P84** Fixed P83: new `infer_input_shape(expr) -> Option<Vec<usize>>`
  walks the RMIL expression to find the first shape-bearing op
...
declarations parse, lower to RMIL, encode to RMIB, decode losslessly,
...
- **P94** Closed the **execution** side of the open catalog: new
  ...
  wrapper gets the RMIB blob on stdin + env metadata, returns a
...
backend appears in the ontology and dispatches real RMIB bytecode.
...
| Measured RMIB compression | 68.8% on FlashAttnBlock |
