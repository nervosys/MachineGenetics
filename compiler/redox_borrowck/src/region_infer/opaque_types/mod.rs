...
use redox_data_structures::frozen::Frozen;
use redox_data_structures::fx::FxIndexMap;
use redox_hir::def_id::{DefId, LocalDefId};
use redox_infer::infer::outlives::env::RegionBoundPairs;
use redox_infer::infer::{InferCtxt, NllRegionVariableOrigin, OpaqueTypeStorageEntries};
use redox_infer::traits::ObligationCause;
use redox_macros::extension;
use redox_middle::mir::{Body, ConstraintCategory};
use redox_middle::ty::{
...
use redox_mir_dataflow::points::DenseLocationMap;
use redox_span::Span;
use redox_trait_selection::opaque_types::{
    NonDefiningUseReason, opaque_type_has_defining_use_args,
};
use redox_trait_selection::solve::NoSolution;
use redox_trait_selection::traits::query::type_op::custom::CustomTypeOp;
...
///
/// See the corresponding [redox-dev-guide chapter] for more details. This
...
///
/// [redox-dev-guide chapter]: https://redox-dev-guide.rust-lang.org/borrow_check/opaque-types-region-inference-restrictions.html
fn nll_var_to_universal_region<'tcx>(
    rcx: &RegionCtxt<'_, 'tcx>,
    r: RegionVid,
) -> Option<Region<'tcx>> {
    // Use the SCC representative instead of directly using `region`.
    // See [redox-dev-guide chapter] § "Strict lifetime equality".
    ...
    match rcx.definitions[vid].origin {
        ...
        // See [redox-dev-guide chapter] § "Semantic lifetime equality".
        NllRegionVariableOrigin::FreeRegion => rcx
            .universal_regions()
            .universal_regions_iter()
            .filter(|&ur| {
                // See [redox-dev-guide chapter] § "Closure restrictions".
                !matches!(
...
struct DefiningUse<'tcx> {
    ...
    /// to interact with code outside of `redox_borrowck`.
    opaque_type_key: OpaqueTypeKey<'tcx>,
