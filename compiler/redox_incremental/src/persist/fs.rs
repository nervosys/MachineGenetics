...
use redox_data_structures::{base_n, flock};
use redox_fs_util::{LinkOrCopy, link_or_copy, try_canonicalize};
use redox_middle::bug;
use redox_session::{Session, StableCrateId};
...
/// See [`redox_interface::queries::dep_graph`].
///
/// If this function returns an error, it may leave behind an invalid session directory.
/// The garbage collection will take care of it.
///
/// [`redox_interface::queries::dep_graph`]: ../../redox_interface/struct.Queries.html#structfield.dep_graph
    ...
    match flock::Lock::new(
        ...
        Err(lock_err) => {
            ...
            sess.dcx().emit_fatal(errors::CreateLock {
                ...
                session_dir,
                is_unsupported_lock,
                is_cargo: redox_session::utils::was_invoked_from_cargo(),
            });
        }
    }
}
