...
use redox_errors::{Applicability, Diag, ErrorGuaranteed, pluralize, struct_span_code_err};
use redox_hir as hir;
use redox_hir::def::{DefKind, Res};
use redox_hir::{HirId, HirIdMap, Node};
use redox_hir_analysis::check::{check_abi, check_custom_abi};
use redox_hir_analysis::hir_ty_lowering::HirTyLowerer;
use redox_infer::traits::{ObligationCauseCode, ObligationInspector, WellFormedLoc};
...
macro_rules! type_error_struct {
    ($dcx:expr, $span:expr, $typ:expr, $code:expr, $($message:tt)*) => ({
        let mut err = redox_errors::struct_span_code_err!($dcx, $span, $code, $($message)*);
...
/// called from within redox. Note, this is not a query, and thus is not cached.
...
fn report_unexpected_variant_res(
    ...
    let path_str = redox_hir_pretty::qpath_to_string(&tcx, qpath);
    let mut err = tcx
        .dcx()
...
fn fatally_break_rust(tcx: TyCtxt<'_>, span: Span) -> ! {
    ...
    diag.note(format!("redox {} running on {}", tcx.sess.cfg_version, config::host_tuple(),));
