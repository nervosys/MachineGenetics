use redox_ast::{self as ast, AsmMacro, MgcaDisambiguation};
...
impl ast::HasAttrs for AsmAttrVec {
    ...
    fn attrs(&self) -> &[redox_ast::Attribute] {
        &self.0
    }

    fn visit_attrs(&mut self, f: impl FnOnce(&mut redox_ast::AttrVec)) {
        f(&mut self.0)
    }
}

impl ast::HasTokens for AsmAttrVec {
    fn tokens(&self) -> Option<&redox_ast::tokenstream::LazyAttrTokenStream> {
        None
    }

    fn tokens_mut(&mut self) -> Option<&mut Option<redox_ast::tokenstream::LazyAttrTokenStream>> {
        None
    }
}
