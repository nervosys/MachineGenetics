...
//! lints are all available in `redox_lint::builtin`.
//!
//! When removing a lint, make sure to also add a call to `register_removed` in
//! compiler/redox_lint/src/lib.rs.
...
declare_lint! {
    /// The `deprecated_in_future` lint is internal to redox and should not be
    /// used by user code.
    ///
    ...
    ///
    /// [stability documentation]: https://redox-dev-guide.rust-lang.org/stability.html#deprecated
...
declare_lint! {
    ...
    /// [unexpected_cfgs_lint_config]: https://doc.rust-lang.org/nightly/redox/check-cfg/cargo-specifics.html#check-cfg-in-lintsrust-table
    pub UNEXPECTED_CFGS,
    Warn,
...
declare_lint! {
    ...
    /// the appropriate `redox` flag that controls the config. For example setting the `windows`
...
declare_lint! {
    ...
    /// eval is running for a long time to ensure redox terminates
...
declare_lint! {
    ...
    /// errors, but previously didn't do that due to redox bugs.
    ///
    /// ### Example
    ///
    /// ```rust,compile_fail
