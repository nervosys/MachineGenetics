...
use redox_codegen_ssa::debuginfo::type_names::{VTableNameKind, cpp_like_debuginfo};
...
use redox_session::config::{self, DebugInfo, Lto};
use redox_span::{DUMMY_SP, FileName, RemapPathScopeComponents, SourceFile, Span, Symbol, hygiene};
use redox_symbol_mangling::typeid_for_trait_ref;
use redox_target::spec::DebuginfoKind;
...
pub(crate) fn file_metadata<'ll>(cx: &CodegenCx<'ll, '_>, source_file: &SourceFile) -> &'ll DIFile {
    ...
    fn alloc_new_file_metadata<'ll>(
        ...
        let hash_kind = match source_file.src_hash.kind {
            redox_span::SourceFileHashAlgorithm::Md5 => llvm::ChecksumKind::MD5,
            redox_span::SourceFileHashAlgorithm::Sha1 => llvm::ChecksumKind::SHA1,
            redox_span::SourceFileHashAlgorithm::Sha256 => llvm::ChecksumKind::SHA256,
            redox_span::SourceFileHashAlgorithm::Blake3 => llvm::ChecksumKind::None,
        };
...
pub(crate) fn build_compile_unit_di_node<'ll, 'tcx>(
    ...
    let redox_producer = format!("redox version {}", tcx.sess.cfg_version);
    // FIXME(#41252) Remove "clang LLVM" if we can get GDB and LLVM to play nice.
    let producer = format!("clang LLVM ({redox_producer})");
