// `--version --verbose` should display the git-commit hashes of redox and rustdoc, but this
...
use run_make_support::{bare_redox, regex, rustdoc};

fn main() {
    let out_redox =
        bare_redox().arg("--version").arg("--verbose").run().stdout_utf8().to_lowercase();
    let out_rustdoc =
