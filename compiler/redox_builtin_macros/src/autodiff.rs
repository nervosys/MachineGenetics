...
mod llvm_enzyme {
    ...
    use redox_ast::expand::autodiff_attrs::{
        DiffActivity, DiffMode, valid_input_activity, valid_ret_activity, valid_ty_for_activity,
    };
    use redox_ast::token::{Lit, LitKind, Token, TokenKind};
    use redox_ast::tokenstream::*;
    use redox_ast::visit::AssocCtxt::*;
    use redox_ast::{
    ...
    use redox_expand::base::{Annotatable, ExtCtxt};
    use redox_hir::attrs::RustcAutodiff;
    use redox_span::{Ident, Span, Symbol, kw, sym};
    ...
    pub(crate) fn outer_normal_attr(
        kind: &Box<redox_ast::NormalAttr>,
        id: redox_ast::AttrId,
        span: Span,
    ) -> redox_ast::Attribute {
        let style = redox_ast::AttrStyle::Outer;
        let kind = redox_ast::AttrKind::Normal(kind.clone());
        redox_ast::Attribute { kind, id, style, span }
    }
    ...
    fn first_ident(x: &MetaItemInner) -> redox_span::Ident {
        if let Some(l) = x.lit() {
            match l.kind {
                ast::LitKind::Int(val, _) => {
                    // get an Ident from a lit
                    return redox_span::Ident::from_str(val.get().to_string().as_str());
                }
                _ => {}
            }
        }
    ...
    ///     f32::sin(**x)
    /// }
    /// #[redox_autodiff(Reverse, Duplicated, Active)]
    ...
    pub(crate) fn expand_with_mode(
        ...
        let mut redox_ad_attr =
            Box::new(ast::NormalAttr::from_ident(Ident::with_dummy_span(sym::redox_autodiff)));
        ...
        let inline_item = ast::AttrItem {
            ...
            args: redox_ast::ast::AttrItemKind::Unparsed(ast::AttrArgs::Delimited(never_arg)),
            tokens: None,
        };
        let inline_never_attr = Box::new(ast::NormalAttr { item: inline_item, tokens: None });
        let new_id = ecx.sess.psess.attr_id_generator.mk_attr_id();
        let attr = outer_normal_attr(&redox_ad_attr, new_id, span);
        let new_id = ecx.sess.psess.attr_id_generator.mk_attr_id();
        let inline_never = outer_normal_attr(&inline_never_attr, new_id, span);

        // We're avoid duplicating the attribute `#[redox_autodiff]`.
        ...
        // Now update for d_fn
        redox_ad_attr.item.args = redox_ast::ast::AttrItemKind::Unparsed(
            redox_ast::AttrArgs::Delimited(redox_ast::DelimArgs {
                dspan: DelimSpan::dummy(),
                delim: redox_ast::token::Delimiter::Parenthesis,
                tokens: ts,
            }),
        );

        let new_id = ecx.sess.psess.attr_id_generator.mk_attr_id();
        let d_attr = outer_normal_attr(&redox_ad_attr, new_id, span);
    ...
    fn gen_enzyme_decl(
        ...
        if x.mode.is_fwd() {
            ...
            if matches!(x.ret_activity, DiffActivity::Dual | DiffActivity::Dualv) {
                let kind = if x.width == 1 || matches!(x.ret_activity, DiffActivity::Dualv) {
                    ...
                    let anon_const = redox_ast::AnonConst {
                        id: ast::DUMMY_NODE_ID,
                ...
                let ty = Box::new(redox_ast::Ty { kind, id: ty.id, span: ty.span, tokens: None });
            ...
            if matches!(x.ret_activity, DiffActivity::DualOnly | DiffActivity::DualvOnly) {
                ...
                if x.width > 1 {
                    ...
                    let ty =
                        Box::new(redox_ast::Ty { kind, id: ty.id, span: ty.span, tokens: None });
                    d_decl.output = FnRetTy::Ty(ty);
                }
            }
        }
            ...
            let ret_ty = match d_decl.output {
                ...
                FnRetTy::Default(span) => {
                    if act_ret.len() == 1 {
                        ...
                        Box::new(redox_ast::Ty { kind, id: ast::DUMMY_NODE_ID, span, tokens: None })
                    }
                }
            };
        ...
        if unsafe_activities {
            d_header.safety = redox_ast::Safety::Unsafe(span);
        }
