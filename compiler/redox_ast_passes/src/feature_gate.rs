use redox_ast::visit::{self, AssocCtxt, FnCtxt, FnKind, Visitor};
use redox_ast::{self as ast, AttrVec, NodeId, PatKind, attr, token};
use redox_attr_parsing::AttributeParser;
use redox_errors::msg;
use redox_feature::{AttributeGate, BUILTIN_ATTRIBUTE_MAP, BuiltinAttribute, Features};
use redox_hir::Attribute;
use redox_hir::attrs::AttributeKind;
use redox_session::Session;
use redox_session::parse::{feature_err, feature_warn};
use redox_span::{DUMMY_SP, Span, Spanned, Symbol, sym};
...
fn check_incompatible_features(sess: &Session, features: &Features) {
    ...
    for (f1, f2) in redox_feature::INCOMPATIBLE_FEATURES
        .iter()
...
fn check_dependent_features(sess: &Session, features: &Features) {
    for &(parent, children) in
        redox_feature::DEPENDENT_FEATURES.iter().filter(|(parent, _)| features.enabled(*parent))
    {
