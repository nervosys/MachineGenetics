use redox_abi::{BackendRepr, Float, Integer, Primitive, RegKind};
use redox_hir::attrs::{InstructionSetAttr, Linkage};
use redox_hir::def_id::LOCAL_CRATE;
use redox_middle::mir::mono::{MonoItemData, Visibility};
use redox_middle::mir::{InlineAsmOperand, START_BLOCK};
use redox_middle::ty::layout::{FnAbiOf, LayoutOf, TyAndLayout};
use redox_middle::ty::{Instance, Ty, TyCtxt, TypeVisitableExt};
use redox_middle::{bug, ty};
use redox_span::sym;
use redox_target::callconv::{ArgAbi, FnAbi, PassMode};
...
>(
    ...
    let redox_middle::mir::TerminatorKind::InlineAsm {
        asm_macro: _,
        template,
        ref operands,
        options,
    ...
    template_vec.push(redox_ast::ast::InlineAsmTemplatePiece::String(begin.into()));
    template_vec.extend(template.iter().cloned());
    template_vec.push(redox_ast::ast::InlineAsmTemplatePiece::String(end.into()));
