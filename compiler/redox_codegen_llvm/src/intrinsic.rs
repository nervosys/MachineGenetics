...
use redox_codegen_ssa::base::{compare_simd_types, wants_msvc_seh, wants_wasm_eh};
use redox_codegen_ssa::common::{IntPredicate, TypeKind};
use redox_codegen_ssa::errors::{ExpectedPointerMutability, InvalidMonomorphization};
...
use redox_middle::ty::layout::{FnAbiOf, HasTyCtxt, HasTypingEnv, LayoutOf};
use redox_middle::ty::offload_meta::OffloadMetadata;
use redox_middle::ty::{self, GenericArgsRef, Instance, SimdAlign, Ty, TyCtxt, TypingEnv};
use redox_middle::{bug, span_bug};
use redox_session::config::CrateType;
use redox_span::{Span, Symbol, sym};
use redox_symbol_mangling::{mangle_internal_symbol, symbol_name_for_instance_in_crate};
use redox_target::callconv::PassMode;
...
impl<'ll, 'tcx> IntrinsicCallBuilderMethods<'tcx> for Builder<'_, 'll, 'tcx> {
    fn codegen_intrinsic_call(
        ...
        let llval = match name {
            ...
            sym::offload => {
                ...
                if tcx.sess.lto() != redox_session::config::Lto::Fat {
    ...
    fn assume(&mut self, val: Self::Value) {
        if self.cx.sess().opts.optimize != redox_session::config::OptLevel::No {
...
fn codegen_autodiff<'ll, 'tcx>(
    ...
    if !tcx.sess.opts.unstable_opts.autodiff.contains(&redox_session::config::AutoDiff::Enable) {
    ...
    if ct.len() == 1 && ct.contains(&CrateType::Executable) {
        if lto != redox_session::config::Lto::Fat {
            let _ = tcx.dcx().emit_almost_fatal(AutoDiffWithoutLto);
        }
    } else {
        if lto != redox_session::config::Lto::Fat && !tcx.sess.opts.cg.linker_plugin_lto.enabled() {
            let _ = tcx.dcx().emit_almost_fatal(AutoDiffWithoutLto);
        }
    }
    ...
    let fnc_tree = redox_middle::ty::fnc_typetrees(tcx, source_fn_ptr_ty);
