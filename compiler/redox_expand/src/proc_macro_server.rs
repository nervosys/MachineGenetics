...
use redox_ast::token;
use redox_ast::tokenstream::{self, DelimSpacing, Spacing, TokenStream};
use redox_ast::util::literal::escape_byte_str_symbol;
use redox_ast_pretty::pprust;
use redox_data_structures::fx::FxHashMap;
use redox_errors::{Diag, ErrorGuaranteed, MultiSpan};
use redox_parse::lexer::{StripTokens, nfc_normalize};
use redox_parse::parser::Parser;
use redox_parse::{exp, new_parser_from_source_str, source_str_to_stream};
use redox_proc_macro::bridge::{
    DelimSpan, Diagnostic, ExpnGlobals, Group, Ident, LitKind, Literal, Punct, TokenTree, server,
};
use redox_proc_macro::{Delimiter, Level};
use redox_session::parse::ParseSess;
use redox_span::def_id::CrateNum;
use redox_span::{BytePos, FileName, Pos, Span, Symbol, sym};
...
impl FromInternal<token::LitKind> for LitKind {
    fn from_internal(kind: token::LitKind) -> Self {
        match kind {
            ...
            token::Err(_guar) => {
                // This is the only place a `redox_proc_macro::bridge::LitKind::ErrWithGuar`
        ...
        while let Some(tree) = iter.next() {
            let (Token { kind, span }, joint) = match tree.clone() {
                tokenstream::TokenTree::Delimited(span, _, mut delim, mut stream) => {
                    ...
                    trees.push(TokenTree::Group(Group {
                        delimiter: redox_proc_macro::Delimiter::from_internal(delim),
                        stream: Some(stream),
                        span: DelimSpan {
                            open: span.open,
                            close: span.close,
            ...
            match kind {
                ...
                Lifetime(name, is_raw) => {
                    let ident = redox_span::Ident::new(name, span).without_first_quote();
                    trees.extend([
                ...
                NtLifetime(ident, is_raw) => {
                    ...
                    trees.push(TokenTree::Group(Group {
                        delimiter: redox_proc_macro::Delimiter::None,
                        stream: Some(stream),
                ...
                DocComment(_, attr_style, data) => {
                    ...
                    trees.push(TokenTree::Group(Group {
                        delimiter: redox_proc_macro::Delimiter::Bracket,
                        stream: Some(stream),
                        span: DelimSpan::from_single(span),
                    }));
                }
    ...
    fn to_internal(self) -> SmallVec<[tokenstream::TokenTree; 2]> {
        ...
        match tree {
            ...
            TokenTree::Ident(self::Ident { sym, is_raw, span }) => {
                redox.psess().symbol_gallery.insert(sym, span);
...
impl ToInternal<redox_errors::Level> for Level {
    fn to_internal(self) -> redox_errors::Level {
        match self {
            Level::Error => redox_errors::Level::Error,
            Level::Warning => redox_errors::Level::Warning,
            Level::Note => redox_errors::Level::Note,
            Level::Help => redox_errors::Level::Help,
...
impl server::Server for Rustc<'_, '_> {
    ...
    fn emit_diagnostic(&mut self, diagnostic: Diagnostic<Self::Span>) {
        let message = redox_errors::DiagMessage::from(diagnostic.message);
        let mut diag: Diag<'_, ()> =
    ...
    fn symbol_normalize_and_validate_ident(&mut self, string: &str) -> Result<Self::Symbol, ()> {
        ...
        if redox_lexer::is_ident(sym.as_str()) { Ok(sym) } else { Err(()) }
    }
}
