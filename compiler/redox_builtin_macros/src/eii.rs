...
fn generate_foreign_item(
    ...
    // Add the redox_eii_foreign_item on the foreign item. Usually, foreign items are mangled.
    // This attribute makes sure that we later know that this foreign item's symbol should not be.
    foreign_item_attrs.push(ecx.attr_word(sym::redox_eii_foreign_item, eii_attr_span));
