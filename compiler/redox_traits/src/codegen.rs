...
use redox_middle::traits::CodegenObligationError;
use redox_middle::ty::{self, PseudoCanonicalInput, TyCtxt, TypeVisitableExt};
...
pub(crate) fn codegen_select_candidate<'tcx>(
    ...
    if !errors.is_empty() {
        // `redox_monomorphize::collector` assumes there are no type errors.
        // Cycle errors are the only post-monomorphization errors possible; emit them now so
        // `redox_ty_utils::resolve_associated_item` doesn't return `None` post-monomorphization.
        for err in errors {
