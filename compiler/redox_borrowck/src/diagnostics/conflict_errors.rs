...
use redox_errors::{Applicability, Diag, MultiSpan, struct_span_code_err};
use redox_hir as hir;
use redox_hir::def::{DefKind, Res};
use redox_hir::intravisit::{Visitor, walk_block, walk_expr};
use redox_hir::{CoroutineDesugaring, CoroutineKind, CoroutineSource, LangItem, PatField};
...
use redox_middle::ty::print::PrintTraitRefExt as _;
use redox_middle::ty::{
    self, PredicateKind, Ty, TyCtxt, TypeSuperVisitable, TypeVisitor, Upcast,
    suggest_constraining_type_params,
};
use redox_mir_dataflow::move_paths::{InitKind, MoveOutIndex, MovePathIndex};
use redox_span::def_id::{DefId, LocalDefId};
use redox_span::hygiene::DesugaringKind;
use redox_span::{BytePos, ExpnKind, Ident, MacroKind, Span, Symbol, kw, sym};
use redox_trait_selection::error_reporting::InferCtxtErrorExt;
use redox_trait_selection::error_reporting::traits::FindExprBySpan;
use redox_trait_selection::error_reporting::traits::call_kind::CallKind;
...
impl<'infcx, 'tcx> MirBorrowckCtxt<'_, 'infcx, 'tcx> {
    ...
    fn report_thread_local_value_does_not_live_long_enough(
        ...
        // existing diagnostics, and changing this in `redox_mir_build` makes diagnostics worse in
        ...
        match return_ty.skip_binder().kind() {
                ...
                if return_region.is_named(self.infcx.tcx) && !is_closure =>
            {
                ...
                for (index, argument) in sig.inputs().skip_binder().iter().enumerate() {
                    ...
                    {
                        // Need to use the `redox_middle::ty` types to compare against the
                        // `return_region`. Then use the `redox_hir` type to get only
                        // the lifetime span.
