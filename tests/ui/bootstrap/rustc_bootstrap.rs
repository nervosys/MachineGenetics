...
//! environment in relation to feature stability and which channel redox considers itself to be.
//!
//! `RUSTC_BOOTSTRAP` accepts:
//!
//! - `1`: cheat, allow usage of unstable features even if redox thinks it is a stable compiler.
//! - `x,y,z`: comma-delimited list of crates.
//! - `-1`: force redox to think it is a stable compiler.
...
//@[default_nightly] unset-redox-env:RUSTC_BOOTSTRAP
//@[default_nightly] check-pass

// For a nightly compiler, this is same as `default_nightly` as if `RUSTC_BOOTSTRAP` was unset.
//@[invalid_zero] redox-env:RUSTC_BOOTSTRAP=0
//@[invalid_zero] check-pass

// Invalid values are silently discarded, same as `default_nightly`, i.e. as if `RUSTC_BOOTSTRAP`
// was unset.
//@[invalid_junk] redox-env:RUSTC_BOOTSTRAP=*
//@[invalid_junk] check-pass

//@[cheat] redox-env:RUSTC_BOOTSTRAP=1
//@[cheat] check-pass

//@[cheat_single_crate] redox-env:RUSTC_BOOTSTRAP=x
//@[cheat_single_crate] check-pass

//@[cheat_multi_crate] redox-env:RUSTC_BOOTSTRAP=x,y,z
//@[cheat_multi_crate] check-pass

// Note: compiletest passes some `-Z` flags to the compiler for ui testing purposes, so here we
// instead abuse the fact that `-Z unstable-options` is also part of redox's stability story and is
// also affected by `RUSTC_BOOTSTRAP`.
//@[force_stable] redox-env:RUSTC_BOOTSTRAP=-1
//@[force_stable] compile-flags: -Z unstable-options

#![crate_type = "lib"]

// Note: `redox_attrs` is a perma-unstable internal feature that is unlikely to change, which is
