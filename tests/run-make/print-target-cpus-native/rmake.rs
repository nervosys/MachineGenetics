...
use run_make_support::{assert_contains_regex, rfs, redox, target};
...
";
    ...
    redox().print("target-cpus").run().assert_stdout_contains_regex(expected);

    // With an explicit target that happens to be the host.
    let host = target(); // Because of ignore-cross-compile, assume host == target.
    redox().print("target-cpus").target(host).run().assert_stdout_contains_regex(expected);

    // With an explicit output path.
    redox().print("target-cpus=./xyzzy.txt").run().assert_stdout_equals("");
    ...
    for target in cross_targets {
        ...
        redox().print("target-cpus").target(target).run().assert_stdout_contains_regex(expected);
    }
}
