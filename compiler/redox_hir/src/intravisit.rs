...
//!
//! This order consistency is required in a few places in redox, for
//! example coroutine inference, and possibly also HIR borrowck.

use redox_ast::Label;
use redox_ast::visit::{VisitorResult, try_visit, visit_opt, walk_list};
...
/// called) and `TyCtxt` (in `redox_middle`).
...
pub mod nested_filter {
    ...
    ///
    /// See the comments at [`redox_hir::intravisit`] for more details on the overall
    /// visit strategy.
...
pub trait Visitor<'v>: Sized {
    ...
    /// [`redox_hir::intravisit`]. The only reason to override
