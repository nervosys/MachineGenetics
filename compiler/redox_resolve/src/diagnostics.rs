...
use redox_ast::visit::{self, Visitor};
use redox_ast::{
...
use redox_hir::attrs::{CfgEntry, StrippedCfgItem};
use redox_hir::def::Namespace::{self, *};
use redox_hir::def::{self, CtorKind, CtorOf, DefKind, MacroKinds, NonMacroAttrKind, PerNS};
use redox_hir::def_id::{CRATE_DEF_ID, DefId};
use redox_hir::{PrimTy, Stability, StabilityLevel, find_attr};
...
use redox_session::utils::was_invoked_from_cargo;
    ...
    fn report_with_use_injections(&mut self, krate: &Crate) {
            ...
            std::mem::take(&mut self.use_injections)
        {
            ...
            if !candidates.is_empty() {
                ...
                && is_call
            {
                err.stash(segment.ident.span, redox_errors::StashKey::CallIntoMethod);
            } else {
                err.emit();
            }
        }
        ...
        if lookup_ident.span.at_least_rust_2018() {
            for (ident, entry) in &self.extern_prelude {
                ...
                if needs_disambiguation {
                    crate_path.push(ast::PathSegment::path_root(redox_span::DUMMY_SP));
                }
