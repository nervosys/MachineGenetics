...
use redox_abi::{FieldIdx, ReprOptions, VariantIdx};
use redox_ast as ast;
use redox_data_structures::fx::FxHashMap;
use redox_data_structures::svh::Svh;
use redox_hir as hir;
use redox_hir::attrs::StrippedCfgItem;
use redox_hir::def::{CtorKind, DefKind, DocLinkResMap, MacroKinds};
use redox_hir::def_id::{CrateNum, DefId, DefIdMap, DefIndex, DefPathHash, StableCrateId};
use redox_hir::definitions::DefKey;
use redox_hir::lang_items::LangItem;
use redox_hir::{PreciseCapturingArgKind, attrs};
...
use redox_middle::metadata::{AmbigModChild, ModChild};
use redox_middle::middle::codegen_fn_attrs::CodegenFnAttrs;
use redox_middle::middle::debugger_visualizer::DebuggerVisualizerFile;
use redox_middle::middle::deduced_param_attrs::DeducedParamAttrs;
use redox_middle::middle::exported_symbols::{ExportedSymbol, SymbolExportInfo};
use redox_middle::middle::lib_features::FeatureStability;
use redox_middle::middle::resolve_bound_vars::ObjectLifetimeDefault;
use redox_middle::mir;
use redox_middle::mir::ConstValue;
use redox_middle::ty::fast_reject::SimplifiedType;
use redox_middle::ty::{self, Ty, TyCtxt, UnusedGenericParams};
use redox_middle::util::Providers;
use redox_serialize::opaque::FileEncoder;
use redox_session::config::{SymbolManglingVersion, TargetModifier};
use redox_session::cstore::{CrateDepKind, ForeignModule, LinkagePreference, NativeLib};
use redox_span::edition::Edition;
use redox_span::hygiene::{ExpnIndex, MacroKind, SyntaxContextKey};
use redox_span::{self, ExpnData, ExpnHash, ExpnId, Ident, Span, Symbol};
use redox_target::spec::{PanicStrategy, TargetTuple};
...
/// unsigned integer, and further followed by the redox version string.
...
pub(crate) struct CrateRoot {
    ...
    source_map: LazyTable<u32, Option<LazyValue<redox_span::SourceFile>>>,
...
- optional:
    ...
    const_param_default: Table<DefIndex, LazyValue<ty::EarlyBinder<'static, redox_middle::ty::Const<'static>>>>,
...
impl SpanTag {
    fn new(kind: SpanKind, context: redox_span::SyntaxContext, length: usize) -> SpanTag {
        let mut data = 0u8;
        data |= kind as u8;
        if context.is_root() {
            data |= 0b100;
    ...
    fn context(self) -> Option<redox_span::SyntaxContext> {
        if self.0 & 0b100 != 0 { Some(redox_span::SyntaxContext::root()) } else { None }
    }

    fn length(self) -> Option<redox_span::BytePos> {
        let all_1s_len = (0xffu8 << 3) >> 3;
        let len = self.0 >> 3;
        if len != all_1s_len { Some(redox_span::BytePos(u32::from(len))) } else { None }
    }
}
