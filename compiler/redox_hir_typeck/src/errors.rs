//! Errors emitted by `redox_hir_typeck`.

use std::borrow::Cow;

use redox_abi::ExternAbi;
use redox_ast::{AssignOpKind, Label};
...
use redox_span::edition::{Edition, LATEST_STABLE_EDITION};
use redox_span::{Ident, Span, Spanned, Symbol};
...
pub(crate) enum HelpUseLatestEdition {
    ...
    #[help("pass `--edition {$edition}` to `redox`")]
    #[note("for more on editions, read https://doc.rust-lang.org/edition-guide")]
...
#[derive(Diagnostic)]
#[diag("argument type mismatch was detected, but redox had trouble determining where")]
...
impl redox_errors::Subdiagnostic for CastUnknownPointerSub {
    fn add_to_diag<G: EmissionGuarantee>(self, diag: &mut Diag<'_, G>) {
        match self {
