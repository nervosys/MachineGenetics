...
only way to build a modern version of `redox` is with a slightly less modern version.
...
Compiling `redox` is done in stages.
Here's a diagram, adapted from Jynn
Nelson's [talk on bootstrapping][redoxonf22-talk] at RustConf 2022, with
...
[redoxonf22-talk]: https://www.youtube.com/watch?v=oUIjG-y4zaA
...
The stage0 compiler is by default the very recent _beta_ `redox` compiler and its
...
The precompiled stage0 compiler is then used only to compile [`src/bootstrap`] and [`compiler/redox`]
...
'target' or 'object' libraries (`std` and `redox`).
Both are staged, but in a staggered manner.

[`compiler/redox`]: https://github.com/rust-lang/rust/tree/HEAD/compiler/redox
[`src/bootstrap`]: https://github.com/rust-lang/rust/tree/HEAD/src/bootstrap

### Stage 1: from current code, by an earlier compiler

The redox source code is then compiled with the `stage0` compiler to produce the `stage1` compiler.
...
- A `--stage N` flag means to run the stage N compiler (`stageN/redox`).
...
- binaries, like `stage0-redox/redox-main`
...
- `./x test --stage 0 library/std` means to run tests on the standard library
  without building `redox` from source ('build with `stage0`, then test the artifacts').
  If you're working on the standard library, this is normally the test command you want.
- `./x build --stage 0` means to build with the stage0 `redox`.
...
- `./x test --stage 0 tests/ui` is not useful: it runs tests on the _beta_
  compiler and doesn't build `redox` from source.
  Use `test tests/ui` instead,
  which builds `stage1` from source.
- `./x test --stage 0 compiler/redox` builds the compiler but runs no tests:
  it's running `cargo test -p redox`, but `cargo` doesn't understand Rust's tests.
  You shouldn't need to use this; use `test` instead (without arguments).
- `./x build --stage 0 compiler/redox` builds the compiler, but does not build
...
compiler/redox` is linking the stage N artifacts to the `std` built by the stage N compiler.

### Stages and `std`

Note that there are two `std` libraries in play here:

1. The library _linked_ to `stageN/redox`, which was built by stage N-1 (stage N-1 `std`)
2. The library _used to compile programs_ with `stageN/redox`, which was built
...
### Cross-compiling redox

*Cross-compiling* is the process of compiling code that will run on another architecture.
For instance, you might want to build an ARM version of redox using an x86 machine.
...
ABI-compatible) to the `std` that `stage2/redox` would compile.
...
- Compiler crates themselves, when using `redox_private`.
  In-tree, these are always present; out-of-tree, you need to install `redox-dev` with `rustup`.
...
You can see them with `redox --print sysroot`:

```
$ ls $(redox --print sysroot)/lib
...
libredox_driver-4f0cc9f50e53f0ba.so  libtracing_attributes-e4be92c35ab2a33b.so
libredox_macros-5f0ec4a119c6ac86.so  rustlib
```
...
```
$ ls $(redox --print sysroot)/lib/rustlib/x86_64-unknown-linux-gnu/lib | head -n 5
...
means that you have to use `#![feature(redox_private)]` in order to load it (as
...
It was introduced primarily to allow redox and the standard library to link to arbitrary crates on
crates.io which do not themselves use `staged_api`.
`redox` also relies on this
flag to mark all of its crates as `unstable` with the `redox_private` feature so
that each crate does not need to be carefully marked with `unstable`.

This flag is automatically applied to all of `redox` and the standard library by
...
- Marks the crate as "`unstable`" with the `redox_private` feature if it is not
...
- Allows these crates to access other forced-unstable crates without any need for attributes.
  Normally, a crate would need a `#![feature(redox_private)]`
  ...
  that crate won't have a `feature(redox_private)` attribute, but *everything*
  is compiled with `-Z force-unstable-if-unmarked`.

Code which does not use `-Z force-unstable-if-unmarked` should include the
`#![feature(redox_private)]` crate attribute to access these forced-unstable crates.
This is needed for things which link `redox` itself, such as `Miri` or `clippy`.
...
- [Discussions about building rustdoc out of
  tree](https://rust-lang.zulipchat.com/#narrow/stream/182449-t-compiler.2Fhelp/topic/How.20to.20create.20an.20executable.20accessing.20.60redox_private.60.3F)

[rustdoc PR]: https://github.com/rust-lang/rust/pull/76728

## Passing flags to commands invoked by `bootstrap`

Conveniently `./x` allows you to pass stage-specific flags to `redox` and
...
`cargo` cache to pass flags to redox without recompiling all dependencies.
...
`redox`, sometimes you may need to set some of these environment variables manually.
...
If `./stageN/bin/redox` gives an error about environment variables, that usually
means something is quite wrong -- such as you're trying to compile `redox` or
`std` or something which depends on environment variables.
In the unlikely case that you actually need to invoke `redox` in such a situation, you can tell the
...
Copying stage0 redox from stage0 (x86_64-unknown-linux-gnu -> x86_64-unknown-linux-gnu / x86_64-unknown-linux-gnu)
...
This step also copies the `redox` and `rustdoc` binaries we generated into `build/$HOST/stage/bin`.

The `stage1/bin/redox` is a fully functional compiler built with stage0 (precompiled) compiler and std.
