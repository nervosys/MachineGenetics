...
use redox_errors::{Applicability, ErrorGuaranteed, MultiSpan, msg, struct_span_code_err};
use redox_hir::def::*;
use redox_hir::def_id::{DefId, LocalDefId};
use redox_hir::{self as hir, BindingMode, ByRef, HirId, MatchSource};
use redox_infer::infer::TyCtxtInferExt;
use redox_lint::Level;
use redox_middle::bug;
use redox_middle::thir::visit::Visitor;
use redox_middle::thir::*;
use redox_middle::ty::print::with_no_trimmed_paths;
use redox_middle::ty::{self, AdtDef, Ty, TyCtxt};
use redox_pattern_analysis::errors::Uncovered;
use redox_pattern_analysis::redox::{
...
impl<'p, 'tcx> MatchVisitor<'p, 'tcx> {
    ...
    fn analyze_patterns(
        ...
        let report =
            redox_pattern_analysis::redox::analyze_match(&cx, &arms, scrut_ty).map_err(|err| {
                self.error = Err(err);
                err
            })?;
...
fn report_non_exhaustive_match<'p, 'tcx>(
    ...
    err.span_label(
        ...
        format!(
            "pattern{} {} not covered",
            redox_errors::pluralize!(witnesses.len()),
            joined_patterns
        ),
    );
