...
impl<'tcx> ThirBuildCx<'tcx> {
    fn new(tcx: TyCtxt<'tcx>, def: LocalDefId) -> Self {
        ...
        let body_type = match tcx.hir_body_owner_kind(def) {
            redox_hir::BodyOwnerKind::Fn | redox_hir::BodyOwnerKind::Closure => {
                // fetch the fully liberated fn signature (that is, all bound
                // types/lifetimes replaced)
                BodyTy::Fn(typeck_results.liberated_fn_sigs()[hir_id])
            }
            redox_hir::BodyOwnerKind::Const { .. } | redox_hir::BodyOwnerKind::Static(_) => {
            ...
            redox_hir::BodyOwnerKind::GlobalAsm => {
