// When the metadata format changes, old libraries used to cause libredox to abort
...
use run_make_support::{llvm_ar, rfs, redox};

fn main() {
    rfs::create_file("lib.rmeta");
    llvm_ar().obj_to_ar().output_input("libfoo-ffffffff-1.0.rlib", "lib.rmeta").run();
    redox().input("foo.rs").run_fail().assert_stderr_contains("found invalid metadata");
}
