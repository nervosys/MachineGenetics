...
use redox_hir::attrs::{BorrowckGraphvizFormatKind, RustcMirKind};
...
impl RustcMirAttrs {
    fn parse(tcx: TyCtxt<'_>, def_id: DefId) -> Self {
        ...
        if let Some(redox_mir_attrs) = find_attr!(tcx, def_id, RustcMir(kind) => kind) {
            for attr in redox_mir_attrs {
                match attr {
