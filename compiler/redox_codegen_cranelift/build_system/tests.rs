...
use crate::redox_info::get_default_sysroot;
...
pub(crate) fn run_tests(
    ...
    let stdlib_source =
        get_default_sysroot(&bootstrap_host_compiler.redox).join("lib/rustlib/src/rust");
...
impl<'a> TestRunner<'a> {
        ...
        for TestCase { config, cmd } in tests {
            ...
            match *cmd {
                ...
                TestCaseCmd::BuildLib { source, crate_types } => {
                    self.run_redox([source, "--crate-type", crate_types]);
                }
                TestCaseCmd::BuildBin { source } => {
                    self.run_redox([source]);
                }
                ...
                TestCaseCmd::JitBin { source, args } => {
                    let mut jit_cmd = self.redox_command([
                        "-Zunstable-options",
                        "-Cllvm-args=jit-mode",
                        "-Cprefer-dynamic",
                        source,
    ...
    #[must_use]
    fn redox_command<I, S>(&self, args: I) -> Command
    where
        I: IntoIterator<Item = S>,
        S: AsRef<OsStr>,
    {
        let mut cmd = Command::new(&self.target_compiler.redox);
    ...
    fn run_redox<I, S>(&self, args: I)
    where
        I: IntoIterator<Item = S>,
        S: AsRef<OsStr>,
    {
        spawn_and_wait(self.redox_command(args));
    }
