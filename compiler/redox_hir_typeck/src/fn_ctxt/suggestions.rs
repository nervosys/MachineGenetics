...
use redox_errors::{Applicability, Diag, MultiSpan, listify, msg};
...
use redox_hir_analysis::hir_ty_lowering::HirTyLowerer;
use redox_hir_analysis::suggest_impl_trait;
...
use redox_span::{ExpnKind, Ident, MacroKind, Span, Spanned, Symbol, sym};
use redox_trait_selection::error_reporting::InferCtxtErrorExt;
use redox_trait_selection::error_reporting::traits::DefIdOrName;
...
use super::FnCtxt;
use crate::errors;
use crate::fn_ctxt::redox_span::BytePos;
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    pub(crate) fn suggest_remove_last_method_call(
            ...
            && (name.starts_with("to_") || name.starts_with("as_") || name == "into")
        {
            let span = if let Some(recv_span) = recv_expr.span.find_ancestor_inside(expr.span) {
                ...
                expr.span.with_lo(method.span.lo() - redox_span::BytePos(1))
            };
                ...
                diag.multipart_suggestion(
                    ...
                    vec![
                        (
                            blk.span
                                .shrink_to_lo()
                                .with_hi(redox_span::BytePos(blk.span.lo().0 + 1)),
                            "[".to_string(),
                        ),
                        (
                            blk.span
                                .shrink_to_hi()
                                .with_lo(redox_span::BytePos(blk.span.hi().0 - 1)),
                            "]".to_string(),
                        ),
                    ],
                    Applicability::MachineApplicable,
    ...
    pub(crate) fn suggest_floating_point_literal(
        ...
        match expr.kind {
            ...
            ExprKind::Lit(Spanned {
                node: redox_ast::LitKind::Int(lit, redox_ast::LitIntType::Unsuffixed),
                span,
            }) => {
    ...
    pub(crate) fn suggest_null_ptr_for_literal_zero_given_to_ptr_arg(
        ...
        let ExprKind::Lit(Spanned { node: redox_ast::LitKind::Int(Pu128(0), _), span }) = expr.kind
        else {
            return false;
        };
    ...
    pub(crate) fn is_field_suggestable(
        ...
        field.vis.is_accessible_from(self.tcx.parent_module(hir_id), self.tcx)
            ...
            && !matches!(
                self.tcx.eval_stability(field.did, None, redox_span::DUMMY_SP, None),
                redox_middle::middle::stability::EvalResult::Deny { .. }
            )
    ...
    pub(crate) fn suggest_semicolon_in_repeat_expr(
            ...
            && second.hir_id == expr.hir_id
        {
            ...
            // Checking this here instead of redox_hir::hir because
            // this check needs access to `self.tcx` but redox_hir
