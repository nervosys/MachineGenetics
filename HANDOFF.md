...
| 29 | **The contract verifier's two entry points disagree, and `Verified` is unreachable from source** | `verify_item` builds a clause's condition with `condition_parts.join(" ")`, so `@req(xs.len() >= 0)` reaches `check_condition` as `xs . len ( ) >= 0` — while every pattern that function matches (`.contains(".len() >= 0")`, `.contains(".is_ok()")`, `.ends_with(".exists()")`) is written in *source* spelling. **Measured 2026-09-08, the same contract through both doors**: `verify/contracts` with the caller-supplied string `xs.len() >= 0` returns `Verified` / `proved`; `verify/module` on that identical text written in MAGE returns `Partial` / `Unreached`. The consequence is bigger than the asymmetry — `CheckResult::Verified` is **unreachable for any contract written in source**, and `Violated` has no branch producing it at all, so `VerifyStatus::Verified` cannot be reached through `verify/module` by any program. **The decision, not the code, is what is missing.** `Token` carries a `span`, so slicing the real text is cheap; but doing it turns on a branch that makes a *substring match* emit `Proved`, the strongest word in `verdict.rs`, for real programs — and `check_condition` says of itself that "a real implementation would use symbolic execution or SMT solving". Deciding that a `.contains()` test may certify a contract is a claim about what this compiler asserts, and it belongs to whoever owns the verifier. Filed rather than fixed for that reason. |
