...
use redox_span::hygiene::ExpnData;
use redox_span::source_map::{FilePathMapping, SourceMap};
use redox_span::{FileName, RealFileName, Span};
use serde::Serialize;
...
impl Emitter for JsonEmitter {
    ...
    fn emit_unused_externs(&mut self, lint_level: redox_lint_defs::Level, unused_externs: &[&str]) {
...
struct Diagnostic {
    ...
    /// The message as redox would render it.
...
impl Diagnostic {
    /// Converts from `redox_errors::DiagInner` to `Diagnostic`.
...
impl DiagnosticSpan {
    ...
    fn from_span_full(
        ...
        if start.col.0 == 0
            ...
            && after.starts_with('\n')
        {
            span = span.with_hi(span.hi() + redox_span::BytePos(1));
        }
