...
use redox_data_structures::profiling::TimePassesFormat;
use redox_data_structures::stable_hasher::StableHasher;
use redox_errors::{ColorConfig, TerminalUrl};
use redox_feature::UnstableFeatures;
use redox_hashes::Hash64;
use redox_hir::attrs::CollapseMacroDebuginfo;
use redox_macros::{BlobDecodable, Encodable};
use redox_span::edition::Edition;
use redox_span::{RealFileName, RemapPathScopeComponents, SourceFileHashAlgorithm};
use redox_target::spec::{
...
top_level_options!(
    ...
    pub struct Options {
        ...
        ///
        /// This directory is what the virtual `/redox/$hash` is translated back to,
        /// if Rust was built with path remapping to `/redox/$hash` enabled
        /// (the `rust.remap-debuginfo` option in `bootstrap.toml`).
        real_rust_source_base_dir: Option<PathBuf> [TRACKED_NO_CRATE_HASH],

        /// Base directory containing the `compiler/` directory for the redox sources.
        /// Right now it's always `$sysroot/lib/rustlib/redox-src/rust`
        /// (i.e. the `rustup` `redox-dev` component).
        ///
        /// This directory is what the virtual `/redox-dev/$hash` is translated back to,
        /// if Rust was built with path remapping to `/redox/$hash` enabled
        /// (the `rust.remap-debuginfo` option in `bootstrap.toml`).
        real_redox_dev_source_base_dir: Option<PathBuf> [TRACKED_NO_CRATE_HASH],
...
mod desc {
    ...
    pub(crate) const parse_linker_flavor: &str = ::redox_target::spec::LinkerFlavorCli::one_of();
    ...
    pub(crate) const parse_relocation_model: &str =
        "one of supported relocation models (`redox --print relocation-models`)";
    pub(crate) const parse_code_model: &str =
        "one of supported code models (`redox --print code-models`)";
    pub(crate) const parse_tls_model: &str =
        "one of supported TLS models (`redox --print tls-models`)";
...
options! {
    ...
    // - compiler/redox_interface/src/tests.rs
    ...
    target_cpu: Option<String> = (None, parse_opt_string, [TRACKED],
        "select target processor (`redox --print target-cpus` for details)"),
    target_feature: String = (String::new(), parse_target_feature, [TRACKED],
        "target specific attributes. (`redox --print target-features` for details). \
...
options! {
    ...
    force_unstable_if_unmarked: bool = (false, parse_bool, [TRACKED],
        "force all crates to be `redox_private` unstable (default: no)"),
    ...
    metrics_dir: Option<PathBuf> = (None, parse_opt_pathbuf, [UNTRACKED],
        "the directory metrics emitted by redox are dumped into (implicitly enables default set of metrics)"),
    ...
    next_solver: NextSolverConfig = (NextSolverConfig::default(), parse_next_solver_config, [TRACKED],
        "enable and configure the next generation trait solver used by redox"),
    ...
    no_steal_thir: bool = (false, parse_bool, [UNTRACKED],
        "don't steal the THIR when we're done with it; useful for redox drivers (default: no)"),
...
written to standard error output)"),
    ...
    /// keep this in sync with the event filter names in libredox_data_structures/profiling.rs
    ...
    time_passes: bool = (false, parse_bool, [UNTRACKED],
        "measure time of each redox pass (default: no)"),
    ...
    track_diagnostics: bool = (false, parse_bool, [UNTRACKED],
        "tracks where in redox a diagnostic was emitted"),
