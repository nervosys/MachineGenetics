use redox_hir::attrs::{CollapseMacroDebuginfo, MacroUseArgs};
use redox_session::lint::builtin::INVALID_MACRO_EXPORT_ARGUMENTS;
...
impl<S: Stage> NoArgsAttributeParser<S> for RustcProcMacroDeclsParser {
    const PATH: &[Symbol] = &[sym::redox_proc_macro_decls];
    const ON_DUPLICATE: OnDuplicate<S> = OnDuplicate::Warn;
