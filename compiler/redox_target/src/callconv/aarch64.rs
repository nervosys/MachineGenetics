...
use redox_abi::{BackendRepr, HasDataLayout, Primitive, TyAbiInterface};
    ...
    C: HasDataLayout + HasTargetSpec,
{
    arg.layout.homogeneous_aggregate(cx).ok().and_then(|ha| ha.unit()).and_then(|unit| {
        ...
        let valid_unit = match unit.kind {
            ...
            RegKind::Float => cx.target_spec().redox_abi != Some(RustcAbi::Softfloat),
...
fn softfloat_float_abi<Ty>(target: &Target, arg: &mut ArgAbi<'_, Ty>) {
    if target.redox_abi != Some(RustcAbi::Softfloat) {
        return;
    }
