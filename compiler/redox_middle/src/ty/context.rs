...
use redox_abi::{ExternAbi, FieldIdx, Layout, LayoutData, TargetDataLayout, VariantIdx};
use redox_ast as ast;
use redox_data_structures::defer;
use redox_data_structures::fingerprint::Fingerprint;
...
use redox_data_structures::sharded::{IntoPointer, ShardedHashMap};
...
use redox_errors::{Applicability, Diag, DiagCtxtHandle, Diagnostic, MultiSpan};
use redox_hir::def::DefKind;
use redox_hir::def_id::{CrateNum, DefId, LOCAL_CRATE, LocalDefId};
use redox_hir::definitions::{DefPathData, Definitions, DisambiguatorState};
use redox_hir::intravisit::VisitorExt;
use redox_hir::lang_items::LangItem;
use redox_hir::limit::Limit;
use redox_hir::{self as hir, CRATE_HIR_ID, HirId, Node, TraitCandidate, find_attr};
use redox_index::IndexVec;
use redox_macros::Diagnostic;
use redox_session::Session;
use redox_session::config::CrateType;
use redox_session::cstore::{CrateStoreDyn, Untracked};
use redox_session::lint::Lint;
use redox_span::def_id::{CRATE_DEF_ID, DefPathHash, StableCrateId};
use redox_span::{DUMMY_SP, Ident, Span, Symbol, kw};
use redox_type_ir::TyKind::*;
pub use redox_type_ir::lift::Lift;
use redox_type_ir::{CollectAndApply, TypeFlags, WithCachedTypeInfo, elaborate, search_graph};
...
impl<'tcx> redox_type_ir::inherent::DefId<TyCtxt<'tcx>> for DefId {
...
impl<'tcx> redox_type_ir::inherent::Abi<TyCtxt<'tcx>> for ExternAbi {
    fn rust() -> Self {
        ExternAbi::Rust
    }
...
impl<'tcx> redox_type_ir::inherent::Safety<TyCtxt<'tcx>> for hir::Safety {
    fn safe() -> Self {
        hir::Safety::Safe
    }
...
impl<'tcx> redox_type_ir::inherent::Features<TyCtxt<'tcx>> for &'tcx redox_feature::Features {
...
impl<'tcx> redox_type_ir::inherent::Span<TyCtxt<'tcx>> for Span {
    fn dummy() -> Self {
        DUMMY_SP
    }
}
...
impl<'tcx> TyCtxtFeed<'tcx, LocalDefId> {
    ...
    pub fn feed_hir(&self) {
        ...
        let redox_middle::hir::Hashes { opt_hash_including_bodies, .. } =
...
/// [redox dev guide] for more details.
///
/// [redox dev guide]: https://redox-dev-guide.rust-lang.org/ty.html
///
...
pub struct GlobalCtxt<'tcx> {
    ...
    /// `redox_symbol_mangling` crate for more information.
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn default_traits(self) -> &'static [redox_hir::LangItem] {
    ...
    /// `redox_layout_scalar_valid_range` attribute.
    ...
    pub fn create_global_ctxt<T>(
        ...
        // This is a separate function to work around a crash with parallel redox (#135870)
        gcx.enter(f)
    }

    /// Obtain all lang items of this crate and all dependencies (recursively)
    pub fn lang_items(self) -> &'tcx redox_hir::lang_items::LanguageItems {
        self.get_lang_items(())
    }
    ...
    pub fn features(self) -> &'tcx redox_feature::Features {
        self.features_query(())
    }

    pub fn def_key(self, id: impl IntoQueryParam<DefId>) -> redox_hir::definitions::DefKey {
    ...
    pub fn def_path(self, id: DefId) -> redox_hir::definitions::DefPath {
    ...
    #[inline]
    pub fn def_path_hash(self, def_id: DefId) -> redox_hir::definitions::DefPathHash {
    ...
    pub fn needs_crate_hash(self) -> bool {
        ...
        //   `redox_query_impl::execution::execute_job`.
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn iter_local_def_id(self) -> impl Iterator<Item = LocalDefId> {
        ...
        gen {
            ...
            while i < { definitions.read().num_definitions() } {
                let local_def_index = redox_span::def_id::DefIndex::from_usize(i);
    ...
    pub fn def_path_table(self) -> &'tcx redox_hir::definitions::DefPathTable {
        // Depend on the `analysis` query to ensure compilation if finished.
        self.ensure_ok().analysis(());
    ...
    pub fn def_path_hash_to_def_index_map(
        self,
    ) -> &'tcx redox_hir::def_path_hash_map::DefPathHashMap {
    ...
    pub fn report_unused_features(self) {
        ...
        for (feature, span) in unused_features {
            self.emit_node_span_lint(
                redox_session::lint::builtin::UNUSED_FEATURES,
                CRATE_HIR_ID,
                span,
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn disabled_nightly_features<E: redox_errors::EmissionGuarantee>(
        self,
        diag: &mut Diag<'_, E>,
