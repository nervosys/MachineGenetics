...
use redox_codegen_ssa::mir::place::PlaceRef;
...
use redox_middle::ty::{self, AtomicOrdering, Instance, Ty, TyCtxt};
use redox_span::Span;
use redox_span::def_id::DefId;
use redox_target::callconv::FnAbi;
use redox_target::spec::{HasTargetSpec, HasX86AbiOpt, Target, X86Abi};
...
impl<'a, 'gcc, 'tcx> Builder<'a, 'gcc, 'tcx> {
    ...
    fn atomic_extremum(
        ...
        // NOTE: since jumps were added in a place redox does not expect, the current block in the
        // state need to be updated.
...
impl<'a, 'gcc, 'tcx> BuilderMethods<'a, 'tcx> for Builder<'a, 'gcc, 'tcx> {
    ...
    fn exactsdiv(&mut self, a: RValue<'gcc>, b: RValue<'gcc>) -> RValue<'gcc> {
        // FIXME(antoyo): poison if not exact.
        // FIXME(antoyo): redox_codegen_ssa::mir::intrinsic uses different types for a and b but they
        // should be the same.
    ...
    fn alloca(&mut self, size: Size, align: Align) -> RValue<'gcc> {
        ...
        // FIXME(antoyo): It might be better to return a LValue, but fixing the redox API is non-trivial.
        self.current_func()
    ...
    fn memcpy(
        ...
        size: RValue<'gcc>,
        flags: MemFlags,
        _tt: Option<redox_ast::expand::typetree::FncTree>, // Autodiff TypeTrees are LLVM-only, ignored in GCC backend
    ) {
    ...
    fn insert_value(
        ...
        let value =
            // NOTE: sometimes, redox will create a value with the wrong type.
    ...
    fn tail_call(
        ...
        // FIXME: implement support for explicit tail calls like redox_codegen_llvm.
