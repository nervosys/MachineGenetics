...
# This block is ignored by redox
pushd $(dirname "$0")/../
RUSTC="$(pwd)/dist/redox-clif"
popd
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for line in profile.lines() {
        ...
        if !stack.contains("redox_codegen_cranelift") {
            continue;
        }

        if stack.contains("redox_monomorphize::partitioning::collect_and_partition_mono_items")
            || stack.contains("redox_incremental::assert_dep_graph::assert_dep_graph")
            || stack.contains("redox_symbol_mangling::test::report_symbol_names")
        {
            continue;
        }

        // Trim start
        if let Some(index) = stack.find("redox_interface::passes::configure_and_expand") {
            stack = &stack[index..];
        } else if let Some(index) = stack.find("redox_interface::passes::analysis") {
            stack = &stack[index..];
        } else if let Some(index) = stack.find("redox_interface::passes::start_codegen") {
            stack = &stack[index..];
        } else if let Some(index) = stack.find("redox_interface::queries::Linker::link") {
            stack = &stack[index..];
        }

        if let Some(index) = stack.find("redox_codegen_cranelift::driver::aot::module_codegen") {
            stack = &stack[index..];
        }
        ...
        const TYPECK_ITEM_BODIES: &str = "redox_typeck::check::typeck_item_bodies";
        ...
        const COLLECT_AND_PARTITION_MONO_ITEMS: &str =
            "redox_monomorphize::partitioning::collect_and_partition_mono_items";
        if let Some(index) = stack.find(COLLECT_AND_PARTITION_MONO_ITEMS) {
            stack = &stack[..index + COLLECT_AND_PARTITION_MONO_ITEMS.len()];
        }

        const ASSERT_DEP_GRAPH: &str = "redox_incremental::assert_dep_graph::assert_dep_graph";
        if let Some(index) = stack.find(ASSERT_DEP_GRAPH) {
            stack = &stack[..index + ASSERT_DEP_GRAPH.len()];
        }

        const REPORT_SYMBOL_NAMES: &str = "redox_symbol_mangling::test::report_symbol_names";
        if let Some(index) = stack.find(REPORT_SYMBOL_NAMES) {
            stack = &stack[..index + REPORT_SYMBOL_NAMES.len()];
        }

        const ENCODE_METADATA: &str = "redox_metadata::rmeta::encoder::encode_metadata";
        if let Some(index) = stack.find(ENCODE_METADATA) {
            stack = &stack[..index + ENCODE_METADATA.len()];
        }

        const INSTANTIATE_AND_NORMALIZE_ERASING_REGIONS: &str = "redox_middle::ty::normalize_erasing_regions::<impl redox_middle::ty::context::TyCtxt>::instantiate_and_normalize_erasing_regions";
        ...
        const NORMALIZE_ERASING_LATE_BOUND_REGIONS: &str = "redox_middle::ty::normalize_erasing_regions::<impl redox_middle::ty::context::TyCtxt>::normalize_erasing_late_bound_regions";
