...
use run_make_support::{build_native_static_lib, cc, dynamic_lib_name, is_darwin, llvm_nm, redox};

fn main() {
    cc().input("foo.c").arg("-c").out_exe("foo.o").run();
    build_native_static_lib("foo");

    redox().input("foo.rs").arg("-lstatic=foo").crate_type("cdylib").run();
    ...
    redox().input("foo_export.rs").arg("-lstatic:+export-symbols=foo").crate_type("cdylib").run();
