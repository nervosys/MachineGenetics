...
use redox_codegen_ssa::{TargetConfig, target_features};
use redox_data_structures::fx::FxHashSet;
use redox_data_structures::small_c_str::SmallCStr;
use redox_fs_util::path_to_c_string;
use redox_middle::bug;
use redox_session::Session;
use redox_session::config::{PrintKind, PrintRequest};
...
unsafe fn configure_llvm(sess: &Session) {
...

    {
        ...
        add("redox -Cllvm-args=\"...\" with", true);
    ...
    redox_llvm::initialize_available_targets();
...
///
/// Check the current redox fork of LLVM in the repo at
...
pub(crate) fn print(req: &PrintRequest, out: &mut String, sess: &Session) {
    ...
    match req.kind {
        ...
        _ => bug!("redox_codegen_llvm can't handle print request: {:?}", req),
    }
}
...
fn print_target_features(sess: &Session, tm: &llvm::TargetMachine, out: &mut String) {
    ...
    let mut redox_target_features = sess
        .target
    ...
    redox_target_features.extend_from_slice(&[(
        "crt-static",
        "Enables C Run-time Libraries to be statically linked",
    )]);
    // ... we need to sort the list again.
    redox_target_features.sort();

    llvm_target_features.retain(|(f, _d)| !known_llvm_target_features.contains(f));

    let max_feature_len = llvm_target_features
        .iter()
        .chain(redox_target_features.iter())
        .map(|(feature, _desc)| feature.len())
        .max()
        .unwrap_or(0);

    writeln!(out, "Features supported by redox for this target:").unwrap();
    for (feature, desc) in &redox_target_features {
    ...
    writeln!(out, "For example, redox -C target-cpu=mycpu -C target-feature=+feature1,-feature2\n")
        .unwrap();
    writeln!(out, "Code-generation features cannot be used in cfg or #[target_feature],").unwrap();
    writeln!(out, "and may be renamed or removed in a future version of LLVM or redox.\n").unwrap();
}
