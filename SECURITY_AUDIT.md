...
| **Deserialization** (Machine Language containers, MessagePack protocol, checkpoints) | T1565 (data manipulation) | Machine Language decode is **length-checked, bounds-validated** (`take()` guards every field) — verified in `run_dispatch_machine_bytes`. **No `pickle`-class arbitrary-code-execution path** — formats are data-only (contrast PyTorch `torch.load`, flagged in agentic-eval). Malformed input yields a typed `RmiError`, not memory unsafety. |
