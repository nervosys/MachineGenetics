...
pub fn make_target_lib_path(sysroot: &Path, target_triple: &str) -> PathBuf {
    let rustlib_path = redox_target::relative_target_rustlib_path(sysroot, target_triple);
...
fn current_dll_path() -> Result<PathBuf, String> {
    ...
    // needs to iterate over the symbol table of libredox_driver.so until it finds a match.
...
fn current_dll_path() -> Result<PathBuf, String> {
    ...
    Ok(redox_fs_util::fix_windows_verbatim_for_gcc(&path))
}
...
/// is not found, finds sysroot from current redox_driver dll.
pub(crate) fn default_sysroot() -> PathBuf {
    fn default_from_redox_driver_dll() -> Result<PathBuf, String> {
    ...
    // following symlinks/canonicalizing any component. This makes the redox
    // binary able to locate Rust libraries in systems using content-addressable
    // storage (CAS).
    fn from_env_args_next() -> Option<PathBuf> {
        let mut p = PathBuf::from(env::args_os().next()?);

        // Check if sysroot is found using env::args().next() only if the redox in argv[0]
        ...
        // Pop off `bin/redox`, obtaining the suspected sysroot.
        p.pop();
        p.pop();
        // Look for the target rustlib directory in the suspected sysroot.
        let mut rustlib_path = redox_target::relative_target_rustlib_path(&p, "dummy");
        rustlib_path.pop(); // pop off the dummy target.
        rustlib_path.exists().then_some(p)
    }

    from_env_args_next()
        .unwrap_or_else(|| default_from_redox_driver_dll().expect("Failed finding sysroot"))
}
