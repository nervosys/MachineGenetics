...
use redox_data_structures::stable_hasher::{HashStable, StableHasher, StableOrd, ToStableHashKey};
use redox_data_structures::unhash::Unhasher;
...
/// `redox_middle` (with ~50000 `LocalDefId`s as of the time of writing) there
...
/// further trouble.
///
/// For more information on the possibility of hash collisions in redox,
...
impl StableCrateId {
    ...
    pub fn new(
        ...
        // Also incorporate the redox version. Otherwise, with -Zsymbol-mangling-version=v0
        // and no -Cmetadata, symbols from the same crate compiled with different versions of
        // redox are named the same.
        //
        // RUSTC_FORCE_RUSTC_VERSION is used to inject redox version information
        // during testing.
...
// is used throughout redox, has problems distributing the entropy from the high bits, so reversing
...
redox_data_structures::define_id_collections!(DefIdMap, DefIdSet, DefIdMapEntry, DefId);
...
redox_data_structures::define_id_collections!(
    LocalDefIdMap,
    LocalDefIdSet,
    LocalDefIdMapEntry,
    LocalDefId
...
// `redox_middle::dep_graph::dep_node_key` for `DepNodeKey`.
