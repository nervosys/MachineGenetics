use redox_abi::{HasDataLayout, Size, TagEncoding, Variants};
        ...
        macro_rules! encode_store {
            ($curr_idx: expr, $endian: expr, $bytes: expr) => {
                let bytes = match $endian {
                    redox_abi::Endian::Little => $bytes.to_le_bytes(),
                    redox_abi::Endian::Big => $bytes.to_be_bytes(),
                };
