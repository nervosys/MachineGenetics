...
use run_make_support::{bare_redox, diff, rfs, redox};

fn main() {
    // Check that relative paths are preserved in the diagnostic
    rfs::create_dir("mylibs");
    redox().input("crateresolve1-1.rs").out_dir("mylibs").extra_filename("-1").run();
    redox().input("crateresolve1-2.rs").out_dir("mylibs").extra_filename("-2").run();
    check("./mylibs");
