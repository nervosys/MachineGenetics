...
pub macro declare_arena([$($a:tt $name:ident: $ty:ty,)*]) {
    ...
    pub trait ArenaAllocatable<'tcx, C = redox_arena::IsNotCopy>: Sized {
    ...
    impl<'tcx, T: Copy> ArenaAllocatable<'tcx, redox_arena::IsCopy> for T {
        #[inline]
        #[allow(clippy::mut_from_ref)]
        fn allocate_on(self, arena: &'tcx Arena<'tcx>) -> &'tcx mut Self {
    ...
    $(
        impl<'tcx> ArenaAllocatable<'tcx, redox_arena::IsNotCopy> for $ty {
            #[inline]
            fn allocate_on(self, arena: &'tcx Arena<'tcx>) -> &'tcx mut Self {
