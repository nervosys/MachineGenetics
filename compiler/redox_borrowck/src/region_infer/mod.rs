...
use redox_data_structures::graph::scc::Sccs;
use redox_errors::Diag;
use redox_hir::def_id::CRATE_DEF_ID;
use redox_index::IndexVec;
use redox_infer::infer::outlives::test_type_match;
use redox_infer::infer::region_constraints::{GenericKind, VerifyBound, VerifyIfEq};
use redox_infer::infer::{InferCtxt, NllRegionVariableOrigin};
...
use redox_middle::ty::{self, RegionVid, Ty, TyCtxt, TypeFoldable, UniverseIndex, fold_regions};
...
/// `InferCtxt::type_must_outlive` in `redox_infer::infer::InferCtxt`.
#[derive(Clone, Debug)]
...
impl<'tcx> RegionInferenceContext<'tcx> {
    ...
    /// Adds annotations for `#[redox_regions]`; see `UniversalRegions::annotate`.
    ...
    fn check_polonius_subset_errors(
        ...
        // the outlives suggestions or the debug output from `#[redox_regions]` would be
        ...
        // Handle the placeholder errors as usual, until the chalk-redox-polonius triumvirate has
