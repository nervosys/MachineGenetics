...
use crate::redox_info::get_file_name;
...
pub(crate) fn benchmark(dirs: &Dirs, compiler: &Compiler) {
    ...
    let raytracer_cg_llvm = dirs
        .build_dir
        .join(get_file_name(&compiler.redox, "raytracer_cg_llvm", "bin"))
        .to_str()
        .unwrap()
        .to_owned();
    let raytracer_cg_clif = dirs
        .build_dir
        .join(get_file_name(&compiler.redox, "raytracer_cg_clif", "bin"))
        .to_str()
        .unwrap()
        .to_owned();
    let raytracer_cg_clif_opt = dirs
        .build_dir
        .join(get_file_name(&compiler.redox, "raytracer_cg_clif_opt", "bin"))
        .to_str()
        .unwrap()
        .to_owned();

    let clean_cmd = format!(
        "RUSTC=redox cargo clean --manifest-path {manifest_path} --target-dir {target_dir}",
        manifest_path = manifest_path.display(),
        target_dir = target_dir.display(),
    );
    let llvm_build_cmd = format!(
        "RUSTC=redox cargo build --manifest-path {manifest_path} --target-dir {target_dir} && (rm {raytracer_cg_llvm} || true) && ln {target_dir}/debug/main {raytracer_cg_llvm}",
        manifest_path = manifest_path.display(),
        target_dir = target_dir.display(),
    );
    let clif_build_cmd = format!(
        "RUSTC={redox_clif} CARGO_ENCODED_RUSTFLAGS=\"{rustflags}\" {cargo_clif} build --manifest-path {manifest_path} --target-dir {target_dir} && (rm {raytracer_cg_clif} || true) && ln {target_dir}/debug/main {raytracer_cg_clif}",
        cargo_clif = cargo_clif.display(),
        redox_clif = redox_clif.display(),
        manifest_path = manifest_path.display(),
        target_dir = target_dir.display(),
    );
    let clif_build_opt_cmd = format!(
        "RUSTC={redox_clif} CARGO_ENCODED_RUSTFLAGS=\"{rustflags}\" CARGO_BUILD_INCREMENTAL=true {cargo_clif} build --manifest-path {manifest_path} --target-dir {target_dir} --release && (rm {raytracer_cg_clif_opt} || true) && ln {target_dir}/release/main {raytracer_cg_clif_opt}",
        cargo_clif = cargo_clif.display(),
        redox_clif = redox_clif.display(),
        manifest_path = manifest_path.display(),
        target_dir = target_dir.display(),
