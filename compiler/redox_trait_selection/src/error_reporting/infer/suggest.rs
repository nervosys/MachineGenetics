...
use redox_hir::{MatchSource, Node};
use redox_middle::traits::{MatchExpressionArmCause, ObligationCause, ObligationCauseCode};
use redox_middle::ty::error::TypeError;
use redox_middle::ty::print::with_no_trimmed_paths;
use redox_middle::ty::{self as ty, GenericArgKind, IsSuggestable, Ty, TypeVisitableExt};
...
impl<'tcx> TypeErrCtxt<'_, 'tcx> {
    ...
    // FIXME: Remove once `redox_hir_typeck` is migrated to diagnostic structs
...
impl<'tcx> TypeErrCtxt<'_, 'tcx> {
    ...
    fn could_remove_semicolon(
        ...
        let span = if last_stmt.span.from_expansion() {
            let mac_call = redox_span::source_map::original_sp(last_stmt.span, blk.span);
