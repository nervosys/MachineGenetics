...
// in redox flags without a compilation failure or the removal of expected symbols.
// See https://github.com/rust-lang/rust/pull/100101

use run_make_support::{llvm_ar, llvm_nm, rfs, rust_lib_name, redox};

fn main() {
    // Build a strangely named dependency.
    redox().input("native_dep.rs").crate_type("staticlib").output("native_dep.ext").run();

    redox().input("rust_dep.rs").crate_type("rlib").arg("-Zpacked_bundled_libs").run();
