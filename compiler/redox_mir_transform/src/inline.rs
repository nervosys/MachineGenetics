...
use redox_hir::attrs::{InlineAttr, OptimizeAttr};
...
use redox_middle::ty::{self, Instance, InstanceKind, Ty, TyCtxt, TypeFlags, TypeVisitableExt};
use redox_session::config::{DebugInfo, OptLevel};
...
// by custom redox drivers, running all the steps by themselves. See #114628.
pub struct Inline;
...
impl<'tcx> crate::MirPass<'tcx> for ForceInline {
    fn is_enabled(&self, _: &redox_session::Session) -> bool {
        true
    }
...
fn check_mir_is_available<'tcx, I: Inliner<'tcx>>(
    ...
    if let Some(callee_def_id) = callee_def_id.as_local()
        && !inliner
            .tcx()
            .is_lang_item(inliner.tcx().parent(caller_def_id), redox_hir::LangItem::FnOnce)
    {
