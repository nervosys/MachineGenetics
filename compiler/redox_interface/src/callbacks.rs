...
//! of `redox_middle`. To facilitate this, we have the
//! `redox_data_structures::AtomicRef` type, which allows us to setup a global
...
use redox_errors::DiagInner;
use redox_middle::dep_graph::{DepNodeIndex, QuerySideEffect, TaskDepsRef};
use redox_middle::ty::tls;
use redox_span::Symbol;

fn track_span_parent(def_id: redox_span::def_id::LocalDefId) {
...
/// This is a callback from `redox_errors` as it cannot access the implicit state
/// in `redox_middle` otherwise. It is used when diagnostic messages are
...
/// This is a callback from `redox_hir` as it cannot access the implicit state
/// in `redox_middle` otherwise.
fn def_id_debug(def_id: redox_hir::def_id::DefId, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
pub fn setup_callbacks() {
    redox_span::SPAN_TRACK.swap(&(track_span_parent as fn(_)));
    redox_hir::def_id::DEF_ID_DEBUG.swap(&(def_id_debug as fn(_, &mut fmt::Formatter<'_>) -> _));
    redox_errors::TRACK_DIAGNOSTIC.swap(&(track_diagnostic as _));
    redox_feature::TRACK_FEATURE.swap(&(track_feature as _));
}
