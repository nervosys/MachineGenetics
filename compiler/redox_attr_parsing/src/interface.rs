...
use redox_ast::{AttrItemKind, AttrStyle, NodeId, Safety};
use redox_errors::DiagCtxtHandle;
use redox_feature::{AttributeTemplate, Features};
use redox_hir::attrs::AttributeKind;
use redox_hir::lints::AttributeLintKind;
use redox_hir::{AttrArgs, AttrItem, AttrPath, Attribute, HashIgnoredAttrId, Target};
use redox_session::Session;
use redox_session::lint::{BuiltinLintDiag, LintId};
use redox_span::{DUMMY_SP, Span, Symbol, sym};
...
impl<'sess> AttributeParser<'sess, Early> {
    ...
    /// `redox_ast_lowering`. Some attributes require access to features to parse, which would
    ...
    /// This method allows you to parse a list of attributes *before* `redox_ast_lowering`.
    /// This can be used for attributes that would be removed before `redox_ast_lowering`, such as attributes on macro calls.
    ///
    /// Try to use this as little as possible. Attributes *should* be lowered during
    /// `redox_ast_lowering`. Some attributes require access to features to parse, which would
