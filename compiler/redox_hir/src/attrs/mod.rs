...
//! Formerly `redox_attr_data_structures`.
//!
//! For detailed documentation about attribute processing,
//! see [redox_attr_parsing](https://doc.rust-lang.org/nightly/nightly-redox/redox_attr_parsing/index.html).
    ...
    ($attributes_list: expr, $pattern: pat $(if $guard: expr)? => $e: expr) => {{
        'done: {
            for i in $attributes_list {
                ...
                let i: &redox_hir::Attribute = i;
                match i {
                    redox_hir::Attribute::Parsed($pattern) $(if $guard)? => {
                        break 'done Some($e);
                    }
                    redox_hir::Attribute::Unparsed(..) => {}
