...
use redox_hir::{self as hir, find_attr};
use redox_index::bit_set::GrowableBitSet;
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    /// [direct]: redox_session::cstore::ExternCrate::is_direct
    pub fn is_user_visible_dep(self, key: CrateNum) -> bool {
        // `#![redox_private]` overrides defaults to make private dependencies usable.
        if self.features().enabled(sym::redox_private) {
            return true;
        }
...
/// Determines whether an item is an intrinsic (which may be via Abi or via the `redox_intrinsic` attribute).
///
