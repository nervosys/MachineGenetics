...
impl<'a, 'hir> LoweringContext<'a, 'hir> {
    ...
    pub(super) fn lower_contract(
        &mut self,
        body: impl FnOnce(&mut Self) -> redox_hir::Expr<'hir>,
        contract: &redox_ast::FnContract,
    ) -> redox_hir::Expr<'hir> {
    ...
    fn lower_decls(&mut self, contract: &redox_ast::FnContract) -> &'hir [redox_hir::Stmt<'hir>] {
    ...
    fn lower_postcond_checker(
        &mut self,
        ens: &Box<redox_ast::Expr>,
    ) -> &'hir redox_hir::Expr<'hir> {
        ...
        self.expr_call_lang_item_fn(
            ens_span,
            redox_hir::LangItem::ContractBuildCheckEnsures,
    ...
    fn lower_contract_check_just_precond(
        &mut self,
        contract_decls: &'hir [redox_hir::Stmt<'hir>],
        precond: redox_hir::Stmt<'hir>,
    ) -> redox_hir::Stmt<'hir> {
        let stmts = self
            .arena
        ...
        let precond_check = redox_hir::ExprKind::If(
    ...
    fn lower_contract_check_with_postcond(
        &mut self,
        contract_decls: &'hir [redox_hir::Stmt<'hir>],
        precond: Option<redox_hir::Stmt<'hir>>,
        postcond_checker: &'hir redox_hir::Expr<'hir>,
    ) -> &'hir redox_hir::Expr<'hir> {
        let stmts = self
            .arena
            .alloc_from_iter(contract_decls.into_iter().map(|d| *d).chain(precond.into_iter()));
        ...
        let postcond_checker = self.arena.alloc(self.expr_enum_variant_lang_item(
            ...
            redox_hir::lang_items::LangItem::OptionSome,
    ...
    fn wrap_body_with_contract_check(
        &mut self,
        body: impl FnOnce(&mut Self) -> redox_hir::Expr<'hir>,
        contract_check: &'hir redox_hir::Expr<'hir>,
        postcond_span: redox_span::Span,
    ) -> &'hir redox_hir::Block<'hir> {
        let check_ident: redox_span::Ident =
            redox_span::Ident::from_str_and_span("__ensures_checker", postcond_span);
        let (check_hir_id, postcond_decl) = {
            // Set up the postcondition `let` statement.
            let (checker_pat, check_hir_id) = self.pat_ident_binding_mode_mut(
                postcond_span,
                check_ident,
                redox_hir::BindingMode::NONE,
            );
            (
                check_hir_id,
                self.stmt_let_pat(
                    ...
                    redox_hir::LocalSource::Contract,
                ),
            )
        };
    ...
    pub(super) fn checked_return(
        &mut self,
        opt_expr: Option<&'hir redox_hir::Expr<'hir>>,
    ) -> redox_hir::ExprKind<'hir> {
        let checked_ret =
    ...
    pub(super) fn inject_ensures_check(
        ...
        //     }
        // }
        let ret_ident: redox_span::Ident = redox_span::Ident::from_str_and_span("__ret", span);

        // Set up the return `let` statement.
        let (ret_pat, ret_hir_id) =
            self.pat_ident_binding_mode_mut(span, ret_ident, redox_hir::BindingMode::NONE);
        ...
        let contract_check = self.expr_call_lang_item_fn_mut(
            span,
            redox_hir::LangItem::ContractCheckEnsures,
        ...
        let attrs: redox_ast::AttrVec = thin_vec![self.unreachable_code_attr(span)];
        self.lower_attrs(contract_check.hir_id, &attrs, span, redox_hir::Target::Expression);
