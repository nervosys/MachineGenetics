...
pub(crate) fn codegen_bitcast(fx: &mut FunctionCx<'_, '_, '_>, dst_ty: Type, val: Value) -> Value {
    ...
    flags.set_endianness(match fx.tcx.data_layout.endian {
        redox_abi::Endian::Big => cranelift_codegen::ir::Endianness::Big,
        redox_abi::Endian::Little => cranelift_codegen::ir::Endianness::Little,
    });
...
impl<'tcx> redox_abi::HasDataLayout for FunctionCx<'_, '_, 'tcx> {
...
impl<'tcx> redox_abi::HasDataLayout for FullyMonomorphizedLayoutCx<'tcx> {
    fn data_layout(&self) -> &redox_abi::TargetDataLayout {
        &self.0.data_layout
    }
}
