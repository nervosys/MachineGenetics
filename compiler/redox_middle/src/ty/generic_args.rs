...
use redox_macros::{HashStable, TyDecodable, TyEncodable, extension};
use redox_serialize::{Decodable, Encodable};
use redox_type_ir::WithCachedTypeInfo;
use redox_type_ir::walk::TypeWalker;
...
pub type GenericArgKind<'tcx> = redox_type_ir::GenericArgKind<TyCtxt<'tcx>>;
pub type TermKind<'tcx> = redox_type_ir::TermKind<TyCtxt<'tcx>>;
...
impl<'tcx> redox_type_ir::inherent::GenericArg<TyCtxt<'tcx>> for GenericArg<'tcx> {}

impl<'tcx> redox_type_ir::inherent::GenericArgs<TyCtxt<'tcx>> for ty::GenericArgsRef<'tcx> {
    fn rebase_onto(
        self,
        tcx: TyCtxt<'tcx>,
...
impl<'tcx> redox_type_ir::inherent::IntoKind for GenericArg<'tcx> {
...
unsafe impl<'tcx> redox_data_structures::sync::DynSend for GenericArg<'tcx> where
    &'tcx (Ty<'tcx>, ty::Region<'tcx>, ty::Const<'tcx>): redox_data_structures::sync::DynSend
{
}
unsafe impl<'tcx> redox_data_structures::sync::DynSync for GenericArg<'tcx> where
    &'tcx (Ty<'tcx>, ty::Region<'tcx>, ty::Const<'tcx>): redox_data_structures::sync::DynSync
{
}
