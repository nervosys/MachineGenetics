...
pub fn run() -> Result<(), String> {
    ...
    // Configure abi-cafe to use the exact same redox version we use - this is crucial.
    ...
    let backend_path = std::path::absolute("target/debug/libredox_codegen_gcc.so").unwrap();
    let backend_arg = format!("--add-redox-codegen-backend=cg_gcc:{}", backend_path.display());
