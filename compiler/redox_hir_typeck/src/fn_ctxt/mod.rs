...
use redox_hir::attrs::{DivergingBlockBehavior, DivergingFallbackBehavior};
use redox_hir::def_id::{DefId, LocalDefId};
use redox_hir::{self as hir, HirId, ItemLocalMap, find_attr};
use redox_hir_analysis::hir_ty_lowering::{
    HirTyLowerer, InherentAssocCandidate, RegionInferReason,
};
use redox_infer::infer::{self, RegionVariableOrigin};
...
/// with [`InferCtxt`].
///
/// This is in contrast to `redox_hir_analysis::collect::ItemCtxt`, which is
...
pub(crate) struct FnCtxt<'a, 'tcx> {
    ...
    /// Whether the current crate enables the `redox_attrs` feature.
    /// This allows to skip processing attributes in many places.
    pub(super) has_redox_attrs: bool,
}
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    pub(crate) fn new(
        ...
        FnCtxt {
            ...
            has_redox_attrs: root_ctxt.tcx.features().redox_attrs(),
        }
    }
    ...
    ///
    /// [`InferCtxtErrorExt::err_ctxt`]: redox_trait_selection::error_reporting::InferCtxtErrorExt::err_ctxt
...
impl<'tcx> HirTyLowerer<'tcx> for FnCtxt<'_, 'tcx> {
    ...
    fn lower_fn_sig(
        &self,
        decl: &redox_hir::FnDecl<'tcx>,
        _generics: Option<&redox_hir::Generics<'_>>,
        _hir_id: redox_hir::HirId,
