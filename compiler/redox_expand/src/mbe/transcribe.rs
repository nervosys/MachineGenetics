...
use redox_ast::{ExprKind, StmtKind, TyKind, UnOp};
use redox_data_structures::fx::FxHashMap;
use redox_errors::{Diag, DiagCtxtHandle, PResult, listify, pluralize};
use redox_parse::lexer::nfc_normalize;
use redox_parse::parser::ParseNtResult;
use redox_session::parse::ParseSess;
use redox_span::hygiene::{LocalExpnId, Transparency};
use redox_span::{
    ...
    match lockstep_iter_size(seq, tscx.interp, &tscx.repeats) {
        LockstepIterSize::Unconstrained => {
            ...
            for ident in meta_vars {
                if let Some(name) = redox_span::edit_distance::find_best_match_for_name(
                    &repeatable_names[..],
                    ident.name,
                    None,
                ) {
                    typo_repeatable = Some(VarTypoSuggestionRepeatable { span: ident.span, name });
                } else if let Some(name) = redox_span::edit_distance::find_best_match_for_name(
...
fn metavar_expr_concat<'tx>(
    ...
    if !redox_lexer::is_ident(symbol.as_str()) {
        return Err(dcx.struct_span_err(
            concatenated_span,
