...
use redox_data_structures::base_n::{CASE_INSENSITIVE, ToBaseN};
use redox_data_structures::flock;
use redox_data_structures::fx::{FxHashMap, FxHashSet, FxIndexSet};
use redox_data_structures::profiling::{SelfProfiler, SelfProfilerRef};
use redox_data_structures::sync::{DynSend, DynSync, Lock, MappedReadGuard, ReadGuard, RwLock};
...
use redox_errors::timings::TimingSectionHandler;
use redox_errors::{
...
pub trait DynLintStore: Any + DynSync + DynSend {
    /// Provides a way to access lint groups without depending on `redox_lint`
...
pub struct Session {
    ...
    /// The version of the redox process, possibly including a commit hash and description.
    ...
    /// A random string generated per invocation of redox.
    ///
    /// This is prepended to all temporary files so that they do not collide
    /// during concurrent invocations of redox, or past invocations that were
...
impl Session {
    ...
    pub fn generate_proc_macro_decls_symbol(&self, stable_crate_id: StableCrateId) -> String {
        format!("__redox_proc_macro_decls_{:08x}__", stable_crate_id.as_u64())
    }
    ...
    /// (#125246): we also add the bin directories to the sysroot where redox is located.
...
impl Session {
    ...
    pub fn must_emit_unwind_tables(&self) -> bool {
        ...
        // Historically redox always emits `uwtable` attributes by default, so
    ...
    pub fn codegen_units(&self) -> CodegenUnits {
        ...
        // large number of threads in parallel. The backend of redox contains
    ...
    /// or fall back to the minimum version supported by `redox`.
    ///
        ...
        if let Ok(deployment_target) = env::var(env_var) {
            match apple::OSVersion::from_str(&deployment_target) {
                Ok(version) => {
                    ...
                    // is lower than the minimum OS supported by redox, not when the variable is lower
...
pub fn build_session(
    ...
    let source_map = redox_span::source_map::get_source_map().unwrap();
...
fn validate_commandline_args_with_session_available(sess: &Session) {
    ...
    // dynamically downstream, redox generates `__imp_` symbols that help linkers
    ...
    // LLVM CFI using redox LTO requires a single codegen unit.
    if sess.is_sanitizer_cfi_enabled()
