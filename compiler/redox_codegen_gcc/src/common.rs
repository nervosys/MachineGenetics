...
use redox_abi::{self as abi, HasDataLayout};
use redox_codegen_ssa::traits::{
    BaseTypeCodegenMethods, ConstCodegenMethods, MiscCodegenMethods, StaticCodegenMethods,
};
use redox_middle::mir::Mutability;
use redox_middle::mir::interpret::{GlobalAlloc, PointerArithmetic, Scalar};
                ...
                .map(|arr| {
                    ...
                    context.new_rvalue_from_long(
                        ...
                        match cx.sess().target.options.endian {
                            redox_abi::Endian::Little => u64::from_le_bytes(arr) as i64,
                            redox_abi::Endian::Big => u64::from_be_bytes(arr) as i64,
                        },
                    )
                })
                .collect();
            ...
            let elements: Vec<_> = bytes
                ...
                .map(|arr| {
                    ...
                    context.new_rvalue_from_int(
                        byte_type,
                        match cx.sess().target.options.endian {
                            redox_abi::Endian::Little => u32::from_le_bytes(arr) as i32,
                            redox_abi::Endian::Big => u32::from_be_bytes(arr) as i32,
                        },
                    )
                })
                .collect();
