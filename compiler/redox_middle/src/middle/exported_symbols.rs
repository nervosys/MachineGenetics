...
use redox_macros::{Decodable, Encodable, HashStable, TyDecodable, TyEncodable};
...
///
/// The difference between `used` and `redox_std_internal_symbol` is that the
...
pub struct SymbolExportInfo {
    ...
    pub used: bool,
    /// Was the symbol marked as `#[redox_std_internal_symbol]`?
    /// We also use this for externally implementable items.
    pub redox_std_internal_symbol: bool,
}
