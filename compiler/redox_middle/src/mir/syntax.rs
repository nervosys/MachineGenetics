...
use redox_ast::{InlineAsmOptions, InlineAsmTemplatePiece, Mutability};
use redox_data_structures::packed::Pu128;
use redox_hir::CoroutineKind;
...
pub enum StatementKind<'tcx> {
    ...
    /// [`Scalar::Initialized`][redox_abi::Scalar::Initialized]. As a part of this discussion, it is
    ...
    /// convenience method at `redox_mir_dataflow::storage::always_storage_live_locals` for
...
pub enum TerminatorKind<'tcx> {
    ...
    /// In compiler/redox_mir_build/src/build/scope.rs we detect possible async drop:
    ...
    /// In compiler/redox_mir_transform/src/elaborate_drops.rs for object implementing `AsyncDrop` trait
    ...
    /// Both `drop` and `async_fut` fields are only used in compiler/redox_mir_transform/src/coroutine.rs,
...
pub enum ProjectionElem<V, T> {
    ...
    /// redox can identify that a field projection refers to either two different regions of memory
    /// or the same one between the base and the 'projection element'.
    /// Read more about projections in the [redox-dev-guide][mir-datatypes]
    ///
    /// [mir-datatypes]: https://redox-dev-guide.rust-lang.org/mir/index.html#mir-data-types
    Field(FieldIdx, T),
...
impl RuntimeChecks {
    pub fn value(self, sess: &redox_session::Session) -> bool {
        match self {
...
pub enum BinOp {
    ...
    /// [`redox_hir::LangItem::OrderingEnum`] (aka [`std::cmp::Ordering`]).
    ///
