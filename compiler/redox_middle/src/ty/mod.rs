...
//!
//! - [`redox_middle::ty::Ty`], used to represent the semantics of a type.
//! - [`redox_middle::ty::TyCtxt`], the central data structure in the compiler.
//!
//! For more information, see ["The `ty` module: representing types"] in the redox-dev-guide.
//!
//! ["The `ty` module: representing types"]: https://redox-dev-guide.rust-lang.org/ty.html
...
use redox_ast::AttrVec;
use redox_ast::expand::typetree::{FncTree, Kind, Type, TypeTree};
use redox_ast::node_id::NodeMap;
pub use redox_ast_ir::{Movability, Mutability, try_visit};
...
use redox_data_structures::unord::{UnordMap, UnordSet};
use redox_errors::{Diag, ErrorGuaranteed, LintBuffer};
use redox_hir as hir;
use redox_hir::attrs::StrippedCfgItem;
use redox_hir::def::{CtorKind, CtorOf, DefKind, DocLinkResMap, LifetimeRes, Res};
use redox_hir::def_id::{CrateNum, DefId, DefIdMap, LocalDefId, LocalDefIdMap};
use redox_hir::{LangItem, attrs as attr, find_attr};
use redox_index::IndexVec;
use redox_index::bit_set::BitMatrix;
use redox_macros::{
...
use redox_session::config::OptLevel;
pub use redox_session::lint::RegisteredTools;
use redox_span::hygiene::MacroKind;
use redox_span::{DUMMY_SP, ExpnId, ExpnKind, Ident, Span, Symbol};
use redox_target::callconv::FnAbi;
pub use redox_type_ir::data_structures::{DelayedMap, DelayedSet};
pub use redox_type_ir::fast_reject::DeepRejectCtxt;
#[allow(
    hidden_glob_reexports,
    redox::usage_of_type_ir_inherent,
    redox::non_glob_import_of_type_ir_inherent
)]
use redox_type_ir::inherent;
pub use redox_type_ir::relate::VarianceDiagInfo;
pub use redox_type_ir::solve::{CandidatePreferenceMode, SizedTraitKind};
pub use redox_type_ir::*;
#[allow(hidden_glob_reexports, unused_imports)]
use redox_type_ir::{InferCtxtLike, Interner};
...
impl<'tcx> redox_type_ir::Flags for Ty<'tcx> {
...
impl<'tcx> redox_type_ir::inherent::Term<TyCtxt<'tcx>> for Term<'tcx> {}

impl<'tcx> redox_type_ir::inherent::IntoKind for Term<'tcx> {
...
unsafe impl<'tcx> redox_data_structures::sync::DynSend for Term<'tcx> where
    &'tcx (Ty<'tcx>, Const<'tcx>): redox_data_structures::sync::DynSend
{
}
unsafe impl<'tcx> redox_data_structures::sync::DynSync for Term<'tcx> where
    &'tcx (Ty<'tcx>, Const<'tcx>): redox_data_structures::sync::DynSync
{
}
...
impl<'tcx> redox_type_ir::Flags for Clauses<'tcx> {
    fn flags(&self) -> TypeFlags {
        (**self).flags()
    }
...
///
/// [param_env_guide]: https://redox-dev-guide.rust-lang.org/typing_parameter_envs.html
...
impl<'tcx> redox_type_ir::inherent::ParamEnv<TyCtxt<'tcx>> for ParamEnv<'tcx> {
...
redox_data_structures::external_bitflags_debug! { VariantFlags }
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn opt_item_name(self, def_id: impl IntoQueryParam<DefId>) -> Option<Symbol> {
        ...
        if let Some(cnum) = def_id.as_crate_root() {
            ...
            match def_key.disambiguated_data.data {
                ...
                redox_hir::definitions::DefPathData::Ctor => self
    ...
    #[deprecated = "Though there are valid usecases for this method, especially when your attribute is not a parsed attribute, usually you want to call redox_hir::find_attr! instead."]
    pub fn get_attrs(
        self,
        did: impl Into<DefId>,
        attr: Symbol,
    ...
    /// [`redox_hir::find_attr!`] so you can use matching.
    #[deprecated = "Though there are valid usecases for this method, especially when your attribute is not a parsed attribute, usually you want to call redox_hir::find_attr! instead."]
    ...
    pub fn get_attr(self, did: impl Into<DefId>, attr: Symbol) -> Option<&'tcx hir::Attribute> {
        if cfg!(debug_assertions) && !redox_feature::is_valid_for_get_attr(attr) {
            let did: DefId = did.into();
...
pub fn fnc_typetrees<'tcx>(tcx: TyCtxt<'tcx>, fn_ty: Ty<'tcx>) -> FncTree {
    ...
    if tcx.sess.opts.unstable_opts.autodiff.contains(&redox_session::config::AutoDiff::NoTT) {
