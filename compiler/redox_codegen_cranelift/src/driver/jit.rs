...
use redox_codegen_ssa::CrateInfo;
...
pub(crate) fn run_jit(tcx: TyCtxt<'_>, crate_info: &CrateInfo, jit_args: Vec<String>) -> ! {
    if !tcx.crate_types().contains(&redox_session::config::CrateType::Executable) {
...
fn dep_symbol_lookup_fn(
    ...
    let data = &crate_info.dependency_formats[&redox_session::config::CrateType::Executable];
    ...
    for &cnum in crate_info.used_crates.iter().rev() {
        ...
        match data[cnum] {
            ...
            Linkage::Static => {
                ...
                diag.note("redox_codegen_cranelift can only load dylibs in JIT mode.");
                diag.emit();
            }
            Linkage::Dynamic => {
