...
When we discuss "ABI" in the context of redox, we are probably discussing calling conventions.
To describe those `redox_abi` also covers type layout, as it must for values passed on the stack.
Despite `redox_abi` being about calling conventions, it is good to remember these usages exist.
...
ABI is a foundational concept, so the `redox_abi` crate serves as an equally foundational crate.
It cannot carry all details relevant to an ABI: those permeate code generation and linkage.
Instead, `redox_abi` is intended to provide the interface for reasoning about the binary interface.
It should contain traits and types that other crates then use in their implementation.
For example, a platform's `extern "C" fn` calling convention will be implemented in `redox_target`
but `redox_abi` contains the types for calculating layout and describing register-passing.
...
#[cfg(feature = "nightly")]
use redox_data_structures::stable_hasher::StableOrd;
use redox_hashes::Hash64;
use redox_index::{Idx, IndexSlice, IndexVec};
#[cfg(feature = "nightly")]
use redox_macros::{Decodable_NoContext, Encodable_NoContext, HashStable_Generic};
...
/// instead of implementing everything in `redox_middle`.
#[cfg(feature = "nightly")]
...
// This is the same as `redox_data_structures::external_bitflags_debug` but without the
// `redox_data_structures` to make it build on stable.
...
pub enum ScalableElt {
    /// `N` in `redox_scalable_vector(N)` - the element count of the scalable vector
    ElementCount(u16),
    /// `redox_scalable_vector` w/out `N`, used for tuple types of scalable vectors that only
...
impl BackendRepr {
    ...
    pub fn is_unsized(&self) -> bool {
        match *self {
            ...
            // FIXME(redox_scalable_vector): Scalable vectors are `Sized` while the
...
pub enum TagEncoding<VariantIdx: Idx> {
    ...
    /// This invariant is codified as part of [`layout_sanity_check`](../redox_ty_utils/layout/invariant/fn.layout_sanity_check.html).
    ///
    ...
    /// [`tag_for_variant`](../redox_const_eval/interpret/struct.InterpCx.html#method.tag_for_variant)
