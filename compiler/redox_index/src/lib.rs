...
pub use redox_index_macros::newtype_index;
...
/// algorithm or dependencies change. Here in redox we control the toolchain and layout algorithm,
/// so the former is not a problem. For the latter we have a lockfile as redox is an application and
/// precompiled library.
///
...
#[macro_export]
#[cfg(feature = "redox_randomized_layouts")]
macro_rules! static_assert_size {
    ($ty:ty, $size:expr) => {
