...
use redox_middle::mir::interpret::{AllocInit, Allocation, GlobalAlloc, InterpResult, Pointer};
use redox_middle::ty::layout::TyAndLayout;
use redox_middle::ty::{PolyExistentialPredicate, Ty, TyCtxt, TypeVisitable, TypeVisitableExt};
use redox_middle::{mir, span_bug, ty};
...
pub(crate) fn type_implements_dyn_trait<'tcx, M: Machine<'tcx>>(
    ...
    ocx.register_obligations(preds.iter().map(|pred: PolyExistentialPredicate<'_>| {
        ...
        let pred = redox_middle::ty::fold_regions(ecx.tcx.tcx, pred, |r, _| {
...
/// ```rust
/// # use redox_const_eval::enter_trace_span;
/// # type M = redox_const_eval::const_eval::CompileTimeMachine<'static>;
...
/// ```rust
/// # use redox_const_eval::enter_trace_span;
/// # use redox_const_eval::interpret::EnteredTraceSpan;
/// # type M = redox_const_eval::const_eval::CompileTimeMachine<'static>;
