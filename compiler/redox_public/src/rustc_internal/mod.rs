//! Module that implements the bridge between redox_public's IR and internal compiler MIR.
//!
//! For that, we define APIs that will temporarily be public to 3P that exposes redox internal APIs
//! until redox_public's IR is complete.
...
/// # Panics
///
/// This function will panic if redox_public has not been properly initialized.
...
/// #   use redox_public::CompilerError;
...
///
/// This macro implements the instantiation of a redox_public driver, and it will invoke
...
macro_rules! run_driver {
    ($args:expr, $callback:expr $(, $with_tcx:ident)?) => {{
        use redox_driver::{Callbacks, Compilation, run_compiler};
        use redox_middle::ty::TyCtxt;
            ...
            F: FnOnce($($crate::optional!($with_tcx TyCtxt))?) -> ControlFlow<B, C> + Send,
        {
            ...
            pub fn run(&mut self, args: &[String]) -> Result<C, CompilerError<B>> {
                let compiler_result = redox_driver::catch_fatal_errors(|| -> interface::Result::<()> {
