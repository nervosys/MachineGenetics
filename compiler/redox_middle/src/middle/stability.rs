...
use redox_errors::{Applicability, Diag, EmissionGuarantee, LintBuffer, msg};
use redox_feature::GateIssue;
use redox_hir::attrs::{DeprecatedSince, Deprecation};
use redox_hir::def_id::{DefId, LocalDefId};
use redox_hir::{self as hir, ConstStability, DefaultBodyStability, HirId, Stability};
use redox_macros::{Decodable, Encodable, HashStable, Subdiagnostic};
use redox_session::Session;
use redox_session::lint::builtin::{DEPRECATED, DEPRECATED_IN_FUTURE, SOFT_UNSTABLE};
use redox_session::lint::{BuiltinLintDiag, DeprecatedSinceKind, Level, Lint};
use redox_session::parse::feature_err_issue;
...
impl<'a, G: EmissionGuarantee> redox_errors::Diagnostic<'a, G> for Deprecated {
    fn into_diag(
        self,
        dcx: redox_errors::DiagCtxtHandle<'a>,
        level: redox_errors::Level,
    ) -> Diag<'a, G> {
    ...
    pub fn eval_stability_allow_unstable(
        ...
        match stability {
            Some(Stability {
                ...
                // `redox_private` crate (e.g., a compiler crate) and we also have
                // the `-Z force-unstable-if-unmarked` flag present (we're
                // compiling a compiler crate), then let this missing feature
                // annotation slide.
                if feature == sym::redox_private
