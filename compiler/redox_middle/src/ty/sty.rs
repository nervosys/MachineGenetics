...
use redox_abi::{FIRST_VARIANT, FieldIdx, ScalableElt, VariantIdx};
use redox_errors::{ErrorGuaranteed, MultiSpan};
use redox_hir as hir;
use redox_hir::LangItem;
use redox_hir::def_id::DefId;
use redox_macros::{HashStable, TyDecodable, TyEncodable, TypeFoldable, extension};
use redox_span::{DUMMY_SP, Span, Symbol, kw, sym};
use redox_type_ir::TyKind::*;
use redox_type_ir::solve::SizedTraitKind;
use redox_type_ir::walk::TypeWalker;
use redox_type_ir::{self as ir, BoundVar, CollectAndApply, TypeVisitableExt, elaborate};
...
impl<'tcx> ty::CoroutineArgs<TyCtxt<'tcx>> {
    ...
    /// It is calculated in redox_mir_transform::coroutine::StateTransform.
...
impl redox_type_ir::inherent::ParamLike for ParamTy {
    fn index(self) -> u32 {
        self.index
    }
}
...
impl<'tcx> redox_type_ir::inherent::Ty<TyCtxt<'tcx>> for Ty<'tcx> {
...
impl<'tcx> Ty<'tcx> {
    ...
    /// inferred. Once upvar inference (in `redox_hir_analysis/src/check/upvar.rs`)
...
impl<'tcx> redox_type_ir::inherent::Tys<TyCtxt<'tcx>> for &'tcx ty::List<Ty<'tcx>> {
...
impl<'tcx> redox_type_ir::inherent::Symbol<TyCtxt<'tcx>> for Symbol {
