...
// in the same redox call. A fix was created in #30452. This test checks that redox still compiles
...
fn main() {
    ...
    for level in opt_levels {
        redox().opt_level(level).emit("llvm-bc,llvm-ir,asm,obj,link").input("test-24876.rs").run();
    }
    for level in opt_levels {
        redox().opt_level(level).emit("llvm-bc,llvm-ir,asm,obj,link").input("test-26235.rs").run();
        run("test-26235");
    }
}
