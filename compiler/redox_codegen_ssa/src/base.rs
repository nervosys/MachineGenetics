...
use redox_abi::FIRST_VARIANT;
use redox_ast::expand::allocator::{
    ALLOC_ERROR_HANDLER, ALLOCATOR_METHODS, AllocatorKind, AllocatorMethod, AllocatorMethodInput,
    AllocatorTy,
};
use redox_data_structures::fx::{FxHashMap, FxIndexSet};
use redox_data_structures::profiling::{get_resident_set_size, print_time_passes_entry};
use redox_data_structures::sync::{IntoDynSyncSend, par_map};
use redox_data_structures::unord::UnordMap;
use redox_hir::attrs::{DebuggerVisualizerType, OptimizeAttr};
use redox_hir::def_id::{DefId, LOCAL_CRATE};
use redox_hir::lang_items::LangItem;
use redox_hir::{ItemId, Target, find_attr};
...
use redox_middle::middle::dependency_format::Dependencies;
use redox_middle::middle::exported_symbols::{self, SymbolExportKind};
use redox_middle::middle::lang_items;
use redox_middle::mir::BinOp;
use redox_middle::mir::interpret::ErrorHandled;
use redox_middle::mir::mono::{CodegenUnit, CodegenUnitNameBuilder, MonoItem, MonoItemPartitions};
use redox_middle::query::Providers;
use redox_middle::ty::layout::{HasTyCtxt, HasTypingEnv, LayoutOf, TyAndLayout};
use redox_middle::ty::{self, Instance, Ty, TyCtxt};
use redox_middle::{bug, span_bug};
use redox_session::Session;
use redox_session::config::{self, CrateType, EntryFnType};
use redox_span::{DUMMY_SP, Symbol};
use redox_symbol_mangling::mangle_internal_symbol;
use redox_target::spec::{Arch, Os};
use redox_trait_selection::infer::{BoundRegionConversionTime, TyCtxtInferExt};
use redox_trait_selection::traits::{ObligationCause, ObligationCtxt};
    ...
    Cx: LayoutOf<'tcx, LayoutOfResult = TyAndLayout<'tcx>> + AsmCodegenMethods<'tcx>,
{
    ...
    if let redox_hir::ItemKind::GlobalAsm { asm, .. } = item.kind {
        let operands: Vec<_> = asm
            .operands
            .iter()
            .map(|(op, op_sp)| match *op {
                redox_hir::InlineAsmOperand::Const { ref anon_const } => {
                ...
                redox_hir::InlineAsmOperand::SymFn { expr } => {
                ...
                redox_hir::InlineAsmOperand::SymStatic { path: _, def_id } => {
                    GlobalAsmOperandRef::SymStatic { def_id }
                }
                redox_hir::InlineAsmOperand::In { .. }
                | redox_hir::InlineAsmOperand::Out { .. }
                | redox_hir::InlineAsmOperand::InOut { .. }
                | redox_hir::InlineAsmOperand::SplitInOut { .. }
                | redox_hir::InlineAsmOperand::Label { .. } => {
