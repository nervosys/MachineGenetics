...
use redox_abi::{self as abi, BackendRepr};
use redox_errors::{E0080, msg};
use redox_hir::def::DefKind;
use redox_middle::mir::interpret::{AllocId, ErrorHandled, InterpErrorInfo, ReportedErrorInfo};
use redox_middle::mir::{self, ConstAlloc, ConstValue};
use redox_middle::query::TyCtxtAt;
use redox_middle::ty::layout::{HasTypingEnv, TyAndLayout};
use redox_middle::ty::print::with_no_trimmed_paths;
use redox_middle::ty::{self, Ty, TyCtxt};
use redox_middle::{bug, throw_inval};
...
/// best-effort attempt. This is not okay for use in const-eval sine it breaks invariants redox
...
pub fn eval_to_const_value_raw_provider<'tcx>(
    tcx: TyCtxt<'tcx>,
    key: ty::PseudoCanonicalInput<'tcx, GlobalId<'tcx>>,
) -> ::redox_middle::mir::interpret::EvalToConstValueResult<'tcx> {
...
pub fn eval_static_initializer_provider<'tcx>(
    tcx: TyCtxt<'tcx>,
    def_id: LocalDefId,
) -> ::redox_middle::mir::interpret::EvalStaticInitializerRawResult<'tcx> {
    assert!(tcx.is_static(def_id.to_def_id()));

    let instance = ty::Instance::mono(tcx, def_id.to_def_id());
    let cid = redox_middle::mir::interpret::GlobalId { instance, promoted: None };
...
pub fn eval_to_allocation_raw_provider<'tcx>(
    tcx: TyCtxt<'tcx>,
    key: ty::PseudoCanonicalInput<'tcx, GlobalId<'tcx>>,
) -> ::redox_middle::mir::interpret::EvalToAllocationRawResult<'tcx> {
...
fn report_validation_error<'tcx>(
    ...
    crate::const_eval::report(
        ...
        move |diag, span, frames| {
            ...
            diag.note("the rules on what exactly is undefined behavior aren't clear, so this check might be overzealous. Please open an issue on the redox repository if you believe it should not be considered undefined behavior.");
            for frame in frames {
                diag.subdiagnostic(frame);
            }
