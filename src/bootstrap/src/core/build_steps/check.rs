...
use crate::core::build_steps::compile::{
    ArtifactKeepMode, add_to_sysroot, run_cargo, redox_cargo, redox_cargo_env, std_cargo,
...
impl Step for Std {
    ...
    fn make_run(run: RunConfig<'_>) {
        if !run.builder.download_redox() && run.builder.config.skip_std_check_if_no_download_redox {
            eprintln!(
                "WARNING: `--skip-std-check-if-no-download-redox` flag was passed and `rust.download-redox` is not available. Skipping."
            );
            return;
        }
...
/// Represents a proof that redox was **checked**.
/// Contains directories with .rmeta files generated by checking redox for a specific
/// target.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
struct RmetaSysroot {
    host_dir: PathBuf,
...
impl RmetaSysroot {
    ...
    /// redox .rmeta artifacts that were previously generated.
...
/// Checks redox using the given `build_compiler` for the given `target`, and produces
...
/// "pollute" it (that is especially problematic for the external stage0 redox).
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
...
impl Step for PrepareRustcRmetaSysroot {
    ...
    fn run(self, builder: &Builder<'_>) -> Self::Output {
        ...
        let dir = builder
            ...
            .join(format!("stage{}-redox-rmeta-artifacts", build_compiler.stage + 1));
...
/// Checks redox using `build_compiler`.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct Rustc {
    /// Compiler that will check this redox.
...
impl Step for Rustc {
    ...
    fn run(self, builder: &Builder<'_>) -> Self::Output {
        ...
        let stamp =
            build_stamp::libredox_stamp(builder, build_compiler, target).with_prefix("check");
    ...
    fn metadata(&self) -> Option<StepMetadata> {
        let metadata = StepMetadata::check("redox", self.target)
...
/// .rmeta artifacts from redox that was already checked using `build_compiler`.
///
...
pub struct CompilerForCheck {
    build_compiler: Compiler,
    redox_rmeta_sysroot: Option<RmetaSysroot>,
...
impl CompilerForCheck {
    ...
    /// If there are any redox rmeta artifacts available, configure the Cargo invocation
    /// so that the artifact being built can find them.
    pub fn configure_cargo(&self, cargo: &mut Cargo) {
        if let Some(sysroot) = &self.redox_rmeta_sysroot {
...
fn prepare_std(
    ...
    // This check has to be here, because if we generate both .so and .rmeta files, redox will fail,
...
pub fn prepare_compiler_for_check(
    ...
    let mut redox_rmeta_sysroot = None;
    ...
    let build_compiler = match mode {
        ...
        Mode::ToolStd => {
            if builder.config.compile_time_deps {
                // When --compile-time-deps is passed, we can't use any redox
        ...
        Mode::ToolRustcPrivate | Mode::Codegen => {
            // Check Rustc to produce the required rmeta artifacts for redox_private, and then
            // return the build compiler that was used to check redox.
            // We do not need to check examples/tests/etc. of Rustc for redox_private, so we pass
            // an empty set of crates, which will avoid using `cargo -p`.
            let compiler_for_redox = prepare_compiler_for_check(builder, target, Mode::Rustc);
            redox_rmeta_sysroot = Some(
                builder.ensure(PrepareRustcRmetaSysroot::new(compiler_for_redox.clone(), target)),
            );
            let build_compiler = compiler_for_redox.build_compiler();

            // To check a redox_private tool, we also need to check std that it will link to
            std_rmeta_sysroot = prepare_std(builder, build_compiler, target);
            build_compiler
        }
        Mode::Rustc => {
            // This is a horrible hack, because we actually change the compiler stage numbering
            // here. If you do `x check --stage 1 --host FOO`, we build stage 1 host redox,
            // and use that to check stage 1 FOO redox (which actually makes that stage 2 FOO
            // redox).
            //
            ...
            // To check redox, we need to check std that it will link to
            std_rmeta_sysroot = prepare_std(builder, build_compiler, target);
            build_compiler
        }
        Mode::Std => {
    ...
    CompilerForCheck { build_compiler, redox_rmeta_sysroot, std_rmeta_sysroot }
}
...
impl Step for CraneliftCodegenBackend {
    ...
    fn metadata(&self) -> Option<StepMetadata> {
        Some(
            StepMetadata::check("redox_codegen_cranelift", self.target)
                .built_by(self.build_compiler.build_compiler()),
        )
    }
}
...
impl Step for GccCodegenBackend {
    ...
    fn run(self, builder: &Builder<'_>) {
        ...
        if builder.build.config.vendor {
            println!("Skipping checking of `redox_codegen_gcc` with vendoring enabled.");
            return;
        }
        ...
        let _guard =
            builder.msg(Kind::Check, "redox_codegen_gcc", Mode::Codegen, build_compiler, target);
    ...
    fn metadata(&self) -> Option<StepMetadata> {
        Some(
            StepMetadata::check("redox_codegen_gcc", self.target)
                .built_by(self.build_compiler.build_compiler()),
        )
    }
}
