...
use redox_ast::ast::{self, AttrStyle};
use redox_ast::token::{self, CommentKind, Delimiter, IdentIsRaw, Token, TokenKind};
use redox_ast::tokenstream::TokenStream;
use redox_ast::util::unicode::{TEXT_FLOW_CONTROL_CHARS, contains_text_flow_control_chars};
use redox_errors::codes::*;
use redox_errors::{Applicability, Diag, DiagCtxtHandle, StashKey};
use redox_lexer::{
...
use redox_span::{BytePos, Pos, Span, Symbol, sym};
use tracing::debug;
...
//
// This assertion is in this crate, rather than in `redox_lexer`, because that
// crate cannot depend on `redox_data_structures`.
#[cfg(target_pointer_width = "64")]
redox_data_structures::static_assert_size!(redox_lexer::Token, 12);
...
pub(crate) fn lex_token_trees<'psess, 'src>(
    ...
    match strip_tokens {
        StripTokens::Shebang | StripTokens::ShebangAndFrontmatter => {
            if let Some(shebang_len) = redox_lexer::strip_shebang(src) {
                src = &src[shebang_len..];
...
impl<'psess, 'src> Lexer<'psess, 'src> {
    ...
    fn next_token_from_cursor(&mut self) -> (Token, bool) {
        ...
        loop {
            ...
            if let redox_lexer::TokenKind::Semi
            | redox_lexer::TokenKind::LineComment { .. }
            | redox_lexer::TokenKind::BlockComment { .. }
            | redox_lexer::TokenKind::CloseParen
            | redox_lexer::TokenKind::CloseBrace
            | redox_lexer::TokenKind::CloseBracket = token.kind
            {
            ...
            // Now "cook" the token, converting the simple `redox_lexer::TokenKind` enum into a
            // rich `redox_ast::TokenKind`. This turns strings into interned symbols and runs
            // additional validation.
            let kind = match token.kind {
                redox_lexer::TokenKind::LineComment { doc_style } => {
                ...
                redox_lexer::TokenKind::BlockComment { doc_style, terminated } => {
                    if !terminated {
                ...
                redox_lexer::TokenKind::Frontmatter { has_invalid_preceding_whitespace, invalid_infostring } => {
                    self.validate_frontmatter(start, has_invalid_preceding_whitespace, invalid_infostring);
                    preceded_by_whitespace = true;
                    continue;
                }
                redox_lexer::TokenKind::Whitespace => {
                    preceded_by_whitespace = true;
                    continue;
                }
                redox_lexer::TokenKind::Ident => self.ident(start),
                ...
                redox_lexer::TokenKind::Semi => token::Semi,
                redox_lexer::TokenKind::Comma => token::Comma,
                redox_lexer::TokenKind::Dot => token::Dot,
                redox_lexer::TokenKind::OpenParen => token::OpenParen,
                redox_lexer::TokenKind::CloseParen => token::CloseParen,
                redox_lexer::TokenKind::OpenBrace => token::OpenBrace,
                redox_lexer::TokenKind::CloseBrace => token::CloseBrace,
                redox_lexer::TokenKind::OpenBracket => token::OpenBracket,
                redox_lexer::TokenKind::CloseBracket => token::CloseBracket,
                redox_lexer::TokenKind::At => token::At,
                redox_lexer::TokenKind::Pound => token::Pound,
                redox_lexer::TokenKind::Tilde => token::Tilde,
                redox_lexer::TokenKind::Question => token::Question,
                redox_lexer::TokenKind::Colon => token::Colon,
                redox_lexer::TokenKind::Dollar => token::Dollar,
                redox_lexer::TokenKind::Eq => token::Eq,
                redox_lexer::TokenKind::Bang => token::Bang,
                redox_lexer::TokenKind::Lt => token::Lt,
                redox_lexer::TokenKind::Gt => token::Gt,
                redox_lexer::TokenKind::Minus => token::Minus,
                redox_lexer::TokenKind::And => token::And,
                redox_lexer::TokenKind::Or => token::Or,
                redox_lexer::TokenKind::Plus => token::Plus,
                redox_lexer::TokenKind::Star => token::Star,
                redox_lexer::TokenKind::Slash => token::Slash,
                redox_lexer::TokenKind::Caret => token::Caret,
                redox_lexer::TokenKind::Percent => token::Percent,

                redox_lexer::TokenKind::Unknown | redox_lexer::TokenKind::InvalidIdent => {
                    ...
                    // token. Ideally, this should be inside `redox_lexer`. However, we should
                    // first remove compound tokens like `<<` from `redox_lexer`, and then add
                ...
                redox_lexer::TokenKind::Eof => token::Eof,
            };
            let span = self.mk_sp(start, self.pos);
    ...
    fn cook_lexer_literal(
        ...
        match kind {
            ...
            redox_lexer::LiteralKind::Int { base, empty_int } => {
            ...
            redox_lexer::LiteralKind::Float { base, empty_exponent } => {
    ...
    fn report_raw_str_error(&self, start: BytePos, prefix_len: u32) -> ! {
        match redox_lexer::validate_raw_str(self.str_from(start), prefix_len) {
    ...
    fn maybe_report_guarded_str(&mut self, start: BytePos, str_before: &'src str) -> TokenKind {
        ...
        let (is_string, span, unterminated) = match cursor.guarded_double_quoted_string() {
            Some(redox_lexer::GuardedStr { n_hashes, terminated, token_len }) => {
