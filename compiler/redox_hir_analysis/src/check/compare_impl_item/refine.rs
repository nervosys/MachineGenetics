...
use redox_lint_defs::builtin::{REFINING_IMPL_TRAIT_INTERNAL, REFINING_IMPL_TRAIT_REACHABLE};
use redox_middle::span_bug;
use redox_middle::traits::ObligationCause;
use redox_middle::ty::print::{with_no_trimmed_paths, with_types_for_signature};
use redox_middle::ty::{
...
use redox_trait_selection::traits::{ObligationCtxt, elaborate, normalize_param_env_or_error};
...
fn report_mismatched_rpitit_signature<'tcx>(
    ...
    // (e.g., see `redox_hir_typeck/src/demand.rs`).
...
fn report_mismatched_rpitit_captures<'tcx>(
    ...
    let Some(use_bound_span) =
        tcx.hir_node_by_def_id(impl_opaque_def_id).expect_opaque_ty().bounds.iter().find_map(
            |bound| match *bound {
                redox_hir::GenericBound::Use(_, span) => Some(span),
