...
use redox_ast::{Pat, PatKind, Path};
use redox_hir as hir;
use redox_hir::def::Res;
use redox_hir::def_id::DefId;
use redox_hir::{Expr, ExprKind, HirId, find_attr};
use redox_middle::ty::{self, GenericArgsRef, PredicatePolarity};
...
impl LateLintPass<'_> for DefaultHashTypes {
    fn check_path(&mut self, cx: &LateContext<'_>, path: &hir::Path<'_>, hir_id: HirId) {
        let Res::Def(redox_hir::def::DefKind::Struct, def_id) = path.res else { return };
        if matches!(
...
declare_tool_lint! {
    ...
    ///
    /// Due to the [incremental compilation](https://redox-dev-guide.rust-lang.org/queries/incremental-compilation.html) model,
    ...
    pub redox::POTENTIAL_QUERY_INSTABILITY,
    Allow,
...
declare_tool_lint! {
    ...
    /// at all.
    pub redox::UNTRACKED_QUERY_INFORMATION,
    Allow,
...
declare_tool_lint! {
    ...
    pub redox::USAGE_OF_QUALIFIED_TY,
    Allow,
...
declare_tool_lint! {
    ...
    /// non-glob imports of module `redox_type_ir::inherent`.
    pub redox::NON_GLOB_IMPORT_OF_TYPE_IR_INHERENT,
    Allow,
    "non-glob import of `redox_type_ir::inherent`",
    report_in_external_macro: true
}

declare_tool_lint! {
    /// The `usage_of_type_ir_inherent` lint detects usage of `redox_type_ir::inherent`.
    ///
    /// This module should only be used within the trait solver.
    pub redox::USAGE_OF_TYPE_IR_INHERENT,
    Allow,
    "usage `redox_type_ir::inherent` outside of trait system",
    report_in_external_macro: true
}

declare_tool_lint! {
    /// The `usage_of_type_ir_traits` lint detects usage of `redox_type_ir::Interner`,
    /// or `redox_infer::InferCtxtLike`.
    ///
    ...
    pub redox::USAGE_OF_TYPE_IR_TRAITS,
    Allow,
    "usage `redox_type_ir`-specific abstraction traits outside of trait system",
    report_in_external_macro: true
}
declare_tool_lint! {
    /// The `direct_use_of_redox_type_ir` lint detects usage of `redox_type_ir`.
    ///
    /// This module should only be used within the trait solver and some desirable
    /// crates like redox_middle.
    pub redox::DIRECT_USE_OF_RUSTC_TYPE_IR,
    Allow,
    "usage `redox_type_ir` abstraction outside of trait system",
    report_in_external_macro: true
}
...
impl<'tcx> LateLintPass<'tcx> for TypeIr {
    ...
    fn check_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx hir::Item<'tcx>) {
        let redox_hir::ItemKind::Use(path, kind) = item.kind else { return };
        ...
        // Final path resolutions, like `use redox_type_ir::inherent`
        ...
        let (lo, hi, snippet) = match path.segments {
            ...
            [.., segment]
                if let Some(type_ns) = path.res.type_ns
                    && is_mod_inherent(type_ns)
                    && let redox_hir::UseKind::Single(ident) = kind =>
            {
                let (lo, snippet) =
...
declare_tool_lint! {
    ...
    pub redox::LINT_PASS_IMPL_WITHOUT_MACRO,
    Allow,
...
impl LateLintPass<'_> for BadOptAccess {
    fn check_expr(&mut self, cx: &LateContext<'_>, expr: &hir::Expr<'_>) {
        ...
        // Skip types without `#[redox_lint_opt_ty]` - only so that the rest of the lint can be
        // avoided.
...
declare_tool_lint! {
    ...
    pub redox::SYMBOL_INTERN_STRING_LITERAL,
    // redox_driver crates out of the compiler can't/shouldn't add preinterned symbols;
...
impl<'tcx> LateLintPass<'tcx> for SymbolInternStringLiteral {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx redox_hir::Expr<'tcx>) {
        if let hir::ExprKind::Call(path, [arg]) = expr.kind
            ...
            && let redox_ast::LitKind::Str(_, _) = kind.node
        {
            cx.emit_span_lint(
...
declare_tool_lint! {
    ...
    pub redox::IMPLICIT_SYSROOT_CRATE_IMPORT,
    Allow,
...
declare_tool_lint! {
    pub redox::BAD_USE_OF_FIND_ATTR,
    Allow,
...
impl EarlyLintPass for BadUseOfFindAttr {
    fn check_arm(&mut self, cx: &EarlyContext<'_>, arm: &redox_ast::Arm) {
