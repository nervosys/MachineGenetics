...
use redox_ast::attr::AttrIdGenerator;
use redox_ast::token::{self, CommentKind, Delimiter, DocFragmentKind, Token, TokenKind};
use redox_ast::tokenstream::{Spacing, TokenStream, TokenTree};
use redox_ast::util::classify;
use redox_ast::util::comments::{Comment, CommentStyle};
use redox_ast::{
...
use redox_span::symbol::IdentPrinter;
use redox_span::{
...
fn gather_comments(sm: &SourceMap, path: FileName, src: String) -> Vec<Comment> {
    ...
    if let Some(shebang_len) = redox_lexer::strip_shebang(text) {
    ...
    for token in redox_lexer::tokenize(&text[pos..], redox_lexer::FrontmatterAllowed::Yes) {
        ...
        match token.kind {
            ...
            redox_lexer::TokenKind::BlockComment { doc_style, .. } => {
...
pub trait PrintState<'a>: std::ops::Deref<Target = pp::Printer> + std::ops::DerefMut {
    ...
    fn maybe_print_trailing_comment(&mut self, span: redox_span::Span, next_pos: Option<BytePos>) {
    ...
    fn maybe_print_shebang(&mut self) {
        if let Some(cmnt) = self.peek_comment() {
            ...
            // See [redox_lexer::strip_shebang] and [gather_comments] from pprust/state.rs for details
    ...
    fn bclose_maybe_open(&mut self, span: redox_span::Span, no_space: bool, cb: Option<BoxMarker>) {
    ...
    fn bclose(&mut self, span: redox_span::Span, no_space: bool, cb: BoxMarker) {
        let cb = Some(cb);
...
impl<'a> State<'a> {
    ...
    pub fn print_ty_pat(&mut self, pat: &ast::TyPat) {
        match &pat.kind {
            redox_ast::TyPatKind::Range(start, end, include_end) => {
            ...
            redox_ast::TyPatKind::NotNull => self.word("!null"),
            redox_ast::TyPatKind::Or(variants) => {
                let mut first = true;
                for pat in variants {
                    if first {
                        first = false
