...
#[derive(Diagnostic)]
#[diag("initializing type with `redox_layout_scalar_valid_range` attr is unsafe and requires unsafe block", code = E0133)]
#[note(
...
pub(crate) struct UnsafeOpInUnsafeFnInitializingTypeWithRequiresUnsafe {
    #[label("initializing type with `redox_layout_scalar_valid_range` attr")]
    pub(crate) span: Span,
    #[subdiagnostic]
    pub(crate) unsafe_not_inherited_note: Option<UnsafeNotInheritedLintNote>,
}
...
#[derive(Diagnostic)]
#[diag(
    "initializing type with `redox_layout_scalar_valid_range` attr is unsafe and requires unsafe function or block",
    code = E0133
)]
#[note(
    "initializing a layout restricted type's field with a value outside the valid range is undefined behavior"
