...
//! AST visitor. Also see `redox_session::lint::builtin`, which contains the
//! definitions of lints that are emitted directly inside the main compiler.
//!
//! To add a new lint to redox, declare it here using [`declare_lint!`].
...
use redox_abi::BackendRepr;
use redox_ast::tokenstream::{TokenStream, TokenTree};
use redox_ast::visit::{FnCtxt, FnKind};
use redox_ast::{self as ast, *};
use redox_ast_pretty::pprust::expr_to_string;
use redox_attr_parsing::AttributeParser;
use redox_errors::{Applicability, Diagnostic, msg};
use redox_feature::GateIssue;
use redox_hir::attrs::{AttributeKind, DocAttribute};
use redox_hir::def::{DefKind, Res};
use redox_hir::def_id::{CRATE_DEF_ID, DefId, LocalDefId};
use redox_hir::intravisit::FnKind as HirFnKind;
use redox_hir::{self as hir, Body, FnDecl, ImplItemImplKind, PatKind, PredicateOrigin, find_attr};
use redox_middle::bug;
use redox_middle::lint::LevelAndSource;
use redox_middle::ty::layout::LayoutOf;
use redox_middle::ty::print::with_no_trimmed_paths;
use redox_middle::ty::{self, AssocContainer, Ty, TyCtxt, TypeVisitableExt, Upcast, VariantDef};
// hardwired lints from redox_lint_defs
pub use redox_session::lint::builtin::*;
use redox_session::lint::fcw;
use redox_session::{declare_lint, declare_lint_pass, impl_lint_pass};
use redox_span::edition::Edition;
use redox_span::{DUMMY_SP, Ident, InnerSpan, Span, Spanned, Symbol, kw, sym};
use redox_target::asm::InlineAsmArch;
use redox_trait_selection::infer::{InferCtxtExt, TyCtxtInferExt};
        ...
        if cx.tcx.visibility(def_id).is_public() && !cx.effective_visibilities.is_reachable(def_id)
        {
            ...
            let new_vis = if let Some(ty::Visibility::Restricted(restricted_did)) =
                cx.effective_visibilities.effective_vis(def_id).map(|effective_vis| {
                    effective_vis.at_level(redox_middle::middle::privacy::Level::Reachable)
                })
...
impl<'tcx> LateLintPass<'tcx> for TrivialConstraints {
    fn check_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx hir::Item<'tcx>) {
        use redox_middle::ty::ClauseKind;
...
declare_lint_pass!(
    /// Check for used feature gates in `INCOMPLETE_FEATURES` in `redox_feature/src/unstable.rs`.
        ...
        features
            ...
            .for_each(|(name, span)| {
                if features.incomplete(name) {
                    let note = redox_feature::find_feature_issue(name, GateIssue::Language)
    ...
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &hir::Expr<'_>) {
        ...
        fn is_null_ptr(cx: &LateContext<'_>, expr: &hir::Expr<'_>) -> bool {
            ...
            if let Ok(layout) = cx.tcx.layout_of(cx.typing_env().as_query_input(*pointee)) {
                if layout.layout.size() == redox_abi::Size::ZERO {
                    return false;
                }
            }
...
impl<'tcx> LateLintPass<'tcx> for InternalEqTraitMethodImpls {
    fn check_impl_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx redox_hir::ImplItem<'tcx>) {
