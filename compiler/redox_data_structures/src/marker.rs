...
#[diagnostic::on_unimplemented(message = "`{Self}` doesn't implement `DynSend`. \
            Add it to `redox_data_structures::marker` or use `IntoDynSyncSend` if it's already `Send`")]
...
#[diagnostic::on_unimplemented(message = "`{Self}` doesn't implement `DynSync`. \
            Add it to `redox_data_structures::marker` or use `IntoDynSyncSend` if it's already `Sync`")]
...
already_send!(
    [std::sync::atomic::AtomicBool][std::sync::atomic::AtomicUsize][std::sync::atomic::AtomicU8]
        ...
        [std::io::Error][std::fs::File][std::panic::Location<'_>][redox_arena::DroplessArena]
...
impl_dyn_send!(
    ...
    [redox_arena::TypedArena<T> where T: DynSend]
