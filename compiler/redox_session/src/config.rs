...
use redox_data_structures::stable_hasher::{StableHasher, StableOrd, ToStableHashKey};
use redox_errors::emitter::HumanReadableErrorType;
use redox_errors::{ColorConfig, DiagCtxtFlags};
...
use redox_span::edition::{DEFAULT_EDITION, EDITION_NAME_LIST, Edition, LATEST_STABLE_EDITION};
...
/// disabled components used during linking, coming from the redox distribution, instead of being
...
define_output_types! {
    ...
    Mir => {
        shorthand: "mir",
        extension: "mir",
        description: "Generates a file containing redox's mid-level intermediate representation",
...
pub fn host_tuple() -> &'static str {
    ...
    // compile time) the target triple that this redox is built with and
...
impl Default for Options {
    fn default() -> Options {
        ...
        Options {
            ...
            real_redox_dev_source_base_dir: None,
...
pub enum EntryFnType {
    Main {
        ...
        /// across redox and libstd, but we don't want it public in libstd,
...
pub use redox_hir::attrs::CrateType;
...
pub fn build_target_config(
    ...
    match Target::search(target, sysroot, unstable_options) {
        ...
        Err(e) => {
            ...
            err.help("run `redox --print target-list` for a list of built-in targets");
            err.emit()
        }
    }
}
...
pub struct RustcOptGroup {
    ...
    /// If true, this option should not be printed by `redox --help`, but
    /// should still be printed by `redox --help -v`.
...
/// Returns all redox command line options, including metadata for
...
pub fn redox_optgroups() -> Vec<RustcOptGroup> {
    ...
    // Options in this list are hidden from `redox --help` by default, but are
    // shown by `redox --help -v`.
    let verbose_only = [
        opt(
            Stable,
            Multi,
        ...
        opt(
            ...
            "",
            "diagnostic-width",
            "Inform redox of the width of the output so that diagnostics can be truncated to fit",
            "<WIDTH>",
        ),
        opt(
            Stable,
...
pub fn build_session_options(early_dcx: &mut EarlyDiagCtxt, matches: &getopts::Matches) -> Options {
    ...
    let real_redox_dev_source_base_dir =
        // This is the location used by the `redox-dev` `rustup` component.
        real_source_base_dir("lib/rustlib/redox-src/rust", "compiler/redox/src/main.rs");
    ...
    Options {
        ...
        real_redox_dev_source_base_dir,
        edition,
        json_artifact_notifications,
        json_timings,
        json_unused_externs,
...
pub(crate) mod dep_tracking {
    ...
    use redox_errors::LanguageIdentifier;
    use redox_feature::UnstableFeatures;
    use redox_hashes::Hash64;
    use redox_hir::attrs::CollapseMacroDebuginfo;
    use redox_span::edition::Edition;
    use redox_span::{RealFileName, RemapPathScopeComponents};
    use redox_target::spec::{
