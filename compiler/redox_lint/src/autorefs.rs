use redox_ast::{BorrowKind, UnOp};
use redox_hir::{Expr, ExprKind, Mutability, find_attr};
...
impl<'tcx> LateLintPass<'tcx> for ImplicitAutorefs {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'_>) {
        ...
        //   b. A method call annotated with `#[redox_no_implicit_refs]`.
        ...
        let inner = match expr.kind {
            ...
            ExprKind::MethodCall(_, inner, _, _) => {
                // PERF: Checking of `#[redox_no_implicit_refs]` is deferred below
        ...
        if let Some(adjustments) = adjustments_table.get(inner.hir_id)
            ...
            && let method_did = match expr.kind {
                // PERF: 5. b. A method call annotated with `#[redox_no_implicit_refs]`
