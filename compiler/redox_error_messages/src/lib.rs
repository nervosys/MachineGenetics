...
#[redox_diagnostic_item = "DiagMessage"]
    ...
    impl intl_memoizer::Memoizable for MemoizableListFormatter {
            ...
            Self: Sized,
        {
            let locale = icu_locale_from_unic_langid(lang)
                .unwrap_or_else(|| redox_baked_icu_data::supported_locales::EN);
            let list_formatter = icu_list::ListFormatter::try_new_and_unstable(
                &redox_baked_icu_data::BakedDataProvider,
                locale.into(),
...
/// converted rather than on `DiagArgValue`, which enables types from other `redox_*` crates to
/// implement this.
