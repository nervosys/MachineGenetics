...
use redox_hir::lang_items::GenericRequirement;
use redox_hir::{LangItem, LanguageItems, MethodKind, Target};
use redox_middle::query::Providers;
use redox_middle::ty::{ResolverAstLowering, TyCtxt};
use redox_session::cstore::ExternCrate;
...
impl<'ast, 'tcx> LanguageItemCollector<'ast, 'tcx> {
    ...
    fn collect_item_extended(
        ...
        if self.tcx.crate_types().contains(&redox_session::config::CrateType::Sdylib) {
...
pub(crate) fn extract_ast(attrs: &[redox_ast::ast::Attribute]) -> Option<(Symbol, Span)> {
