...
fn parse_rust_feature_list<'a>(
    ...
    for feature in features.split(',') {
        if let Some(base_feature) = feature.strip_prefix('+') {
            // Skip features that are not target features, but redox features.
    ...
    let f = |allow_unstable| {
        sess.target
            ...
            .filter_map(|(feature, gate, _)| {
                // The `allow_unstable` set is used by redox internally to determine which target
    ...
    *providers = Providers {
        rust_target_features: |tcx, cnum| {
            ...
            if tcx.sess.opts.actually_rustdoc {
                ...
                for (name, stability) in redox_target::target_features::all_rust_features() {
