...
use redox_errors::{Diag, DiagCtxtHandle, Diagnostic, MultiSpan, msg};
use redox_feature::{Features, GateIssue};
...
use redox_session::lint::{Level, Lint, LintExpectationId, LintId};
        ...
        for &(ref lint_name, level) in &self.sess.opts.lint_opts {
            ...
            match self.store.check_lint_name(lint_name_only, tool_name, self.registered_tools) {
                ...
                CheckLintNameResult::NoLint(suggestion) => {
                    ...
                    let suggestion = suggestion.map(|(replace, from_redox)| {
                        UnknownLintSuggestion::WithoutSpan { replace, from_redox }
                    });
                    let requested_level = RequestedLevel { level, lint_name };
