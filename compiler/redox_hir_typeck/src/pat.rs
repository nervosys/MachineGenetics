...
use redox_hir::pat_util::EnumerateAndAdjustIterator;
use redox_hir::{
    self as hir, BindingMode, ByRef, ExprKind, HirId, LangItem, Mutability, Pat, PatExpr,
    PatExprKind, PatKind, expr_needs_parens,
};
use redox_hir_analysis::autoderef::report_autoderef_recursion_limit_error;
use redox_infer::infer::RegionVariableOrigin;
use redox_middle::traits::PatternOriginExpr;
use redox_middle::ty::{self, Pinnedness, Ty, TypeVisitableExt};
use redox_middle::{bug, span_bug};
use redox_session::lint::builtin::NON_EXHAUSTIVE_OMITTED_PATTERNS;
use redox_session::parse::feature_err;
use redox_span::edit_distance::find_best_match_for_name;
use redox_span::edition::Edition;
use redox_span::{BytePos, DUMMY_SP, Ident, Span, kw, sym};
use redox_trait_selection::infer::InferCtxtExt;
use redox_trait_selection::traits::{ObligationCause, ObligationCauseCode};
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    fn check_pat_expr_unadjusted(&self, lt: &'tcx hir::PatExpr<'tcx>) -> Ty<'tcx> {
        let ty = match &lt.kind {
            redox_hir::PatExprKind::Lit { lit, negated } => {
            ...
            redox_hir::PatExprKind::Path(qpath) => {
    ...
    fn error_tuple_variant_index_shorthand(
            ...
            (variant.ctor_kind(), &pat.kind)
        {
            ...
            if has_shorthand_field_name {
                let path = redox_hir_pretty::qpath_to_string(&self.tcx, qpath);
                let mut err = struct_span_code_err!(
                    self.dcx(),
                    pat.span,
                    E0769,
        ...
        fields
            ...
            .map(|field| {
                match self.tcx.sess.source_map().span_to_snippet(field.pat.span) {
                    ...
                    Err(_) => redox_hir_pretty::pat_to_string(&self.tcx, field.pat),
                }
            })
            .collect::<Vec<String>>()
            .join(", ")
    ...
    ///
    /// Nb: the partner lint for enums lives in `compiler/redox_mir_build/src/thir/pattern/usefulness.rs`.
        ...
        impl<'a, 'b, 'c, 'tcx> Diagnostic<'a, ()> for FieldsNotListed<'b, 'c, 'tcx> {
            fn into_diag(self, dcx: DiagCtxtHandle<'a>, level: Level) -> Diag<'a, ()> {
                ...
                Diag::new(dcx, level, "some fields are not explicitly listed")
                    .with_span_label(pat_span, format!("field{} {} not listed", redox_errors::pluralize!(unmentioned_fields.len()), joined_patterns))
                    .with_help(
