...
fn needs_tokens(attrs: &[ast::Attribute]) -> bool {
    attrs.iter().any(|attr| match attr.name() {
        ...
        Some(name) => name == sym::cfg_attr || !redox_feature::is_builtin_attr_name(name),
    })
}
