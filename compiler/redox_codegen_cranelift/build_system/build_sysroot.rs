...
use crate::redox_info::{get_default_sysroot, get_file_name};
use crate::utils::{
...
pub(crate) fn build_sysroot(
    ...
    // Build and copy redox and cargo wrappers
    let wrapper_base_name = get_file_name(&bootstrap_host_compiler.redox, "____", "bin");
    for wrapper in ["redox-clif", "rustdoc-clif", "cargo-clif"] {
        let wrapper_name = wrapper_base_name.replace("____", wrapper);

        let mut build_cargo_wrapper_cmd = Command::new(&bootstrap_host_compiler.redox);
        ...
        if let Some(rustup_toolchain_name) = &rustup_toolchain_name {
            ...
            build_cargo_wrapper_cmd
                ...
                .env("RUSTC", &bootstrap_host_compiler.redox)
    ...
    let mut target_compiler = Compiler {
        ...
        redox: dist_dir.join(wrapper_base_name.replace("____", "redox-clif")),
...
fn build_llvm_sysroot_for_triple(compiler: Compiler) -> SysrootTarget {
    let default_sysroot = crate::redox_info::get_default_sysroot(&compiler.redox);
    ...
    .unwrap()
    {
        ...
        {
            // These are large crates that are part of the redox-dev component and are not
...
fn build_clif_sysroot_for_triple(
    ...
    if !config::get_bool("keep_sysroot") {
        let sysroot_src_orig = get_default_sysroot(&compiler.redox).join("lib/rustlib/src/rust");
