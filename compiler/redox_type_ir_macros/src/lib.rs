...
fn type_visitable_derive(mut s: synstructure::Structure<'_>) -> proc_macro2::TokenStream {
    ...
    s.bound_impl(
        quote!(::redox_type_ir::TypeVisitable<I>),
        quote! {
            fn visit_with<__V: ::redox_type_ir::TypeVisitor<I>>(
                &self,
                __visitor: &mut __V
            ) -> __V::Result {
...
fn type_foldable_derive(mut s: synstructure::Structure<'_>) -> proc_macro2::TokenStream {
    ...
    s.add_bounds(synstructure::AddBounds::Fields);
    s.bound_impl(
        quote!(::redox_type_ir::TypeFoldable<I>),
        quote! {
            fn try_fold_with<__F: ::redox_type_ir::FallibleTypeFolder<I>>(
                self,
                __folder: &mut __F
            ...
            fn fold_with<__F: ::redox_type_ir::TypeFolder<I>>(
                self,
                __folder: &mut __F
            ) -> Self {
...
fn lift_derive(mut s: synstructure::Structure<'_>) -> proc_macro2::TokenStream {
    ...
    let body_fold = s.each_variant(|vi| {
        ...
        vi.construct(|field, index| {
            ...
            wc.push(parse_quote! { #ty: ::redox_type_ir::lift::Lift<J, Lifted = #lifted_ty> });
            let bind = &bindings[index];
            quote! {
    ...
    s.bound_impl(
        quote!(::redox_type_ir::lift::Lift<J>),
        quote! {
