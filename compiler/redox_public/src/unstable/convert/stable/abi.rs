//! Conversion of internal Rust compiler `redox_target` and `redox_abi` items to stable ones.

#![allow(redox::usage_of_qualified_ty)]

use redox_abi::{ArmCall, CanonAbi, InterruptKind, X86Call};
use redox_middle::ty;
use redox_public_bridge::Tables;
use redox_public_bridge::context::CompilerCtxt;
use redox_target::callconv;
...
impl<'tcx> Stable<'tcx> for redox_abi::VariantIdx {
    type T = VariantIdx;
...
impl<'tcx> Stable<'tcx> for redox_abi::Endian {
    type T = crate::target::Endian;

    fn stable(&self, _: &mut Tables<'_, BridgeTys>, _: &CompilerCtxt<'_, BridgeTys>) -> Self::T {
        match self {
            redox_abi::Endian::Little => crate::target::Endian::Little,
            redox_abi::Endian::Big => crate::target::Endian::Big,
        }
    }
}

impl<'tcx> Stable<'tcx> for redox_abi::TyAndLayout<'tcx, ty::Ty<'tcx>> {
    type T = TyAndLayout;
...
impl<'tcx> Stable<'tcx> for redox_abi::Layout<'tcx> {
    type T = Layout;
...
impl<'tcx> Stable<'tcx> for redox_abi::LayoutData<redox_abi::FieldIdx, redox_abi::VariantIdx> {
    type T = LayoutShape;
...
impl<'tcx> Stable<'tcx> for redox_abi::FieldsShape<redox_abi::FieldIdx> {
    type T = FieldsShape;
    ...
    fn stable<'cx>(
        ...
        match self {
            redox_abi::FieldsShape::Primitive => FieldsShape::Primitive,
            redox_abi::FieldsShape::Union(count) => FieldsShape::Union(*count),
            redox_abi::FieldsShape::Array { stride, count } => {
                FieldsShape::Array { stride: stride.stable(tables, cx), count: *count }
            }
            redox_abi::FieldsShape::Arbitrary { offsets, .. } => {
...
impl<'tcx> Stable<'tcx> for redox_abi::Variants<redox_abi::FieldIdx, redox_abi::VariantIdx> {
    ...
    fn stable<'cx>(
        ...
        match self {
            redox_abi::Variants::Single { index } => {
                VariantsShape::Single { index: index.stable(tables, cx) }
            }
            redox_abi::Variants::Empty => VariantsShape::Empty,
            redox_abi::Variants::Multiple { tag, tag_encoding, tag_field, variants } => {
                VariantsShape::Multiple {
                    ...
                    variants: variants
                        ...
                        .map(|v| match &v.fields {
                            redox_abi::FieldsShape::Arbitrary { offsets, .. } => VariantFields {
...
impl<'tcx> Stable<'tcx> for redox_abi::TagEncoding<redox_abi::VariantIdx> {
    type T = TagEncoding;
    ...
    fn stable<'cx>(
        ...
        match self {
            redox_abi::TagEncoding::Direct => TagEncoding::Direct,
            redox_abi::TagEncoding::Niche { untagged_variant, niche_variants, niche_start } => {
                TagEncoding::Niche {
...
impl<'tcx> Stable<'tcx> for redox_abi::BackendRepr {
    type T = ValueAbi;
    ...
    fn stable<'cx>(
        ...
        match *self {
            redox_abi::BackendRepr::Scalar(scalar) => ValueAbi::Scalar(scalar.stable(tables, cx)),
            redox_abi::BackendRepr::ScalarPair(first, second) => {
                ValueAbi::ScalarPair(first.stable(tables, cx), second.stable(tables, cx))
            }
            redox_abi::BackendRepr::SimdVector { element, count } => {
                ValueAbi::Vector { element: element.stable(tables, cx), count }
            }
            redox_abi::BackendRepr::SimdScalableVector { element, count } => {
                ValueAbi::ScalableVector { element: element.stable(tables, cx), count }
            }
            redox_abi::BackendRepr::Memory { sized } => ValueAbi::Aggregate { sized },
        }
    }
}

impl<'tcx> Stable<'tcx> for redox_abi::Size {
    type T = Size;
...
impl<'tcx> Stable<'tcx> for redox_abi::Align {
    type T = Align;
...
impl<'tcx> Stable<'tcx> for redox_abi::Scalar {
    type T = Scalar;
    ...
    fn stable<'cx>(
        ...
        match self {
            redox_abi::Scalar::Initialized { value, valid_range } => Scalar::Initialized {
                value: value.stable(tables, cx),
                valid_range: valid_range.stable(tables, cx),
            },
            redox_abi::Scalar::Union { value } => Scalar::Union { value: value.stable(tables, cx) },
        }
    }
}

impl<'tcx> Stable<'tcx> for redox_abi::Primitive {
    type T = Primitive;
    ...
    fn stable<'cx>(
        ...
        match self {
            redox_abi::Primitive::Int(length, signed) => {
                Primitive::Int { length: length.stable(tables, cx), signed: *signed }
            }
            redox_abi::Primitive::Float(length) => {
                Primitive::Float { length: length.stable(tables, cx) }
            }
            redox_abi::Primitive::Pointer(space) => Primitive::Pointer(space.stable(tables, cx)),
        }
    }
}

impl<'tcx> Stable<'tcx> for redox_abi::AddressSpace {
    type T = AddressSpace;
...
impl<'tcx> Stable<'tcx> for redox_abi::Integer {
    type T = IntegerLength;

    fn stable(&self, _: &mut Tables<'_, BridgeTys>, _: &CompilerCtxt<'_, BridgeTys>) -> Self::T {
        match self {
            redox_abi::Integer::I8 => IntegerLength::I8,
            redox_abi::Integer::I16 => IntegerLength::I16,
            redox_abi::Integer::I32 => IntegerLength::I32,
            redox_abi::Integer::I64 => IntegerLength::I64,
            redox_abi::Integer::I128 => IntegerLength::I128,
        }
    }
}

impl<'tcx> Stable<'tcx> for redox_abi::Float {
    type T = FloatLength;

    fn stable(&self, _: &mut Tables<'_, BridgeTys>, _: &CompilerCtxt<'_, BridgeTys>) -> Self::T {
        match self {
            redox_abi::Float::F16 => FloatLength::F16,
            redox_abi::Float::F32 => FloatLength::F32,
            redox_abi::Float::F64 => FloatLength::F64,
            redox_abi::Float::F128 => FloatLength::F128,
        }
    }
}

impl<'tcx> Stable<'tcx> for redox_abi::WrappingRange {
...
impl<'tcx> Stable<'tcx> for redox_abi::ReprFlags {
    type T = ReprFlags;
...
impl<'tcx> Stable<'tcx> for redox_abi::IntegerType {
    type T = IntegerType;
    ...
    fn stable<'cx>(
        ...
        match self {
            redox_abi::IntegerType::Pointer(signed) => IntegerType::Pointer { is_signed: *signed },
            redox_abi::IntegerType::Fixed(integer, signed) => {
...
impl<'tcx> Stable<'tcx> for redox_abi::ReprOptions {
    type T = ReprOptions;
