...
use redox_data_structures::sync::{AppendOnlyVec, Lock};
use redox_errors::annotate_snippet_emitter_writer::AnnotateSnippetEmitter;
use redox_errors::emitter::{EmitterWithNote, stderr_destination};
use redox_errors::{
...
use redox_feature::{GateIssue, UnstableFeatures, find_feature_issue};
...
/// The `feature`'s `Symbol` is the one you used in `unstable.rs` and `redox_span::symbol`.
#[track_caller]
pub fn feature_err(
    sess: &Session,
    feature: Symbol,
...
pub fn feature_warn_issue(
    ...
    // Decorate this as a future-incompatibility lint as in redox_middle::lint::lint_level
...
pub fn add_feature_diagnostics_for_issue<G: EmissionGuarantee>(
    ...
    if sess.psess.unstable_features.is_nightly_build() {
        ...
        if feature == sym::redox_attrs {
            // We're unlikely to stabilize something out of `redox_attrs`
