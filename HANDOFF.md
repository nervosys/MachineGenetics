...
| 17 | **`rmi`'s `wasm` feature builds, pulls a dependency, and enables no code** | Found on 2026-08-19 by `check-orphan-sources.sh`'s first run, and it is item 15's shape with the failure mode inverted. `compute/wasm.rs` is 208 lines that no `mod` declaration reaches, so it never compiles; `Cargo.toml` still declares `wasm = ["dep:wasm-bindgen", …]`. Measured: `cargo build --features wasm` **succeeds** (exit 0) and `cargo tree --features wasm -i wasm-bindgen` shows **wasm-bindgen v0.2.126** in the graph, for zero compiled lines. Item 15's `cuda` at least fails loudly; this one is silent, so a consumer who enables it gets a heavier dependency tree and concludes the backend is present. Worse, `core/discoverability.rs` publishes `("wasm_backend", "WASM Backend", "WebAssembly SIMD")` into the ontology **unconditionally** — not behind the feature — so an agent querying the crate for its compute backends is told about one whose implementation no compiler has ever read. Same disposition as item 15: `rmi` is vendored and must stay syncable against its own upstream, so wiring the `mod` in, deleting the file, or dropping the feature is its owner's call. Baselined in `scripts/orphan-sources-baseline.txt` so a third orphan fails. |
