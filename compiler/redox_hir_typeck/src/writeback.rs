...
use redox_data_structures::unord::ExtendUnord;
use redox_errors::{E0720, ErrorGuaranteed};
use redox_hir::def_id::LocalDefId;
use redox_hir::intravisit::{self, InferKind, Visitor};
use redox_hir::{self as hir, AmbigArg, HirId, find_attr};
use redox_infer::traits::solve::Goal;
use redox_middle::traits::ObligationCause;
use redox_middle::ty::adjustment::{Adjust, Adjustment, PointerCoercion};
use redox_middle::ty::{
...
use redox_trait_selection::opaque_types::opaque_type_has_defining_use_args;
use redox_trait_selection::solve;
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    pub(crate) fn resolve_type_vars_in_body(
        ...
        let redox_dump_user_args =
            self.has_redox_attrs && find_attr!(self.tcx, item_def_id, RustcDumpUserArgs);

        let mut wbcx = WritebackCx::new(self, body, redox_dump_user_args);
