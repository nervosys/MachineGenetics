...
use redox_errors::{Diag, ErrorGuaranteed};
...
use redox_mir_dataflow::Analysis;
use redox_mir_dataflow::impls::{MaybeStorageLive, always_storage_live_locals};
...
type QualifResults<'mir, 'tcx, Q> =
    redox_mir_dataflow::ResultsCursor<'mir, 'tcx, FlowSensitiveAnalysis<'mir, 'tcx, Q>>;
        ...
        let gate = match op.status_in_item(self.ccx) {
            Status::Unstable {
                ...
                if !safe_to_expose_on_stable
                    ...
                    && !super::redox_allow_const_fn_unstable(self.tcx, self.def_id(), gate)
                {
        ...
        match &terminator.kind {
            ...
            | TerminatorKind::TailCall { func, args, fn_span, .. } => {
                ...
                if let Some(intrinsic) = tcx.intrinsic(callee) {
                    ...
                    // `#[redox_const_stable_indirect]` an attribute that is always safe to add.
                    ...
                    match tcx.lookup_const_stability(callee) {
                        ...
                        Some(hir::ConstStability {
                            ...
                            // All good. Note that a `#[redox_const_stable]` intrinsic (meaning it
                            // can be *directly* invoked from stable const code) does not always
                            // have the `#[redox_intrinsic_const_stable_indirect]` attribute (which controls
