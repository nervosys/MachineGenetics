...
use redox_abi::{Align, AlignFromBytesError};
...
/// `Endian` is in `redox_abi`, which doesn't have access to the macro and serde.
struct EndianWrapper(redox_abi::Endian);
impl FromStr for EndianWrapper {
    type Err = String;
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        redox_abi::Endian::from_str(s).map(Self)
    }
}
...
/// `ExternAbi` is in `redox_abi`, which doesn't have access to the macro and serde.
struct ExternAbiWrapper(redox_abi::ExternAbi);
impl FromStr for ExternAbiWrapper {
    type Err = String;
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        redox_abi::ExternAbi::from_str(s)
            .map(Self)
...
impl schemars::JsonSchema for ExternAbiWrapper {
    ...
    fn json_schema(_: &mut schemars::SchemaGenerator) -> schemars::Schema {
        let all =
            redox_abi::ExternAbi::ALL_VARIANTS.iter().map(|abi| abi.as_str()).collect::<Vec<_>>();
        schemars::json_schema! ({
            "type": "string",
            "enum": all,
        })
