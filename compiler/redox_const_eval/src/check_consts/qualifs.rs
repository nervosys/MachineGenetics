...
use redox_middle::ty::{self, AdtDef, Ty};
use redox_middle::{bug, mir};
use redox_trait_selection::traits::{Obligation, ObligationCause, ObligationCtxt};
        ...
        ocx.register_obligation(Obligation::new(
            ...
            ty::Binder::dummy(ty::TraitRef::new(cx.tcx, destruct_def_id, [ty]))
                .to_host_effect_clause(
                    cx.tcx,
                    match cx.const_kind() {
                        redox_hir::ConstContext::ConstFn => ty::BoundConstness::Maybe,
                        redox_hir::ConstContext::Static(_)
                        | redox_hir::ConstContext::Const { .. } => ty::BoundConstness::Const,
                    },
                ),
        ));
