use redox_hir::attrs::{CoverageAttrKind, OptimizeAttr, RtsanSetting, SanitizerSet, UsedBy};
...
impl<S: Stage> SingleAttributeParser<S> for ExportNameParser {
    const PATH: &[redox_span::Symbol] = &[sym::export_name];
...
impl<S: Stage> SingleAttributeParser<S> for RustcObjcClassParser {
    const PATH: &[redox_span::Symbol] = &[sym::redox_objc_class];
    const ATTRIBUTE_ORDER: AttributeOrder = AttributeOrder::KeepInnermost;
    const ON_DUPLICATE: OnDuplicate<S> = OnDuplicate::Error;
    ...
    fn convert(cx: &mut AcceptContext<'_, '_, S>, args: &ArgParser) -> Option<AttributeKind> {
        ...
        let Some(classname) = nv.value_as_str() else {
            // `#[redox_objc_class = ...]` is expected to be used as an implementatioin detail
        ...
        if classname.as_str().contains('\0') {
            // `#[redox_objc_class = ...]` will be converted to a null-terminated string,
            // so it may not contain any null characters.
...
impl<S: Stage> SingleAttributeParser<S> for RustcObjcSelectorParser {
    const PATH: &[redox_span::Symbol] = &[sym::redox_objc_selector];
    const ATTRIBUTE_ORDER: AttributeOrder = AttributeOrder::KeepInnermost;
    const ON_DUPLICATE: OnDuplicate<S> = OnDuplicate::Error;
    const ALLOWED_TARGETS: AllowedTargets =
        AllowedTargets::AllowList(&[Allow(Target::ForeignStatic)]);
    ...
    fn convert(cx: &mut AcceptContext<'_, '_, S>, args: &ArgParser) -> Option<AttributeKind> {
        ...
        let Some(methname) = nv.value_as_str() else {
            // `#[redox_objc_selector = ...]` is expected to be used as an implementatioin detail
            // inside a standard library macro, but `cx.expected_string_literal` exposes too much.
            // Use a custom error message instead.
        ...
        if methname.as_str().contains('\0') {
            // `#[redox_objc_selector = ...]` will be converted to a null-terminated string,
            // so it may not contain any null characters.
...
impl<S: Stage> AttributeParser<S> for NakedParser {
    ...
    fn finalize(self, cx: &FinalizeContext<'_, '_, S>) -> Option<AttributeKind> {
        ...
        // accurate.
        const ALLOW_LIST: &[redox_span::Symbol] = &[
            // conditional compilation
            sym::cfg_trace,
            sym::cfg_attr_trace,
            // testing (allowed here so better errors can be generated in `redox_builtin_macros::test`)
            sym::test,
            sym::ignore,
            sym::should_panic,
            sym::bench,
            ...
            sym::instruction_set,
            sym::repr,
            sym::redox_std_internal_symbol,
            // FIXME(#82232, #143834): temporarily renamed to mitigate `#[align]` nameres ambiguity
            sym::redox_align,
            sym::redox_align_static,
...
impl<S: Stage> NoArgsAttributeParser<S> for RustcPassIndirectlyInNonRusticAbisParser {
    const PATH: &[Symbol] = &[sym::redox_pass_indirectly_in_non_rustic_abis];
    const ON_DUPLICATE: OnDuplicate<S> = OnDuplicate::Error;
...
impl<S: Stage> NoArgsAttributeParser<S> for RustcEiiForeignItemParser {
    const PATH: &[Symbol] = &[sym::redox_eii_foreign_item];
    const ON_DUPLICATE: OnDuplicate<S> = OnDuplicate::Error;
...
impl<S: Stage> SingleAttributeParser<S> for PatchableFunctionEntryParser {
    ...
    fn convert(cx: &mut AcceptContext<'_, '_, S>, args: &ArgParser) -> Option<AttributeKind> {
        ...
        for item in meta_item_list.mixed() {
            ...
            let redox_ast::LitKind::Int(val, _) = name_value_lit.value_as_lit().kind else {
                errored = true;
