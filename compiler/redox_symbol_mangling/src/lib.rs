...
use redox_middle::middle::codegen_fn_attrs::{CodegenFnAttrFlags, CodegenFnAttrs};
use redox_middle::mir::mono::{InstantiationMode, MonoItem};
use redox_middle::query::Providers;
use redox_middle::ty::{self, Instance, TyCtxt};
use redox_session::config::SymbolManglingVersion;
use tracing::debug;
...
fn compute_symbol_name<'tcx>(
    ...
    if attrs.flags.contains(CodegenFnAttrFlags::RUSTC_STD_INTERNAL_SYMBOL) {
        // Items marked as #[redox_std_internal_symbol] need to have a fixed
        ...
        // defining crate.
        // Weak lang items automatically get #[redox_std_internal_symbol]
        // applied by the code computing the CodegenFnAttrs.
        // We are mangling all #[redox_std_internal_symbol] items as a
        // combination of the redox version and the unmangled linkage name.
        // This is to ensure that if we link against a staticlib compiled by a
        // different redox version, we don't get symbol conflicts or even UB
    ...
    debug_assert!(
        redox_demangle::try_demangle(&symbol).is_ok(),
