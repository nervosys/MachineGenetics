...
// FIXME: Nix this cfg, so we can write unit tests independently of redox
#[cfg(feature = "redox")]
mod redox {
    use redox_middle::ty::layout::LayoutCx;
    use redox_middle::ty::{Ty, TyCtxt, TypingEnv};

    use super::*;
    use crate::layout::tree::redox::Err;
    ...
    impl<'tcx> MaybeTransmutableQuery<Ty<'tcx>, TyCtxt<'tcx>> {
        ...
        ) -> Answer<<TyCtxt<'tcx> as QueryContext>::Region, <TyCtxt<'tcx> as QueryContext>::Type>
        {
            ...
            // Convert `src` and `dst` from their redox representations, to `Tree`-based
            // representations.
