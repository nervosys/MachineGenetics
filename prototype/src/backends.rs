...
pub fn builtin_backends() -> Vec<BackendDescriptor> {
    vec![
        ...
        BackendDescriptor::builtin(
            "metal", "gpu", "Apple", "a Metal binding, which does not exist yet",
            "Apple Metal GPU. NOT IMPLEMENTED: compute/metal.rs has no Metal binding and no `metal` cargo feature exists. Requesting it returns an error.",
            false,
        ),
        BackendDescriptor::builtin(
            "apple_ane", "npu", "Apple", "an ANE binding, which does not exist yet",
            "Apple Neural Engine. NOT IMPLEMENTED: no ANE binding, no `apple_ane` feature. Requesting it returns an error.",
            false,
        ),
        BackendDescriptor::builtin(
            "vulkan", "gpu", "any (AMD/Intel/NVIDIA)",
            "a Vulkan binding, which does not exist yet",
            "Cross-vendor GPU via Vulkan. NOT IMPLEMENTED: no Vulkan binding, no `vulkan` feature. Requesting it returns an error.",
            false,
        ),
        BackendDescriptor::builtin(
            "webgpu", "gpu", "any", "cargo feature `gpu` (rmi), which builds the real wgpu backend",
            "wgpu-based cross-platform GPU. Real, behind rmi feature `gpu` (compute/wgpu_backend.rs); without it, requesting WebGPU returns an error.",
            false,
        ),
        BackendDescriptor::builtin(
            "qualcomm", "npu", "Qualcomm", "a QNN binding, which does not exist yet",
            "Qualcomm Hexagon DSP/NPU. NOT IMPLEMENTED: no QNN binding, no `qualcomm` feature. Requesting it returns an error.",
            false,
        ),
        BackendDescriptor::builtin(
            "blas", "cpu", "any",
