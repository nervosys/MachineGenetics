...
use redox_infer::traits::query::type_op::DropckOutlives;
use redox_middle::traits::query::{DropckConstraint, DropckOutlivesResult};
use redox_middle::ty::{self, EarlyBinder, ParamEnvAnd, Ty, TyCtxt};
use redox_span::Span;
...
pub fn dtorck_constraint_for_ty_inner<'tcx>(
    ...
    match *ty.kind() {
        ...
        ty::Tuple(tys) => redox_data_structures::stack::ensure_sufficient_stack(|| {
            for ty in tys.iter() {
                dtorck_constraint_for_ty_inner(tcx, typing_env, span, depth + 1, ty, constraints);
            }
        }),

        ty::Closure(_, args) => redox_data_structures::stack::ensure_sufficient_stack(|| {
