...
use redox_hir::def::{self, CtorKind, DefKind, Namespace};
use redox_hir::def_id::{DefIdMap, DefIdSet, LOCAL_CRATE, ModDefId};
use redox_hir::definitions::{DefKey, DefPathDataName};
use redox_hir::limit::Limit;
use redox_macros::{Lift, extension};
use redox_session::cstore::{ExternCrate, ExternCrateSource};
use redox_span::{Ident, RemapPathScopeComponents, Symbol, kw, sym};
use redox_type_ir::{FieldInfo, Upcast as _, elaborate};
...
define_helper!(
    ...
    // This function is used by `redox_public` and downstream redox-driver in
    ...
    /// of various redox types, for example `std::vec::Vec` would be trimmed to `Vec`,
    ...
    fn pretty_print_type(&mut self, ty: Ty<'tcx>) -> Result<(), PrintError> {
        match *ty.kind() {
            ...
            ty::Bound(debruijn, bound_ty) => match bound_ty.kind {
                ty::BoundTyKind::Anon => {
                    redox_type_ir::debug_bound_var(self, debruijn, bound_ty.var)?
                }
            ...
            ty::Alias(ty::Opaque, ty::AliasTy { def_id, args, .. }) => {
                ...
                // from `redox_middle::ty` during the computation of `tcx.predicates_of`),
    ...
    fn pretty_print_const(
        ...
        match ct.kind() {
            ...
            ty::ConstKind::Bound(debruijn, bound_var) => {
                redox_type_ir::debug_bound_var(self, debruijn, bound_var)?
            }
...
impl<'tcx> redox_errors::IntoDiagArg for TraitRefPrintOnlyTraitPath<'tcx> {
    fn into_diag_arg(self, path: &mut Option<std::path::PathBuf>) -> redox_errors::DiagArgValue {
        ty::tls::with(|tcx| {
            let trait_ref = tcx.short_string(self, path);
            redox_errors::DiagArgValue::Str(std::borrow::Cow::Owned(trait_ref))
        })
    }
}
...
impl<'tcx> redox_errors::IntoDiagArg for TraitRefPrintSugared<'tcx> {
    fn into_diag_arg(self, path: &mut Option<std::path::PathBuf>) -> redox_errors::DiagArgValue {
        ty::tls::with(|tcx| {
            let trait_ref = tcx.short_string(self, path);
            redox_errors::DiagArgValue::Str(std::borrow::Cow::Owned(trait_ref))
...
pub fn trimmed_def_paths(tcx: TyCtxt<'_>, (): ()) -> DefIdMap<Symbol> {
    ...
    // unnecessary.
    //
    // The `redox_middle::ty::print::with_no_trimmed_paths` wrapper can be used to suppress this
