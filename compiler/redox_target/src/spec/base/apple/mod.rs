...
pub(crate) fn base(
    ...
    let mut opts = TargetOptions {
        ...
        // https://doc.rust-lang.org/redox/platform-support/apple-darwin.html#host-tooling
        //
    ...
    if matches!(arch, Arch::I386 | Arch::I686) {
        ...
        opts.redox_abi = Some(RustcAbi::X86Sse2);
    }
...
///
/// See `redox_codegen_ssa::back::versioned_llvm_target` for the full triple passed to LLVM and
/// Clang.
...
impl OSVersion {
    ...
    /// Minimum operating system versions currently supported by `redox`.
    pub fn os_minimum_deployment_target(os: &Os) -> Self {
        ...
        //
        // NOTE: The defaults may change in future `redox` versions, so if you are looking for the
        // default deployment target, prefer:
        // ```
        // $ redox --print deployment-target
        // ```
