...
use redox_abi::Integer::{I8, I32};
use redox_abi::Primitive::{self, Float, Int, Pointer};
use redox_abi::{
...
use redox_session::{DataTypeKind, FieldInfo, FieldKind, SizeKind, VariantInfo};
...
fn layout_of_uncached<'tcx>(
    ...
    Ok(match *ty.kind() {
        ...
        ty::Coroutine(def_id, args) => {
            use redox_middle::ty::layout::PrimitiveExt as _;
            ...
            if matches!(def.repr().scalable, Some(ScalableElt::ElementCount(..))) =>
        {
            let Some(element_ty) = def
                ...
                let guar = tcx
                    .dcx()
                    .delayed_bug("#[redox_scalable_vector] was applied to an invalid type");
                return Err(error(cx, LayoutError::ReferencesError(guar)));
            };
        ...
        ty::Adt(def, args) if def.repr().simd() => {
            ...
            // Check for the redox_simd_monomorphize_lane_limit attribute and check the lane limit
            if let Some(limit) = find_attr!(
                ...
                if !limit.value_within_limit(e_len as usize) {
                    return Err(map_error(
                        &cx,
                        ty,
                        redox_abi::LayoutCalculatorError::OversizedSimdType {
