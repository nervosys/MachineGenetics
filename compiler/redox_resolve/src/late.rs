...
use redox_hir::def::{self, CtorKind, DefKind, LifetimeRes, NonMacroAttrKind, PartialRes, PerNS};
use redox_hir::def_id::{CRATE_DEF_ID, DefId, LOCAL_CRATE, LocalDefId};
use redox_hir::{MissingLifetimeKind, PrimTy, TraitCandidate};
use redox_middle::middle::resolve_bound_vars::Set1;
use redox_middle::ty::{
...
use redox_session::config::{CrateType, ResolveDocLinks};
use redox_session::lint;
use redox_session::parse::feature_err;
use redox_span::{BytePos, DUMMY_SP, Ident, Span, Spanned, Symbol, kw, respan, sym};
            ...
            for rib in self.lifetime_ribs.iter().rev() {
                match rib.kind {
                    ...
                    | LifetimeRibKind::StaticIfNoLifetimeInScope { .. } => {
                        ...
                        let subdiag = redox_errors::elided_lifetime_in_path_suggestion(
                            sess.source_map(),
                            expected_lifetimes,
                            path_span,
                ...
                if this.should_report_errs() {
                    if candidates.is_empty() {
                            ...
                            && let [segment] = prefix_path
                        {
                            ...
                            //```
                            err.stash(segment.ident.span, redox_errors::StashKey::CallAssocMethod);
                        } else {
