...
| | |
|---|---|
| Tests | **2,985** — rmi 1,384 · prototype 1,268 · ribosome 168 · germline 112 · forge 53 |
...
Applied to three more surfaces on 2026-09-08 and 2026-09-15: the **CLI** (an
unrecognised flag was opened as a filename, item 28), the **38 RAP methods** of
which the previous sweep had examined about seven (three answered about an
empty subject, item 30), and the **scripts that resolve a crate binary**, where
a six-day-stale compiler was measuring a pinned figure (item 31).

**1b — A second question, and it finds a different kind of defect: *what would
this checker fail to notice?*** Not *does it fail closed* — `check-skb-tree.sh`
fails closed correctly, reports honestly, and is still blind to a
`RuleDatabase` variant that exists and is absent from `RuleDatabase::ALL`.
Measured 2026-09-15: it printed `ok skb/ matches a fresh --emit-skb: 255 rules
across 8 databases` while **256 rules across 9 databases** existed, because it
diffs the committed tree against a fresh `--emit-skb` that `emit_tree` builds
from that same list. **Its reference is generated by the construct it is
checking, so it can only confirm self-consistency.** Running it a thousand
times would never show this; the only way to see it is to ask where its
expected value comes from.

Be precise about the scope, because the first version of item 32 was not and
had to be corrected. The blindness needs *both* a per-item artifact and an item
list shared with the reference. `check-ontology-fresh.sh` has the same
committed-vs-fresh shape and is **not** vulnerable: the ontology is one JSON
file, so a dropped section is present on one side and absent on the other and
the diff fails. `check-skb-tree` writes one file per database, so a database
that is never emitted has no file on *either* side — nothing to diff, nothing
...
item 30), a summary line whose "8 databases" matched by coincidence (item 32),
an overstated claim in item 32 itself, and a detector written to find literal
figures that excluded the exact shape it was hunting. Each was caught by
breaking the thing and watching, never by reading it.
...
### The same question, asked of the RAP surface

The checker audit's question — *what does it print when its subject is empty?*
— was pointed at RAP on 2026-09-08. The 2026-09-07 sweep had examined about
**seven of the 38 methods**; probing all 38 with a module that declares nothing
found three more, recorded as item 30. The one worth carrying:

> **`verify/module` said `ok: true` because the source parsed.** The arm
> immediately above it in the same file, `verify/contracts`, had been fixed for
> this the day before and carries a comment explaining why a one-bit answer
> cannot separate "verified" from "nobody claimed anything". Two doors into one
> oracle; only one was shut.

**That is the third one-short sweep recorded on the same day**, after the
heredoc copies and `capability/check` — and the fourth counting my own first
attempt at the unknown-flag guard, which caught `--help` and left `-h`. The
pattern is not carelessness about the fix; it is that **the population is never
enumerated before the first instance is repaired**. `rap/methods` returns the
list. Nothing stopped anyone asking it for all 38 and probing each.

**Two of the three bugs had a test asserting them**, which is why they survived
a suite of 59 RAP tests. `test_skb_query` is the sharper case: it asserted
`ok == true` for a query matching *nothing*, so the assertion was about a
hardcoded constant rather than about a result, and it went green for as long as
the constant did. Making `ok` mean something turned it red on the first run.
**A test whose subject is empty is the same defect as a checker whose subject
is empty**, and this repository now has instances of both in the same week.
...
| `scripts/internals-doc-baseline.txt` | `internals/*.md` items with no definition in `prototype/src`, outside blocks labelled as design | **0** — was 15 of 36; the 12 blocks holding them are now labelled `**Not implemented.**` and skipped, and the run prints how many were skipped so the gap stays visible |
...
| `scripts/check-ci-paths.sh` | that CI is actually triggered by every file a checker reads, **and that every checker is actually run** — the instrument on the instruments. Four directions: a file `check-doc-counts.sh` pins that no `paths` entry covers; a file a *content* checker compiles that no entry covers; an entry naming a file that does not exist (a typo covers nothing and looks exactly like coverage); and a `check-*.sh` no CI step reaches, directly or through a script CI runs. **It has found three gaps so far.** Seven of fourteen pinned files uncovered, `HANDOFF.md` and `SECURITY_AUDIT.md` among them. Then 76 of 101 markdown files and 83 of 101 MAGE sources, including `MAGE_SPEC.md` and all of `quick-start/`, `agent-guide/`, `cookbook/` and `training/prompts/`. Then its own fourth direction, which counted a *mention* in a comment as an invocation and so reported all 15 checkers reachable while one was not — a false clean, found only by breaking it. Reachability is transitive (`check-doc-counts.sh` runs via `test-all.sh`) and comments are not invocations; both are now tested. |
| `scripts/check-rap-methods.sh` | that RAP's advertised surface is its real surface. The protocol states its methods **three times** — the `"ns/verb" =>` dispatch arms, the `METHODS` list the server publishes for discovery, and `internals/07-rap-server.md` — and nothing compared them. **No defect was found: 37, 37, and all 37 named.** That is the point of writing it down; the failure modes are silent in both directions, an arm with no list entry being a method that works and cannot be discovered, and a list entry with no arm being one an agent is told about and gets an error from. For an agent protocol the advertised surface *is* the surface, and the chapter was at **3 of 37** as recently as 2026-08-25. Three enforced directions plus a fourth ratcheted: thirteen method-shaped names in the chapter are not served, and all thirteen are legitimate — a table mapping planned names to what shipped, plus `namespace/verb`, which is the naming *convention*. Baselined at 13, may only shrink, and a **new** phantom fails. Verified by breaking all four. |
| `scripts/check-skb-tree.sh` | that `skb/` still matches what the compiler serves. The directory used to hold **56 rules nothing read** while `builtin_rules()` served **255** — and not the same corpus either: `BR-`/`LT-`/`TS-`/`CC-` on disk against `BOR-`/`LIF-`/`TYP-`/`CON-` in the binary, two stray `MEM-`/`TC-` rules, and two whole databases with no file. `check-orphan-sources.sh` could not see it, because that finds `.rs` files no `mod` reaches, not **data** nothing loads. The tree is now generated by `--emit-skb`; this regenerates it and diffs, in both directions — a hand-edited file, a missing file, an orphan the generator does not produce. All three verified by breaking them. |
...
| 23 | ~~**`skb/` is a 56-rule JSON tree that nothing reads**~~ | **Decided and closed 2026-09-02: generate it.** The three options were load, generate, or delete. Loading would have made 56 rules authoritative and thrown away the 199 that actually run, needing a schema migration on the way; deleting would lose a machine-readable corpus worth having on disk. `mage-parse --emit-skb` now writes the tree from `builtin_rules()` — **255 rules across eight databases**, including `AgentElision` (30) and `SwarmSafety` (15), which had no file at all — plus a manifest and the JSON Schema, because a hand-written schema beside a generated corpus is the same parallel-copy problem one level down. **The old schema proved that**: it required `version` and `scope` and described seventeen fields against rules that carry nine, so it validated nothing actually present; the emitted one is checked against all 255. `check-skb-tree.sh` regenerates and diffs in CI, both directions — a hand-edited file, a missing file, and an orphan the generator does not produce, each verified by breaking it. Registering the flag also tripped `every_flag_in_main_is_published_and_vice_versa`, which is a **fourth** copy of the CLI flag list; the repo's own guard caught it. |
| 22 | ~~**No editor here has a language server, and two claimed one**~~ | **Decided and closed 2026-09-02: RAP is an agent protocol, and this repository will stop pretending it is an editor one.** The evidence is §7.1's own design goals plus the method list, which is agent-shaped — `build/heal`, `nl/generate`, `abl/encode`, `abl/run`. An LSP server is a different thing with a different lifecycle (`initialize`, `textDocument` sync, incremental updates, push diagnostics, position encodings); writing one is a project, and calling RAP one was the defect. Neovim and Helix no longer register a language server, and **eight invented settings went with it** — `checkOnSave`, `diagnostics.skb`, `diagnostics.effects`, `completion.autoimport`, `completion.sigils`, `inlayHints.typeHints`, `inlayHints.effectHints`, `inlayHints.costHints`, none of which RAP has a method for; a user pointing `rap_cmd` at their own shim would have had MAGE's imaginary configuration sent to it. What editors *do* get: tree-sitter highlighting everywhere, and formatting, which needs no protocol at all now that `--fmt-compact -` reads stdin (item 25). If someone wants LSP later, the honest shape is a separate binary speaking LSP on stdio and calling RAP over TCP — a translator, not a rename. **Writing this up produced one more phantom of its own**: `evolve/run`, invented as an example in a sentence about the list being agent-shaped, caught by `check-rap-methods.sh` before commit. |
| 29 | ~~**The contract verifier's two entry points disagree, and `Verified` is unreachable from source**~~ | **Closed 2026-09-15, and it never needed the decision this row was filed for.** The parser rebuilt a clause with `condition_parts.join(" ")`, so `@ens(|result| result.shape.len() > 0)` reached `check_condition` — and agents, through `verify/module`'s `checks[].condition` — as `| result | result . shape . len ( ) > 0`. **Three things this row said were wrong.** *(1)* It called the fix a decision, because restoring the text would make `check_condition`'s patterns reachable and let a `.contains()` test emit `Proved`. Measured instead of assumed: the corpus holds **12 contract clauses**, the patterns are `.len() >= 0`, `.len() > -1` and `.is_ok()` with `=>`, and the only two clauses using `.len()` are `.len() > 0`. Nothing matches, so nothing changes — confirmed after the fix, all 12 still `Unknown`, statuses still `Partial: 8, Unspecified: 1`. The deciding fact was the absence of demand, exactly as in item 21, and three commands would have shown it before the row was written. *(2)* It said `Token` carries a span "so slicing the real text is cheap". The spans exist but `parse()` takes `&[Token]` and has no source to slice; threading it through would change a public signature with many callers. The fix needs no source at all — consecutive byte spans reveal whether whitespace separated two tokens. *(3)* It described **one** site. There were **four**: `parse_contract_clause`, `collect_paren_text` (which is what the `spec` block's clauses actually go through), the `~>` refinement predicate, and the `optimize` metric. **I fixed the first one, rebuilt, measured, and the text was still mangled** — then spent a round of diagnosis on token spans and stale binaries before running `grep -rn 'join(" ")'`, which answers it in one second and should have come before the fix. All four now use one `push_token_text` helper; deleting the duplication rather than repairing instances, as with `find_crate_bin` and `RuleDatabase::ALL`. Verified: prototype 1,268 green, and `check-mg-sources`, `check-examples`, `check-doc-blocks` and `check-vocabulary` all pass — the four that exercise the parser across every `.mg` source and 208 documentation blocks. |
| 33 | **The ontology publishes one wrapper protocol and not its sibling** | `wrapper_protocol` documents the **agent** wrapper contract — `RDX_BENCH_MODE`, `RDX_TASK_ID`, `RDX_TASK_DESCRIPTION`, `RDX_PARSE_ERROR`, and five stream/exit conventions — which is what `scripts/agent_wrappers/smart_fixer.sh` reads. There is a **second** wrapper protocol: `scripts/backend_wrappers/demo_subprocess_backend.sh` and `backends.rs` speak `RDX_BACKEND`, `RDX_ITEM_NAME` and `RDX_INPUT_SHAPE`. **Measured 2026-09-15: those three appear zero times in `prototype/src/ontology.rs`**, and are documented only in `benchmarks/STATUS.md` prose. Eight `RDX_*` variables exist in the tree; four are published. **The published half is accurate**, which is what makes this a scope question rather than a correctness one: `prototype/src/bin/reliability_bench.rs` is the producer and sets all four, `RDX_BENCH_MODE` and `RDX_TASK_ID` on every call and `RDX_PARSE_ERROR` and `RDX_TASK_DESCRIPTION` only in the refine branch — matching the section down to its *"set on refine only"* annotations. Both are supported extension points with a demo script apiece, so an agent asked to write a *backend* wrapper finds the *agent* wrapper contract in the index and nothing about the one it needs. **Filed rather than fixed, and the measurement is attached on purpose** — item 29 was filed as blocked without anyone checking whether the block was real, and dissolved in three commands when someone did. This one does not dissolve: publishing it means a new ontology section, which moves `onto_sections` — pinned in `DOCS.md`, `UNIFICATION.md` and `benchmarks/STATUS.md` — and is a decision about what the agent-discoverable index is *for*, not a gap to be quietly filled. The cheap alternative, folding three `env` rows into `wrapper_protocol`, would put two unrelated protocols in one section and make the section's name a lie. Whoever owns the ontology's scope decides which. |
| 21 | **Bounds are reported, and still not enforced** | The reporting half shipped 2026-09-01: every bound at all ten surface forms warns that it constrains nothing. **Enforcement assessed and deliberately not built, 2026-09-03** — and the reason is not cost. It is that **no program in this repository writes a bound.** Across **101 `.mg` sources**: zero bounds, zero `~>` clauses, one trait declaration (`T Module`), 95 impl blocks. All eighteen bounds in the tree live in *prose* — `quick-start/`, `migration-guide/`, `MAGE_PROPOSAL.md` — where they teach the syntax. So a checker would have **no call site to check** on any file here, and its correctness could only ever be demonstrated against tests written to demonstrate it. The machinery is real work — `types.rs` has **0** mentions of `bounds` and **0** handling of `ItemKind::Trait`/`Impl`, so it needs an impl table, obligation collection at call sites, and answers on coherence and orphan rules — but the deciding fact is the absence of demand, not the presence of effort. **Re-measured 2026-09-15 and unchanged**: 101 sources, zero bounds, zero `~>` clauses, one trait declaration, 95 impl blocks — the same figures the 2026-09-03 decision rested on. Recorded because this row *is* an absence claim, and `check-crypto-inventory.sh` states the hazard exactly: "there is no X" stays green by doing nothing, and every scope change silently expires it. A dated re-measurement is the cheapest guard available until something enforces it. **Revisit when a `.mg` source writes a bound and means it**; that is the signal, and `check-mg-sources.sh` walks every one of them, so it is cheap to look for. Until then the warning is proportionate: it tells the truth, and a partial solver would look like enforcement while not being it. |
...
| 18 | ~~**`rmi` publishes five hardware backends that are CPU code, and they report invented device properties**~~ | **Closed 2026-09-02.** Worse than filed on two counts, and one of them was the opposite of the finding. `metal` and `apple_ane` reported themselves **available on macOS/iOS**, `qualcomm` on **Android or any Linux host**, and `vulkan`/`webgpu` on **every platform** — so a caller asking for the best accelerator was handed a backend named after an API the build has no binding for, and ran on the CPU believing otherwise. They published `total_memory: 16 GB`, `available_memory: 12 GB`, `compute_units: 10`, and `ane_tops() == 15.8` (the M2's published rating) from code that never queried a device. **And five tests asserted all of it** — `assert!(backend.ane_tops() > 0.0)` pinned the fabrication and would have failed anyone who made it honest. Now: `is_available()` is `false` on all five, names say "not implemented", figures are `0` (zero reads as *unknown*; a placeholder is a number a scheduler divides by), and the five tests are inverted to pin the honesty. `get_backend_by_type` returns an error rather than an impostor. The descriptors also promised `feature:metal`, `feature:vulkan`, `feature:qualcomm` — **none of those cargo features exists**, so an agent was told to flip a flag that is not there; corrected, and the ontology now says so. **The opposite finding:** `compute/wgpu_backend.rs` is 1,103 lines that genuinely drive wgpu behind `feature = "gpu"`, implements `Backend`, is `pub` — and **nothing outside its own file ever named it**. `BackendType::WebGpu` handed out the stub even with the feature on. A working GPU backend, shadowed by a placeholder, in the crate's own dispatcher. Now routed. |
...
| 30 | ~~**Three RAP methods answered about an empty subject**~~ | **Closed 2026-09-08.** The 2026-09-07 sweep of the RAP surface examined about seven of the **38** methods; probing all 38 with a module that declares nothing found three more. **`verify/module` returned `ok: true` on the *parse* succeeding** — so `ok: true, results: []` for a module with nothing to verify, and `ok: true` over a lone `Partial`, the status `evidence()` maps to `Unreached` precisely because it is "a claim nothing adjudicated, not a weak pass". Its sibling `verify/contracts` is the arm **immediately above it in the same file** and was fixed for this on 2026-09-07, with a comment saying a one-bit answer cannot separate "verified" from "nobody claimed anything": the sweep stopped one method short, in the file that documents the lesson. It now reports `ok`/`verdict`/`detail` off a `verdict::Tally`. **`skb/query` hardcoded `ok: true` while `doc/query` — the same `skb::query_by_fqn` call — answered `!matches.is_empty()`**: two methods, one operation, opposite booleans; aligned to the reading this document had already ruled correct, with `count` added. **`language/tokens` was the only one of the 38 with no `ok` at all**, so a generic client read a successful lex as falsy and could not distinguish a source full of `TokenKind::Error` from a clean one; it now reports `ok` and `lex_errors`. **Two of the three bugs had tests asserting them.** `test_verify_module` asserted `ok == true` for a module with nothing to verify — item 18's shape exactly — and `test_skb_query` asserted `ok == true` for `{by: "fqn", value: "Vec"}`, a query matching **nothing**, since the SKB is keyed on names like `std.io.read_file`. That assertion had never been about a result; it was about a constant, and only the hardcoded boolean kept it green. Making `ok` mean something turned it red on the first run, which is how the empty subject surfaced. |
| 31 | ~~**The doc-count pipeline could not say why it failed, and two things were wrong under it**~~ | **Closed 2026-09-08**, after `check-doc-counts.sh` reported `INCOMPLETE - N documented count(s) had nothing to compare against` three times with three different causes and `N` as the only clue. Diagnosis meant re-running a multi-minute pipeline and counting which keys escaped. **The verdict was right every time** — an unchecked claim is an unguarded claim, and it refused to pass — but *failing closed silently is a separate defect from failing open*, and this document had only ever chased the second. `emit-doc-counts.sh` now carries an `ERR` trap naming the line, and it found the rest in one run. **Under it: (a)** `benchmarks/capstone/run.sh` guessed at `<crate>/target/release/<bin>.exe`. On a machine whose `~/.cargo/config.toml` sets one shared target directory, `forge/target/` does not exist — so the benchmark exited 1, its `\| grep -E '^capstone_'` matched nothing, and `pipefail` + `errexit` killed the emitter at that line. Worse where the guess *did* resolve: `prototype/target/release/mage-parse.exe` existed, **six days stale**, so `capstone_dispatched` — a pinned README figure — would have been measured by an old compiler. `find-mage-parse.sh` exists for exactly this and its own comment says "six checkers each carried their own copy"; the sweep converted the **checkers** and left `capstone/run.sh`, `demo_agent_workflow.sh`, `demo_rap_workflow.sh` and `smart_fixer.sh` still guessing, with **no resolver for `forge` at all**. Generalised to `find_crate_bin <crate> <bin> [profile]`, and **all of them converted** — `capstone/run.sh`, `constructs/run.sh`, `cross_lang/run.sh`, `demo_agent_workflow.sh`, `demo_rap_workflow.sh` and `agent_wrappers/smart_fixer.sh`, each verified by running it. **This row said “the other three are unswept” and there were five**, which is the session's own finding committed into the note recording it: the number came from what I remembered rather than from `grep -rn 'target/release'`, which is the whole distinction. Two of the five were not cosmetic. `constructs/run.sh` feeds **four pinned figures** in `ARCHITECTURE_DSL.md` and resolved to the six-day-old leftover; measured both ways it gives 126/141/82/839 either way, so the figures were right and *the mechanism verifying them was not* — right by luck of nothing having changed, which is the same sentence this document already carries about the “0 of 155” result. And `smart_fixer.sh` is a **fixer**: a stale compiler there does not mis-measure, it accepts or rejects the wrong programs. **(b)** `test-all.sh`'s `COUNTS["$name"]="$(grep … \| grep … \| awk …)"` dies at the *assignment* when a crate prints no `test result` line, so the `failed` array and the `FAILED: <crate>` line it exists to print are never reached — **a crate that fails to build answered with a bare exit 1 naming nothing**. `emit-doc-counts.sh` documents this precise trap and the lesson had not reached its own caller. One `awk`, which exits 0 on no match. **A bug of my own, in the fix:** `find_crate_bin` returned the extensionless name, which MSYS executes happily (it resolves `foo` → `foo.exe`) and which a *native* consumer cannot open — `forge` answered `FORGE_MG points at …/mage-parse, which is not a file` and capstone reported "forge check did not pass" about a compiler sitting right there. `.exe` is checked first now, and the divergence from `find_mage_parse`'s stated ordering is written down where it happens. |
| 32 | ~~**`check-skb-tree.sh` cannot see a database missing from `RuleDatabase::ALL`**~~ | **Closed 2026-09-15. This row is a correction of itself — the first version overstated the finding in three ways, and the reason is the caveat this document already carries about two other checkers: I ran a unit test in a broken state and *read* the checker instead of running it.** What is true, measured with a ninth database holding one rule, wired through both exhaustive matches and left out of `ALL`: `check-skb-tree.sh` **passes**, printing `ok skb/ matches a fresh --emit-skb: 255 rules across 8 databases` while **256 rules across 9 databases** exist. It diffs the committed tree against a fresh `--emit-skb` that `emit_tree` builds from `ALL`, so both sides omit the same database — its reference is generated by the construct it is checking — and its summary line counts what it emitted rather than what exists. **What was wrong. (1)** The row said *"drop a variant and both sides lose those rules together"*. Removal does not compile: `ALL` is `[RuleDatabase; 8]` and a shorter literal is a type error, and before `ALL` existed the script's orphan loop — *"a file in the tree the generator does not produce"* — caught it. **The direction I injected was the one already guarded, and I never ran the checker in that state.** **(2)** It said *"three existing checks stayed green"*. False: `rule_total_count_255` fails in the real scenario. The suite was never blind. **(3)** *"Structurally invisible"* was overstated — `check-skb-tree` alone is blind, the suite is not. **What survives is narrower and still worth the test.** The signal that does fire says only *the total changed*, and its obvious remedy — bump 255 to 256 — leaves `skb/` short with every check green. `per_database_counts_sum_to_the_total` fails with *which* variant is missing, so the contribution is **diagnosis, not detection**; detection existed and pointed at a number instead of a cause. Adding a variant is a compile error twice over (`database_file_stem`, and the `air.skb.*` map in `rmi_ontology_adapter`), so only the last step — from *it compiles* to *it is in `ALL`* — is unforced. The duplicate array was deleted rather than guarded twice: `RuleDatabase::ALL` is the single list and both callers derive from it. |
...
- Prototype tests **1,066 → 1,268**, all green — checked against the live run, so
  ...
  across five crates **2,985**; documented-count pins **94**, up from 46 — the
