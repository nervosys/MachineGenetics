...
use redox_const_eval::check_consts::{self, ConstCx};
use redox_const_eval::util;
...
use redox_middle::{bug, query, span_bug};
use redox_span::{DUMMY_SP, Spanned, sym};
use tracing::debug;
...
fn mir_built(tcx: TyCtxt<'_>, def: LocalDefId) -> &Steal<Body<'_>> {
    // Delegate to the main MIR building code in the `redox_mir_build` crate.
...
pub(crate) fn run_optimization_passes<'tcx>(tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
    ...
    pm::run_passes(
        ...
        &[
            ...
            // Perform inlining of `#[redox_force_inline]`-annotated callees.
            &inline::ForceInline,
