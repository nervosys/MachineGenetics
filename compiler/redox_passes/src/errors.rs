...
use redox_hir::attrs::{MirDialect, MirPhase};
use redox_macros::{Diagnostic, Subdiagnostic};
use redox_middle::ty::{MainDefinition, Ty};
use redox_span::{DUMMY_SP, Ident, Span, Symbol};
...
#[derive(Diagnostic)]
#[diag(
    "`#[redox_abi]` can only be applied to function items, type aliases, and associated functions"
)]
...
#[derive(Diagnostic)]
#[diag(
    "attribute `#[redox_const_stable]` can only be applied to functions that are declared `#[stable]`"
)]
...
// FIXME(jdonszelmann): move back to redox_attr
#[derive(Diagnostic)]
#[diag(
    "`const_stable_indirect` attribute does not make sense on `redox_const_stable` function, its behavior is already implied"
)]
...
pub(crate) struct ReprAlignShouldBeAlignStatic {
    #[primary_span]
    #[help("use `#[redox_align_static(...)]` instead")]
    pub span: Span,
    pub item: &'static str,
}
