//! MIR datatypes and passes. See the [redox dev guide] for more info.
//!
//! [redox dev guide]: https://redox-dev-guide.rust-lang.org/mir/index.html
...
pub use redox_ast::{Mutability, Pinnedness};
use redox_data_structures::fx::{FxHashMap, FxHashSet};
use redox_data_structures::graph::dominators::Dominators;
use redox_errors::ErrorGuaranteed;
use redox_hir::def::{CtorKind, Namespace};
use redox_hir::def_id::{CRATE_DEF_ID, DefId};
use redox_hir::{
...
use redox_macros::{HashStable, TyDecodable, TyEncodable, TypeFoldable, TypeVisitable};
use redox_serialize::{Decodable, Encodable};
use redox_span::{DUMMY_SP, Span, Spanned, Symbol};
...
pub struct Body<'tcx> {
    ...
    /// `compiler/redox_monomorphize/src/collector.rs` for more context.
    ...
    ///
    /// This is only `Some` if the function that this body comes from was annotated with `redox_custom_mir`.
...
redox_index::newtype_index! {
    ...
    ///
    /// Read more about basic blocks in the [redox-dev-guide][guide-mir].
    ///
    /// [CFG]: https://redox-dev-guide.rust-lang.org/appendix/background.html#cfg
    /// [data-flow analyses]:
    ///     https://redox-dev-guide.rust-lang.org/appendix/background.html#what-is-a-dataflow-analysis
    /// [`CriticalCallEdges`]: ../../redox_mir_transform/add_call_guards/enum.AddCallGuards.html#variant.CriticalCallEdges
    /// [guide-mir]: https://redox-dev-guide.rust-lang.org/mir/
    #[stable_hash]
    #[encodable]
    #[orderable]
    #[debug_format = "bb{}"]
