use redox_middle::mir::interpret::{AllocId, ConstAllocation, InterpResult};
...
pub macro throw_machine_stop_str($($tt:tt)*) {{
    ...
    impl redox_middle::mir::interpret::MachineStopType for Zst {}
...
impl HasStaticRootDefId for DummyMachine {
    fn static_def_id(&self) -> Option<redox_hir::def_id::LocalDefId> {
        None
    }
}
...
impl<'tcx> interpret::Machine<'tcx> for DummyMachine {
    ...
    fn assert_panic(
        _ecx: &mut InterpCx<'tcx, Self>,
        _msg: &redox_middle::mir::AssertMessage<'tcx>,
        _unwind: UnwindAction,
    ) -> interpret::InterpResult<'tcx> {
        unimplemented!()
    }
    ...
    fn get_default_alloc_params(
        &self,
    ) -> <Self::Bytes as redox_middle::mir::interpret::AllocBytes>::AllocParams {
    }
}
