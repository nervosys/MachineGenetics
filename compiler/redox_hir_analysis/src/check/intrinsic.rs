//! Type-checking for the `#[redox_intrinsic]` intrinsics that the compiler exposes.

use redox_abi::ExternAbi;
use redox_errors::DiagMessage;
use redox_hir::{self as hir, LangItem};
use redox_middle::traits::{ObligationCause, ObligationCauseCode};
use redox_middle::ty::{self, Const, Ty, TyCtxt};
...
/// Remember to add all intrinsics here, in `compiler/redox_codegen_llvm/src/intrinsic.rs`,
...
pub(crate) fn check_intrinsic_type(
    ...
    let (n_tps, n_cts, inputs, output) = match intrinsic_name {
        ...
        sym::redox_peek => (1, 0, vec![param(0)], param(0)),
