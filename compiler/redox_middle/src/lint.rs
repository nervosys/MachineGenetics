...
use redox_data_structures::sorted_map::SortedMap;
use redox_errors::{Diag, DiagLocation, Diagnostic, MultiSpan};
use redox_hir::{HirId, ItemLocalId};
use redox_lint_defs::EditionFcw;
use redox_macros::{Decodable, Encodable, HashStable};
use redox_session::Session;
use redox_session::lint::builtin::{self, FORBIDDEN_LINT_GROUPS};
use redox_session::lint::{FutureIncompatibilityReason, Level, Lint, LintExpectationId, LintId};
use redox_span::{DUMMY_SP, ExpnKind, Span, Symbol, kw};
...
pub enum LintLevelSource {
    /// Lint is at the default level as declared in redox.
    Default,
...
pub struct LintExpectation {
    ...
    /// goes for `rustdoc`. This will be `None` for redox lints
...
pub fn emit_lint_base<'a, D: Diagnostic<'a, ()> + 'a>(
    ...
    fn emit_lint_base_impl<'a>(
        ...
        span: Option<MultiSpan>,
        decorate: Box<
            dyn FnOnce(redox_errors::DiagCtxtHandle<'a>, redox_errors::Level) -> Diag<'a, ()> + 'a,
        >,
    ) {
        ...
        let err_level = match level {
            ...
            Level::Expect => {
                ...
                // need the lint diagnostic to be emitted to `redox_error::DiagCtxtInner`.
                //
                // We can also not mark the lint expectation as fulfilled here right away, as it
                // can still be cancelled in the decorate function. All of this means that we simply
                // create a `Diag` and continue as we would for warnings.
                redox_errors::Level::Expect
            }
            Level::ForceWarn => redox_errors::Level::ForceWarning,
            Level::Warn => redox_errors::Level::Warning,
            Level::Deny | Level::Forbid => redox_errors::Level::Error,
        };
        ...
        if disable_suggestions {
            ...
            // In redox, for the find_attr macro, we want to always emit this.
            // This completely circumvents normal lint checking, which usually doesn't happen for macros from other crates.
            // However, we kind of want that when using find_attr from another redox crate. So we cheat a little.
        ...
        // 1. If the corresponding `redox_errors::Level` is beyond warning, i.e. `ForceWarning(_)`
        //    or `Error`, then the diagnostic will be emitted regardless of CLI options.
        // 2. If the corresponding `redox_errors::Level` is warning, then that can be affected by
        //    `-A warnings` or `--cap-lints=xxx` on the command line. In which case, the diagnostic
        //    will be emitted if `can_emit_warnings` is true.
        let skip = err_level == redox_errors::Level::Warning && !sess.dcx().can_emit_warnings();
