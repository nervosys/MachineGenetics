...
#[cfg(feature = "redox")]
impl From<redox_abi::Endian> for Endian {
    fn from(order: redox_abi::Endian) -> Endian {
        match order {
            redox_abi::Endian::Little => Endian::Little,
            redox_abi::Endian::Big => Endian::Big,
        }
    }
}
...
pub(crate) mod redox {
    ...
    use redox_middle::ty::layout::{HasTyCtxt, LayoutCx, LayoutError};
    use redox_middle::ty::{
    ...
    use crate::layout::redox::{Def, layout_of};

    #[derive(Debug, Copy, Clone)]
    pub(crate) enum Err {
        /// The layout of the type is not yet supported.
        NotYetSupported,
        /// This error will be surfaced elsewhere by redox, so don't surface it.
        UnknownLayout,
        /// Overflow size
        SizeOverflow,
