...
pub(crate) fn report_error_if_not_applied_to_span(
    ...
    if !type_matches_path(info.ty.inner_type(), &["redox_span", "Span"])
        && !type_matches_path(info.ty.inner_type(), &["redox_errors", "MultiSpan"])
    {
...
/// `Applicability` of a suggestion - mirrors `redox_errors::Applicability` - and used to represent
    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        tokens.extend(match self {
            Applicability::MachineApplicable => {
                quote! { redox_errors::Applicability::MachineApplicable }
            }
            Applicability::MaybeIncorrect => {
                quote! { redox_errors::Applicability::MaybeIncorrect }
            }
            Applicability::HasPlaceholders => {
                quote! { redox_errors::Applicability::HasPlaceholders }
            }
            Applicability::Unspecified => {
                quote! { redox_errors::Applicability::Unspecified }
            }
        });
    }
}
    ...
    pub(crate) fn to_suggestion_style(&self) -> TokenStream {
        match self {
            SuggestionKind::Normal => {
                quote! { redox_errors::SuggestionStyle::ShowCode }
            }
            SuggestionKind::Short => {
                quote! { redox_errors::SuggestionStyle::HideCodeInline }
            }
            SuggestionKind::Hidden => {
                quote! { redox_errors::SuggestionStyle::HideCodeAlways }
            }
            SuggestionKind::Verbose => {
                quote! { redox_errors::SuggestionStyle::ShowAlways }
            }
            SuggestionKind::ToolOnly => {
                quote! { redox_errors::SuggestionStyle::CompletelyHidden }
            }
        }
    }
