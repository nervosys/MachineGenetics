...
use redox_data_structures::flat_map_in_place::FlatMapInPlace;
use redox_errors::E0802;
...
pub(crate) fn expand_deriving_coerce_pointee(
    ...
    // # Validity assertion which will be checked later in `redox_hir_analysis::coherence::builtins`.
    {
        let trait_path =
...
impl<'a> ast::mut_visit::MutVisitor for TypeSubstitution<'a> {
    ...
    fn visit_where_predicate_kind(&mut self, kind: &mut ast::WherePredicateKind) {
        match kind {
            redox_ast::WherePredicateKind::BoundPredicate(bound) => {
                bound
            ...
            redox_ast::WherePredicateKind::RegionPredicate(_)
            | redox_ast::WherePredicateKind::EqPredicate(_) => {}
        }
    }
}
...
impl<'a, 'b> redox_ast::visit::Visitor<'a> for DetectNonGenericPointeeAttr<'a, 'b> {
    fn visit_attribute(&mut self, attr: &'a redox_ast::Attribute) -> Self::Result {
        if attr.has_name(sym::pointee) {
            self.cx.dcx().emit_err(errors::NonGenericPointee { span: attr.span });
        }
    }

    fn visit_generic_param(&mut self, param: &'a redox_ast::GenericParam) -> Self::Result {
        ...
        match &param.kind {
            GenericParamKind::Type { default } => {
                ...
                redox_ast::visit::visit_opt!(error_on_pointee, visit_ty, default);
            }

            GenericParamKind::Const { .. } | GenericParamKind::Lifetime => {
                redox_ast::visit::walk_generic_param(&mut error_on_pointee, param);
            }
        }
    }

    fn visit_ty(&mut self, t: &'a redox_ast::Ty) -> Self::Result {
        let mut error_on_pointee = AlwaysErrorOnGenericParam { cx: self.cx };
...
impl<'a, 'b> redox_ast::visit::Visitor<'a> for AlwaysErrorOnGenericParam<'a, 'b> {
    fn visit_attribute(&mut self, attr: &'a redox_ast::Attribute) -> Self::Result {
        if attr.has_name(sym::pointee) {
            self.cx.dcx().emit_err(errors::NonGenericPointee { span: attr.span });
        }
