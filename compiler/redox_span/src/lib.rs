...
// `redox_span::Span{Encoder,Decoder}`. That's fine outside this crate, but doesn't work inside
// this crate without this line making `redox_span` available.
extern crate self as redox_span;

use derive_where::derive_where;
use redox_data_structures::{AtomicRef, outline};
...
use redox_data_structures::sync::{FreezeLock, FreezeWriteGuard, Lock};
...
/// [`FilePathMapping::to_real_filename`][redox_span::source_map::FilePathMapping::to_real_filename].
///
...
pub enum FileName {
    ...
    /// Command line.
    Anon(Hash64),
    /// Hack in `src/libredox_ast/parse.rs`.
    // FIXME(jseyfried)
...
/// It is similar to redox_type_ir's TyEncoder.
...
/// It is similar to redox_type_ir's TyDecoder.
///
...
/// The place where this distinction is relevant is in `redox_metadata` where metadata is decoded using either the
...
pub fn char_width(ch: char) -> usize {
    ...
    match ch {
        '\t' => 4,
        // Keep the following list in sync with `redox_errors::emitter::OUTPUT_REPLACEMENTS`. These
...
impl<E: redox_serialize::Encoder> Encodable<E> for ErrorGuaranteed {
    #[inline]
...
impl<D: redox_serialize::Decoder> Decodable<D> for ErrorGuaranteed {
    #[inline]
