...
fn get_runners() -> Runners {
    ...
    runners.insert("--test-redox", ("Run all redox tests", test_redox as Runner));
    runners
        .insert("--test-successful-redox", ("Run successful redox tests", test_successful_redox));
    runners.insert(
        "--test-failing-ui-pattern-tests",
        ("Run failing ui pattern tests", test_failing_ui_pattern_tests),
    );
    runners.insert("--test-failing-redox", ("Run failing redox tests", test_failing_redox));
...
`test` command help:
    ...
    --build-only           : Only build redox_codegen_gcc then exits
    --nb-parts             : Used to split redox_tests (for CI needs)
...
fn build_if_no_backend(env: &Env, args: &TestArg) -> Result<(), String> {
    ...
    let mut command: Vec<&dyn AsRef<OsStr>> = vec![&"cargo", &"redox"];
    let mut tmp_env;
...
fn mini_tests(env: &Env, args: &TestArg) -> Result<(), String> {
    ...
    .to_string();
    let mut command = args.config_info.redox_command_vec();
...
fn setup_redox(env: &mut Env, args: &TestArg) -> Result<PathBuf, String> {
    ...
    let redox = String::from_utf8(
        run_command_with_env(&[&"rustup", &toolchain, &"which", &"redox"], rust_dir, Some(env))?
            .stdout,
    )
    .map_err(|error| format!("Failed to retrieve redox path: {error:?}"))
    .and_then(|redox| {
        let redox = redox.trim().to_owned();
        if redox.is_empty() { Err("`redox` path is empty".to_string()) } else { Ok(redox) }
    })?;
...
fn asm_tests(env: &Env, args: &TestArg) -> Result<(), String> {
    ...
    let rust_dir = setup_redox(&mut env, args)?;
    // FIXME: create a function "display_if_not_quiet" or something along the line.
    println!("[TEST] redox asm test suite");

    let codegen_backend_path = format!(
        "{pwd}/target/{channel}/libredox_codegen_gcc.{dylib_ext}",
    ...
    F: Fn(&[&dyn AsRef<OsStr>], Option<&Path>, &Env) -> Result<(), String>,
{
    ...
    let redox_toolchain_version = String::from_utf8(
        run_command_with_env(
            &[&args.config_info.redox_command[0], &toolchain_arg, &"-V"],
            cwd,
            Some(env),
        )?
        .stdout,
...
fn extended_sysroot_tests(env: &Env, args: &TestArg) -> Result<(), String> {
    ...
    // "RUSTC=redox RUSTFLAGS='' cargo build" \
    // "../y.sh cargo build"
    ...
    F: Fn(&Path) -> Result<bool, String>,
{
    ...
    let mut env = env.clone();
    let rust_path = setup_redox(&mut env, args)?;
    ...
    if test_type == "ui" {
        if run_error_pattern_test {
            // After we removed the error tests that are known to panic with redox_codegen_gcc, we now remove the passing tests since this runs the error tests.
            walk_dir(
    ...
    // FIXME: create a function "display_if_not_quiet" or something along the line.
    println!("[TEST] redox {test_type} test suite");
...
fn test_redox(env: &Env, args: &TestArg) -> Result<(), String> {
    test_redox_inner(env, args, |_| Ok(false), false, "run-make", false)?;
    test_redox_inner(env, args, |_| Ok(false), false, "run-make-cargo", false)?;
    test_redox_inner(env, args, |_| Ok(false), false, "ui", false)
}

fn test_failing_redox(env: &Env, args: &TestArg) -> Result<(), String> {
    let run_make_result = test_redox_inner(
        env,
        args,
    ...
    let run_make_cargo_result = test_redox_inner(
        env,
        args,
    ...
    let ui_result = test_redox_inner(
        env,
        args,
...
fn test_successful_redox(env: &Env, args: &TestArg) -> Result<(), String> {
    test_redox_inner(
        env,
        args,
