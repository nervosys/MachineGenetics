...
//! Feature gate checking itself is done in `redox_ast_passes/src/feature_gate.rs`
//! at the moment.
//!
...
impl UnstableFeatures {
    ...
    /// `std::env::var("RUSTC_BOOTSTRAP")` with the `env_var_redox_bootstrap`
    /// arg.
    fn from_environment_value(
        krate: Option<&str>,
        env_var_redox_bootstrap: Result<String, std::env::VarError>,
    ) -> Self {
        ...
        let bootstrap = env_var_redox_bootstrap.ok();
