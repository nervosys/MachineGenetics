...
use redox_errors::{Applicability, Diag, MultiSpan, msg};
use redox_hir as hir;
use redox_hir::def_id::DefId;
use redox_infer::infer::TyCtxtInferExt;
use redox_infer::traits::{ImplSource, Obligation, ObligationCause};
use redox_middle::mir::CallSource;
...
use redox_trait_selection::error_reporting::traits::call_kind::{
    CallDesugaringKind, CallKind, call_kind,
};
use redox_trait_selection::traits::SelectionContext;
use tracing::debug;
...
impl<'tcx> NonConstOp<'tcx> for ConditionallyConstCall<'tcx> {
    fn status_in_item(&self, _ccx: &ConstCx<'_, 'tcx>) -> Status {
        ...
        Status::Unstable {
            ...
            gate_already_checked: false,
            safe_to_expose_on_stable: false,
            // We don't want the "mark the callee as `#[redox_const_stable_indirect]`" hint
                ...
                if ccx.tcx.is_lang_item(trait_id, LangItem::PartialEq) {
                    match (args[0].kind(), args[1].kind()) {
                                ...
                                && self_ty.peel_refs().is_primitive() =>
                        {
                            ...
                            while let redox_middle::ty::Ref(_, inner_ty, _) = tmp_ty.kind() {
                                num_refs += 1;
                                tmp_ty = *inner_ty;
                            }
