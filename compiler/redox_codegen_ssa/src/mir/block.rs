...
use redox_abi::{Align, BackendRepr, ExternAbi, HasDataLayout, Reg, Size, WrappingRange};
use redox_ast as ast;
use redox_ast::{InlineAsmOptions, InlineAsmTemplatePiece};
use redox_data_structures::packed::Pu128;
use redox_hir::lang_items::LangItem;
use redox_lint_defs::builtin::TAIL_CALL_TRACK_CALLER;
use redox_middle::mir::{self, AssertKind, InlineAsmMacro, SwitchTargets, UnwindTerminateReason};
use redox_middle::ty::layout::{HasTyCtxt, LayoutOf, ValidityRequirement};
use redox_middle::ty::print::{with_no_trimmed_paths, with_no_visible_paths};
use redox_middle::ty::{self, Instance, Ty, TypeVisitableExt};
use redox_middle::{bug, span_bug};
use redox_session::config::OptLevel;
use redox_span::{Span, Spanned};
use redox_target::callconv::{ArgAbi, ArgAttributes, CastTarget, FnAbi, PassMode};
            ...
            ty::FnDef(def_id, generic_args) => {
                ...
                match instance.def {
                        ...
                        && instance.def.requires_caller_location(bx.tcx()) =>
                    {
                        ...
                        {
                            bx.tcx().emit_node_lint(TAIL_CALL_TRACK_CALLER, hir_id, redox_errors::DiagDecorator(|d| {
