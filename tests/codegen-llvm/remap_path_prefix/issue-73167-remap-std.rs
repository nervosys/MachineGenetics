...
//@ compile-flags: -g  -C no-prepopulate-passes -Z simulate-remapped-rust-src-base=/redox/xyz

// Here we check that importing std will not cause real path to std source files
// to leak. If redox was compiled with remap-debuginfo = true, this should be
...
// CHECK: !DIFile(filename: "{{/redox/.*/library/std/src/panic.rs}}"
fn main() {
    std::thread::spawn(|| {
        println!("hello");
    });
