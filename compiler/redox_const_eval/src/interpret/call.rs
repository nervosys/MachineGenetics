...
use redox_abi::{self as abi, ExternAbi, FieldIdx, Integer, VariantIdx};
use redox_hir::def_id::DefId;
use redox_hir::find_attr;
use redox_middle::ty::layout::{IntegerExt, TyAndLayout};
use redox_middle::ty::{self, AdtDef, Instance, Ty, VariantDef};
use redox_middle::{bug, mir, span_bug};
use redox_target::callconv::{ArgAbi, FnAbi};
...
impl<'tcx, M: Machine<'tcx>> InterpCx<'tcx, M> {
    ...
    pub fn check_argument_compat(
        ...
        if self.layout_compat(caller_abi.layout, callee_abi.layout)? {
            ...
            // (This can fail e.g. if `#[redox_nonnull_optimization_guaranteed]` is used incorrectly.)
    ...
    pub(super) fn init_drop_in_place_call(
        ...
        // implementation fail -- a problem shared by redox.
