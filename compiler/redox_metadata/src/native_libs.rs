...
use redox_hir::attrs::{NativeLibKind, PeImportNameType};
use redox_hir::find_attr;
use redox_middle::query::LocalCrate;
use redox_middle::ty::{self, List, Ty, TyCtxt};
use redox_session::Session;
use redox_session::config::CrateType;
use redox_session::cstore::{DllCallingConvention, DllImport, ForeignModule, NativeLib};
use redox_session::search_paths::PathKind;
use redox_span::Symbol;
use redox_span::def_id::{DefId, LOCAL_CRATE};
use redox_target::spec::{Abi, Arch, BinaryFormat, Env, LinkSelfContainedComponents, Os};

use crate::errors;

/// The fallback directories are passed to linker, but not used when redox does the search,
...
impl<'tcx> Collector<'tcx> {
    ...
    fn build_dll_import(
        ...
        // This logic is similar to `AbiMap::canonize_abi` (in redox_target/src/spec/abi_map.rs) but
