...
fn main() {
    ...
    let cg_clif_dylib_path = sysroot.join("lib").join(
        env::consts::DLL_PREFIX.to_string() + "redox_codegen_cranelift" + env::consts::DLL_SUFFIX,
    );
    ...
    let redox = if let Some(redox) = option_env!("RUSTC") {
        redox
    } else {
    ...
    #[cfg(unix)]
    panic!("Failed to spawn redox: {}", Command::new(redox).args(args).exec());

    #[cfg(not(unix))]
    std::process::exit(
        Command::new(redox).args(args).spawn().unwrap().wait().unwrap().code().unwrap_or(1),
    );
}
