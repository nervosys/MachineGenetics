...
//!
//! See <https://redox-dev-guide.rust-lang.org/diagnostics.html> for an
...
use redox_errors::{Diagnostic, LintBuffer, MultiSpan};
use redox_feature::Features;
use redox_hir as hir;
use redox_hir::def::Res;
use redox_hir::def_id::{CrateNum, DefId};
use redox_hir::definitions::{DefPathData, DisambiguatedDefPathData};
use redox_hir::{Pat, PatKind};
use redox_middle::bug;
use redox_middle::lint::LevelAndSource;
use redox_middle::middle::privacy::EffectiveVisibilities;
use redox_middle::ty::layout::{LayoutError, LayoutOfHelpers, TyAndLayout};
use redox_middle::ty::print::{PrintError, PrintTraitRefExt as _, Printer, with_no_trimmed_paths};
use redox_middle::ty::{self, GenericArg, RegisteredTools, Ty, TyCtxt, TypingEnv, TypingMode};
use redox_session::lint::{FutureIncompatibleInfo, Lint, LintExpectationId, LintId};
use redox_session::{DynLintStore, Session};
use redox_span::edit_distance::find_best_match_for_names;
use redox_span::{Ident, Span, Symbol, sym};
use tracing::debug;
...
impl DynLintStore for LintStore {
    fn lint_groups_iter(&self) -> Box<dyn Iterator<Item = redox_session::LintGroup> + '_> {
        Box::new(self.get_lint_groups().map(|(name, lints, is_externally_loaded)| {
            redox_session::LintGroup { name, lints, is_externally_loaded }
        }))
    }
}
...
enum TargetLint {
    ...
    ///
    /// This is used by redox to avoid warning about old rustdoc lints before rustdoc registers
    /// them as tool lints.
    Ignored,
}
...
pub enum CheckLintNameResult<'a> {
    ...
    /// redox lint. The `Option<String>` indicates if the lint has been
    /// renamed.
...
impl LintStore {
        ...
        for lint in lints {
            ...
            if let Some(FutureIncompatibleInfo { reason, .. }) = lint.future_incompatible {
                if let Some(edition) = reason.edition() {
                    ...
                    // future version of redox will cause existing code to stop compiling.
    ...
    ///
    /// This is used by redox to avoid warning about old rustdoc lints before rustdoc registers them as tool lints.
    #[track_caller]
    ...
    pub fn check_lint_name(
        ...
        if let Some(tool_name) = tool_name {
            // FIXME: redox and rustdoc are considered tools for lints, but not for attributes.
            if tool_name != sym::redox
        ...
        if let Some(tool_name) = tool_name {
            match self.by_name.get(&complete_name) {
                ...
                // to treat tool_lints and redox lints different and can use the code below.
                _ => {}
            }
        }
        match self.by_name.get(&complete_name) {
    ...
    fn no_lint_suggestion(&self, lint_name: &str, tool_name: &str) -> CheckLintNameResult<'_> {
        ...
        let is_redox = res.map_or_else(
            || false,
            |s| name_lower.contains("::") && !s.as_str().starts_with(tool_name),
        );
        let suggestion = res.map(|s| (s, is_redox));
...
pub trait LintContext {
    ...
    ///
    /// [`emit_lint_base`]: redox_middle::lint::emit_lint_base#decorate-signature
    #[track_caller]
...
impl<'tcx> LateContext<'tcx> {
    ...
    /// be used for pretty-printing HIR by redox_hir_pretty.
