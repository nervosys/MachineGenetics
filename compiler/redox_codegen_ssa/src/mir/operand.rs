...
use redox_middle::mir::interpret::{Pointer, Scalar, alloc_range};
use redox_middle::mir::{self, ConstValue};
use redox_middle::ty::layout::{LayoutOf, TyAndLayout};
use redox_middle::ty::{self, Ty};
use redox_middle::{bug, span_bug};
use redox_session::config::{AnnotateMoves, DebugInfo, OptLevel};
...
impl<'a, 'tcx, V: CodegenObject> OperandRef<'tcx, V> {
    ...
    fn from_const_alloc<Bx: BuilderMethods<'a, 'tcx, Value = V>>(
        ...
        alloc: redox_middle::mir::interpret::ConstAllocation<'tcx>,
        offset: Size,
    ) -> Self {
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    fn move_copy_annotation_instance(
        ...
        let instance = ty::Instance::expect_resolve(
            ...
            def_id,
            generic_args,
            redox_span::DUMMY_SP, // span only used for error messages
        );
