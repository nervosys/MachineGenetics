...
use redox_hir::intravisit::{self, InferKind, Visitor, VisitorExt};
use redox_hir::{self as hir, GenericParamKind, HirId, Node, PreciseCapturingArgKind, find_attr};
use redox_infer::infer::{InferCtxt, TyCtxtInferExt};
use redox_infer::traits::{DynCompatibilityViolation, ObligationCause};
use redox_middle::hir::nested_filter;
use redox_middle::query::Providers;
use redox_middle::ty::util::{Discr, IntTypeExt};
use redox_middle::ty::{
...
use redox_trait_selection::error_reporting::traits::suggestions::NextTypeParamName;
...
/// Adds query implementations to the [Providers] vtable, see [`redox_middle::query`]
...
///
/// [`InferCtxt`]: redox_infer::infer::InferCtxt
///
/// # Trait predicates
///
            ...
            diag.multipart_suggestion(
                format!(
                    "try replacing `_` with the type{} in the corresponding trait method \
                        signature",
                    redox_errors::pluralize!(infer_replacements.len()),
                ),
                infer_replacements,
...
impl<'tcx> HirTyLowerer<'tcx> for ItemCtxt<'tcx> {
    ...
    fn lower_assoc_item_path(
        ...
        span: Span,
        item_def_id: DefId,
        item_segment: &redox_hir::PathSegment<'tcx>,
        ...
        if def.repr().c() {
                ...
                && !(min_discr >= 0 && max_discr <= c_uint_max)
            {
                ...
                tcx.emit_node_span_lint(
                    redox_session::lint::builtin::REPR_C_ENUMS_LARGER_THAN_INT,
...
fn impl_trait_header(tcx: TyCtxt<'_>, def_id: LocalDefId) -> ty::ImplTraitHeader<'_> {
    ...
    let is_redox_reservation = find_attr!(tcx, def_id, RustcReservationImpl(..));
    ...
    ty::ImplTraitHeader {
        ...
        polarity: polarity_of_impl(tcx, of_trait, is_redox_reservation),
...
fn coroutine_kind(tcx: TyCtxt<'_>, def_id: LocalDefId) -> Option<hir::CoroutineKind> {
    match tcx.hir_node_by_def_id(def_id) {
        Node::Expr(&hir::Expr {
            kind:
                hir::ExprKind::Closure(&redox_hir::Closure {
...
fn coroutine_for_closure(tcx: TyCtxt<'_>, def_id: LocalDefId) -> DefId {
    let &redox_hir::Closure { kind: hir::ClosureKind::CoroutineClosure(_), body, .. } =
