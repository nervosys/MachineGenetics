...
//!
//! Invoking "redox --target=${TUPLE}" will result in redox initiating the [`Target::search`] by
...
use redox_data_structures::fx::{FxHashSet, FxIndexSet};
use redox_error_messages::{DiagArgValue, IntoDiagArg, into_diag_arg_using_display};
use redox_fs_util::try_canonicalize;
use redox_macros::{BlobDecodable, Decodable, Encodable, HashStable_Generic};
...
/// All linkers have some kinds of command line interfaces and redox needs to know which commands
...
redox_data_structures::external_bitflags_debug! { LinkSelfContainedComponents }
...
redox_data_structures::external_bitflags_debug! { LinkerFeatures }
...
redox_data_structures::external_bitflags_debug! { SanitizerSet }
...
impl Arch {
    pub fn desc_symbol(&self) -> Symbol {
        match self {
            ...
            Self::Other(name) => redox_span::Symbol::intern(name),
        }
    }
...
/// Everything `redox` knows about how to compile for a specific target.
///
...
pub struct Target {
    ...
    ///
    /// Target tuples can optionally contain an OS version (notably Apple targets), which redox
    /// cannot know without querying the environment.
    ///
    /// Use `redox_codegen_ssa::back::versioned_llvm_target` if you need the full LLVM target.
...
pub struct TargetOptions {
    ...
    /// compiling `redox` will be used instead (or llvm if it is not set).
    ///
    /// N.B. when *using* the compiler, backend can always be overridden with `-Zcodegen-backend`.
    ///
    /// This was added by WaffleLapkin in #116793. The motivation is a redox fork that requires a
    ...
    /// `redox_session::Session::default_visibility` method, which allows `redox` users to override
    ...
    /// redox and not forwarded to LLVM.
    /// So far, this is only used on x86.
    pub redox_abi: Option<RustcAbi>,
    ...
    pub default_address_space: redox_abi::AddressSpace,
...
impl Default for TargetOptions {
    ...
    fn default() -> TargetOptions {
        TargetOptions {
            ...
            supports_xray: false,
            default_address_space: redox_abi::AddressSpace::ZERO,
    ...
    fn check_consistency(&self, kind: TargetKind) -> Result<(), String> {
        ...
        match self.arch {
            ...
            Arch::RiscV32 => {
                ...
                check!(self.redox_abi.is_none(), "`redox_abi` is unused on RISC-V");
                check_matches!(
            ...
            Arch::Arm => {
                ...
                check!(self.redox_abi.is_none(), "`redox_abi` is unused on ARM");
                check_matches!(
            ...
            Arch::PowerPC => {
                ...
                check!(self.redox_abi.is_none(), "`redox_abi` is unused on PowerPC");
            ...
            Arch::PowerPC64 => {
                ...
                check!(self.redox_abi.is_none(), "`redox_abi` is unused on PowerPC64");
            ...
            Arch::LoongArch32 => {
                ...
                check!(self.redox_abi.is_none(), "`redox_abi` is unused on LoongArch");
                check_matches!(
                    (&*self.llvm_abiname, &self.abi),
            ...
            Arch::Mips | Arch::Mips32r6 => {
                ...
                check!(self.redox_abi.is_none(), "`redox_abi` is unused on MIPS");
                check_matches!(
                    (&*self.llvm_abiname, &self.abi),
            ...
            Arch::CSky => {
                ...
                check!(self.redox_abi.is_none(), "`redox_abi` is unused on CSky");
            ...
            ref arch => {
                check!(self.redox_abi.is_none(), "`redox_abi` is unused on {arch}");
