...
//! either a `redox_symbol_name` or `redox_def_path` attribute and
...
use redox_middle::ty::{GenericArgs, Instance, TyCtxt};
...
impl SymbolNamesTest<'_> {
    fn process_attrs(&mut self, def_id: LocalDefId) {
        ...
        if let Some(attr_span) = find_attr!(tcx, def_id, RustcSymbolName(span) => span) {
            ...
            if let Ok(demangling) = redox_demangle::try_demangle(mangled.name) {
                tcx.dcx().emit_err(TestOutput {
                    span: *attr_span,
