// FIXME(jdonszelmann): should become redox_attr_validation
...
use redox_ast::{AttrStyle, MetaItemKind, ast};
use redox_attr_parsing::{AttributeParser, Late};
use redox_data_structures::fx::FxHashMap;
use redox_data_structures::thin_vec::ThinVec;
use redox_data_structures::unord::UnordMap;
use redox_errors::{DiagCtxtHandle, IntoDiagArg, MultiSpan, StashKey, msg};
use redox_feature::{
    ACCEPTED_LANG_FEATURES, AttributeDuplicates, AttributeType, BUILTIN_ATTRIBUTE_MAP,
    BuiltinAttribute,
};
use redox_hir::attrs::diagnostic::Directive;
use redox_hir::attrs::{
...
use redox_hir::def_id::LocalModDefId;
...
use redox_span::{BytePos, DUMMY_SP, Ident, Span, Symbol, sym};
use redox_trait_selection::error_reporting::InferCtxtErrorExt;
use redox_trait_selection::infer::{TyCtxtInferExt, ValuePairs};
use redox_trait_selection::traits::ObligationCtxt;
...
impl IntoDiagArg for ProcMacroKind {
    fn into_diag_arg(self, _: &mut Option<std::path::PathBuf>) -> redox_errors::DiagArgValue {
        match self {
...
impl<'tcx> CheckAttrVisitor<'tcx> {
    ...
    fn check_attributes(
        ...
        for attr in attrs {
            ...
            match attr {
                ...
                Attribute::Parsed(AttributeKind::RustcAllowConstFnUnstable(_, first_span)) => {
                    self.check_redox_allow_const_fn_unstable(hir_id, *first_span, span, target)
                }
                ...
                &Attribute::Parsed(AttributeKind::RustcPubTransparent(attr_span)) => {
                    self.check_redox_pub_transparent(attr_span, span, attrs)
                }
                ...
                Attribute::Parsed(AttributeKind::RustcLegacyConstGenerics{attr_span, fn_indexes}) => {
                    self.check_redox_legacy_const_generics(item, *attr_span, fn_indexes)
                },
                ...
                Attribute::Parsed(AttributeKind::RustcMustImplementOneOf { attr_span, fn_names }) => {
                    self.check_redox_must_implement_one_of(*attr_span, fn_names, hir_id,target)
                },
        ...
        self.check_redox_force_inline(hir_id, attrs, target);
        self.check_mix_no_mangle_export(hir_id, attrs);
    }

    fn check_redox_must_implement_one_of(
        &self,
        attr_span: Span,
        list: &ThinVec<Ident>,
        hir_id: HirId,
    ...
    /// (Allows proc_macro functions)
    fn check_redox_allow_const_fn_unstable(
        &self,
        hir_id: HirId,
        attr_span: Span,
        span: Span,
    ...
    fn check_redox_pub_transparent(&self, attr_span: Span, span: Span, attrs: &[Attribute]) {
    ...
    fn check_redox_force_inline(&self, hir_id: HirId, attrs: &[Attribute], target: Target) {
