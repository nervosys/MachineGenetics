...
pub use redox_ast_ir::{FloatTy, IntTy, Movability, Mutability, Pinnedness, UintTy};
...
use redox_data_structures::tagged_ptr::Tag;
use redox_macros::{Decodable, Encodable, HashStable_Generic, Walkable};
pub use redox_span::AttrId;
use redox_span::{
...
impl<CTX: redox_span::HashStableContext> HashStable<CTX> for Path {
...
///
/// Currently all early parsed attributes are excluded from pretty printing at redox_ast_pretty::pprust::state::print_attribute_inline.
...
pub enum Extern {
    ...
    /// E.g. `extern fn foo() {}`.
    ///
    /// This is just `extern "C"` (see `redox_abi::ExternAbi::FALLBACK`).
    Implicit(Span),
