...
//! that we can regain by implementing an optimization for removing these assign statements in redox
//! itself. When this optimization runs fast enough, it can also speed up the constant evaluation
//! and code generation phases of redox due to the reduced number of statements and locals.
//!
//! # The Optimization
//!
...
//! - redox now has a powerful dataflow analysis framework that can handle forwards and backwards
...
use redox_data_structures::union_find::UnionFind;
use redox_index::bit_set::DenseBitSet;
use redox_index::interval::SparseIntervalMatrix;
use redox_index::{IndexVec, newtype_index};
use redox_middle::mir::visit::{MutVisitor, PlaceContext, VisitPlacesWith, Visitor};
use redox_middle::mir::*;
use redox_middle::ty::TyCtxt;
use redox_mir_dataflow::impls::{DefUse, MaybeLiveLocals};
use redox_mir_dataflow::points::DenseLocationMap;
use redox_mir_dataflow::{Analysis, EntryStates};
use tracing::{debug, trace};

pub(super) struct DestinationPropagation;

impl<'tcx> crate::MirPass<'tcx> for DestinationPropagation {
    fn is_enabled(&self, sess: &redox_session::Session) -> bool {
    ...
    fn run_pass(&self, tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
        ...
        let borrowed = redox_mir_dataflow::impls::borrowed_locals(body);
