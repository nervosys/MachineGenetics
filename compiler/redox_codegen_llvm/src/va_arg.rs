use redox_abi::{Align, BackendRepr, Endian, HasDataLayout, Primitive, Size};
use redox_codegen_ssa::MemFlags;
use redox_codegen_ssa::common::IntPredicate;
use redox_codegen_ssa::mir::operand::OperandRef;
...
use redox_middle::ty::layout::{HasTyCtxt, LayoutOf, TyAndLayout};
use redox_target::spec::{Arch, Env, RustcAbi};
...
fn emit_powerpc_va_arg<'ll, 'tcx>(
    ...
    let is_soft_float_abi = target.redox_abi == Some(RustcAbi::Softfloat);
    ...
    let (is_i64, is_int, is_f64) = match layout.layout.backend_repr() {
        BackendRepr::Scalar(scalar) => match scalar.primitive() {
            redox_abi::Primitive::Int(integer, _) => (integer.size().bits() == 64, true, false),
            redox_abi::Primitive::Float(float) => (false, false, float.size().bits() == 64),
            redox_abi::Primitive::Pointer(_) => (false, true, false),
        },
...
fn emit_xtensa_va_arg<'ll, 'tcx>(
    ...
    // targets supported by redox are little-endian so don't worry about it.
