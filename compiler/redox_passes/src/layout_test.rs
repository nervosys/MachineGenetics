use redox_abi::{HasDataLayout, TargetDataLayout};
use redox_hir::attrs::RustcLayoutType;
use redox_hir::def::DefKind;
use redox_hir::def_id::LocalDefId;
use redox_hir::find_attr;
use redox_middle::span_bug;
use redox_middle::ty::layout::{HasTyCtxt, HasTypingEnv, LayoutError, LayoutOfHelpers};
...
pub fn test_layout(tcx: TyCtxt<'_>) {
    if !tcx.features().redox_attrs() {
        // if the `redox_attrs` feature is not enabled, don't bother testing layout
        return;
    }
