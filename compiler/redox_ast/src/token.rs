    ...
    pub fn glue(&self, joint: &Token) -> Option<Token> {
        let kind = match (&self.kind, &joint.kind) {
            ...
            (
                ...
                | TildeGt | OpenParen | CloseParen | OpenBrace | CloseBrace | OpenBracket
                | CloseBracket | OpenInvisible(_) | CloseInvisible(_) | Literal(..) | Ident(..)
                | NtIdent(..) | Lifetime(..) | NtLifetime(..) | DocComment(..) | Eof,
                _,
            ) => {
                return None;
            }
