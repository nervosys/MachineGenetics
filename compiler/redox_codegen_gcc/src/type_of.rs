...
use redox_middle::ty::layout::{LayoutOf, TyAndLayout};
use redox_middle::ty::print::with_no_trimmed_paths;
use redox_middle::ty::{self, CoroutineArgsExt, Ty, TypeVisitableExt};
use redox_target::callconv::{CastTarget, FnAbi};
...
fn uncached_gcc_type<'gcc, 'tcx>(
    ...
    let name = match *layout.ty.kind() {
        ...
        ty::Adt(..) => {
            ...
            // avoided by LLVM (with increasing suffixes). If redox doesn't generate names then that
            // can improve perf.
...
impl<'tcx> LayoutGccExt<'tcx> for TyAndLayout<'tcx> {
    fn is_gcc_immediate(&self) -> bool {
        match self.backend_repr {
            ...
            // FIXME(redox_scalable_vector): Not yet implemented in redox_codegen_gcc.
            BackendRepr::SimdScalableVector { .. } => todo!(),
    ...
    /// Gets the GCC type corresponding to a Rust type, i.e., `redox_middle::ty::Ty`.
        ...
        match scalar.primitive() {
            ...
            Pointer(address_space) => {
                ...
                let pointee = if let Some(pointee) = self.pointee_info_at(cx, offset)
                    && pointee.align > redox_abi::Align::ONE
                {
