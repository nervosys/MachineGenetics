...
use redox_abi::TyAndLayout;
use redox_hir::def::Namespace;
use redox_hir::def_id::LocalDefId;
use redox_span::Spanned;
use redox_type_ir::{ConstKind, TypeFolder, VisitorResult, try_visit};
...
TrivialLiftImpls! {
    ...
    redox_type_ir::BoundConstness,
    redox_type_ir::PredicatePolarity,
...
TrivialTypeTraversalImpls! {
    ...
    redox_abi::FieldIdx,
    redox_abi::VariantIdx,
    redox_ast::InlineAsmOptions,
    redox_ast::InlineAsmTemplatePiece,
    redox_hir::CoroutineKind,
    redox_hir::HirId,
    redox_hir::MatchSource,
    redox_hir::RangeEnd,
    redox_hir::def_id::LocalDefId,
    redox_span::Ident,
    redox_span::Span,
    redox_span::Symbol,
    redox_target::asm::InlineAsmRegOrRegClass,
    // tidy-alphabetical-end
}
...
impl<'tcx> TypeVisitable<TyCtxt<'tcx>> for redox_span::ErrorGuaranteed {
    fn visit_with<V: TypeVisitor<TyCtxt<'tcx>>>(&self, visitor: &mut V) -> V::Result {
        visitor.visit_error(*self)
    }
}

impl<'tcx> TypeFoldable<TyCtxt<'tcx>> for redox_span::ErrorGuaranteed {
    fn try_fold_with<F: FallibleTypeFolder<TyCtxt<'tcx>>>(
        self,
        _folder: &mut F,
    ) -> Result<Self, F::Error> {
