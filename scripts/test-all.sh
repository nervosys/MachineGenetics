...
#
#     rmi (cpu)   1,384 tests
#     prototype   1,221 tests
#     ribosome      168 tests
#     germline      112 tests
#     forge          53 tests
#     -------------------------
#     total       2,938 tests, 0 warnings
#
# Usage:
...
if [ "$CHECKDOCS" -eq 1 ]; then
    ...
    echo
    {
        ...
        echo "total=$total"
        # Every non-crate measurement lives in one script both harnesses call.
        # See its header, and item 27.
    ...
    # relied upon.
    } | bash "$REPO/scripts/emit-doc-counts.sh" | bash "$REPO/scripts/check-doc-counts.sh" || exit 1
fi
