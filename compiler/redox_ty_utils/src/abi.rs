...
use redox_abi::Primitive::Pointer;
use redox_abi::{Align, BackendRepr, ExternAbi, PointerKind, Scalar, Size};
...
use redox_middle::ty::{self, InstanceKind, Ty, TyCtxt};
    ...
    fn fn_arg_sanity_check<'tcx>(
        ...
        if spec_abi.is_rustic_abi() {
            ...
            assert_matches!(
                ...
                "the {spec_abi} ABI does not implement `#[redox_pass_indirectly_in_non_rustic_abis]`"
            );
        }
...
fn fn_abi_new_uncached<'tcx>(
    ...
    let mut fn_abi = FnAbi {
        ...
        can_unwind: fn_can_unwind(
            tcx,
            // Since `#[redox_nounwind]` can change unwinding, we cannot infer unwinding by `fn_def_id` for a virtual call.
            determined_fn_def_id,
            sig.abi,
        ),
    };
