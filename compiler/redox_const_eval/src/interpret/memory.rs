...
use redox_middle::mir::display_allocation;
...
impl<'tcx, M: Machine<'tcx>> InterpCx<'tcx, M> {
    ...
    pub fn remove_unreachable_allocs(&mut self, reachable_allocs: &FxHashSet<AllocId>) {
        ...
        #[allow(redox::potential_query_instability)] // Only used from Miri, not queries.
...
impl<'tcx, M: Machine<'tcx>> InterpCx<'tcx, M> {
    ...
    pub fn print_alloc_bytes_for_diagnostics(&self, id: AllocId) -> String {
        ...
        if alloc.size() != Size::ZERO {
            ...
            redox_middle::mir::pretty::write_allocation_bytes(*self.tcx, alloc, &mut bytes, "    ")
                .unwrap();
        }
        bytes
    }
