...
use redox_data_structures::jobserver::{self, Acquired};
use redox_data_structures::memmap::Mmap;
use redox_data_structures::profiling::{SelfProfilerRef, VerboseTimingGuard};
...
use redox_fs_util::link_or_copy;
use redox_hir::find_attr;
use redox_incremental::{
    copy_cgu_workproduct_to_incr_comp_cache_dir, in_incr_comp_dir, in_incr_comp_dir_sess,
};
use redox_macros::{Decodable, Encodable};
use redox_metadata::fs::copy_to_stdout;
...
use redox_span::{FileName, InnerSpan, Span, SpanData};
use redox_target::spec::{MergeFunctions, SanitizerSet};
use tracing::debug;
...
impl ModuleConfig {
    fn new(kind: ModuleKind, tcx: TyCtxt<'_>, no_builtins: bool) -> ModuleConfig {
            ...
            || (sess.opts.cg.linker_plugin_lto.enabled() && !no_builtins)
        {
            ...
            // native linker or redox itself.
            //
            // Note, however, that the linker-plugin-lto requested here is
            // explicitly ignored for `#![no_builtins]` crates. These crates are
            // specifically ignored by redox's LTO passes and wouldn't work if
...
pub struct CodegenContext {
    ...
    pub split_debuginfo: redox_target::spec::SplitDebuginfo,
    pub split_dwarf_kind: redox_session::config::SplitDwarfKind,
...
// A cut-down version of `redox_errors::DiagInner` that impls `Send`, which
// can be used to send diagnostics from codegen threads to the main thread.
// It's missing the following fields from `redox_errors::DiagInner`.
...
// A cut-down version of `redox_errors::Subdiag` that impls `Send`. It's
// missing the following fields from `redox_errors::Subdiag`.
// - `span`: it doesn't impl `Send`.
...
fn start_executing_work<B: ExtraBackendMethods>(
    ...
    fn queue_full_enough(items_in_queue: usize, workers_running: usize) -> bool {
        ...
        // give tokens back to the job server. That enables some other redox to
        ...
        // redox can make use of the concurrency, then we've squandered it.
        //
...
impl Emitter for SharedEmitter {
    fn emit_diagnostic(&mut self, mut diag: redox_errors::DiagInner) {
        ...
        assert_eq!(diag.sort_span, redox_span::DUMMY_SP);
        ...
        loop {
            ...
            match message {
                Ok(SharedEmitterMessage::Diagnostic(diag)) => {
                    ...
                    let dcx = sess.dcx();
                    let mut d =
                        redox_errors::DiagInner::new_with_messages(diag.level, diag.messages);
