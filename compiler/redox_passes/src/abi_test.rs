use redox_hir::attrs::RustcAbiAttrKind;
use redox_hir::def::DefKind;
use redox_hir::def_id::LocalDefId;
use redox_hir::find_attr;
use redox_middle::span_bug;
use redox_middle::ty::layout::{FnAbiError, LayoutError};
use redox_middle::ty::{self, GenericArgs, Instance, Ty, TyCtxt};
use redox_span::Span;
...
pub fn test_abi(tcx: TyCtxt<'_>) {
    if !tcx.features().redox_attrs() {
        // if the `redox_attrs` feature is not enabled, don't bother testing ABI
        return;
    }
...
fn dump_abi_of_fn_item(
    ...
    // Check out the `#[redox_abi(..)]` attribute to tell what to dump.
    ...
    match attr_kind {
        RustcAbiAttrKind::Debug => {
            let ty::FnPtr(sig_tys, hdr) = ty.kind() else {
                span_bug!(
                    attr_span,
                    "`#[redox_abi(debug)]` on a type alias requires function pointer type"
                );
            };
            let abi =
                unwrap_fn_abi(
        ...
        RustcAbiAttrKind::AssertEq => {
            let ty::Tuple(fields) = ty.kind() else {
                span_bug!(
                    attr_span,
                    "`#[redox_abi(assert_eq)]` on a type alias requires pair type"
                );
            };
            ...
            let ty::FnPtr(sig_tys1, hdr1) = field1.kind() else {
                span_bug!(
                    attr_span,
                    "`#[redox_abi(assert_eq)]` on a type alias requires pair of function pointer types"
                );
            };
