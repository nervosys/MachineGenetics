...
use redox_abi::{HasDataLayout, Size, TargetDataLayout, VariantIdx};
use redox_codegen_ssa::back::versioned_llvm_target;
use redox_codegen_ssa::base::{wants_msvc_seh, wants_wasm_eh};
...
use redox_middle::middle::codegen_fn_attrs::PatchableFunctionEntry;
...
pub(crate) unsafe fn create_module<'ll>(
        ...
        if sess.target.arch == Arch::AArch64 {
            ...
            bug!(
                "branch-protection used on non-AArch64 target; \
                  this should be checked in redox_session."
            );
        }
    }
    ...
    let redox_producer =
        format!("redox version {}", option_env!("CFG_VERSION").expect("CFG_VERSION"));

    let name_metadata = cx.create_metadata(redox_producer.as_bytes());
