...
use redox_span::{DesugaringKind, Span, Symbol, kw};
...
impl Directive {
    ...
    ///
    /// We can't check this while parsing the attribute because `redox_attr_parsing` doesn't have
    ...
    pub fn format(&self, args: &FormatArgs) -> String {
        ...
        for piece in &self.pieces {
            match piece {
                ...
                // It's only `redox_onunimplemented` from here
...
pub struct ConditionOptions {
    ...
    /// Match on a variant of redox_infer's `ObligationCauseCode`.
