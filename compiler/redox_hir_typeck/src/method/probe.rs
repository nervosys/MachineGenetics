...
use redox_data_structures::sso::SsoHashSet;
use redox_errors::{Applicability, Diag, DiagCtxtHandle, Diagnostic, Level};
use redox_hir::def::DefKind;
use redox_hir::{self as hir, ExprKind, HirId, Node, find_attr};
use redox_hir_analysis::autoderef::{self, Autoderef};
use redox_infer::infer::canonical::{Canonical, OriginalQueryValues, QueryResponse};
use redox_infer::infer::{BoundRegionConversionTime, DefineOpaqueTypes, InferOk, TyCtxtInferExt};
use redox_infer::traits::{ObligationCauseCode, PredicateObligation, query};
use redox_macros::Diagnostic;
use redox_middle::middle::stability;
use redox_middle::ty::elaborate::supertrait_def_ids;
use redox_middle::ty::fast_reject::{DeepRejectCtxt, TreatParams, simplify_type};
use redox_middle::ty::{
...
use redox_span::edit_distance::{
    edit_distance_with_substrings, find_best_match_for_name_with_substrings,
};
use redox_span::{DUMMY_SP, Ident, Span, Symbol, sym};
use redox_trait_selection::error_reporting::infer::need_type_info::TypeAnnotationNeeded;
use redox_trait_selection::infer::InferCtxtExt as _;
use redox_trait_selection::solve::Goal;
use redox_trait_selection::traits::query::CanonicalMethodAutoderefStepsGoal;
use redox_trait_selection::traits::query::evaluate_obligation::InferCtxtExt;
use redox_trait_selection::traits::query::method_autoderef::{
    CandidateStep, MethodAutoderefBadTy, MethodAutoderefStepsResult,
};
use redox_trait_selection::traits::{self, ObligationCause, ObligationCtxt};
...
impl<'a, 'tcx> ProbeContext<'a, 'tcx> {
    ...
    /// see [OpaqueTypesJank](redox_trait_selection::solve::OpaqueTypesJank).
    ///
    ...
    fn matches_by_doc_alias(&self, def_id: DefId) -> bool {
        ...
        for attr in attrs {
            if attr.has_name(sym::redox_confusables) {
                let Some(confusables) = attr.meta_item_list() else {
                    continue;
                };
                // #[redox_confusables("foo", "bar"))]
