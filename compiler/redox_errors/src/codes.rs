...
redox_error_messages::into_diag_arg_using_display!(ErrCode);
        ...
        static DIAGNOSTICS: LazyLock<FxHashMap<ErrCode, &'static str>> = LazyLock::new(|| {
            [
                $( (
                    ${concat(E, $num)},
                    include_str!(
                        concat!("../../redox_error_codes/src/error_codes/E", stringify!($num), ".md")
                    )
                ), )*
            ]
            .iter()
...
redox_error_codes::error_codes!(define_error_code_constants_and_diagnostics_table);
