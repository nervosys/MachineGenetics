...
//!
//! This is roughly based on the symbol interner from `redox_span` and the
//! DroplessArena from `redox_arena`. It is unfortunately a complete
...
impl Symbol {
    ...
    // Mimics the behavior of `Symbol::can_be_raw` from `redox_span`
...
/// Basic interner for a `Symbol`, inspired by the one in `redox_span`.
struct Interner {
    arena: arena::Arena,
