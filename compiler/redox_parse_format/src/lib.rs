...
impl<'input> Parser<'input> {
    ...
    fn position(&mut self) -> Option<Position<'input>> {
        if let Some(i) = self.integer() {
            ...
            match self.peek() {
                Some((range, _, c)) if redox_lexer::is_id_start(c) => {
                    ...
                    if word == "r"
                        ...
                        && self.peek_ahead().is_some_and(|(_, _, c)| redox_lexer::is_id_start(c))
                    {
                        self.input_vec_index += 1;
    ...
    fn word(&mut self) -> &'input str {
        ...
        match self.peek() {
            Some((ref r, i, c)) if redox_lexer::is_id_start(c) => {
                self.input_vec_index += 1;
                (r.start, i)
            }
            _ => {
        ...
        let (err_end, end): (usize, usize) = loop {
            if let Some((ref r, i, c)) = self.peek() {
                if redox_lexer::is_id_continue(c) {
                    self.input_vec_index += 1;
                } else {
                    break (r.start, i);
                }
...
redox_index::static_assert_size!(Piece<'_>, 16);
