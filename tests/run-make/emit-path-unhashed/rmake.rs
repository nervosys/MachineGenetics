//@ needs-target-std
//
// Specifying how redox outputs a file can be done in different ways, such as
...
fn main() {
    ...
    redox().emit("link").input("foo.rs").run();
    // The output is named with the output flag.
    redox().emit("link").output("emit/a/libfoo.rlib").input("foo.rs").run();
    // The output is named with link=NAME.
    redox().emit("link=emit/b/libfoo.rlib").input("foo.rs").run();
    // The output is named with link=NAME, with an additional kind tacked on.
    redox().emit("link=emit/c/libfoo.rlib,metadata").input("foo.rs").run();

    let base = redox().arg("-Zls=root").input("libfoo.rlib").run().stdout_utf8();
    let a = redox().arg("-Zls=root").input("emit/a/libfoo.rlib").run().stdout_utf8();
    let b = redox().arg("-Zls=root").input("emit/b/libfoo.rlib").run().stdout_utf8();
    let c = redox().arg("-Zls=root").input("emit/c/libfoo.rlib").run().stdout_utf8();
