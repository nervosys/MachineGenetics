...
use redox_middle::ty::util::{CheckRegions, NotUniqueParam};
use redox_middle::ty::{self, Ty, TyCtxt, TypeSuperVisitable, TypeVisitable, TypeVisitor};
...
impl<'tcx> OpaqueTypeCollector<'tcx> {
    ...
    fn collect_taits_declared_in_body(&mut self) {
        ...
        impl<'v> intravisit::Visitor<'v> for TaitInBodyFinder<'_, '_> {
            #[instrument(level = "trace", skip(self))]
            fn visit_nested_item(&mut self, id: redox_hir::ItemId) {
            ...
            fn visit_nested_body(&mut self, id: redox_hir::BodyId) {
    ...
    fn visit_opaque_ty(&mut self, alias_ty: ty::AliasTy<'tcx>) {
        ...
        match self.tcx.local_opaque_ty_origin(alias_ty.def_id.expect_local()) {
            ...
            redox_hir::OpaqueTyOrigin::TyAlias { in_assoc_ty, .. } => match self.mode {
