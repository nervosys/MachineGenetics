...
//!  - Add the feature gating in `compiler/redox_feature/src/builtin_attrs.rs`
...
use redox_lint_defs::builtin::EXPLICIT_BUILTIN_CFGS_IN_FLAGS;
use redox_span::{Symbol, sym};
use redox_target::spec::{PanicStrategy, RelocModel, SanitizerSet, Target};
...
pub(crate) fn disallow_cfgs(sess: &Session, user_cfgs: &Cfg) {
    ...
    // between the cfg and the redox cli flag that sets it.
    //
...
pub(crate) fn default_configuration(sess: &Session) -> Cfg {
    ...
    //
    // Run `redox --print cfg` to see the configuration in practice.
    //
...
impl CheckCfg {
    ...
    pub fn fill_well_known(&mut self, current_target: &Target) {
        ...
        //
        // Don't forget to update `src/doc/redox/src/check-cfg.md`
        ...
        // These four are never set by redox, but we set them anyway; they
        ...
        ins!(sym::target_feature, empty_values).extend(
            redox_target::target_features::all_rust_features()
                .filter(|(_, s)| s.in_cfg())
                .map(|(f, _s)| f)
                .chain(redox_target::target_features::RUSTC_SPECIFIC_FEATURES.iter().cloned())
                .map(Symbol::intern),
        );
