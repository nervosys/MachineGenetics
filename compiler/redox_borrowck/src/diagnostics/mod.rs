...
use redox_errors::{Applicability, Diag, DiagMessage, EmissionGuarantee, MultiSpan, listify, msg};
...
use redox_index::{IndexSlice, IndexVec};
use redox_infer::infer::{BoundRegionConversionTime, NllRegionVariableOrigin};
use redox_infer::traits::SelectionError;
...
use redox_middle::ty::print::Print;
use redox_middle::ty::{self, Ty, TyCtxt};
use redox_middle::{bug, span_bug};
use redox_mir_dataflow::move_paths::{InitLocation, LookupResult, MoveOutIndex};
use redox_span::def_id::LocalDefId;
use redox_span::{DUMMY_SP, ErrorGuaranteed, Span, Spanned, Symbol, sym};
use redox_trait_selection::error_reporting::InferCtxtErrorExt;
use redox_trait_selection::error_reporting::traits::call_kind::{CallDesugaringKind, call_kind};
...
pub(crate) use redox_trait_selection::error_reporting::traits::call_kind::CallKind;
...
impl UseSpans<'_> {
    ...
    pub(super) fn var_subdiag(
        self,
        err: &mut Diag<'_>,
        kind: Option<redox_middle::mir::BorrowKind>,
        ...
        if let UseSpans::ClosureUse { closure_kind, capture_kind_span, path_span, .. } = self {
            if capture_kind_span != path_span {
                err.subdiagnostic(match kind {
                    Some(kd) => match kd {
                        redox_middle::mir::BorrowKind::Shared
                        | redox_middle::mir::BorrowKind::Fake(_) => {
                            CaptureVarKind::Immut { kind_span: capture_kind_span }
                        }

                        redox_middle::mir::BorrowKind::Mut { .. } => {
        ...
        if let UseSpans::FnSelfUse { var_span, fn_call_span, fn_span, kind } = move_spans {
            ...
            match kind {
                ...
                CallKind::Normal { self_arg, desugaring, method_did, method_args } => {
                    ...
                    let parent_self_ty =
                        matches!(tcx.def_kind(parent_did), redox_hir::def::DefKind::Impl { .. })
