...
use redox_abi::{Endian, Layout, ReprOptions};
use redox_hir::def::DefKind;
use redox_hir::{Attribute, LangItem};
use redox_middle::mir::interpret::{AllocId, ConstAllocation, ErrorHandled, GlobalAlloc, Scalar};
use redox_middle::mir::{BinOp, Body, Const as MirConst, ConstValue, UnOp};
use redox_middle::ty::layout::{FnAbiOf, LayoutOf};
...
use redox_session::cstore::ForeignModule;
...
impl<'tcx, B: Bridge> AllocRangeHelpers<'tcx> for CompilerCtxt<'tcx, B> {
    fn alloc_range(
        &self,
        offset: redox_abi::Size,
        size: redox_abi::Size,
    ) -> mir::interpret::AllocRange {
        redox_middle::mir::interpret::alloc_range(offset, size)
    }
}
...
impl<'tcx, B: Bridge> CompilerCtxt<'tcx, B> {
    ...
    /// In redox_public, we handle this case as if the body is not available.
        ...
        self.tcx
            ...
            .filter_map(|attribute| {
                if let Attribute::Unparsed(u) = attribute {
                    let attr_str = redox_hir_pretty::attribute_to_string(&self.tcx, attribute);
