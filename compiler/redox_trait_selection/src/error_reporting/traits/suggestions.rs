...
use redox_hir::intravisit::{Visitor, VisitorExt};
use redox_hir::lang_items::LangItem;
use redox_hir::{
    self as hir, AmbigArg, CoroutineDesugaring, CoroutineKind, CoroutineSource, Expr, HirId, Node,
    expr_needs_parens,
};
use redox_infer::infer::{BoundRegionConversionTime, DefineOpaqueTypes, InferCtxt, InferOk};
use redox_middle::middle::privacy::Level;
use redox_middle::traits::IsConstable;
    ...
    pub(super) fn suggest_fn_call(
            ...
            && obligation.cause.span.contains(*arg_span)
        {
            ...
            let removal_span = if let Ok(snippet) =
                sm.span_to_snippet(arg_span.with_hi(arg_span.hi() + redox_span::BytePos(1)))
                && snippet.ends_with(' ')
            {
                // There's a space after `||`, include it in the removal
                arg_span.with_hi(arg_span.hi() + redox_span::BytePos(1))
            } else {
                *arg_span
            };
            err.span_suggestion_verbose(
         ...
         -> bool {
            ...
            let mut label = || {
                ...
                if has_custom_message {
                    ...
                    err.messages = vec![(redox_errors::DiagMessage::from(msg), Style::NoStyle)];
                }
                if is_sized {
                    err.span_label(
                        span,
