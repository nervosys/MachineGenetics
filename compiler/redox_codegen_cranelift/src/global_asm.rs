...
use redox_codegen_ssa::traits::{AsmCodegenMethods, GlobalAsmOperandRef};
...
impl<'tcx> redox_abi::HasDataLayout for GlobalAsmContext<'_, 'tcx> {
    ...
    for piece in template {
        match *piece {
            ...
            InlineAsmTemplatePiece::Placeholder { operand_idx, modifier: _, span } => {
                use redox_codegen_ssa::back::symbol_export::escape_symbol_name;
    ...
    pub(crate) fn new(tcx: TyCtxt<'_>) -> Self {
        GlobalAsmConfig {
            ...
            target: match &tcx.sess.opts.target_triple {
                redox_target::spec::TargetTuple::TargetTuple(triple) => triple.clone(),
                redox_target::spec::TargetTuple::TargetJson { path_for_rustdoc, .. } => {
