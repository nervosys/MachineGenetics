...
use redox_hir::{self as hir, BindingMode, ByRef, Node};
...
use redox_middle::ty::{self, InstanceKind, Ty, TyCtxt, Upcast};
use redox_span::{BytePos, DesugaringKind, Span, Symbol, kw, sym};
            ...
            let reason = if let PlaceBase::Upvar(upvar_id) = closure_kind_origin.base {
                ...
                for captured_place in captured_places {
                    let captured_place_kinds = captured_place
                        ...
                        .map(|proj| proj.kind)
                        .collect::<Vec<_>>();
                    if redox_middle::ty::is_ancestor_or_same_capture(
...
fn suggest_ref_mut(tcx: TyCtxt<'_>, span: Span) -> Option<Span> {
    ...
    if let Some(rest) = pattern_str.strip_prefix("ref")
        && rest.starts_with(redox_lexer::is_whitespace)
    {
