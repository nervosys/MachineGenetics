...
pub use NtPatKind::*;
pub use TokenKind::*;
use redox_macros::{Decodable, Encodable, HashStable_Generic};
use redox_span::edition::Edition;
use redox_span::symbol::IdentPrintMode;
use redox_span::{DUMMY_SP, ErrorGuaranteed, Span, kw, sym};
...
/// Compact attribute kinds recognized in canonical syntax mode.
/// Each corresponds to a `@X` abbreviation that expands to a standard attribute.
#[derive(Clone, Copy, PartialEq, Eq, Hash, Encodable, Decodable, Debug, HashStable_Generic)]
pub enum CompactAttr {
    /// `@d` → `#[derive(...)]`
    Derive,
    /// `@r` → `#[repr(...)]`
    Repr,
...
impl CompactAttr {
    /// Try to match an identifier suffix to a compact attribute kind.
    pub fn from_suffix(s: &str) -> Option<Self> {
        match s {
            ...
            "t" => Some(Self::Test),
            "i" => Some(Self::Inline),
            "as" => Some(Self::AgentSpec),
            "ac" => Some(Self::AgentContract),
    ...
    /// Return the display string for this compact attribute (e.g. `"@d"`).
    ...
    /// Return the suffix identifier as a `Symbol` (for decomposition).
    pub fn suffix_symbol(self) -> Symbol {
...
pub enum TokenKind {
    ...
    /// `?`
    Question,
    /// `?=` (Redox: refinement operator)
    QuestionEq,
    /// `~>` (Redox: pipeline operator)
    TildeGt,
    /// `+f` / `+fn` (Redox canonical: async fn)
    PlusFn,
    /// `-f` / `-fn` (Redox canonical: const fn)
    MinusFn,
    /// `!f` / `!fn` (Redox canonical: unsafe fn)
    BangFn,
    /// `*f` / `*fn` (Redox canonical: extern fn)
    StarFn,
    /// Compact attribute (Redox canonical): `@d`, `@r`, `@t`, `@i`, `@as`, etc.
    CompactAttribute(CompactAttr),
...
impl TokenKind {
    ...
    /// redox parser. If the operator token can be broken into two tokens, the
...
impl Token {
    ...
    pub fn is_punct(&self) -> bool {
        match self.kind {
            ...
            | SlashEq | PercentEq | CaretEq | AndEq | OrEq | ShlEq | ShrEq | At | AtAt | Dot
            | DotDot | DotDotDot | DotDotEq | Comma | Semi | Colon | PathSep | RArrow | LArrow
            | FatArrow | Pound | Dollar | Question | QuestionEq | TildeGt | SingleQuote => true,

            OpenParen | CloseParen | OpenBrace | CloseBrace | OpenBracket | CloseBracket
            | OpenInvisible(_) | CloseInvisible(_) | Literal(..) | DocComment(..) | Ident(..)
            | NtIdent(..) | Lifetime(..) | NtLifetime(..) | PlusFn | MinusFn | BangFn | StarFn
            | CompactAttribute(..) | Eof => false,
        }
    }
    ...
    pub fn glue(&self, joint: &Token) -> Option<Token> {
        let kind = match (&self.kind, &joint.kind) {
            ...
            (
                Le | EqEq | Ne | Ge | AndAnd | OrOr | PlusEq | MinusEq | StarEq | SlashEq
                | PercentEq | CaretEq | AndEq | OrEq | ShlEq | ShrEq | AtAt | DotDotDot | DotDotEq
                | Comma | Semi | PathSep | RArrow | LArrow | FatArrow | Pound | Dollar | QuestionEq
                | TildeGt | PlusFn | MinusFn | BangFn | StarFn | CompactAttribute(..) | OpenParen
