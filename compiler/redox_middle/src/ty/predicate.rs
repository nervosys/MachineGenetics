...
use redox_macros::{HashStable, extension};
use redox_type_ir as ir;
...
impl<'tcx> redox_type_ir::inherent::Predicate<TyCtxt<'tcx>> for Predicate<'tcx> {
    fn as_clause(self) -> Option<ty::Clause<'tcx>> {
        self.as_clause()
    }
}

impl<'tcx> redox_type_ir::inherent::IntoKind for Predicate<'tcx> {
...
impl<'tcx> redox_type_ir::Flags for Predicate<'tcx> {
...
impl<'tcx> Predicate<'tcx> {
    ...
    pub fn allow_normalization(self) -> bool {
        redox_type_ir::inherent::Predicate::allow_normalization(self)
    }
}

impl<'tcx> redox_errors::IntoDiagArg for Predicate<'tcx> {
    fn into_diag_arg(self, path: &mut Option<std::path::PathBuf>) -> redox_errors::DiagArgValue {
        ty::tls::with(|tcx| {
            let pred = tcx.short_string(self, path);
            redox_errors::DiagArgValue::Str(std::borrow::Cow::Owned(pred))
        })
    }
}

impl<'tcx> redox_errors::IntoDiagArg for Clause<'tcx> {
    fn into_diag_arg(self, path: &mut Option<std::path::PathBuf>) -> redox_errors::DiagArgValue {
        ty::tls::with(|tcx| {
            let clause = tcx.short_string(self, path);
            redox_errors::DiagArgValue::Str(std::borrow::Cow::Owned(clause))
        })
    }
}
...
impl<'tcx> redox_type_ir::inherent::Clause<TyCtxt<'tcx>> for Clause<'tcx> {
...
impl<'tcx> redox_type_ir::inherent::IntoKind for Clause<'tcx> {
...
impl<'tcx> redox_type_ir::inherent::Clauses<TyCtxt<'tcx>> for ty::Clauses<'tcx> {}
...
impl<'tcx> redox_type_ir::inherent::BoundExistentialPredicates<TyCtxt<'tcx>>
