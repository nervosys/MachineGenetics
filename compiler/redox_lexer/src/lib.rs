...
//!
//! The idea with `redox_lexer` is to make a reusable library,
//! by separating out pure lexing and redox-specific concerns, like spans,
//! error reporting, and interning. So, redox_lexer operates directly on `&str`,
//! produces simple tokens which are a pair of type-tag and a bit of original text,
//! and does not report errors, instead storing them as flags on the token.
//!
//! Tokens produced by this lexer are not yet ready for parsing the Rust syntax.
//! For that see [`redox_parse::lexer`], which converts this basic token stream
...
//! lexeme types.
//!
//! [`redox_parse::lexer`]: ../redox_parse/lexer/index.html
...
/// type as `Int`. (Compare against `redox_ast::token::LitKind` and
/// `redox_ast::ast::LitKind`).
...
/// `redox` allows files to have a shebang, e.g. "#!/usr/bin/rustrun",
...
impl<'a> Cursor<'a> {
    ...
    ///
    /// Used by `redox_parse::lexer` to lex for guarded strings
