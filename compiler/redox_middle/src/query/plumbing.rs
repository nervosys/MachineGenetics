...
use redox_data_structures::hash_table::HashTable;
use redox_data_structures::sharded::Sharded;
use redox_data_structures::sync::{AtomicU64, WorkerLocal};
use redox_errors::Diag;
use redox_hir::def_id::{DefId, LocalDefId};
use redox_hir::hir_id::OwnerId;
use redox_span::Span;
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    /// is already cached in memory.)
    ///
    /// [`Steal`]: redox_data_structures::steal::Steal
    #[inline(always)]
    ...
    (
        ...
        $(
            pub mod $name {
                ...
                // Increase this limit if necessary, but do try to keep the size low if possible
                #[cfg(target_pointer_width = "64")]
                #[cfg(not(feature = "redox_randomized_layouts"))]
                const _: () = {
        ...
        pub struct QueryVTables<'tcx> {
            $(
                pub $name: ::redox_middle::query::plumbing::QueryVTable<'tcx, $name::Cache<'tcx>>,
            )*
        }
...
mod sealed {
    use redox_hir::def_id::{LocalModDefId, ModDefId};
