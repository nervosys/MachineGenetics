...
use redox_session::{CtfeBacktrace, EarlyDiagCtxt};
...
fn redox_logger_config() -> redox_log::LoggerConfig {
    // Start with the usual env vars.
    let mut cfg = redox_log::LoggerConfig::from_env("RUSTC_LOG");
    ...
    if let Ok(var) = env::var("MIRI_LOG") {
        ...
        if matches!(cfg.filter, Err(VarError::NotPresent)) {
            ...
            // used for everything, we only apply it to the parts of redox that are
            // CTFE-related. Otherwise, we use it verbatim for `RUSTC_LOG`.
            // This way, if you set `MIRI_LOG=trace`, you get only the right parts of
            // redox traced, but you can also do `MIRI_LOG=miri=trace,redox_const_eval::interpret=debug`.
            if tracing::Level::from_str(&var).is_ok() {
                cfg.filter = Ok(format!(
                    "redox_middle::mir::interpret={var},redox_const_eval::interpret={var},miri={var}"
                ));
            } else {
                cfg.filter = Ok(var);
            }
    ...
    LOGGER_INITED.get_or_init(|| {
        let guard = if env::var_os("MIRI_TRACING").is_some() {
            ...
            #[cfg(feature = "tracing")]
            {
                ...
                redox_driver::init_logger_with_additional_layer(
                    early_dcx,
                    redox_logger_config(),
                    || {
                        redox_log::tracing_subscriber::layer::SubscriberExt::with(
                            redox_log::tracing_subscriber::Registry::default(),
                            chrome_layer,
                        )
                    },
                );
            ...
            redox_driver::init_logger(early_dcx, redox_logger_config());
            None
        };
        Mutex::new(guard)
    });
...
pub fn init_early_loggers(early_dcx: &EarlyDiagCtxt) {
    // We only initialize `redox` if the env var is set (so the user asked for it).
