...
use redox_errors::pluralize;
use redox_hir::lang_items::LangItem;
use redox_hir::{self as hir, CoroutineDesugaring, CoroutineKind, find_attr};
use redox_index::bit_set::{BitMatrix, DenseBitSet, GrowableBitSet};
use redox_index::{Idx, IndexVec, indexvec};
use redox_middle::mir::visit::{MutVisitor, MutatingUseContext, PlaceContext, Visitor};
...
use redox_mir_dataflow::impls::{
    MaybeBorrowedLocals, MaybeLiveLocals, MaybeRequiresStorage, MaybeStorageLive,
    always_storage_live_locals,
};
use redox_mir_dataflow::{
    Analysis, Results, ResultsCursor, ResultsVisitor, visit_reachable_results,
};
use redox_span::def_id::{DefId, LocalDefId};
use redox_span::{DUMMY_SP, Span, dummy_spanned};
use redox_trait_selection::error_reporting::InferCtxtErrorExt;
use redox_trait_selection::infer::TyCtxtInferExt as _;
use redox_trait_selection::traits::{ObligationCause, ObligationCauseCode, ObligationCtxt};
...
fn create_coroutine_resume_function<'tcx>(
    ...
    use redox_middle::mir::AssertKind::{ResumedAfterPanic, ResumedAfterReturn};
...
fn check_must_not_suspend_def(
    ...
    if let Some(reason_str) = find_attr!(tcx, def_id, MustNotSupend {reason} => reason) {
        ...
        tcx.emit_node_span_lint(
            redox_session::lint::builtin::MUST_NOT_SUSPEND,
            hir_id,
            data.source_span,
