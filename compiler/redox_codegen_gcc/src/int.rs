...
use redox_abi::{CanonAbi, Endian, ExternAbi};
use redox_codegen_ssa::common::{IntPredicate, TypeKind};
use redox_codegen_ssa::traits::{BackendTypes, BaseTypeCodegenMethods, BuilderMethods, OverflowOp};
use redox_middle::ty::{self, Ty};
use redox_target::callconv::{ArgAbi, ArgAttributes, FnAbi, PassMode};
use redox_type_ir::{Interner, TyKind};
...
impl<'a, 'gcc, 'tcx> Builder<'a, 'gcc, 'tcx> {
    ...
    pub fn gcc_checked_binop(
        ...
        use redox_middle::ty::IntTy::*;
        use redox_middle::ty::UintTy::*;
    ...
    pub fn gcc_icmp(
        ...
        if self.is_non_native_int_type(a_type) || self.is_non_native_int_type(b_type) {
            ...
            // NOTE: gcc cannot compare pointers to different objects, but redox does that, so cast them to usize.
