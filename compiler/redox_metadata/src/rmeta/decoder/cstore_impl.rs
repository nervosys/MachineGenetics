...
use redox_hir::attrs::Deprecation;
use redox_hir::def::{CtorKind, DefKind};
use redox_hir::def_id::{CrateNum, DefId, DefIdMap, LOCAL_CRATE};
use redox_hir::definitions::{DefKey, DefPath, DefPathHash};
use redox_middle::arena::ArenaAllocatable;
...
use redox_middle::middle::stability::DeprecationEntry;
use redox_middle::queries::ExternProviders;
...
use redox_serialize::Decoder;
use redox_session::StableCrateId;
use redox_session::cstore::{CrateStore, ExternCrate};
use redox_span::hygiene::ExpnId;
use redox_span::{Span, Symbol, kw};
...
macro_rules! provide_one {
    ...
    ($tcx:ident, $def_id:ident, $other:ident, $cdata:ident, $name:ident => $compute:block) => {
        fn $name<'tcx>(
            $tcx: TyCtxt<'tcx>,
            def_id_arg: redox_middle::queries::$name::Key<'tcx>,
        ) -> redox_middle::queries::$name::ProvidedValue<'tcx> {
            let _prof_timer =
            ...
            use redox_middle::dep_graph::DepKind;
            if DepKind::$name != DepKind::crate_hash && $tcx.dep_graph.is_fully_enabled() {
                $tcx.ensure_ok().crate_hash($def_id.krate);
            }

            let cdata = redox_data_structures::sync::FreezeReadGuard::map(CStore::from_tcx($tcx), |c| {
...
pub(in crate::rmeta) fn provide(providers: &mut Providers) {
    ...
    providers.queries = redox_middle::query::Providers {
