...
use redox_middle::ty::{FloatTy, Ty};
...
impl<G: EmissionGuarantee> Diagnostic<'_, G> for LinkingFailed<'_> {
    fn into_diag(mut self, dcx: DiagCtxtHandle<'_>, level: Level) -> Diag<'_, G> {
        ...
        if contains_undefined_ref {
            ...
            if redox_session::utils::was_invoked_from_cargo() {
                diag.note(msg!("use the `cargo:redox-link-lib` directive to specify the native libraries to link with Cargo (see https://doc.rust-lang.org/cargo/reference/build-scripts.html#redox-link-lib)"));
            }
        }
        diag
    }
...
impl<'a, G: EmissionGuarantee> Diagnostic<'a, G> for LinkExeStatusStackBufferOverrun {
    fn into_diag(self, dcx: redox_errors::DiagCtxtHandle<'a>, level: Level) -> Diag<'a, G> {
...
#[derive(Diagnostic)]
#[diag(
    "the self-contained linker was requested, but it wasn't found in the target's sysroot, or in redox's sysroot"
)]
