...
use redox_span::{DUMMY_SP, STDLIB_STABLE_CRATES, Span, Symbol, sym};
...
impl<'a> StripUnconfigured<'a> {
    ...
    pub(crate) fn expand_cfg_attr(&self, cfg_attr: &Attribute, recursive: bool) -> Vec<Attribute> {
        ...
        let Some((cfg_predicate, expanded_attrs)) =
            redox_attr_parsing::parse_cfg_attr(cfg_attr, &self.sess, self.features)
        else {
        ...
        if expanded_attrs.is_empty() {
            self.sess.psess.buffer_lint(
                redox_lint_defs::builtin::UNUSED_ATTRIBUTES,
                cfg_attr.span,
                ast::CRATE_NODE_ID,
