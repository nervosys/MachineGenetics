...
///   `redox_middle`.
/// * `MetadataEncodable` is used in `redox_metadata` for types that contain
///   `redox_metadata::rmeta::Lazy`.
/// * `TyEncodable` should be used for types that are only serialized in crate
///   metadata or the incremental cache. This is most types in `redox_middle`.
...
///   `redox_middle`.
/// * `MetadataDecodable` is used in `redox_metadata` for types that contain
///   `redox_metadata::rmeta::Lazy`.
