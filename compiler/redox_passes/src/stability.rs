...
use redox_ast_lowering::stability::extern_abi_stability;
use redox_data_structures::fx::FxIndexMap;
use redox_data_structures::unord::{ExtendUnord, UnordMap, UnordSet};
use redox_feature::{EnabledLangFeature, EnabledLibFeature, UNSTABLE_LANG_FEATURES};
use redox_hir::attrs::{AttributeKind, DeprecatedSince};
use redox_hir::def::{DefKind, Res};
use redox_hir::def_id::{CRATE_DEF_ID, LOCAL_CRATE, LocalDefId, LocalModDefId};
...
use redox_middle::middle::lib_features::{FeatureStability, LibFeatures};
use redox_middle::middle::privacy::EffectiveVisibilities;
use redox_middle::middle::stability::{AllowUnstable, Deprecated, DeprecationEntry, EvalResult};
use redox_middle::query::{LocalCrate, Providers};
use redox_middle::ty::print::with_no_trimmed_paths;
use redox_middle::ty::{AssocContainer, TyCtxt};
use redox_session::lint;
use redox_session::lint::builtin::{DEPRECATED, INEFFECTIVE_UNSTABLE_TRAIT_IMPL};
...
/// with the `redox_private` feature. This is intended for use when
/// compiling library and `redox_*` crates themselves so we can leverage crates.io
...
fn lookup_const_stability(tcx: TyCtxt<'_>, def_id: LocalDefId) -> Option<ConstStability> {
    ...
    // FIXME(const_trait_impl): how is this supposed to interact with `#[redox_const_stable_indirect]`?
    ...
    fn visit_path(&mut self, path: &hir::Path<'tcx>, id: hir::HirId) {
        if let Some(def_id) = path.res.opt_def_id() {
            ...
            if item_is_allowed {
                ...
                // path segment twice
                //
                // We include special cases via #[redox_allowed_through_unstable_modules] for items
...
// This is `pub` for rustdoc. redox should call it through `check_mod_unstable_api_usage`.
