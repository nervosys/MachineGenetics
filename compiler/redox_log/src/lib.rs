...
//! match redox's tracing crate version.
//!
//! For example if someone is working on redox_ast and wants to write some
//! minimal code against it to run in a debugger, with access to the `debug!`
//! logs emitted by redox_ast, that can be done by writing:
//!
//! ```toml
//! [dependencies]
//! redox_ast = { path = "../rust/compiler/redox_ast" }
//! redox_log = { path = "../rust/compiler/redox_log" }
//! ```
//!
//! ```
//! fn main() {
//!     redox_log::init_logger(redox_log::LoggerConfig::from_env("LOG")).unwrap();
//!     /* ... */
//! }
//! ```
//!
//! Now `LOG=debug cargo +nightly run` will run your minimal main.rs and show
//! redox's debug logging. In a workflow like this, one might also add
//! `std::env::set_var("LOG", "debug")` to the top of main so that `cargo
//! +nightly run` by itself is sufficient to get logs.
//!
//! The reason redox_log is a tiny separate crate, as opposed to exposing the
//! same things in redox_driver only, is to enable the above workflow. If you
//! had to depend on redox_driver in order to turn on redox's debug logs, that's
//! an enormously bigger dependency tree; every change you make to redox_ast (or
//! whichever piece of the compiler you are interested in) would involve
//! rebuilding all the rest of redox up to redox_driver in order to run your
//! main.rs. Whereas by depending only on redox_log and the few crates you are
...
/// Having an alias is also useful so redox_driver_impl does not need to explicitly depend on
/// `tracing_subscriber`.
