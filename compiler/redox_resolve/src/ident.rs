...
use Determinacy::*;
use Namespace::*;
use redox_ast::{self as ast, NodeId};
use redox_errors::ErrorGuaranteed;
use redox_hir::def::{DefKind, MacroKinds, Namespace, NonMacroAttrKind, PartialRes, PerNS};
use redox_middle::ty::Visibility;
use redox_middle::{bug, span_bug};
use redox_session::lint::builtin::PROC_MACRO_DERIVE_RESOLUTION_FALLBACK;
use redox_session::parse::feature_err;
use redox_span::edition::Edition;
use redox_span::hygiene::{ExpnId, ExpnKind, LocalExpnId, MacroKind, SyntaxContext};
                ...
                None => {
                    ...
                    ctor.map(|(ctor_kind, ctor_def_id)| {
                        let ctor_res = Res::Def(
                            DefKind::Ctor(redox_hir::def::CtorOf::Struct, ctor_kind),
                            ctor_def_id,
                        );
