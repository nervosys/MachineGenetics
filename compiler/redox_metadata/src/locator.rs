...
//! is a redox-defined file format (currently just an ar archive) while a dylib
...
//!
//! ```text
//! redox a.rs --extern b1=path/to/libb1.rlib --extern b2=path/to/libb2.rlib
//! ```
//!
...
//! deal with this in redox is to add hashes to the end of each symbol.
//!
...
use redox_data_structures::owned_slice::{OwnedSlice, slice_owned};
use redox_data_structures::svh::Svh;
use redox_errors::{DiagArgValue, IntoDiagArg};
use redox_fs_util::try_canonicalize;
use redox_session::cstore::CrateSource;
use redox_session::filesearch::FileSearch;
use redox_session::search_paths::PathKind;
use redox_session::utils::CanonicalizedPath;
use redox_session::{Session, config};
...
fn get_metadata_section<'p>(
    ...
    let raw_bytes = match flavor {
        ...
        CrateFlavor::SDylib => {
            ...
            let tmp_path = match TempFileBuilder::new().prefix("redox").tempdir() {
...
enum MetadataError<'a> {
    ...
    LoadFailure(String),
    /// The file was present, but compiled with a different redox version.
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            MetadataError::VersionMismatch { expected_version, found_version } => {
                f.write_str(&format!(
                    "redox version mismatch. expected {}, found {}",
                    expected_version, found_version,
                ))
            }
        }
