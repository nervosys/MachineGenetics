...
| 26 | **`cross_lang/run.sh` is cited by the README and run by nothing** | Found 2026-09-02. `README.md` sources its cross-language 5/5 executability table and its 173-token figure from this script, and nothing ran it. **Made honest 2026-09-02; whether CI should run it is still open.** Four defects, all of the same family — reporting a failure it never measured. (1) The `.exe` fallback *appended* instead of stripping, so on Linux it looked for `mage-parse.exe.exe`, `mage_all` produced nothing, and the table would have published **MAGE failing all five of its own tasks** — the third copy of that line, wrong in `constructs/run.sh`, right in `capstone/run.sh`. (2) A missing toolchain was indistinguishable from a broken one: no `node` scored 0/5, no `rustc` printed "compile/run FAILED". Absent now reads *not measured*. (3) `go run` compiles and runs in one step, so a **cold build cache** scored Go 0/5 — observed, then passing on the next run of the same script; it now builds as its own step like Rust. (4) The Python line asserted "runtime not installed on this host" unconditionally — false even when written, since python3 is present. The real reason is that **there is no `tasks.py`**, and the row now says so and will start measuring the day someone adds one. What remains a decision: whether CI pins its toolchains and runs it, which is a judgement about what the benchmark means when a language is absent. |
