//! Crate that implements what will become the redox side of redox_public.
//!
//! This crate serves as a proxy for making calls to redox queries.
//!
//! This crate is not intended to be invoked directly by users.
//! This crate is the public API of redox that will be invoked by the `redox_public` crate.
//!
...
use redox_data_structures::fx::{self, FxIndexMap};
use redox_middle::mir;
use redox_middle::mir::interpret::AllocId;
use redox_middle::ty::{self, Ty, TyCtxt};
use redox_span::Span;
use redox_span::def_id::{CrateNum, DefId, LOCAL_CRATE};
...
#[deprecated(note = "please use `redox_public::redox_internal` instead")]
pub mod redox_internal {}
...
pub struct Tables<'tcx, B: Bridge> {
    ...
    pub spans: IndexMap<redox_span::Span, B::Span>,
    ...
    pub layouts: IndexMap<redox_abi::Layout<'tcx>, B::Layout>,
}
...
impl<'tcx, B: Bridge> Tables<'tcx, B> {
    ...
    pub fn layout_id(&mut self, layout: redox_abi::Layout<'tcx>) -> B::Layout {
        self.layouts.create_or_fetch(layout)
    }

    pub fn crate_item(&mut self, did: redox_span::def_id::DefId) -> B::CrateItem {
        B::CrateItem::new(self.create_def_id(did))
    }

    pub fn adt_def(&mut self, did: redox_span::def_id::DefId) -> B::AdtDef {
        B::AdtDef::new(self.create_def_id(did))
    }

    pub fn foreign_module_def(&mut self, did: redox_span::def_id::DefId) -> B::ForeignModuleDef {
        B::ForeignModuleDef::new(self.create_def_id(did))
    }

    pub fn foreign_def(&mut self, did: redox_span::def_id::DefId) -> B::ForeignDef {
        B::ForeignDef::new(self.create_def_id(did))
    }

    pub fn fn_def(&mut self, did: redox_span::def_id::DefId) -> B::FnDef {
        B::FnDef::new(self.create_def_id(did))
    }

    pub fn closure_def(&mut self, did: redox_span::def_id::DefId) -> B::ClosureDef {
        B::ClosureDef::new(self.create_def_id(did))
    }

    pub fn coroutine_def(&mut self, did: redox_span::def_id::DefId) -> B::CoroutineDef {
        B::CoroutineDef::new(self.create_def_id(did))
    }

    pub fn coroutine_closure_def(
        &mut self,
        did: redox_span::def_id::DefId,
    ) -> B::CoroutineClosureDef {
        B::CoroutineClosureDef::new(self.create_def_id(did))
    }

    pub fn alias_def(&mut self, did: redox_span::def_id::DefId) -> B::AliasDef {
        B::AliasDef::new(self.create_def_id(did))
    }

    pub fn param_def(&mut self, did: redox_span::def_id::DefId) -> B::ParamDef {
        B::ParamDef::new(self.create_def_id(did))
    }

    pub fn br_named_def(&mut self, did: redox_span::def_id::DefId) -> B::BrNamedDef {
        B::BrNamedDef::new(self.create_def_id(did))
    }

    pub fn trait_def(&mut self, did: redox_span::def_id::DefId) -> B::TraitDef {
        B::TraitDef::new(self.create_def_id(did))
    }

    pub fn generic_def(&mut self, did: redox_span::def_id::DefId) -> B::GenericDef {
        B::GenericDef::new(self.create_def_id(did))
    }

    pub fn const_def(&mut self, did: redox_span::def_id::DefId) -> B::ConstDef {
        B::ConstDef::new(self.create_def_id(did))
    }

    pub fn impl_def(&mut self, did: redox_span::def_id::DefId) -> B::ImplDef {
        B::ImplDef::new(self.create_def_id(did))
    }

    pub fn region_def(&mut self, did: redox_span::def_id::DefId) -> B::RegionDef {
    ...
    pub fn assoc_def(&mut self, did: redox_span::def_id::DefId) -> B::AssocDef {
        B::AssocDef::new(self.create_def_id(did))
    }

    pub fn opaque_def(&mut self, did: redox_span::def_id::DefId) -> B::OpaqueDef {
        B::OpaqueDef::new(self.create_def_id(did))
    }

    pub fn prov(&mut self, aid: redox_middle::mir::interpret::AllocId) -> B::Prov {
        B::Prov::new(self.create_alloc_id(aid))
    }

    pub fn static_def(&mut self, did: redox_span::def_id::DefId) -> B::StaticDef {
        B::StaticDef::new(self.create_def_id(did))
    }
}

/// A trait defining types that are used to emulate redox_public components, which is really
/// useful when programming in redox_public-agnostic settings.
...
/// Similar to redox's `FxIndexMap`, `IndexMap` with extra
    ...
    F: FnMut(DefId) -> Option<T>,
{
    if krate == LOCAL_CRATE {
        ...
        (0..num_definitions)
            .filter_map(move |i| {
                let def_id = DefId { krate, index: redox_span::def_id::DefIndex::from_usize(i) };
                func(def_id)
            })
            .collect()
    }
