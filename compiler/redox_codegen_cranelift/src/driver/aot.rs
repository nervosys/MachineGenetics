...
use redox_codegen_ssa::assert_module_sources::CguReuse;
use redox_codegen_ssa::back::write::produce_final_output_artifacts;
use redox_codegen_ssa::base::determine_cgu_reuse;
use redox_codegen_ssa::{CompiledModule, CompiledModules, ModuleKind};
...
use redox_hir::attrs::Linkage as RLinkage;
use redox_middle::dep_graph::{WorkProduct, WorkProductId};
use redox_middle::middle::codegen_fn_attrs::CodegenFnAttrFlags;
use redox_middle::mir::mono::{CodegenUnit, MonoItem, MonoItemData, Visibility};
use redox_session::Session;
use redox_session::config::{OutputFilenames, OutputType};
        ...
        for module_codegen in self.modules {
            ...
            if let Some((work_product_id, work_product)) = existing_work_product {
                ...
                let work_product = if disable_incr_cache {
                    ...
                    redox_incremental::copy_cgu_workproduct_to_incr_comp_cache_dir(
                        sess,
                        &module_regular.name,
                        &[
...
fn make_module(sess: &Session, name: String) -> UnwindModule<ObjectModule> {
    ...
    // explicitly disable it on MinGW as redox already disables it by default on MinGW and as such
    // isn't tested. If redox enables it in the future on MinGW, we can re-enable it too once it has
    // been on MinGW.
...
fn reuse_workproduct_for_cgu(
    ...
    let source_file_regular = redox_incremental::in_incr_comp_dir_sess(
        tcx.sess,
        work_product.saved_files.get("o").expect("no saved object file in work product"),
    );

    if let Err(err) = redox_fs_util::link_or_copy(&source_file_regular, &obj_out_regular) {
        return Err(format!(
            "unable to copy {} to {}: {}",
    ...
    let source_file_global_asm = if let Some(asm_o) = work_product.saved_files.get("asm.o") {
        let source_file_global_asm = redox_incremental::in_incr_comp_dir_sess(tcx.sess, asm_o);
        if let Err(err) = redox_fs_util::link_or_copy(&source_file_global_asm, &obj_out_global_asm)
        {
            return Err(format!(
                "unable to copy {} to {}: {}",
        ...
        match mono_item {
            MonoItem::Fn(instance) => {
                ...
                if flags.contains(CodegenFnAttrFlags::NAKED) {
                    redox_codegen_ssa::mir::naked_asm::codegen_naked_asm(
            ...
            MonoItem::GlobalAsm(item_id) => {
                redox_codegen_ssa::base::codegen_global_asm(
                    &mut GlobalAsmContext { tcx, global_asm: &mut global_asm },
                    item_id,
                );
            }
...
pub(crate) fn run_aot(tcx: TyCtxt<'_>) -> Box<OngoingCodegen> {
    ...
    redox_codegen_ssa::assert_module_sources::assert_module_sources(tcx, &|cgu_reuse_tracker| {
        ...
        tcx.sess.time("codegen mono items", || {
            let modules: Vec<_> = par_map(todo_cgus, |(_, cgu)| {
                ...
                let (module, _) = tcx.dep_graph.with_task(
                    ...
                    module_codegen,
                    Some(redox_middle::dep_graph::hash_result),
                );
