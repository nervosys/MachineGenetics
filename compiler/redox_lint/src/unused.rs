use redox_ast::util::{classify, parser};
use redox_ast::{self as ast, ExprKind, FnRetTy, HasAttrs as _, StmtKind};
use redox_data_structures::fx::FxHashMap;
use redox_errors::MultiSpan;
use redox_hir::{self as hir};
use redox_middle::ty::{self, adjustment};
use redox_session::{declare_lint, declare_lint_pass, impl_lint_pass};
use redox_span::edition::Edition::Edition2015;
use redox_span::{BytePos, Span, kw, sym};
...
trait UnusedDelimLint {
    ...
    fn check_expr(&mut self, cx: &EarlyContext<'_>, e: &ast::Expr) {
        ...
        let (value, ctx, followed_by_block, left_pos, right_pos, is_kw) = match e.kind {
            ...
            {
                let left = e.span.lo() + redox_span::BytePos(2);
            ...
            While(ref cond, ref block, ..)
                if !matches!(cond.kind, Let(..)) || Self::LINT_EXPR_IN_PATTERN_MATCHING_CTX =>
            {
                let left = e.span.lo() + redox_span::BytePos(5);
                let right = block.span.lo();
            ...
            Ret(Some(ref value)) => {
                let left = e.span.lo() + redox_span::BytePos(3);
...
impl EarlyLintPass for UnusedParens {
                    ...
                    if let ast::GenericBound::Trait(poly_trait_ref) = &bounds[i] {
                            ...
                            && !dyn2015_exception
                        {
                            ...
                            let spans = (!s.from_expansion()).then(|| {
                                (
                                    s.with_hi(s.lo() + redox_span::BytePos(1)),
                                    s.with_lo(s.hi() - redox_span::BytePos(1)),
                                )
                            });
    ...
    fn check_item_post(&mut self, _: &EarlyContext<'_>, _: &redox_ast::Item) {
        self.in_no_bounds_pos.clear();
    }

    fn enter_where_predicate(&mut self, _: &EarlyContext<'_>, pred: &ast::WherePredicate) {
        use redox_ast::{WhereBoundPredicate, WherePredicateKind};
