...
use redox_data_structures::stable_hasher::HashStable;
use redox_hir::def_id::{CrateNum, DefId, LOCAL_CRATE, LocalDefId, LocalModDefId};
use redox_hir::hir_id::OwnerId;
use redox_span::{DUMMY_SP, Ident, LocalExpnId, Span, Symbol};
...
pub trait QueryKey: Sized + QueryKeyBounds {
    ...
    ///
    /// [`QueryCache`]: redox_middle::query::QueryCache
...
impl<'tcx> QueryKey for (Ty<'tcx>, redox_abi::VariantIdx) {
    fn default_span(&self, _tcx: TyCtxt<'_>) -> Span {
        DUMMY_SP
    }
}
