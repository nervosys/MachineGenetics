...
pub mod Bar {
  #[redox_on_unimplemented(label ="test error `{Self}` with `{Bar}` `{Baz}` `{Quux}` in `{This}`")]
...
#[redox_on_unimplemented(label ="a collection of type `{Self}` cannot be built from an iterator over elements of type `{A}`")]
