...
use redox_middle::dep_graph::DepGraph;
use redox_middle::ty::TyCtxt;
use redox_session::Session;
use redox_session::config::{self, OutputFilenames, OutputType};
...
impl Linker {
    ...
    pub fn link(self, sess: &Session, codegen_backend: &dyn CodegenBackend) {
        ...
        sess.time("serialize_work_products", || {
            redox_incremental::save_work_product_index(sess, &self.dep_graph, work_products)
        });
        ...
        redox_incremental::finalize_session_directory(sess, self.crate_hash);
