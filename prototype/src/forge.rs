...
mod tests {
    ...
    fn publish_and_latest() {
        ...
        let math = reg.latest("mage-math").unwrap();
    ...
    fn search_multiple_capabilities() {
        ...
        // mage-agent has both, mage-math has one
        assert!(!results.is_empty());
        assert_eq!(results[0].package_name, "mage-agent"); // highest score
    }
    ...
    fn compatible_packages() {
        let reg = sample_registry();
        let result = reg.check_compatibility("mage-math", "mage-agent");
    ...
    fn incompatible_missing_cap() {
        ...
        let result = reg.check_compatibility("mage-math", "mage-consumer");
        assert!(!result.compatible);
    ...
    fn compatibility_package_not_found() {
        let reg = sample_registry();
        let result = reg.check_compatibility("nonexistent", "mage-agent");
        assert!(!result.compatible);
    }
    ...
    fn dependency_graph() {
        ...
        assert_eq!(graph["mage-agent"], vec!["mage-math".to_string()]);
        assert!(graph["mage-math"].is_empty());
    }

    #[test]
    fn transitive_capabilities() {
        let reg = sample_registry();
        let caps = reg.transitive_capabilities("mage-agent");
        assert!(caps.contains("code-analysis"));
        assert!(caps.contains("arithmetic"));
        assert!(caps.contains("linear-algebra")); // from mage-math
    }
    ...
    fn search_by_effect() {
        ...
        assert_eq!(results[0].package_name, "mage-io");
    }
    ...
    fn contract_ensures_requires_match() {
        let reg = sample_registry();
        // mage-math ensures "result >= 0", mage-agent requires "result >= 0"
        let result = reg.check_compatibility("mage-math", "mage-agent");
