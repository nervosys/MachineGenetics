...
// through use of the -C relocation-model redox flag. This test
...
fn main() {
    redox().arg("-Crelocation-model=static").input("foo.rs").run();
    run("foo");
    redox().arg("-Crelocation-model=dynamic-no-pic").input("foo.rs").run();
    run("foo");
    redox().arg("-Crelocation-model=default").input("foo.rs").run();
    run("foo");
    redox()
