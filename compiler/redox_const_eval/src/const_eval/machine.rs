...
use redox_abi::{Align, FIRST_VARIANT, Size};
use redox_ast::Mutability;
use redox_data_structures::fx::{FxHashMap, FxIndexMap, IndexEntry};
use redox_hir::def_id::{DefId, LocalDefId};
use redox_hir::{self as hir, CRATE_HIR_ID, LangItem, find_attr};
use redox_middle::mir::AssertMessage;
use redox_middle::mir::interpret::ReportedErrorInfo;
use redox_middle::query::TyCtxtAt;
use redox_middle::ty::layout::{HasTypingEnv, TyAndLayout, ValidityRequirement};
use redox_middle::ty::{self, FieldInfo, Ty, TyCtxt};
...
//
// Should be public because out-of-tree redox consumers need this
...
impl<'tcx> CompileTimeInterpCx<'tcx> {
    ...
    /// All `#[redox_do_not_const_check]` functions MUST be hooked here.
        ...
        if let Some(new_steps) = ecx.machine.num_evaluated_steps.checked_add(1) {
            ...
            if new_steps == limit {
                ...
                let is_error = ecx
                    .tcx
                    .lint_level_at_node(
                        redox_session::lint::builtin::LONG_RUNNING_CONST_EVAL,
                        hir_id,
                    )
                    .level
                    .is_error();
