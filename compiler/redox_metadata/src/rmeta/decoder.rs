...
use redox_data_structures::owned_slice::OwnedSlice;
use redox_data_structures::sync::Lock;
use redox_data_structures::unhash::UnhashMap;
use redox_expand::base::{SyntaxExtension, SyntaxExtensionKind};
use redox_expand::proc_macro::{AttrProcMacro, BangProcMacro, DeriveProcMacro};
use redox_hir::Safety;
use redox_hir::def::Res;
use redox_hir::def_id::{CRATE_DEF_INDEX, LOCAL_CRATE};
use redox_hir::definitions::{DefPath, DefPathData};
use redox_hir::diagnostic_items::DiagnosticItems;
use redox_index::Idx;
use redox_middle::middle::lib_features::LibFeatures;
use redox_middle::mir::interpret::{AllocDecodingSession, AllocDecodingState};
use redox_middle::ty::Visibility;
use redox_middle::ty::codec::TyDecoder;
use redox_middle::{bug, implement_ty_decoder};
use redox_proc_macro::bridge::client::ProcMacro;
use redox_serialize::opaque::MemDecoder;
use redox_serialize::{Decodable, Decoder};
use redox_session::config::TargetModifier;
use redox_session::cstore::{CrateSource, ExternCrate};
use redox_span::hygiene::HygieneDecodeContext;
use redox_span::{
...
/// Holds information about a redox_span::SourceFile imported from another crate.
/// See `imported_source_file()` for more information.
#[derive(Clone)]
struct ImportedSourceFile {
    /// This SourceFile's byte-offset within the source_map of its original crate
    original_start_pos: redox_span::BytePos,
    /// The end of this SourceFile within the source_map of its original crate
    original_end_pos: redox_span::BytePos,
    /// The imported SourceFile's representation within the local source_map
    translated_source_file: Arc<redox_span::SourceFile>,
}
...
impl<'a, 'tcx> TyDecoder<'tcx> for MetadataDecodeContext<'a, 'tcx> {
    ...
    fn decode_alloc_id(&mut self) -> redox_middle::mir::interpret::AllocId {
...
impl<'a, 'tcx> SpanDecoder for MetadataDecodeContext<'a, 'tcx> {
    fn decode_attr_id(&mut self) -> redox_span::AttrId {
    ...
    fn decode_syntax_context(&mut self) -> SyntaxContext {
        ...
        redox_span::hygiene::decode_syntax_context(self, &cdata.hygiene_context, |_, id| {
    ...
    fn decode_expn_id(&mut self) -> ExpnId {
        ...
        let expn_id = redox_span::hygiene::decode_expn_id(cnum, index, |expn_id| {
    ...
    pub(crate) fn check_compatibility(
        ...
        if !self.starts_with(METADATA_HEADER) {
            if self.starts_with(b"rust") {
                return Err(Some("<unknown redox version>".to_owned()));
            }
            return Err(None);
        }
...
impl<'a> CrateMetadataRef<'a> {
    ...
    /// Decodes all traits in the crate (for rustdoc and redox diagnostics).
    ...
    fn expn_hash_to_expn_id(self, tcx: TyCtxt<'_>, index_guess: u32, hash: ExpnHash) -> ExpnId {
        ...
        redox_span::hygiene::register_expn_id(self.cnum, index, data, hash)
    }
    ...
    fn imported_source_file(self, tcx: TyCtxt<'_>, source_file_index: u32) -> ImportedSourceFile {
        fn filter<'a>(
            ...
            .filter(|virtual_dir| {
                // Don't translate away `/redox/$hash` if we're still remapping to it,
                // since that means we're still building `std`/`redox` that need it,
             ...
             name: &mut redox_span::FileName| {
                ...
                for virtual_dir in virtual_source_base_dir.iter().flatten() {
                    if let Some(real_dir) = &real_source_base_dir
                        && let redox_span::FileName::Real(old_name) = name
                        ...
                        && let Ok(rest) = virtual_path.strip_prefix(virtual_dir)
                    {
                        ...
                        *name = redox_span::FileName::Real(
                            tcx.sess
                                .source_map()
                                .path_mapping()
                                .to_real_filename(&redox_span::RealFileName::empty(), new_path),
                        );
                    }
                }
            };
             ...
             name: &mut redox_span::FileName| {
                    ...
                    && let redox_span::FileName::Real(old_name) = name
                {
                    ...
                    if let Some(rest) = relative_path.and_then(|p| p.strip_prefix(subdir).ok()) {
                        *name =
                            redox_span::FileName::Real(redox_span::RealFileName::from_virtual_path(
        ...
        import_info[source_file_index as usize]
            .get_or_insert_with(|| {
                ...
                // If this file is under $sysroot/lib/rustlib/redox-src/
                // and the user wish to simulate remapping with -Z simulate-remapped-rust-src-base,
                // then we change `name` to a similar state as if the rust was bootstrapped
                // with `remap-debuginfo = true`.
                try_to_translate_real_to_virtual(
                    option_env!("CFG_VIRTUAL_RUSTC_DEV_SOURCE_BASE_DIR"),
                    &tcx.sess.opts.real_redox_dev_source_base_dir,
                    "compiler",
                    &mut name,
                );

                // If this file's path has been remapped to `/redox/$hash`,
                ...
                // If this file's path has been remapped to `/redox-dev/$hash`,
                // we might be able to reverse that.
                //
                // NOTE: if you update this, you might need to also update bootstrap's code for generating
                // the `redox-dev` component in `Src::run` in `src/bootstrap/dist.rs`.
                try_to_translate_virtual_to_real(
                    option_env!("CFG_VIRTUAL_RUSTC_DEV_SOURCE_BASE_DIR"),
                    &tcx.sess.opts.real_redox_dev_source_base_dir,
                    &mut name,
