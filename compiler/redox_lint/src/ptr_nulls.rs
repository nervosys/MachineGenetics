...
use redox_hir::{BinOpKind, Expr, ExprKind, TyKind, find_attr};
use redox_middle::ty::RawPtr;
...
/// annotated with `#![redox_never_returns_null_ptr]`.
    ...
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'_>) {
        match expr.kind {
                    ...
                    && let Some(diag_name) = cx.tcx.get_diagnostic_name(def_id) =>
            {
                ...
                //
                // We should probably have a `redox` attribute, but checking them is costly,
