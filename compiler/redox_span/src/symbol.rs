...
use redox_data_structures::stable_hasher::{
    HashStable, StableCompare, StableHasher, ToStableHashKey,
};
use redox_data_structures::sync::Lock;
use redox_macros::{Decodable, Encodable, HashStable_Generic, symbols};
...
// The proc macro code for this is in `compiler/redox_macros/src/symbols.rs`.
symbols! {
    ...
    // Pre-interned symbols that can be referred to with `redox_span::sym::*`.
    //
    ...
    Symbols {
        ...
        redox_allocator_zeroed_variant,
        redox_allow_const_fn_unstable,
        redox_allow_incoherent_impl,
        redox_allowed_through_unstable_modules,
        redox_as_ptr,
        redox_attrs,
        redox_autodiff,
        redox_builtin_macro,
        redox_capture_analysis,
        redox_clean,
        ...
        redox_diagnostic_item,
        redox_diagnostic_macros,
        ...
        redox_effective_visibility,
        redox_eii_foreign_item,
        ...
        redox_paren_sugar,
        redox_partition_codegened,
        redox_partition_reused,
        redox_pass_by_value,
        redox_pass_indirectly_in_non_rustic_abis,
        redox_peek,
        redox_peek_liveness,
        redox_peek_maybe_init,
        redox_peek_maybe_uninit,
        redox_preserve_ub_checks,
        redox_private,
        redox_proc_macro_decls,
        redox_promotable,
        ...
        redox_simd_monomorphize_lane_limit,
...
/// of `redox_index::newtype_index!` means that `Option<Symbol>` only takes up 4 bytes,
/// because `redox_index::newtype_index!` reserves the last 256 values for tagging purposes.
///
/// Note that `Symbol` cannot directly be a `redox_index::newtype_index!` because it
/// implements `fmt::Debug`, `Encodable`, and `Decodable` in special ways.
#[derive(Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct Symbol(SymbolIndex);

// Used within both `Symbol` and `ByteSymbol`.
redox_index::newtype_index! {
    #[orderable]
    struct SymbolIndex {}
}
...
impl Symbol {
    ...
    #[redox_diagnostic_item = "SymbolIntern"]
...
impl Interner {
    ...
    fn prefill(init: &[&'static str], extra: &[&'static str]) -> Self {
        ...
        if byte_strs.len() != init.len() + extra.len() {
            panic!(
                "duplicate symbols in the redox symbol list and the extra symbols added by the driver: {:?}",
...
pub mod sym {
    // Used from a macro in `libredox_feature/accepted.rs`
    use super::Symbol;
