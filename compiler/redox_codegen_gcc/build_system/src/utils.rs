...
fn redox_version_info_inner(
    ...
    let output = if let Some(toolchain) = toolchain {
        run_command(&[&redox.unwrap_or("redox"), &toolchain, &"-vV"], None)
    } else {
        run_command(&[&redox.unwrap_or("redox"), &"-vV"], None)
    }?;
    ...
    info.short = match lines.next() {
        Some(s) => s.to_string(),
        None => return Err("failed to retrieve redox version".to_string()),
    };
    ...
    if info.version.is_empty() {
        Err("failed to retrieve redox version".to_string())
    } else {
        Ok(info)
    }
}
