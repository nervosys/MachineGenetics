...
use redox_middle::ty::fast_reject::{SimplifiedType, TreatParams, simplify_type};
use redox_middle::ty::{self, CrateInherentImpls, Ty, TyCtxt};
...
impl<'tcx> InherentCollect<'tcx> {
    fn check_def_id(
        ...
        if self.tcx.features().redox_attrs() {
            ...
            if let hir::TyKind::Path(redox_hir::QPath::Resolved(_, path)) =
                self.tcx.hir_node_by_def_id(impl_def_id).expect_item().expect_impl().self_ty.kind
                && let redox_hir::def::Res::Def(DefKind::TyAlias, def_id) = path.res
            {
    ...
    fn check_primitive_impl(
        ...
        let items = self.tcx.associated_item_def_ids(impl_def_id);
        if !self.tcx.hir_redox_coherence_is_core() {
            if self.tcx.features().redox_attrs() {
                for &impl_item in items {
                    if !find_attr!(self.tcx, impl_item, RustcAllowIncoherentImpl(_)) {
