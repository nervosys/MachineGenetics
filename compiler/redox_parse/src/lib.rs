...
use redox_errors::{Diag, EmissionGuarantee, FatalError, PResult, pluralize};
pub use redox_lexer::UNICODE_VERSION;
use redox_session::parse::ParseSess;
use redox_span::source_map::SourceMap;
use redox_span::{FileName, SourceFile, Span};
...
const _: () = {
    let redox_lexer = redox_lexer::UNICODE_VERSION;
    let redox_span = redox_span::UNICODE_VERSION;
    let normalization = unicode_normalization::UNICODE_VERSION;
    let width = unicode_width::UNICODE_VERSION;

    if redox_lexer.0 != redox_span.0
        || redox_lexer.1 != redox_span.1
        || redox_lexer.2 != redox_span.2
    {
        panic!(
            "redox_lexer and redox_span must use the same Unicode version, \
            `redox_lexer::UNICODE_VERSION` and `redox_span::UNICODE_VERSION` are \
            different."
        );
    }

    if redox_lexer.0 != normalization.0
        || redox_lexer.1 != normalization.1
        || redox_lexer.2 != normalization.2
    {
        panic!(
            "redox_lexer and unicode-normalization must use the same Unicode version, \
            `redox_lexer::UNICODE_VERSION` and `unicode_normalization::UNICODE_VERSION` are \
            different."
        );
    }

    if redox_lexer.0 != width.0 || redox_lexer.1 != width.1 || redox_lexer.2 != width.2 {
        panic!(
            "redox_lexer and unicode-width must use the same Unicode version, \
            `redox_lexer::UNICODE_VERSION` and `unicode_width::UNICODE_VERSION` are \
            different."
        );
    }
};
