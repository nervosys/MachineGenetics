//! The current redox diagnostics emitter.
//!
...
//!
//! The output types are defined in `redox_session::config::ErrorOutputType`.
...
use redox_error_messages::DiagArgMap;
...
pub trait Emitter {
    ...
    fn emit_unused_externs(
        &mut self,
        _lint_level: redox_lint_defs::Level,
        _unused_externs: &[&str],
    ) {
    }

    /// Checks if should show explanations about "redox --explain"
...
// Keep the following list in sync with `redox_span::char_width`.
...
pub fn stderr_destination(color: ColorConfig) -> Destination {
    ...
    // system for emitting redox errors, so there's no need to buffer
    // anything.
    //
