...
use redox_span::sym::{self};
use redox_span::symbol::kw;
use redox_span::{DUMMY_SP, Ident, Span};
...
impl<'hir> GenericsGenerationResults<'hir> {
    pub(super) fn all_params(
        ...
        // `generics_of` in `redox_hir_analysis` will order them anyway,
    ...
    /// in `redox_hir_analysis`, as we inherit all predicates from delegation signature.
    ...
    fn generate_lifetime_predicate(
        ...
        let create_lifetime = |this: &mut Self| -> &'hir hir::Lifetime {
            this.arena.alloc(hir::Lifetime {
                hir_id: this.next_id(),
                ident: p.name.ident(),
                kind: redox_hir::LifetimeKind::Param(p.def_id),
                source: redox_hir::LifetimeSource::Path {
                    angle_brackets: redox_hir::AngleBrackets::Full,
                },
                syntax: redox_hir::LifetimeSyntax::ExplicitBound,
            })
        };
