...
#[derive(Diagnostic)]
#[diag("`redox_allowed_through_unstable_modules` attribute must be paired with a `stable` attribute", code = E0789)]
...
// FIXME(jdonszelmann) duplicated from `redox_passes`, remove once `check_attr` is integrated.
#[derive(Diagnostic)]
...
#[derive(Diagnostic)]
#[diag("element count in `redox_scalable_vector` is too large: `{$n}`")]
#[note("the value may not exceed `u16::MAX`")]
