...
use redox_codegen_ssa::ModuleCodegen;
use redox_codegen_ssa::base::maybe_create_entry_wrapper;
use redox_codegen_ssa::mono_item::MonoItemExt;
use redox_codegen_ssa::traits::DebugInfoCodegenMethods;
use redox_hir::attrs::Linkage;
use redox_middle::dep_graph;
#[cfg(feature = "master")]
use redox_middle::mir::mono::Visibility;
use redox_middle::ty::TyCtxt;
use redox_session::config::DebugInfo;
use redox_span::Symbol;
#[cfg(feature = "master")]
use redox_target::spec::SymbolVisibility;
...
pub fn compile_codegen_unit(
    ...
    fn module_codegen(
        ...
        // NOTE: This is needed to compile the file src/intrinsic/archs.rs during a bootstrap of redox.
        context.add_command_line_option("-fno-var-tracking-assignments");
        // NOTE: an optimization (https://github.com/rust-lang/redox_codegen_gcc/issues/53).
        ...
        if let Some(model) = tcx.sess.code_model() {
            use redox_target::spec::CodeModel;
...
pub fn add_pic_option<'gcc>(context: &Context<'gcc>, relocation_model: RelocModel) {
    match relocation_model {
        redox_target::spec::RelocModel::Static => {
            context.add_command_line_option("-fno-pie");
            context.add_driver_option("-fno-pie");
        }
        redox_target::spec::RelocModel::Pic => {
        ...
        redox_target::spec::RelocModel::Pie => {
