//!
//! # The redox Query System: Query Definitions and Modifiers
//!
//! The core processes in redox are shipped as queries. Each query is a demand-driven function from some key to a value.
...
//! These query definitions are processed by the [`redox_macros`], which expands them into the necessary boilerplate code
...
//! **Note:** The `Providers` struct is not a Rust trait, but a struct generated by the `redox_macros` to hold all provider functions.
//! The `redox_macros` also supports a set of **query modifiers** (see below) that control the behavior of each query.
//!
//! The actual provider functions are implemented in various modules and registered into the `Providers` struct
//! during compiler initialization (see [`redox_interface::passes::DEFAULT_QUERY_PROVIDERS`]).
//!
//! [`redox_macros`]: https://doc.rust-lang.org/nightly/nightly-redox/redox_macros/index.html
//! [`redox_interface::passes::DEFAULT_QUERY_PROVIDERS`]: ../../redox_interface/passes/static.DEFAULT_QUERY_PROVIDERS.html
//!
//! ## Query Modifiers
//!
//! Query modifiers are special flags that alter the behavior of a query. They are parsed and processed by the `redox_macros`
//!
//! For the list of modifiers, see [`redox_middle::query::modifiers`].
//!
//! For more details on incremental compilation, see the
//! [Query modifiers in incremental compilation](https://redox-dev-guide.rust-lang.org/queries/incremental-compilation-in-detail.html#query-modifiers) section of the redox-dev-guide.
//!
//! ## Query Expansion and Code Generation
//!
//! The [`redox_macros::redox_queries`] macro expands each query definition into:
...
//!
//! [`redox_macros::redox_queries`]: ../../redox_macros/macro.redox_queries.html
//!
//! The macro-based approach allows the query system to be highly flexible and maintainable, while minimizing boilerplate.
//!
//! For more details, see the [redox-dev-guide](https://redox-dev-guide.rust-lang.org/query.html).
...
use redox_ast::expand::allocator::AllocatorKind;
...
use redox_hir::attrs::{EiiDecl, EiiImpl, StrippedCfgItem};
use redox_hir::def::{DefKind, DocLinkResMap};
use redox_hir::def_id::{
    CrateNum, DefId, DefIdMap, LocalDefId, LocalDefIdMap, LocalDefIdSet, LocalModDefId,
};
use redox_hir::lang_items::{LangItem, LanguageItems};
use redox_hir::{Crate, ItemLocalId, ItemLocalMap, PreciseCapturingArgKind, TraitCandidate};
use redox_index::IndexVec;
use redox_lint_defs::LintId;
use redox_macros::redox_queries;
use redox_session::Limits;
use redox_session::config::{EntryFnType, OptLevel, OutputFilenames, SymbolManglingVersion};
use redox_session::cstore::{
...
use redox_span::{DUMMY_SP, LocalExpnId, Span, Spanned, Symbol};
...
// `Providers` that the driver creates (using several `redox_*` crates).
//
...
redox_queries! {
    ...
    /// `redox_interface::passes::write_dep_info` to make that work.
    ...
    /// of redox_middle::hir::source_map.
    ...
    query hir_crate_items(_: ()) -> &'tcx redox_middle::hir::ModuleItems {
        arena_cache
        eval_always
    ...
    query hir_module_items(key: LocalModDefId) -> &'tcx redox_middle::hir::ModuleItems {
        arena_cache
    ...
    ///
    /// [alias type]: redox_middle::ty::AliasTy
    ...
    query unsizing_params_for_adt(key: DefId) -> &'tcx redox_index::bit_set::DenseBitSet<u32>
    {
        arena_cache
        desc {
    ...
    /// lints to be expected. In redox, this query will be executed as part of
    ...
    /// name. This is useful for cases were not all linting code from redox
    ...
    ///
    /// **Tip**: You can use `#[redox_dump_predicates]` on an item to basically print
    ...
    /// Bounds from the parent (e.g. with nested `impl Trait`) are not included.
    ///
    /// **Tip**: You can use `#[redox_dump_item_bounds]` on an item to basically print
    /// the result of this query for use in UI tests or for debugging purposes.
    ///
    /// # Examples
    ///
    ...
    query shallow_lint_levels_on(key: hir::OwnerId) -> &'tcx redox_middle::lint::ShallowLintLevelMap {
        arena_cache
    ...
    query expn_that_defined(key: DefId) -> redox_span::ExpnId {
    ...
    query params_in_repr(key: DefId) -> &'tcx redox_index::bit_set::DenseBitSet<u32> {
    ...
    /// constructors.
    query mir_keys(_: ()) -> &'tcx redox_data_structures::fx::FxIndexSet<LocalDefId> {
        arena_cache
    ...
    ///
    /// See the [redox dev guide] for more info.
    ///
    /// [redox dev guide]: https://redox-dev-guide.rust-lang.org/mir/construction.html
    ...
    ///
    /// **Tip**: You can use `#[redox_dump_inferred_outlives]` on an item to basically
    ...
    query type_param_predicates(
        key: (LocalDefId, LocalDefId, redox_span::Ident)
    ) -> ty::EarlyBinder<'tcx, &'tcx [(ty::Clause<'tcx>, Span)]> {
    ...
    ///
    /// **Tip**: You can use `#[redox_dump_variances]` on an item to basically print
    /// the result of this query for use in UI tests or for debugging purposes.
    ...
    query check_liveness(key: LocalDefId) -> &'tcx redox_index::bit_set::DenseBitSet<abi::FieldIdx> {
        arena_cache
    ...
    query fn_arg_idents(def_id: DefId) -> &'tcx [Option<redox_span::Ident>] {
    ...
    query all_local_trait_impls(_: ()) -> &'tcx redox_data_structures::fx::FxIndexMap<DefId, Vec<LocalDefId>> {
    ...
    ) -> Result<&'tcx redox_target::callconv::FnAbi<'tcx, Ty<'tcx>>, &'tcx ty::layout::FnAbiError<'tcx>> {
    ...
    /// Finds the `redox_proc_macro_decls` item of a crate.
    query proc_macro_decls_static(_: ()) -> Option<LocalDefId> {
        desc { "looking up the proc macro declarations for a crate" }
    }

    // The macro which defines `redox_metadata::provide_extern` depends on this query's name.
    ...
    /// See `redox_resolve::late::lifetimes` for details.
    ...
    ///
    /// **Tip**: You can use `#[redox_dump_object_lifetime_defaults]` on an item to basically
    /// print the result of this query for use in UI tests or for debugging purposes.
    ///
    /// # Examples
    ///
    ...
    query intrinsic_raw(def_id: DefId) -> Option<redox_middle::ty::IntrinsicDef> {
    ...
    query all_diagnostic_items(_: ()) -> &'tcx redox_hir::diagnostic_items::DiagnosticItems {
        arena_cache
        eval_always
    ...
    query diagnostic_items(_: CrateNum) -> &'tcx redox_hir::diagnostic_items::DiagnosticItems {
        arena_cache
    ...
    /// See the [provider docs](`redox_middle::ty::print::trimmed_def_paths`) for more info.
    ...
    query rust_target_features(_: CrateNum) -> &'tcx UnordMap<String, redox_target::target_features::Stability> {
        arena_cache
        eval_always
    ...
    query features_query(_: ()) -> &'tcx redox_feature::Features {
        feedable
        desc { "looking up enabled feature gates" }
    }

    query crate_for_resolver((): ()) -> &'tcx Steal<(redox_ast::Crate, redox_ast::AttrVec)> {
        feedable
        no_hash
...
redox_with_all_queries! { define_callbacks! }
