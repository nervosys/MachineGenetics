...
use redox_codegen_ssa::mir::debuginfo::{DebugScope, FunctionDebugContext};
use redox_codegen_ssa::traits::*;
use redox_data_structures::fx::FxHashMap;
use redox_index::bit_set::DenseBitSet;
use redox_middle::mir::{Body, SourceScope};
use redox_middle::ty::layout::{FnAbiOf, HasTypingEnv};
use redox_middle::ty::{self, Instance};
use redox_session::config::DebugInfo;
use redox_span::{BytePos, DUMMY_SP, hygiene};
...
// FIXME(eddyb) almost all of this should be in `redox_codegen_ssa::mir::debuginfo`.
...
fn make_mir_scope<'ll, 'tcx>(
    ...
    let dbg_scope = match scope_data.inlined {
        Some((callee, _)) => {
            ...
            // if this is moved to `redox_codegen_ssa::mir::debuginfo`.
