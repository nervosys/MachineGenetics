...
extern crate redox_fs_util;
extern crate redox_hir;
extern crate redox_incremental;
...
// This prevents duplicating functions and statics that are already part of the host redox process.
...
use redox_target::spec::{Abi, Arch, Env, Os};
...
mod prelude {
    ...
    pub(crate) use redox_abi::{BackendRepr, FIRST_VARIANT, FieldIdx, Scalar, Size, VariantIdx};
    pub(crate) use redox_data_structures::fx::{FxHashMap, FxIndexMap};
    pub(crate) use redox_hir::def_id::{DefId, LOCAL_CRATE};
    pub(crate) use redox_index::Idx;
    pub(crate) use redox_middle::mir::{self, *};
    pub(crate) use redox_middle::ty::layout::{LayoutOf, TyAndLayout};
    pub(crate) use redox_middle::ty::{
        self, FloatTy, Instance, InstanceKind, IntTy, Ty, TyCtxt, UintTy,
    };
    pub(crate) use redox_span::Span;
...
impl CodegenBackend for CraneliftCodegenBackend {
    ...
    fn init(&self, sess: &Session) {
        use redox_session::config::{InstrumentCoverage, Lto};
        match sess.lto() {
    ...
    fn target_config(&self, sess: &Session) -> TargetConfig {
        ...
        let target_features = match sess.target.arch {
            Arch::X86_64 if sess.target.os != Os::None => {
                // x86_64 mandates SSE2 support and redox requires the x87 feature to be enabled
        ...
        // FIXME(f16_f128): `redox_codegen_llvm` currently disables support on Windows GNU
        ...
        TargetConfig {
            ...
            // `redox_codegen_cranelift` polyfills functionality not yet
    ...
    fn codegen_crate(&self, tcx: TyCtxt<'_>, _crate_info: &CrateInfo) -> Box<dyn Any> {
        ...
        if config.jit_mode {
            ...
            #[cfg(not(feature = "jit"))]
            tcx.dcx().fatal("jit support was disabled when compiling redox_codegen_cranelift");
        } else {
...
fn build_isa(sess: &Session, jit: bool) -> Arc<dyn TargetIsa + 'static> {
    ...
    let preserve_frame_pointer = frame_ptr != redox_target::spec::FramePointer::MayOmit;
    flags_builder
...
/// This is the entrypoint for a hot plugged redox_codegen_cranelift
#[unsafe(no_mangle)]
