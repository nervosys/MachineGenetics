...
/// See `redox_middle::query::modifiers` for documentation of each query modifier.
...
/// functions, to be used by the final output of `redox_queries!`.
///
...
fn add_to_analyzer_stream(query: &Query, analyzer_stream: &mut proc_macro2::TokenStream) {
    ...
    // This dead code exists to instruct rust-analyzer about the link between the `redox_queries`
    ...
    // `redox_queries` emit a field access with the given name's span which allows it to
...
pub(super) fn redox_queries(input: TokenStream) -> TokenStream {
    ...
    TokenStream::from(quote! {
        ...
        /// expanded in `redox_middle::queries`, and makes this macro-generated
        ...
        // `redox_query_impl`, where the functions are actually used?
