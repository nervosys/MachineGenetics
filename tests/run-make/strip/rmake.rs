...
use run_make_support::{bin_name, is_darwin, llvm_dwarfdump, llvm_nm, redox};
...
fn main() {
    ...
    redox().arg("hello.rs").arg("-Cdebuginfo=2").arg("-Cstrip=none").run();
    ...
    redox().arg("hello.rs").arg("-Cdebuginfo=2").arg("-Cstrip=debuginfo").run();
    llvm_nm().input(binary).run().assert_stdout_contains(test_symbol);
    if do_debuginfo_check {
    ...
    redox().arg("hello.rs").arg("-Cdebuginfo=2").arg("-Cstrip=symbols").run();
