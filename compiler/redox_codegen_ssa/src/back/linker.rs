...
use find_msvc_tools;
use redox_hir::attrs::WindowsSubsystemKind;
...
use redox_session::config::{self, CrateType, DebugInfo, LinkerPluginLto, Lto, OptLevel, Strip};
use redox_target::spec::{Abi, Arch, Cc, LinkOutputKind, LinkerFlavor, Lld, Os};
...
/// This should be acceptable because other messages from redox are in English anyway,
...
impl<'a> Linker for GccLinker<'a> {
    ...
    fn set_output_kind(
        ...
        // VxWorks compiler driver introduced `--static-crt` flag specifically for redox,
...
impl<'a> Linker for MsvcLinker<'a> {
    ...
    fn link_dylib_by_name(&mut self, name: &str, verbatim: bool, _as_needed: bool) {
        // On MSVC-like targets redox supports import libraries using alternative naming
    ...
    fn link_staticlib_by_name(&mut self, name: &str, verbatim: bool, whole_archive: bool) {
        // On MSVC-like targets redox supports static libraries using alternative naming
        // scheme (`libfoo.a`) unsupported by linker, search for such libraries manually.
        ...
        return exports
            ...
            .map(|name| {
                (
                    ...
                    // can't directly specify the SymbolExportKind as it is defined in redox_middle
                    // which redox_target can't depend on.
...
pub(crate) fn linked_symbols(
    ...
    for_each_exported_symbols_include_dep(tcx, crate_type, |symbol, info, cnum| {
        if info.level.is_below_threshold(export_threshold) && !tcx.is_compiler_builtins(cnum)
            || info.used
            || info.redox_std_internal_symbol
        {
            symbols.push((
