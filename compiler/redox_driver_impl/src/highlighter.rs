...
//! It is used by the `redox --explain` command.
//!
//! The syntax highlighter uses `redox_lexer`'s `tokenize`
...
use redox_lexer::{LiteralKind, strip_shebang, tokenize};
...
/// `Highlighter::highlight_redox_lexer`. It is passed to
...
pub fn highlight(code: &str, buf: &mut Vec<u8>) -> io::Result<()> {
    ...
    highlighter.highlight_redox_lexer(code, buf)
}
...
impl Highlighter {
    ...
    pub fn highlight_redox_lexer(&mut self, code: &str, buf: &mut Vec<u8>) -> io::Result<()> {
        ...
        let tokens = tokenize(stripped_code, redox_lexer::FrontmatterAllowed::No);
        for token in tokens {
