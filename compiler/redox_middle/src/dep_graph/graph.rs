...
use redox_data_structures::fingerprint::{Fingerprint, PackedFingerprint};
use redox_data_structures::fx::{FxHashMap, FxHashSet};
use redox_data_structures::outline;
use redox_data_structures::profiling::QueryInvocationId;
use redox_data_structures::sharded::{self, ShardedHashMap};
use redox_data_structures::stable_hasher::{HashStable, StableHasher};
use redox_data_structures::sync::{AtomicU64, Lock, is_dyn_thread_safe};
...
use redox_serialize::opaque::{FileEncodeResult, FileEncoder};
...
redox_data_structures::static_assert_size!(Option<DepNodeIndex>, 4);
...
impl DepGraphData {
    ...
    /// dep-graph -- see the [redox dev guide] for more details on
    /// the dep-graph).
    ///
    ...
    /// needed.
    ///
    /// [redox dev guide]: https://redox-dev-guide.rust-lang.org/queries/incremental-compilation.html
    #[inline(always)]
    pub fn with_task<'tcx, A: Debug, R>(
        &self,
        dep_node: DepNode,
