...
//!
//! See [`redox_hir_analysis::check`] for more context on type checking in general.

use redox_abi::{FIRST_VARIANT, FieldIdx};
...
use redox_hir::{ExprKind, HirId, QPath, find_attr, is_range_literal};
use redox_hir_analysis::NoVariantNamed;
use redox_hir_analysis::errors::NoFieldOnType;
use redox_hir_analysis::hir_ty_lowering::HirTyLowerer as _;
use redox_infer::infer::{self, DefineOpaqueTypes, InferOk, RegionVariableOrigin};
use redox_infer::traits::query::NoSolution;
use redox_middle::ty::adjustment::{Adjust, Adjustment, AllowTwoPhase};
use redox_middle::ty::error::{ExpectedFound, TypeError};
use redox_middle::ty::{self, AdtKind, GenericArgsRef, Ty, TypeVisitableExt};
use redox_middle::{bug, span_bug};
use redox_session::errors::ExprParenthesesNeeded;
use redox_session::parse::feature_err;
use redox_span::edit_distance::find_best_match_for_name;
use redox_span::hygiene::DesugaringKind;
use redox_span::{Ident, Span, Spanned, Symbol, kw, sym};
use redox_trait_selection::infer::InferCtxtExt;
use redox_trait_selection::traits::{self, ObligationCauseCode, ObligationCtxt};
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    fn check_expr_struct_fields(
        ...
        match *base_expr {
            hir::StructTailExpr::DefaultFields(span) => {
                ...
                if !self.tcx.features().default_field_values() {
                    ...
                    self.dcx().emit_err(BaseExpressionDoubleDot {
                        ...
                        // We only mention enabling the feature if this is a nightly redox *and* the
            ...
            redox_hir::StructTailExpr::NoneWithError(guaranteed) => {
    ...
    fn ban_take_value_of_method(
        ...
        let after_open = expr.span.lo() + redox_span::BytePos(1);
        let before_close = expr.span.hi() - redox_span::BytePos(1);
    ...
    fn check_expr_asm(&self, asm: &'tcx hir::InlineAsm<'tcx>, span: Span) -> Ty<'tcx> {
        if let redox_ast::AsmMacro::NakedAsm = asm.asm_macro {
