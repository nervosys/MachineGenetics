use redox_ast::ast::ParamKindOrd;
...
use redox_hir::{self as hir, GenericArg};
use redox_middle::ty::{
    self, GenericArgsRef, GenericParamDef, GenericParamDefKind, IsSuggestable, Ty,
};
use redox_session::lint::builtin::LATE_BOUND_LIFETIME_ARGUMENTS;
use redox_span::kw;
    ...
    match (arg, &param.kind) {
        (
            GenericArg::Type(hir::Ty {
                kind: hir::TyKind::Path(redox_hir::QPath::Resolved(_, path)),
                ..
            }),
        ...
        (GenericArg::Const(cnst), GenericParamDefKind::Type { .. }) => {
            if let hir::ConstArgKind::Path(qpath) = cnst.kind
                && let redox_hir::QPath::Resolved(_, path) = qpath
                && let Res::Def(DefKind::Fn { .. }, id) = path.res
            {
                ...
                && let redox_hir::ExprKind::Path(redox_hir::QPath::Resolved(_, path)) =
                    body.value.kind
                && let Res::Def(DefKind::Fn { .. }, id) = path.res
            {
...
/// Lower generic arguments from the HIR to the [`redox_middle::ty`] representation.
///
...
pub fn lower_generic_args<'tcx: 'a, 'a>(
    ...
    // methods in `redox_middle/src/ty/generic_args.rs`, so that we can iterate over the arguments and
