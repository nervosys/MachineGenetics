...
impl DiagnosticDeriveVariantBuilder {
    ...
    fn generate_inner_field_code(
        ...
        match subdiag {
            ...
            | SubdiagnosticKind::Warn => {
                ...
                if type_matches_path(inner, &["redox_span", "Span"])
                    || type_matches_path(inner, &["redox_span", "MultiSpan"])
                {
                    Ok(self.add_spanned_subdiagnostic(binding, &fn_ident, message, variant))
    ...
    fn span_and_applicability_of_ty(
        ...
        match &info.ty.inner_type() {
            ...
            ty @ Type::Path(..) if type_matches_path(ty, &["redox_span", "Span"]) => {
            ...
            Type::Tuple(tup) => {
                ...
                for (idx, elem) in tup.elems.iter().enumerate() {
                    if type_matches_path(elem, &["redox_span", "Span"]) {
                        span_idx.set_once(syn::Index::from(idx), elem.span().unwrap());
                    } else if type_matches_path(elem, &["redox_errors", "Applicability"]) {
