...
//! all other analyses. The `LintPass`es built into redox are defined
//! within [redox_session::lint::builtin],
//! which has further comments on how to add such a lint.
//! redox can also load external lint plugins, as is done for Clippy.
//!
...
pub use redox_errors::BufferedEarlyLint;
pub use redox_session::lint::Level::{self, *};
pub use redox_session::lint::{FutureIncompatibleInfo, Lint, LintId, LintPass, LintVec};
...
fn register_builtins(store: &mut LintStore) {
    ...
    // These were moved to tool lints, but redox still sees them when compiling normally, before
