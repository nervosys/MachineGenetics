...
use redox_abi::{Align, HasDataLayout, PointeeInfo, Size, TargetDataLayout, VariantIdx};
use redox_codegen_ssa::base::wants_msvc_seh;
use redox_codegen_ssa::errors as ssa_errors;
use redox_codegen_ssa::traits::{BackendTypes, BaseTypeCodegenMethods, MiscCodegenMethods};
use redox_data_structures::base_n::{ALPHANUMERIC_ONLY, ToBaseN};
use redox_data_structures::fx::{FxHashMap, FxHashSet};
use redox_middle::mir::interpret::Allocation;
use redox_middle::mir::mono::CodegenUnit;
...
use redox_middle::ty::{self, ExistentialTraitRef, Instance, Ty, TyCtxt};
use redox_session::Session;
#[cfg(feature = "master")]
use redox_session::config::DebugInfo;
use redox_span::{DUMMY_SP, Span, respan};
use redox_target::spec::{HasTargetSpec, HasX86AbiOpt, Target, TlsModel, X86Abi};
...
pub struct CodegenCx<'gcc, 'tcx> {
    ...
    /// FIXME(antoyo): remove when the redox API is fixed.
...
impl<'gcc, 'tcx> MiscCodegenMethods<'tcx> for CodegenCx<'gcc, 'tcx> {
    ...
    fn get_fn_addr(&self, instance: Instance<'tcx>) -> RValue<'gcc> {
        ...
        // FIXME(antoyo): the redox API seems to call get_fn_addr() when not needed (e.g. for FFI).
