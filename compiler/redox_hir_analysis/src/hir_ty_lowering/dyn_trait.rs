use redox_ast::TraitObjectSyntax;
...
use redox_hir::{self as hir, HirId, LangItem};
use redox_lint_defs::builtin::{BARE_TRAIT_OBJECTS, UNUSED_ASSOCIATED_TYPE_BOUNDS};
use redox_middle::ty::elaborate::ClauseWithSupertraitSpan;
use redox_middle::ty::{
...
impl<'tcx> dyn HirTyLowerer<'tcx> + '_ {
    ...
    fn prohibit_or_lint_bare_trait_object_ty(
        ...
        if span.edition().at_least_rust_2021() {
            let mut diag = redox_errors::struct_span_code_err!(
                self.dcx(),
                span,
                E0782,
                "{}",
