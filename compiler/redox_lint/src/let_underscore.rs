...
const SYNC_GUARD_SYMBOLS: [Symbol; 3] = [
    redox_span::sym::MutexGuard,
    redox_span::sym::RwLockReadGuard,
    redox_span::sym::RwLockWriteGuard,
];

impl<'tcx> LateLintPass<'tcx> for LetUnderscore {
    fn check_local(&mut self, cx: &LateContext<'_>, local: &hir::LetStmt<'_>) {
        if matches!(local.source, redox_hir::LocalSource::AsyncFn) {
            return;
        }
        ...
        local.pat.walk_always(|pat| {
            ...
            let sub = NonBindingLetSub {
                ...
                is_assign_desugar: matches!(local.source, redox_hir::LocalSource::AssignDesugar),
            };
            if is_sync_lock {
