...
use redox_middle::mir::{self, *};
use redox_middle::thir::{AdtExpr, AdtExprBase, ArmId, ExprId, ExprKind};
use redox_middle::ty::{self, Ty, TyCtxt, TypeVisitableExt, ValTree};
use redox_middle::{bug, span_bug};
use redox_pattern_analysis::redox::RustcPatCtxt;
use redox_session::lint::Level;
use redox_span::{DUMMY_SP, Span, Spanned};
...
impl<'a, 'tcx> Builder<'a, 'tcx> {
    ...
    pub(crate) fn break_const_continuable_scope(
        ...
        let redox_middle::thir::ExprKind::Scope { value, .. } = self.thir[value].kind else {
        ...
        let dropless_arena = redox_arena::DroplessArena::default();
        ...
        let cx = RustcPatCtxt {
            ...
            typing_env: redox_middle::ty::TypingEnv::non_body_analysis(self.tcx, self.def_id),
            dropless_arena: &dropless_arena,
            match_lint_level: self.hir_id,
            whole_match_span: Some(redox_span::Span::default()),
            scrut_span: redox_span::Span::default(),
            refutable: true,
    ...
    pub(crate) fn maybe_new_source_scope(
        ...
        let (current_root, parent_root) =
            if self.tcx.sess.opts.unstable_opts.maximal_hir_to_mir_coverage {
                // Some consumers of redox need to map MIR locations back to HIR nodes. Currently
                // the only part of redox that tracks MIR -> HIR is the
