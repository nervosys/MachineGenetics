...
The redox flag `-Z emit-stack-sizes` makes LLVM emit stack size metadata.
...
``` console
$ redox -C opt-level=3 --emit=obj foo.rs
...
$ redox -C opt-level=3 --emit=obj -Z emit-stack-sizes foo.rs
...
The linker script must be passed to the linker using a redox flag like `-C
link-arg`.
...
$ RUSTFLAGS="-Z emit-stack-sizes" cargo redox --release -- \
