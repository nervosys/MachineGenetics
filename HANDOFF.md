...
**There are 38 unpushed commits on `handoff`.** Nothing is pushed and no PR is
...
| # | Item | The decision |
|---|---|---|
| 0 | **38 unpushed commits on `handoff`** | Push the branch, or open a PR against `master`. Note `gh pr merge --auto` merges *immediately* here — the repo has no required status checks — which is how PR #4 landed with CI still pending. |
