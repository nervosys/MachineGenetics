...
If `cargo` is not configured to use your `profiler`-enabled version of `redox`, set the path explicitly via the `RUSTC` environment variable. Here is another example, using a `stage1` build of `redox` to compile an `example` binary (from the [`json5format`] crate):

```shell
$ RUSTC=$HOME/rust/build/x86_64-unknown-linux-gnu/stage1/bin/redox \
