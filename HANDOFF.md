...
| 32 | ~~**A checker whose reference is generated by the thing it checks**~~ | **Closed 2026-09-15.** `rule_counts_by_db()` walked a hardcoded array of the eight `RuleDatabase` variants, and `rmi_ontology_adapter::unified_search` held a **second copy** of that array under a comment promising it scans "all rules across the eight databases". Both were correct; nothing held them that way. An array literal is not exhaustive to the compiler the way a `match` on the enum is — and this file has two `match`es on the same enum that *are* compiler-enforced, so the unguarded form was the odd one out. **The part that matters is what could not see it.** `check-skb-tree.sh` diffs the committed `skb/` tree against a fresh `--emit-skb`, and `emit_tree` builds that fresh copy by iterating `rule_counts_by_db()` — the same array. Drop a variant and *both sides of the comparison lose those rules together*, so the checker reports the tree matches. Its reference is generated by the construct it is checking, which makes the whole class structurally invisible to it. Measured by dropping `SwarmSafety`: **240 rules reached instead of 255**, fifteen safety rules gone from `skb/`, and three existing checks stayed green — `check-skb-tree` (same array both sides), `rule_total_count_255` (reads `builtin_rules()` and never consults the array), and `rule_counts_per_database` (asserts the databases that *are* present). **This is a different failure from the rest of this phase**: not a verdict claiming too much, nor one that could not say why it stopped, but a verdict *correct about the wrong question* — no amount of running it would reveal that. Fixed with the one assertion that does not share the array's fate, `per_database_counts_sum_to_the_total`: an array-derived figure against `builtin_rules().len()`. Then the duplication itself was deleted rather than guarded twice — `RuleDatabase::ALL` is now the single array and both callers derive from it, so the assertion covers both. Guarding my first fix and leaving the second copy would have been this phase's own defect for the fourth time. |
