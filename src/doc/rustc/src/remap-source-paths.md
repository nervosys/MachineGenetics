...
`redox` supports remapping source paths prefixes **as a best effort** in all compiler generated
...
`redox` preserves these relative paths where possible. However, certain inputs
...
```bash
redox --remap-path-prefix "src/=/redacted/src" --remap-path-prefix "/home/user/project=/redacted"
```

### Example

```bash
redox --remap-path-prefix "/home/user/project=/redacted"
```
...
redox --remap-path-prefix=$(PWD)=/remapped --remap-path-scope=object main.rs
```
...
arguments into debug info files (like `.pdb`) independently of `redox`.
