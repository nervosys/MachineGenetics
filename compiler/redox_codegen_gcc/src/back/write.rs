...
use redox_codegen_ssa::back::write::{BitcodeSection, CodegenContext, EmitObj, ModuleConfig};
use redox_codegen_ssa::{CompiledModule, ModuleCodegen};
...
use redox_session::config::OutputType;
use redox_target::spec::SplitDebuginfo;
    ...
    let _timer = prof.generic_activity_with_arg("GCC_module_codegen", &*module.name);
    {
        ...
        if config.bitcode_needed() {
            ...
            if config.emit_obj == EmitObj::ObjectCode(BitcodeSection::Full) {
                ...
                // FIXME(antoyo): Send -plugin/usr/lib/gcc/x86_64-pc-linux-gnu/11.1.0/liblto_plugin.so to linker (this should be done when specifying the appropriate redox cli argument).
                context
                    .compile_to_file(OutputKind::ObjectFile, bc_out.to_str().expect("path to str"));
            }
        }
