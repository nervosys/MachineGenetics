...
use redox_ast::expand::allocator::{ALLOC_ERROR_HANDLER, AllocatorKind, global_fn_name};
...
use redox_data_structures::sync::{self, FreezeReadGuard, FreezeWriteGuard};
use redox_data_structures::unord::UnordMap;
use redox_expand::base::SyntaxExtension;
use redox_fs_util::try_canonicalize;
use redox_hir as hir;
use redox_hir::def_id::{CrateNum, LOCAL_CRATE, LocalDefId, StableCrateId};
use redox_hir::definitions::Definitions;
use redox_index::IndexVec;
use redox_middle::bug;
use redox_middle::ty::data_structures::IndexSet;
use redox_middle::ty::{TyCtxt, TyCtxtFeed};
...
use redox_session::cstore::{CrateDepKind, CrateSource, ExternCrate, ExternCrateSource};
use redox_session::lint::{self, BuiltinLintDiag};
use redox_session::output::validate_crate_name;
...
use redox_target::spec::{PanicStrategy, Target};
...
impl CStore {
    ...
    pub fn report_unused_deps(&self, tcx: TyCtxt<'_>) {
        ...
        let level = tcx
            .lint_level_at_node(lint::builtin::UNUSED_CRATE_DEPENDENCIES, redox_hir::CRATE_HIR_ID)
            .level;
    ...
    impl<'ast> visit::Visitor<'ast> for Finder {
        fn visit_item(&mut self, item: &'ast ast::Item) {
            if let Some(ident) = item.kind.ident()
                ...
                && attr::contains_name(&item.attrs, sym::redox_std_internal_symbol)
            {
