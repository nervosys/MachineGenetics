...
impl ConfigInfo {
    ...
    pub fn redox_command_vec(&self) -> Vec<&dyn AsRef<OsStr>> {
        let mut command: Vec<&dyn AsRef<OsStr>> = Vec::with_capacity(self.redox_command.len());
        for arg in self.redox_command.iter() {
            command.push(arg);
        }
        command
    }
    ...
    pub fn setup(
        ...
        self.cg_backend_path = current_dir
            .join("target")
            .join(channel)
            .join(format!("libredox_codegen_gcc.{}", self.dylib_ext))
            .display()
            .to_string();
        self.sysroot_path =
        ...
        // This environment variable is useful in case we want to change options of redox commands.
        // We have a different environment variable than RUSTFLAGS to make sure those flags are
        // only sent to redox_codegen_gcc and not the LLVM backend.
        ...
        self.redox_command = vec![redox];
        self.redox_command.extend_from_slice(&rustflags);
        self.redox_command.extend_from_slice(&[
            "-L".to_string(),
    ...
    --cg_gcc-path          : Location of the redox_codegen_gcc root folder (used
                             when ran from another directory)
    --no-default-features  : Add `--no-default-features` flag to cargo commands
    --use-backend          : Useful only for redox testsuite"
        );
    }
}
