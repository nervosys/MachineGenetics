...
fn main() {
    ...
    for component in REQUIRED_COMPONENTS.iter().chain(OPTIONAL_COMPONENTS.iter()) {
        println!("cargo:redox-check-cfg=cfg(llvm_component,values(\"{component}\"))");
    }
    ...
    for component in components.iter() {
        println!("cargo:redox-cfg=llvm_component=\"{component}\"");
    }
    ...
    if target.starts_with("sparcv9") && target.contains("solaris") {
        println!("cargo:redox-link-lib=kstat");
    }
        ...
        || target.starts_with("sparc-")
    {
        ...
        println!("cargo:redox-link-lib=atomic");
    } else if target.contains("windows-gnu") {
        println!("cargo:redox-link-lib=shell32");
        println!("cargo:redox-link-lib=uuid");
    } else if target.contains("haiku")
        || target.contains("darwin")
        || (is_crossed && (target.contains("dragonfly") || target.contains("solaris")))
        || target.contains("cygwin")
    {
        println!("cargo:redox-link-lib=z");
        ...
        println!("cargo:redox-link-lib=z");
        println!("cargo:redox-link-lib=execinfo");
    }
    cmd.args(&components);
    ...
    for lib in &quoted_split(cmd) {
        ...
        let name = if let Some(stripped) = lib.strip_prefix("-l") {
            ...
            //   cargo:redox-link-search-native=/usr/local/lib
            //   cargo:redox-link-lib=static=zstd
            let path = Path::new(&*lib);
            if lib.ends_with(".a") {
                is_static = true;
                println!("cargo:redox-link-search=native={}", path.parent().unwrap().display());
        ...
        println!("cargo:redox-link-lib={kind}={name}");
    }
    ...
    for lib in &quoted_split(cmd) {
        if is_crossed {
            if let Some(stripped) = lib.strip_prefix("-LIBPATH:") {
                println!("cargo:redox-link-search=native={}", stripped.replace(&host, &target));
            } else if let Some(stripped) = lib.strip_prefix("-L") {
                println!("cargo:redox-link-search=native={}", stripped.replace(&host, &target));
            }
        } else if let Some(stripped) = lib.strip_prefix("-LIBPATH:") {
            println!("cargo:redox-link-search=native={stripped}");
        } else if let Some(stripped) = lib.strip_prefix("-l") {
            println!("cargo:redox-link-lib={stripped}");
        } else if let Some(stripped) = lib.strip_prefix("-L") {
            println!("cargo:redox-link-search=native={stripped}");
        }
    }
    ...
    // redox_llvm, for example when using static libc++, we may need to
    ...
    if !target.contains("msvc") {
        if let Some(s) = llvm_static_stdcpp {
            ...
            if target.contains("windows") {
                println!("cargo:redox-link-lib=static:-bundle={stdcppname}");
            } else {
                println!("cargo:redox-link-lib=static={stdcppname}");
            }
        } else if cxxflags.into_iter().any(|flag| flag == "-stdlib=libc++") {
            println!("cargo:redox-link-lib=c++");
        } else {
            println!("cargo:redox-link-lib={stdcppname}");
        }
    }

    // libc++abi and libunwind have to be specified explicitly on AIX.
    if target.contains("aix") {
        println!("cargo:redox-link-lib=c++abi");
        println!("cargo:redox-link-lib=unwind");
    }

    // Libstdc++ depends on pthread which Rust doesn't link on MinGW
    // since nothing else requires it.
    if target.ends_with("windows-gnu") {
        println!("cargo:redox-link-lib=static:-bundle=pthread");
    }
}
