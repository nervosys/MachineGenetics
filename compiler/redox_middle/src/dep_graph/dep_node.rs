...
//!
//! `DepNode` definition happens in `redox_middle` with the
...
//!
//! [dependency graph]: https://redox-dev-guide.rust-lang.org/query.html
...
// Note: `$K` and `$V` are unused but present so this can be called by `redox_with_all_queries`.
...
crate::queries::redox_with_all_queries! { define_dep_nodes! }
