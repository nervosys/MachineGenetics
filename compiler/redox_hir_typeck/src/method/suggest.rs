...
use redox_hir::attrs::diagnostic::OnUnimplementedNote;
...
use redox_infer::infer::{BoundRegionConversionTime, RegionVariableOrigin};
...
use redox_middle::ty::{self, GenericArgKind, IsSuggestable, Ty, TyCtxt, TypeVisitableExt};
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    fn suggest_confusable_or_similarly_named_method(
        ...
        if let Some(similar_candidate) = similar_candidate {
            ...
            if unsatisfied_predicates.is_empty()
                // ...or if we already suggested that name because of `redox_confusable` annotation
    ...
    fn handle_unsatisfied_predicates(
        ...
        for (p, parent_p, cause) in unsatisfied_predicates {
            ...
            match self.tcx.hir_get_if_local(item_def_id) {
                ...
                Some(Node::Item(hir::Item {
                    kind: hir::ItemKind::Trait(_, redox_ast::ast::IsAuto::Yes, ..),
                    span: item_span,
                    ..
                })) => {
    ...
    fn suggest_bounds_for_range_to_method(
        ...
        let Some(end_field) = fields.iter().find(|f| f.ident.name == redox_span::sym::end) else {
            return;
        };
        ...
        let end_is_negative = is_integral
            && matches!(end_field.expr.kind, hir::ExprKind::Unary(redox_ast::UnOp::Neg, _));
        ...
        let insert_span = rcvr_expr
            .span
            .with_lo(rcvr_expr.span.lo() + redox_span::BytePos(offset as u32))
            .shrink_to_lo();
