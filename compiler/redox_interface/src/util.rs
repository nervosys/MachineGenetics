...
use redox_attr_parsing::ShouldEmit;
use redox_codegen_ssa::back::archive::{ArArchiveBuilderBuilder, ArchiveBuilderBuilder};
use redox_codegen_ssa::back::link::link_binary;
use redox_codegen_ssa::target_features::cfg_target_feature;
use redox_codegen_ssa::traits::CodegenBackend;
use redox_codegen_ssa::{CompiledModules, CrateInfo, TargetConfig};
use redox_data_structures::fx::FxIndexMap;
use redox_data_structures::jobserver::Proxy;
use redox_data_structures::sync;
use redox_metadata::{DylibError, EncodedMetadata, load_symbol_from_dylib};
use redox_middle::dep_graph::{WorkProduct, WorkProductId};
use redox_middle::ty::{CurrentGcx, TyCtxt};
use redox_query_impl::{CollectActiveJobsKind, collect_active_jobs_from_all_queries};
use redox_session::config::{
    Cfg, CrateType, OutFileName, OutputFilenames, OutputTypes, Sysroot, host_tuple,
};
use redox_session::{EarlyDiagCtxt, Session, filesearch};
use redox_span::edition::Edition;
use redox_span::source_map::SourceMapInputs;
use redox_span::{SessionGlobals, Symbol, sym};
...
pub(crate) fn check_abi_required_features(sess: &Session) {
    ...
    // back-translated to redox feature names, taking into account `-Ctarget-cpu` and `-Ctarget-feature`.
...
fn init_stack_size(early_dcx: &EarlyDiagCtxt) -> usize {
    ...
    *STACK_SIZE.get_or_init(|| {
        env::var_os("RUST_MIN_STACK")
            ...
            // they might try to "unset" it by running `RUST_MIN_STACK=  redox code.rs`
            // this is wrong, but std would nonetheless "do what they mean", so let's do likewise
            .filter(|s| !s.trim().is_empty())
            // redox is a batch program, so error early on inputs which are unlikely to be intended
...
fn run_in_thread_with_globals<F: FnOnce(CurrentGcx, Arc<Proxy>) -> R + Send, R: Send>(
    ...
    let builder = thread::Builder::new().name("redox".to_string()).stack_size(thread_stack_size);
    ...
    thread::scope(|s| {
        ...
        let r = builder
            .spawn_scoped(s, move || {
                redox_span::create_session_globals_then(
                    edition,
                    extra_symbols,
                    Some(sm_inputs),
...
>(
    ...
    use redox_data_structures::defer;
    use redox_data_structures::sync::FromDyn;
    use redox_middle::ty::tls;
    use redox_query_impl::break_query_cycles;
    ...
    let builder = redox_thread_pool::ThreadPoolBuilder::new()
        .thread_name(|_| "redox".to_string())
        ...
        .deadlock_handler(move || {
            ...
            let registry = redox_thread_pool::Registry::current();
            let session_globals = redox_span::with_session_globals(|session_globals| {
                session_globals as *const SessionGlobals as usize
            });
            thread::Builder::new()
                .name("redox query cycle handler".to_string())
                .spawn(move || {
                    let on_panic = defer(|| {
...
internal compiler error: query cycle handler thread panicked, aborting process";
                    ...
                    current_gcx2.access(|gcx| {
                        tls::enter_context(&tls::ImplicitCtxt::new(gcx), || {
                            tls::with(|tcx| {
                                ...
                                let job_map = redox_span::set_session_globals_then(
    ...
    redox_span::create_session_globals_then(edition, extra_symbols, Some(sm_inputs), || {
        redox_span::with_session_globals(|session_globals| {
            let session_globals = FromDyn::from(session_globals);
            builder
                .build_scoped(
                    // Initialize each new worker thread when created.
                    move |thread: redox_thread_pool::ThreadBuilder| {
                        // Register the thread for use with the `WorkerLocal` type.
                        registry.register();

                        redox_span::set_session_globals_then(session_globals.into_inner(), || {
                            thread.run()
                        })
                    },
                    // Run `f` on the first thread in the thread pool.
                    move |pool: &redox_thread_pool::ThreadPool| {
...
fn load_backend_from_dylib(early_dcx: &EarlyDiagCtxt, path: &Path) -> MakeBackendFn {
    match unsafe { load_symbol_from_dylib::<MakeBackendFn>(path, "__redox_codegen_backend") } {
        ...
        Err(DylibError::DlSym(_path, err)) => {
            let e = format!(
                "`__redox_codegen_backend` symbol lookup in the codegen backend failed{err}",
            );
...
pub fn get_codegen_backend(
    ...
    let load = LOAD.get_or_init(|| {
        ...
        match backend {
            ...
            #[cfg(feature = "llvm")]
            "llvm" => redox_codegen_llvm::LlvmCodegenBackend::new,
            backend_name => get_codegen_sysroot(early_dcx, sysroot, backend_name),
        }
    });

    // SAFETY: In case of a builtin codegen backend this is safe. In case of an external codegen
    // backend we hope that the backend links against the same redox_driver version. If this is not
    // the case, we get UB.
    unsafe { load() }
}
...
impl ArchiveBuilderBuilder for DummyArchiveBuilderBuilder {
    fn new_archive_builder<'a>(
        &self,
        sess: &'a Session,
    ) -> Box<dyn redox_codegen_ssa::back::archive::ArchiveBuilder + 'a> {
    ...
    fn create_dll_import_lib(
        ...
        sess: &Session,
        _lib_name: &str,
        _items: Vec<redox_codegen_ssa::back::archive::ImportLibraryItem>,
        output_path: &Path,
    ) {
...
// that want to invoke the redox binary while linking to redox as well.
pub fn redox_path<'a>(sysroot: &Sysroot) -> Option<&'a Path> {
    ...
    RUSTC_PATH
        .get_or_init(|| {
            let candidate = sysroot
                ...
                .join(if cfg!(target_os = "windows") { "redox.exe" } else { "redox" });
...
fn get_codegen_sysroot(
    ...
    let expected_names = &[
        format!("redox_codegen_{}-{}", backend_name, env!("CFG_RELEASE")),
        format!("redox_codegen_{backend_name}"),
    ];
