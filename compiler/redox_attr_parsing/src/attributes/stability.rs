...
use redox_hir::target::GenericParamKind;
use redox_hir::{
...
impl<S: Stage> AttributeParser<S> for StabilityParser {
    const ATTRIBUTES: AcceptMapping<Self, S> = &[
        ...
        (
            &[sym::redox_allowed_through_unstable_modules],
...
impl<S: Stage> AttributeParser<S> for BodyStabilityParser {
    const ATTRIBUTES: AcceptMapping<Self, S> = &[(
        &[sym::redox_default_body_unstable],
        template!(List: &[r#"feature = "name", reason = "...", issue = "N""#]),
        |this, cx, args| {
            reject_outside_std!(cx);
...
impl<S: Stage> NoArgsAttributeParser<S> for RustcConstStableIndirectParser {
    const PATH: &[Symbol] = &[sym::redox_const_stable_indirect];
...
/// Read the content of a `stable`/`redox_const_stable` attribute, and return the feature name and
...
pub(crate) fn parse_stability<S: Stage>(
    ...
    let feature = match feature {
        Some(feature) if redox_lexer::is_ident(feature.as_str()) => Ok(feature),
        Some(_bad_feature) => {
