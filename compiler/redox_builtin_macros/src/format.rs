...
use redox_lint_defs::builtin::NAMED_ARGUMENTS_USED_POSITIONALLY;
use redox_lint_defs::{BuiltinLintDiag, LintId};
use redox_parse::exp;
use redox_parse_format as parse;
use redox_span::{BytePos, ErrorGuaranteed, Ident, InnerSpan, Span, Symbol};
...
//  3. Much later, in AST lowering (redox_ast_lowering), that `FormatArgs` structure will be turned
//     into the expression of type `core::fmt::Arguments`.

// See redox_ast/src/format.rs for the FormatArgs structure and glossary.
...
fn make_format_args(
    ...
    let str_style = match fmt_style {
        redox_ast::StrStyle::Cooked => None,
        redox_ast::StrStyle::Raw(raw) => Some(raw as usize),
    };
    ...
    if invalid_refs.is_empty() && !has_unused && !unnamed_arg_after_named_arg {
        for &(index, span, used_as) in &numeric_references_to_named_arg {
            ...
            ecx.buffered_early_lint.push(BufferedEarlyLint {
                ...
                node_id: redox_ast::CRATE_NODE_ID,
