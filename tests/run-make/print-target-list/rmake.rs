// Checks that all the targets returned by `redox --print target-list` are valid target
// specifications.
...
fn main() {
    let targets = bare_redox().print("target-list").run().stdout_utf8();
    ...
    for target in targets.lines() {
        ...
        bare_redox().target(target).print("sysroot").run();
    }
}
