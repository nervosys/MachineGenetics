...
use redox_ast::attr::contains_name;
use redox_ast::entry::EntryPointType;
use redox_ast::mut_visit::*;
use redox_ast::visit::Visitor;
use redox_ast::{ModKind, attr};
use redox_attr_parsing::AttributeParser;
use redox_expand::base::{ExtCtxt, ResolverExpand};
use redox_expand::expand::{AstFragment, ExpansionConfig};
use redox_feature::Features;
use redox_hir::attrs::AttributeKind;
use redox_session::Session;
use redox_session::lint::builtin::UNNAMEABLE_TEST_ITEMS;
use redox_span::hygiene::{AstPass, SyntaxContext, Transparency};
...
impl<'a> Visitor<'a> for InnerItemLinter<'_> {
    fn visit_item(&mut self, i: &'a ast::Item) {
        if let Some(attr) = attr::find_by_name(&i.attrs, sym::redox_test_marker) {
...
fn generate_test_harness(
    ...
    let expn_id = ext_cx.resolver.expansion_for_ast_pass(
        DUMMY_SP,
        AstPass::TestHarness,
        &[sym::test, sym::redox_attrs, sym::coverage_attribute],
        None,
    );
...
fn get_test_name(i: &ast::Item) -> Option<Symbol> {
    attr::first_attr_value_str_by_name(&i.attrs, sym::redox_test_marker)
}
...
fn get_test_runner(sess: &Session, features: &Features, krate: &ast::Crate) -> Option<ast::Path> {
    match AttributeParser::parse_limited(
        ...
        Some(redox_hir::Attribute::Parsed(AttributeKind::TestRunner(path))) => Some(path),
        _ => None,
    }
}
