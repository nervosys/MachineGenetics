...
enum HashStableMode {
    ...
    // trait, when the crate is upstream of `redox_middle`.
    Generic,
...
fn hash_stable_derive_with_mode(
    ...
    let context: syn::Type = match mode {
        HashStableMode::Normal => {
            parse_quote!(::redox_middle::ich::StableHashingContext<'__ctx>)
        }
        HashStableMode::Generic | HashStableMode::NoContext => parse_quote!(__CTX),
    };
    ...
    s.bound_impl(
        quote!(
            ::redox_data_structures::stable_hasher::HashStable<
                #context
            >
        ),
        quote! {
            ...
            fn hash_stable(
                &self,
                __hcx: &mut #context,
                __hasher: &mut ::redox_data_structures::stable_hasher::StableHasher) {
                #discriminant
                match *self { #body }
            }
        },
