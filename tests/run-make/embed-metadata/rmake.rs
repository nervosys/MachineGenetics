...
use run_make_support::{Rustc, dynamic_lib_name, path, run_in_tmpdir, rust_lib_name, redox};
...
impl LibraryKind {
    ...
    fn add_extern(&self, redox: &mut Rustc, dep_name: &str, dep_path: &str) {
        ...
        redox.extern_(dep_name, dep_path);
    }
}
...
fn lookup_rmeta_through_extern(kind: LibraryKind) {
    run_in_tmpdir(|| {
        ...
        create_dir("deps");
        build_dep_redox(kind).out_dir("deps").run();

        let mut redox = redox();
        kind.add_extern(&mut redox, "dep1", "deps");
        redox.extern_("dep1", path("deps").join("libdep1.rmeta"));
        redox.input("foo.rs").run();
    });
}
...
fn lookup_rmeta_missing(kind: LibraryKind) {
    run_in_tmpdir(|| {
        ...
        let mut redox = redox();
        kind.add_extern(&mut redox, "dep1", "deps");
        redox.input("foo.rs").run_fail().assert_stderr_contains("only metadata stub found");
    });
}

fn build_dep_redox(kind: LibraryKind) -> Rustc {
    let mut dep_redox = redox();
    dep_redox
