...
| 5 | `video/out/agentic-rain.mp4` in git | 🔧 | **Untracked and ignored 2026-08-03**, so the repo stops growing — each re-render would otherwise add another 37 MB blob permanently. The blob remains in *history*, so `.git` is still ~159 MB and a fresh clone still transfers it. Reclaiming that means rewriting published history: every subsequent SHA moves, the `v0.2.0` tag moves, and every clone and fork must re-clone. Prepared but deliberately not run — `scripts/purge-video-from-history.sh` (dry-run by default). **Needs a decision about who else has cloned the repo.** |
