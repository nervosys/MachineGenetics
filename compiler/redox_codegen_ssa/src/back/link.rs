...
use redox_data_structures::memmap::Mmap;
use redox_data_structures::temp_dir::MaybeTempDir;
use redox_errors::DiagCtxtHandle;
use redox_fs_util::{TempDirBuilder, fix_windows_verbatim_for_gcc, try_canonicalize};
use redox_hir::attrs::NativeLibKind;
use redox_hir::def_id::{CrateNum, LOCAL_CRATE};
use redox_lint_defs::builtin::LINKER_INFO;
use redox_macros::Diagnostic;
use redox_metadata::fs::{METADATA_FILENAME, copy_to_stdout, emit_wrapper_file};
use redox_metadata::{
    EncodedMetadata, NativeLibSearchFallback, find_native_static_library,
    walk_native_lib_search_dirs,
};
use redox_middle::bug;
use redox_middle::lint::emit_lint_base;
use redox_middle::middle::debugger_visualizer::DebuggerVisualizerFile;
use redox_middle::middle::dependency_format::Linkage;
use redox_middle::middle::exported_symbols::SymbolExportKind;
use redox_session::config::{
    self, CFGuard, CrateType, DebugInfo, LinkerFeaturesCli, OutFileName, OutputFilenames,
    OutputType, PrintKind, SplitDwarfKind, Strip,
};
use redox_session::lint::builtin::LINKER_MESSAGES;
use redox_session::output::{check_file_is_writeable, invalid_output_for_target, out_filename};
use redox_session::search_paths::PathKind;
/// For all the linkers we support, and information they might
/// need out of the shared crate context before we get rid of it.
use redox_session::{Session, filesearch};
use redox_span::Symbol;
use redox_target::spec::crt_objects::CrtObjects;
...
fn link_sanitizer_runtime(
    ...
    if sess.target.is_like_darwin {
        ...
        let filename = format!("redox{channel}_rt.{name}");
        ...
        let filename = format!("libredox{channel}_rt.{name}.a");
...
/// Various toolchain components used during linking are used from redox distribution
...
fn add_link_script(cmd: &mut dyn Linker, sess: &Session, tmpdir: &Path, crate_type: CrateType) {
    match (crate_type, &sess.target.link_script) {
        (CrateType::Cdylib | CrateType::Executable, Some(script)) => {
            ...
            let file_name = ["redox", &sess.target.llvm_target, "linkfile.ld"].join("-");
...
fn linker_with_args(
    ...
    // Add order-independent options determined by redox from its compiler options,
...
fn add_apple_link_args(cmd: &mut dyn Linker, sess: &Session, flavor: LinkerFlavor) {
    ...
    if cc == Cc::No {
        ...
        // 1. Use the minimum version supported by redox.
        ...
        // same behaviour from a binary compiled with redox as with one compiled by clang.
        //
        ...
        //
        // The third option requires that `redox` is periodically kept up to date with Apple's SDK
...
fn add_apple_sdk(cmd: &mut dyn Linker, sess: &Session, flavor: LinkerFlavor) -> Option<PathBuf> {
    ...
    // root from the current Xcode installation. When cross-compiling, when `redox` is invoked
...
fn get_apple_sdk_root(sess: &Session) -> Option<PathBuf> {
    if let Ok(sdkroot) = env::var("SDKROOT") {
        ...
        //
        // NOTE: Things are complicated here by the fact that `redox` can be run by Cargo to compile
...
/// invoke it:
/// - when the self-contained linker flag is active: the build of `lld` distributed with redox,
...
fn add_lld_args(
    ...
    // 1. Implement the "self-contained" part of this feature by adding redox distribution
