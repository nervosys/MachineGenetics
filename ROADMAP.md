...
> **1,064**, rmi **1,380**, ribosome **164**, germline **112**, forge **52** —
> **2,772 tests, 0 failures, 0 warnings**. The crate count went from three to
...
| 159 | RustSec audit, and a real vulnerability it found | ✅ | Only npm had been audited this session; the Rust side was assumed clean **because Dependabot was quiet**. It was not evidence: `cargo audit` found **RUSTSEC-2026-0204** (`crossbeam-epoch 0.9.18`, invalid pointer dereference) in both `prototype` and `rmi`, which Dependabot never reported while reporting 16 npm alerts. Fixed by lockfile bump to 0.9.20; all five workspaces now report zero vulnerabilities. Wired into CI over each lockfile separately — `SECURITY_AUDIT.md` had listed this as "the one concrete CI action item" since 2026-06-04. Deliberately **not** `-D warnings`: the residual `unmaintained`/`unsound` advisories have no patched version, and a permanently-red job is an ignored job. **`rmi` was never affected**, though the first draft of this row said it was: its `Cargo.lock` is git-ignored so the vendored crate cannot inherit this repo's pins, so there is no committed pin to be stale, and a fresh resolve picks 0.9.20 unaided (verified by deleting and regenerating). The 0.9.18 `cargo audit` saw there was a stale artifact in one working copy — a local finding, not a repository finding |
| 158 | Widen the doc check past test counts | ✅ | A **fifth** stale figure: `ARCHITECTURE.md`'s scope banner claimed "976 lib + 132 + 30 tests green" — 1,138 against 1,038 measured — and it is the first one `check-doc-counts.sh` did not catch, because the checker read the layout table below it and never the banner. Now covered, along with the `eval_bench` **73/73** claim that appears in four documents and had never been verified against a run: it is fed from `--bench` the same way the CUDA figure is fed from `--cuda`, and it does hold. 51 counts checked when all three modes run, 38 in the default mode. The lesson is narrower than "check the docs" — a checker only covers what someone remembered to list, so its own coverage is the thing that quietly rots |
| 157 | TLS, without deciding the trust posture | ✅ | I had called this blocked on an operator decision three times. Half right: the posture *is* the operator's. The wrong half was concluding that made TLS undeliverable — in `rustls` the posture already **is** a value, so `tls::acceptor`/`connector` take the caller's `ServerConfig`/`ClientConfig` and supply only plumbing. Pinned self-signed, mutual TLS, and public PKI are all expressible; none is chosen. Deliberately no `with_self_signed()` convenience: one line, would get used, would be the decision being avoided. Four loopback tests with an in-test certificate, three of them negative — untrusted cert refused, plaintext client refused, and **a valid certificate alone still refused** because the fleet key gates every frame inside the session. Optional so encryption is not billed to every consumer: default build is 39 normal deps with no TLS, `--features tls` is 48, and CI checks the feature has not leaked into the default |
...
| 144 | Transport authentication | ✅ | HMAC challenge-response before any frame is served — a worker that answers `Describe` to an unauthenticated peer has already disclosed its capabilities. Server-generated per-connection nonce, so a captured proof cannot be replayed. **Authenticates but does not encrypt**; encryption arrived separately in step 157 and wraps this handshake rather than replacing it |
