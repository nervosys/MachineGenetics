 ...
 *
 * Maybe some missing optimizations enabled by redox's LTO is in there: https://gcc.gnu.org/onlinedocs/gcc/Optimize-Options.html
// cspell:disable-next-line
...
use redox_ast::expand::allocator::AllocatorMethod;
use redox_codegen_ssa::back::lto::{SerializedModule, ThinModule};
use redox_codegen_ssa::back::write::{
    CodegenContext, FatLtoInput, ModuleConfig, SharedEmitter, TargetMachineFactoryFn,
};
use redox_codegen_ssa::base::codegen_crate;
use redox_codegen_ssa::target_features::cfg_target_feature;
use redox_codegen_ssa::traits::{CodegenBackend, ExtraBackendMethods, WriteBackendMethods};
use redox_codegen_ssa::{CompiledModule, CompiledModules, CrateInfo, ModuleCodegen, TargetConfig};
...
use redox_errors::{DiagCtxt, DiagCtxtHandle};
...
use redox_session::config::{OptLevel, OutputFilenames};
use redox_span::Symbol;
use redox_target::spec::{Arch, RelocModel};
...
impl CodegenBackend for GccCodegenBackend {
    ...
    fn init(&self, sess: &Session) {
        fn file_path(sysroot_path: &Path, sess: &Session) -> PathBuf {
            let rustlib_path =
                redox_target::relative_target_rustlib_path(sysroot_path, &sess.host.llvm_target);
            sysroot_path
                .join(rustlib_path)
    ...
    fn join_codegen(
        ...
        ongoing_codegen
            .downcast::<redox_codegen_ssa::back::write::OngoingCodegen<GccCodegenBackend>>()
...
/// This is the entrypoint for a hot plugged redox_codegen_gccjit
#[unsafe(no_mangle)]
