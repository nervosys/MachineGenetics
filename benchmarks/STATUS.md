...
tokens. The mission win lives in the **binary IR (RMIB)** for AI-routed
...
| RMIL transport | `rmil/encode`, `rmil/decode`, `rmil/run` |
...
ontology/full
  ...
  ├─ ir_ops (107)            — every RMIL opcode w/ metadata
  ...
  ├─ rmib                    — binary container format constants
...
MechGen-parse --backends-file=~/.mechgen/backends.json \
              --backend=groq_lpu --run=rmil-bytes model.rmib
```

Wrapper protocol: stdin = RMIB blob, env = `RDX_BACKEND` / `RDX_ITEM_NAME` /
...
2. **Binary IR (RMIB)** for AI items (`net` / `kb` / `agent` / `swarm`).
