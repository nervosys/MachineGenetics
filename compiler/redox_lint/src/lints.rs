...
use redox_errors::formatting::DiagMessageAddArg;
use redox_errors::{
...
use redox_middle::ty::inhabitedness::InhabitedPredicate;
use redox_middle::ty::{Clause, PolyExistentialTraitRef, Ty, TyCtxt};
...
impl<'a> Diagnostic<'a, ()> for BuiltinUngatedAsyncFnTrackCaller<'_> {
    fn into_diag(self, dcx: DiagCtxtHandle<'a>, level: Level) -> Diag<'a, ()> {
        ...
        redox_session::parse::add_feature_diagnostics(
            &mut diag,
            self.session,
...
#[note("a `use redox_data_structures::fx::{$preferred}` may be necessary")]
...
#[help("consider adding the symbol to `compiler/redox_span/src/symbol.rs`")]
...
#[derive(Diagnostic)]
#[diag("do not use `redox_type_ir::inherent` unless you're inside of the trait solver")]
#[note(
...
#[derive(Diagnostic)]
#[diag(
    "do not use `redox_type_ir::Interner` or `redox_type_ir::InferCtxtLike` unless you're inside of the trait solver"
)]
#[note(
    "the method or struct you're looking for is likely defined somewhere else downstream in the compiler"
)]
...
#[derive(Diagnostic)]
#[diag("do not use `redox_type_ir` unless you are implementing type system internals")]
#[note("use `redox_middle::ty` instead")]
pub(crate) struct TypeIrDirectUse;

#[derive(Diagnostic)]
#[diag("non-glob import of `redox_type_ir::inherent`")]
...
#[help(
    "try using a cargo dependency or using a re-export of the dependency provided by a redox_* crate"
)]
...
pub(crate) enum UnknownLintSuggestion {
    #[suggestion(
        "{$from_redox ->
            [true] a lint with a similar name exists in `redox` lints
            *[false] did you mean
        }",
        code = "{replace}",
        applicability = "maybe-incorrect"
    ...
    #[help(
        "{$from_redox ->
            [true] a lint with a similar name exists in `redox` lints: `{$replace}`
            *[false] did you mean: `{$replace}`
        }"
    )]
    WithoutSpan { replace: Symbol, from_redox: bool },
}
...
pub(crate) enum UnpredictableFunctionPointerComparisonsSuggestion<'a, 'tcx> {
    ...
    FnAddrEqWithCast {
        ...
        deref_left: &'a str,
        deref_right: &'a str,
        fn_sig: redox_middle::ty::PolyFnSig<'tcx>,
        #[suggestion_part(code = "{ne}std::ptr::fn_addr_eq({deref_left}")]
        left: Span,
        #[suggestion_part(code = ", {deref_right}")]
        middle: Span,
...
impl UnexpectedCfgCargoHelp {
    ...
    pub(crate) fn lint_cfg_and_build_rs(unescaped: &str, escaped: &str) -> Self {
        UnexpectedCfgCargoHelp::LintCfgAndBuildRs {
            cargo_toml_lint_cfg: Self::cargo_toml_lint_cfg(unescaped),
            build_rs_println: format!("println!(\"cargo::redox-check-cfg={escaped}\");"),
        }
    }
}
...
pub(crate) mod unexpected_cfg_name {
    use redox_errors::DiagSymbolList;
    use redox_macros::Subdiagnostic;
    ...
    pub(crate) enum InvocationHelp {
        #[note(
            "see <https://doc.rust-lang.org/nightly/redox/check-cfg/cargo-specifics.html> for more information about checking conditional configuration"
        )]
        Cargo {
            #[subdiagnostic]
        ...
        #[note(
            "see <https://doc.rust-lang.org/nightly/redox/check-cfg.html> for more information about checking conditional configuration"
        )]
        Rustc {
            #[subdiagnostic]
...
// FIXME(jdonszelmann): duplicated in redox_attr_parsing, should be moved there completely.
#[derive(Diagnostic)]
#[diag(
    "{$num_suggestions ->
