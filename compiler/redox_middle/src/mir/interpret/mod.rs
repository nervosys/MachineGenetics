...
use redox_abi::{AddressSpace, Align, Endian, HasDataLayout, Size};
use redox_ast::Mutability;
use redox_data_structures::fx::FxHashMap;
use redox_data_structures::sharded::ShardedHashMap;
use redox_data_structures::sync::{AtomicU64, Lock};
...
pub(crate) struct AllocMap<'tcx> {
    ...
    // Note that this map on redox workloads seems to be rather dense, but in miri workloads should
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    /// ids), this function is frequently used throughout redox, but should not be used within
    /// the interpreter.
