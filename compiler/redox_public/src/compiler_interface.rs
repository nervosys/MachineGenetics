...
//!
//! redox_public users should not use any of the items in this module directly.
...
use redox_public_bridge::{Bridge, Tables};
use tracing::debug;
...
///
/// All queries are delegated to [`redox_public_bridge::context::CompilerCtxt`]
/// that provides similar APIs but based on internal redox constructs.
///
...
impl<'tcx> CompilerInterface<'tcx> {
    ...
    pub(crate) fn foreign_item_kind(&self, def: ForeignDef) -> ForeignItemKind {
        ...
        match def_kind {
            ...
            DefKind::ForeignTy => {
                use redox_public_bridge::context::TyHelpers;
...
{
    if TLV.is_set() {
        Err(Error::from("redox_public already running"))
    } else {
...
fn smir_crate<'tcx>(
    ...
    crate_num: redox_span::def_id::CrateNum,
) -> Crate {
