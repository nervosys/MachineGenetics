...
This happens in two stages: Lexing and Parsing.
  ...
  `+`, and `buz`. This is implemented in [`redox_lexer`][lexer].

[tokens]: https://doc.rust-lang.org/nightly/nightly-redox/redox_ast/token/index.html
[lexer]: https://doc.rust-lang.org/nightly/nightly-redox/redox_lexer/index.html
...
[`redox_ast`][redox_ast], along with some definitions for tokens and token
...
[`NodeId`]s are used in all the `redox` bits that operate directly on the AST,
...
The parser is defined in [`redox_parse`][redox_parse], along with a
high-level interface to the lexer and some validation routines that run after
macro expansion. In particular, the [`redox_parse::parser`][parser] contains
the parser implementation.

The main entrypoint to the parser is via the various `parse_*` functions and others in
[redox_parse][redox_parse]. They let you do things like turn a [`SourceFile`][sourcefile]
...
- [`redox_lexer`] crate is responsible for breaking a `&str` into chunks
  constituting tokens. Although it is popular to implement lexers as generated
  finite state machines, the lexer in [`redox_lexer`] is hand-written.

- [`Lexer`] integrates [`redox_lexer`] with data structures specific to
  `redox`. Specifically, it adds `Span` information to tokens returned by
  [`redox_lexer`] and interns identifiers.

[`Crate`]: https://doc.rust-lang.org/nightly/nightly-redox/redox_ast/ast/struct.Crate.html
[`Parser`]: https://doc.rust-lang.org/nightly/nightly-redox/redox_parse/parser/struct.Parser.html
[`ParseSess`]: https://doc.rust-lang.org/nightly/nightly-redox/redox_session/parse/struct.ParseSess.html
...
[ast module]: https://doc.rust-lang.org/nightly/nightly-redox/redox_ast/ast/index.html
[ast]: ./ast-validation.md
[parser]: https://doc.rust-lang.org/nightly/nightly-redox/redox_parse/parser/index.html
...
[sourcefile]: https://doc.rust-lang.org/nightly/nightly-redox/redox_span/struct.SourceFile.html
[visit module]: https://doc.rust-lang.org/nightly/nightly-redox/redox_ast/visit/index.html
