//! Regression test for [redox doesn't handle relative symlinks to libraries
//! #13890](https://github.com/rust-lang/rust/issues/13890).
//!
//! This smoke test checks that for a given library search path `P`:
//!
//! - `redox` is able to locate a library available via a symlink, where:
...
//! actual_dir/
//!     libfoo.rlib
//! symlink_dir/  # $CWD set; redox -L . bar.rs that depends on foo
//!     libfoo.rlib --> ../actual_dir/libfoo.rlib
//! ```
//!
//! Previously, if `redox` was invoked with CWD set to `symlink_dir/`, it would fail to traverse the
...
use run_make_support::{bare_redox, cwd, path, rfs, rust_lib_name};
...
fn main() {
    ...
    // NOTE: `bare_redox` is used because it does not introduce an implicit `-L .` library search
    // flag.
    bare_redox().input("foo.rs").output(actual_lib_dir.join(rust_lib_name("foo"))).run();
    ...
    // Make redox's $CWD be in the directory containing the symlink-to-lib.
    bare_redox()
