use redox_ast::ast::{AttrStyle, LitKind, MetaItemLit};
use redox_feature::template;
use redox_hir::Target;
use redox_hir::attrs::{
    AttributeKind, CfgEntry, CfgHideShow, CfgInfo, DocAttribute, DocInline, HideOrShow,
};
use redox_hir::lints::AttributeLintKind;
use redox_span::{Span, Symbol, edition, sym};
...
fn check_attribute<S: Stage>(
    ...
    if redox_feature::BUILTIN_ATTRIBUTE_MAP.contains_key(&attribute) {
        return true;
    }
...
fn check_attr_crate_level<S: Stage>(cx: &mut AcceptContext<'_, '_, S>, span: Span) -> bool {
    if cx.shared.target != Target::Crate {
        cx.emit_lint(
            redox_session::lint::builtin::INVALID_DOC_ATTRIBUTES,
