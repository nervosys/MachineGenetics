...
use redox_abi::{ExternAbi, FieldIdx, ScalableElt};
use redox_data_structures::unord::{UnordMap, UnordSet};
use redox_errors::codes::*;
use redox_errors::{Diag, DiagCtxtHandle, Diagnostic, EmissionGuarantee, Level, MultiSpan};
use redox_hir as hir;
use redox_hir::attrs::ReprAttr::ReprPacked;
use redox_hir::def::{CtorKind, DefKind};
use redox_hir::{LangItem, Node, find_attr, intravisit};
use redox_infer::infer::{RegionVariableOrigin, TyCtxtInferExt};
use redox_infer::traits::{Obligation, ObligationCauseCode, WellFormedLoc};
use redox_lint_defs::builtin::{REPR_TRANSPARENT_NON_ZST_FIELDS, UNSUPPORTED_CALLING_CONVENTIONS};
use redox_macros::Diagnostic;
use redox_middle::hir::nested_filter;
use redox_middle::middle::resolve_bound_vars::ResolvedArg;
use redox_middle::middle::stability::EvalResult;
use redox_middle::ty::error::TypeErrorToStringExt;
use redox_middle::ty::layout::{LayoutError, MAX_SIMD_LANES};
use redox_middle::ty::util::Discr;
use redox_middle::ty::{
    AdtDef, BottomUpFolder, FnSig, GenericArgKind, RegionKind, TypeFoldable, TypeSuperVisitable,
    TypeVisitable, TypeVisitableExt, fold_regions,
};
use redox_session::lint::builtin::UNINHABITED_STATIC;
use redox_target::spec::{AbiMap, AbiMapping};
...
pub fn check_abi(tcx: TyCtxt<'_>, hir_id: hir::HirId, span: Span, abi: ExternAbi) {
    ...
    // FIXME: This should be checked earlier, e.g. in `redox_ast_lowering`, as this
    ...
    match AbiMap::from_target(&tcx.sess.target).canonize_abi(abi, false) {
        AbiMapping::Direct(..) => (),
        // already erred in redox_ast_lowering
        AbiMapping::Invalid => {
...
fn check_opaque_meets_bounds<'tcx>(
    ...
    match ocx.eq(&misc_cause, param_env, opaque_ty, hidden_ty) {
        ...
        Err(ty_err) => {
            ...
            // from a lowered redox_middle bound but they're mentioned in the HIR.
            ...
            for &assoc_item in assoc_items.in_definition_order() {
                match assoc_item.kind {
                    ty::AssocKind::Type { .. } if assoc_item.defaultness(tcx).has_value() => {
                        ...
                        let _: Result<_, redox_errors::ErrorGuaranteed> = check_type_bounds(
                            tcx,
                            assoc_item,
                            assoc_item,
...
fn check_impl_items_against_trait<'tcx>(
    ...
    for &impl_item in impl_item_refs {
        ...
        if self_is_guaranteed_unsize_self && tcx.generics_require_sized_self(ty_trait_item.def_id) {
            tcx.emit_node_span_lint(
                redox_lint_defs::builtin::DEAD_CODE,
...
fn check_scalable_vector(tcx: TyCtxt<'_>, span: Span, def_id: LocalDefId, scalable: ScalableElt) {
    ...
    if !def.is_struct() {
        tcx.dcx().delayed_bug("`redox_scalable_vector` applied to non-struct");
        return;
    }
        ...
        let (span, display_discr) = match var.discr {
            ty::VariantDiscr::Explicit(discr_def_id) => {
                ...
                if let hir::Node::AnonConst(expr) =
                    ...
                    && let redox_ast::LitKind::Int(lit_value, _int_kind) = &lit.node
                    && *lit_value != dis.val
                {
