...
└────────────────────────────┬─────────────────────────────────┘
                             │ lowers to RMIL via rmil_bridge
                             ▼
┌──────────────────────────────────────────────────────────────┐
│  RMI / Framewerx (Rust crate)        ← JAX-equivalent        │
│  • 107 RMIL opcodes (Neural / Symbolic / Control / ...)      │
│  • CpuBackend dispatch                                       │
...
| **Binary transport** | A Framewerx-MG module compiles to an RMIB container. Agents ship bytes, not text. (See `rmil/encode` and `pipeline/recover-and-encode`.) |
...
2. Lowers to RMIL via `rmil_bridge::lower_module` — `Linear` and `ReLU` map to opcodes `0x0002` and `0x0010`.
3. Encodes as RMIB bytes (`rmil/encode` RAP method).
4. Executes on `CpuBackend` (`rmil/run` RAP method).
