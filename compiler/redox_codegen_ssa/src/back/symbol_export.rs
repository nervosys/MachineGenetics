...
use redox_abi::{CanonAbi, X86Call};
use redox_ast::expand::allocator::{AllocatorKind, NO_ALLOC_SHIM_IS_UNSTABLE, global_fn_name};
use redox_data_structures::unord::UnordMap;
use redox_hir::def::DefKind;
use redox_hir::def_id::{CrateNum, DefId, DefIdMap, LOCAL_CRATE, LocalDefId};
...
use redox_middle::ty::{self, GenericArgKind, GenericArgsRef, Instance, SymbolName, Ty, TyCtxt};
use redox_middle::util::Providers;
use redox_session::config::CrateType;
use redox_span::Span;
use redox_symbol_mangling::mangle_internal_symbol;
use redox_target::spec::{Arch, Os, TlsModel};
use tracing::debug;
    ...
    let mut reachable_non_generics: DefIdMap<_> = tcx
        ...
        .map(|def_id| {
            ...
            let info = SymbolExportInfo {
                ...
                redox_std_internal_symbol: codegen_attrs
                    .flags
            ...
            tcx.needs_thread_local_shim(def_id).then(|| {
                (
                    ...
                    SymbolExportInfo {
                        ...
                        used: info.used,
                        redox_std_internal_symbol: info.redox_std_internal_symbol,
                    },
                )
            })
        }))
...
fn exported_generic_symbols_provider_local<'tcx>(
    ...
    if tcx.local_crate_exports_generics() {
        ...
        use redox_middle::mir::mono::{MonoItem, Visibility};
        use redox_middle::ty::InstanceKind;
        ...
        for (mono_item, data) in cgus.iter().flat_map(|cgu| cgu.items().iter()) {
            ...
            if !tcx.sess.opts.share_generics() {
                if tcx.codegen_fn_attrs(mono_item.def_id()).inline
                    == redox_hir::attrs::InlineAttr::Never
                {
            ...
            // Note: These all set redox_std_internal_symbol to false as generic functions must not
...
pub(crate) fn symbol_name_for_instance_in_crate<'tcx>(
    ...
    match symbol {
        ExportedSymbol::NonGeneric(def_id) => {
            redox_symbol_mangling::symbol_name_for_instance_in_crate(
                tcx,
        ...
        ExportedSymbol::DropGlue(ty) => redox_symbol_mangling::symbol_name_for_instance_in_crate(
            tcx,
...
fn calling_convention_for_symbol<'tcx>(
    tcx: TyCtxt<'tcx>,
    symbol: ExportedSymbol<'tcx>,
) -> (CanonAbi, &'tcx [redox_target::callconv::ArgAbi<'tcx, Ty<'tcx>>]) {
...
pub fn escape_symbol_name(tcx: TyCtxt<'_>, symbol: &str, span: Span) -> String {
    ...
    use redox_target::spec::{Arch, BinaryFormat};
    if !symbol.is_empty()
