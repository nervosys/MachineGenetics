...
impl<'a> DiagnosticDerive<'a> {
    ...
    pub(crate) fn into_tokens(self) -> TokenStream {
        ...
        structure.gen_impl(quote! {
            gen impl<'_sess, G> redox_errors::Diagnostic<'_sess, G> for @Self
                where G: redox_errors::EmissionGuarantee
            {
                #[track_caller]
                fn into_diag(
                    self,
                    dcx: redox_errors::DiagCtxtHandle<'_sess>,
                    level: redox_errors::Level
                ) -> redox_errors::Diag<'_sess, G> {
                    #implementation
                }
            }
        })
