...
use redox_abi::{CVariadicStatus, CanonAbi, ExternAbi, InterruptKind};
use redox_ast::visit::{AssocCtxt, BoundKind, FnCtxt, FnKind, Visitor, walk_list};
use redox_ast::*;
use redox_ast_pretty::pprust::{self, State};
use redox_attr_parsing::validate_attr;
use redox_data_structures::fx::FxIndexMap;
use redox_errors::{DiagCtxtHandle, LintBuffer};
        ...
        self.dcx().emit_err(errors::TraitFnConst {
            ...
            remove_const_sugg: (
                ...
                match parent_constness {
                    Some(_) => redox_errors::Applicability::MachineApplicable,
                    None => redox_errors::Applicability::MaybeIncorrect,
                },
            ),
    ...
    fn check_decl_attrs(&self, fn_decl: &FnDecl) {
        fn_decl
            ...
            .filter(|attr| {
                ...
                !attr.has_any_name(&arr) && redox_attr_parsing::is_builtin_attr(*attr)
            })
            .for_each(|attr| {
    ...
    fn visit_item(&mut self, item: &'a Item) {
        ...
        match &item.kind {
            ...
            ItemKind::Fn(
                ...
                let is_intrinsic = item.attrs.iter().any(|a| a.has_name(sym::redox_intrinsic));
            ...
            ItemKind::Struct(ident, generics, vdata) => {
                self.with_tilde_const(Some(TildeConstReason::Struct { span: item.span }), |this| {
                    ...
                    let scalable_vector_attr =
                        item.attrs.iter().find(|attr| attr.has_name(sym::redox_scalable_vector));
