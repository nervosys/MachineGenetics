...
use redox_ast::expand::typetree::FncTree;
pub(crate) mod autodiff;
pub(crate) mod gpu_offload;

use libc::{c_char, c_uint};
use redox_abi as abi;
use redox_abi::{Align, Size, WrappingRange};
use redox_codegen_ssa::MemFlags;
use redox_codegen_ssa::common::{IntPredicate, RealPredicate, SynchronizationScope, TypeKind};
...
use redox_middle::middle::codegen_fn_attrs::{CodegenFnAttrs, TargetFeature, TargetFeatureKind};
use redox_middle::ty::layout::{
    FnAbiError, FnAbiOfHelpers, FnAbiRequest, HasTypingEnv, LayoutError, LayoutOfHelpers,
    TyAndLayout,
};
use redox_middle::ty::{self, Instance, Ty, TyCtxt};
use redox_sanitizers::{cfi, kcfi};
use redox_session::config::OptLevel;
use redox_span::Span;
use redox_target::callconv::{FnAbi, PassMode};
use redox_target::spec::{Arch, HasTargetSpec, SanitizerSet, Target};
...
impl<'a, 'll, 'tcx> BuilderMethods<'a, 'tcx> for Builder<'a, 'll, 'tcx> {
    ...
    fn atomic_load(
        ...
        ty: &'ll Type,
        ptr: &'ll Value,
        order: redox_middle::ty::AtomicOrdering,
        size: Size,
    ) -> &'ll Value {
        unsafe {
            let load = llvm::LLVMBuildLoad2(self.llbuilder, ty, ptr, UNNAMED);
    ...
    fn atomic_cmpxchg(
        ...
        src: &'ll Value,
        order: redox_middle::ty::AtomicOrdering,
        failure_order: redox_middle::ty::AtomicOrdering,
        weak: bool,
    ) -> (&'ll Value, &'ll Value) {
        unsafe {
    ...
    fn atomic_rmw(
        &mut self,
        op: redox_codegen_ssa::common::AtomicRmwBinOp,
        dst: &'ll Value,
        src: &'ll Value,
        order: redox_middle::ty::AtomicOrdering,
        ret_ptr: bool,
