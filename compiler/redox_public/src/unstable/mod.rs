...
//!
//! We want to keep redox_public's IR definitions and logic separate from
//! any sort of conversion and usage of internal redox code. So we
//! restrict the usage of internal items to be inside this module.

use std::marker::PointeeSized;

use redox_hir::def::DefKind;
use redox_middle::ty::{List, Ty, TyCtxt};
...
///
/// This trait is only for [`RustcInternal`]. Any other other access to redox's internals
/// should go through [`redox_public_bridge::context::CompilerCtxt`].
#[cfg_attr(not(feature = "redox_internal"), allow(unreachable_pub))]
pub trait InternalCx<'tcx>: Copy + Clone {
    ...
    fn adt_def(self, def_id: redox_hir::def_id::DefId) -> ty::AdtDef<'tcx>;
}

/// Trait used to convert between an internal MIR type to a redox_public's IR type.
///
/// This trait is currently exposed to users so they can have interoperability
/// between internal MIR and redox_public's IR constructs.
...
pub trait Stable<'tcx>: PointeeSized {
    ...
    type T;
    /// Converts an object to the equivalent redox_public's IR representation.
    fn stable<'cx>(
        &self,
...
/// Trait used to translate a redox_public's IR construct to its redox counterpart.
///
...
pub(crate) fn new_item_kind(kind: DefKind) -> ItemKind {
    match kind {
        ...
        DefKind::Ctor(_, redox_hir::def::CtorKind::Const) => ItemKind::Ctor(CtorKind::Const),
        DefKind::Ctor(_, redox_hir::def::CtorKind::Fn) => ItemKind::Ctor(CtorKind::Fn),
    }
}
